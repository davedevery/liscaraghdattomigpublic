...
function Lze479115de3 {
    param([string]$Severity)
    Lz7c5078ecf9 -Severity $Severity
    try {
...
function Lze020a85137 { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lzf57162d1e8 {
...
function Lzf2e1f3e538 {
    ...
    if (-not $s) { $s = Lz18d0992ec3 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lz9f8e0241b6 {
    param([string]$Value)
    if (Lzf57162d1e8 -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lz90d598ef4d {
    $f = @(Lzb9bfe183f7)
    if ($f.Count -eq 1) { return $f[0] }
    return ''
}
function Lz18c7c0437d {
...
function Lz028440f867 {
    param([string[]]$Picked)
    $result = New-Object System.Collections.Generic.List[string]
    foreach ($e in @(Lzb9bfe183f7)) { [void]$result.Add($e) }
    $dropped = New-Object System.Collections.Generic.List[string]
    foreach ($p in @($Picked | ForEach-Object { "$_".Trim().Trim('/').Trim('\') } | Where-Object { $_ })) {
        if ($result -ccontains $p) { continue }
        $covered = $false
        foreach ($k in @($result)) { if (Lz18c7c0437d -Child $p -Parent $k) { [void]$dropped.Add($p); $covered = $true; break } }
        if ($covered) { continue }
        foreach ($narrow in @($result | Where-Object { Lz18c7c0437d -Child $_ -Parent $p -Strict })) { [void]$result.Remove($narrow); [void]$dropped.Add($narrow) }
        [void]$result.Add($p)
    }
    Lzefde4c697e -Folders @($result)
    if ($dropped.Count) { Lzbf0f0727cf ("Overlapping folder(s) merged, kept the broader one: " + ((@($dropped) | Select-Object -Unique) -join ', ')) }
}
function Lze5c23ff49e {
    $one = $true; try { $one = (@(Lz2052b7b77b).Count -le 1) } catch {}
    $n = @(Lzb9bfe183f7).Count
...
function Lz792d238a74 {
    ...
    try {
        ...
        $norm = Lzcac224b1ae -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lzbf0f0727cf "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz1a0804ff88
    } catch { }
}
function Lzcac224b1ae {
...
function Lzb86474e3ed {
    ...
    return @{ SubfolderCount = @(Lz921206ec39 -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lz91e9843ddd {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lz00ac19677e -Path "sites/$siteId"
    } catch { return $null }
}
function Lz94f3d409a9 {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lz00ac19677e -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lz022420d3b9 {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lz00ac19677e -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz23bfc2b4e2 {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lz66d367b39b -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
function Lz7b68b87c67 {
    param($State, [string]$Current)
    $out = New-Object System.Collections.Generic.List[string]
    foreach ($s in @($State.Lst.SelectedItems)) {
        $name = "$s"
        if (-not (Lz134c13a8f9 -Item $name)) { continue }
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lz94f3d409a9 -Search $s.Txt.Text)
...
"@
    ...
    Lz23bfc2b4e2
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lz23bfc2b4e2 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lz23bfc2b4e2 } })
    $new.Add_Click({
        $n = Lz4b473ebea5 -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lzd02205bcca -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lz23bfc2b4e2; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lz85a3df627a -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lza292a4db58 -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lz85a3df627a -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lz1614eb9768 {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lz921206ec39 -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lz1614eb9768
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lz1614eb9768 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lz1614eb9768 } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lz85a3df627a -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lz7b68b87c67 -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lz85a3df627a -State $script:SP
    $r = $w.ShowDialog()
...
function Lzbf0f0727cf { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz8c74956d89 { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lz87ced39490 {
    ...
    if ($hasRegime) {
        $etaA = Lzd8b154b16f -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lzd8b154b16f -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lzd8b154b16f -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lzd8b154b16f -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lz6c635c1481 {
    ...
    try {
        ...
        $etaText = Lz87ced39490 -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lz9ae86ff93d {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lz2c048d237b -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lz60752e5bc9 -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lz6c635c1481 -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lz26044e743d $Line) + "`r`n")
...
function Lz717205c8f7 { param([string]$Text) Lz9ae86ff93d $Text }
function Lz78aea41900 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lz27ea7be04a $true 'Connecting to Microsoft 365...'
        Lzb16e94bab2
    } catch {
        ...
        Lz27ea7be04a $false; return
    } finally { Lz27ea7be04a $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lzed2a42f5b9 -StartSearch $seed
    if ($picked) {
        ...
        Lzbf0f0727cf "Site set to $picked"
        Lz1a0804ff88
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lz78aea41900 }
    Lz1a0804ff88
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lz03e6b4e1da }
    Lz1a0804ff88
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz1a0804ff88 })
$ctrl.TxtFolder.Add_TextChanged({ Lz1a0804ff88 })
if ($ctrl.LstSource) { $ctrl.LstSource.Add_SelectionChanged({ Lze5c23ff49e }) }
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz1a0804ff88 }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz1a0804ff88 }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lz654b587ed1 -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lz792d238a74 })
$ctrl.TxtLoc.Add_TextChanged({ Lz1a0804ff88 })
$ctrl.TxtLib.Add_TextChanged({ Lz1a0804ff88 })
$script:NestSuspend = $false
function Lz6d13c350e1 {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lz3c7b350430 -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lzbf0f0727cf $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lz6d13c350e1 })
$ctrl.ChkNest.Add_Unchecked({ Lz6d13c350e1 })
$script:ProjectRows = @()
$script:RunStatus = @{}
...
function Lz02212aff99 {
    ...
    if (-not $hasProjects) {
        $sd = $false
        if (Get-Command Test-ConnectionConfigured -ErrorAction SilentlyContinue) { try { $sd = [bool](Lz5495feddfc) } catch {} }
        Lzad0cfe1b9e $sd
    }
...
function Lzd55480fd73 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lzb86474e3ed -ProjectId $p.Id -SubPath $sub) }
...
function Lzcaa32bb37c {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lz00ac19677e -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz668d45d490 {
    ...
    if (($EngineArgs -contains 'Transfer') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck') -or ($EngineArgs -contains 'PreFlight')) {
        try { Lz27ea7be04a $true 'Checking source folders...'; $srcOk = Lzd55480fd73 } finally { Lz27ea7be04a $false }
        if (-not $srcOk) { return }
    }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lz27ea7be04a $true 'Checking destinations...'; $dstOk = Lzcaa32bb37c } finally { Lz27ea7be04a $false }
        if (-not $dstOk) { return }
    }
    ...
    $script:Timer.Add_Tick({
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lz9ae86ff93d $lines[$k] } catch {} }
                }
            }
        } catch {}
        ...
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lz9ae86ff93d $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lz9ae86ff93d $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Format-Span ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lz2e04b2773f) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lz550bad6e1d -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lz9ae86ff93d "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lz9ae86ff93d "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lzbf0f0727cf ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
            ...
            Lz624b2809a0
            if (-not $script:Paused) { Lze479115de3 -Severity $sev }
            if ($script:RunStatus.Count) { try { Lz02212aff99 } catch {} }
            try {
            ...
            try {
                if ($script:CurLabel -eq 'Check readiness' -or $script:CurLabel -eq 'Preview (no upload)') {
                    $newest = Get-ChildItem (Join-Path (Lz2e04b2773f) 'report-*.html') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
function Lza48f7ba54f { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lz6e4d711239 { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lzaea6b8cdb1 {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lz5814c07e7d -ConfigFile $s.Tag; Lz9bb6374808 } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lzffbb49682d { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lze020a85137 -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lzaea6b8cdb1
}
function Lz85a6fed134 {
...
function Lz5814c07e7d {
    param([string]$ConfigFile)
    if ($script:Sched) { Lzb44e6b8b1f -Why 'Schedule cancelled (job changed).' }
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lz6e4d711239 $true
    Lza48f7ba54f $true
    Lzffbb49682d $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lz85a6fed134 -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lzbf0f0727cf 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
}
function Lza59acb94d5 {
    param([string]$Dir)
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lz4b473ebea5 -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Lz67e2f8c951 -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Open it instead, or choose another name.")); return }
    if ((Test-Path $jobDir) -and -not (Lza59acb94d5 -Dir $jobDir)) {
        ...
        Lz3c7b350430 -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lz5814c07e7d -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuJobOpen.Add_Click({
    ...
    [void]$w.ShowDialog()
    if ($script:OpenPick) { Lz5814c07e7d -ConfigFile $script:OpenPick; Lz9bb6374808 }
})
$ctrl.MnuJobSave.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')); return }
    if (Save-Mapping) { Lzbf0f0727cf 'Migration job saved.' }
})
$ctrl.MnuJobSaveAs.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open a migration job first, then Save As to copy it.')); return }
    try {
        $name = Lz4b473ebea5 -Prompt "New name for the copy:" -Title 'Save job as'
        if (-not $name -or -not $name.Trim()) { return }
        $name = $name.Trim()
        $slug = ($name -replace '[^A-Za-z0-9._-]','_').Trim('_'); if (-not $slug) { $slug = 'job' }
        $jobDir = Join-Path $script:JobsRoot $slug
        if (Lz67e2f8c951 -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Choose another name.")); return }
        if ((Test-Path $jobDir) -and -not (Lza59acb94d5 -Dir $jobDir)) {
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz0a9f2cb744 })
$ctrl.MnuSettingsApi.Add_Click({ Lzba53e393af })
$ctrl.MnuSettingsWizard.Add_Click({ Lz6d15d29a2c })
$ctrl.MnuSettingsEmail.Add_Click({ Lz5911037d73 })
$ctrl.MnuSettingsTuning.Add_Click({ Lzec5c7a16bd })
$ctrl.BtnFilters.Add_Click({ Lz638600f225 })
$ctrl.MnuSettingsDecommission.Add_Click({ Lz417abd9bf2 })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lz4ede88f63a -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lzcc521bd1f8 {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lz18d0992ec3 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lzcc521bd1f8 "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lzcc521bd1f8 (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lz27ea7be04a $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lz27ea7be04a $false }
    if (-not $b) { return }
...
function Lz2d1329a8ca { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lzfe455622ca {
    $path = Lz2d1329a8ca
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Datto Workplace to SharePoint Migrator" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lzfe455622ca)) -Caption ('Licence')) | Out-Null })
function Lz329cea6218 {
...
function Lza8e0aa4589 {
    param([string]$For = 'copy')
    if (Test-Path (Lz2d1329a8ca)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        try {
            ...
            Lze020a85137 -Name 'LicenceAppliedId'  -Value "$($lpl.LicenceId)"
            Lze020a85137 -Name 'LicenceAppliedUtc' -Value ((Get-Date).ToUniversalTime().ToString('o'))
        } catch {}
        (Show-Msg -Text ("Licence installed.`n`n" + (Lzfe455622ca)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-FullReinstall {
    ...
    Lz27ea7be04a $true "Downloading the installer for version $($Payload.version)..."
    ...
    catch { Lz27ea7be04a $false; (Show-Msg -Text ("The installer could not be downloaded, so nothing has been changed.`n`n$($_.Exception.Message)") -Caption ('Reinstall') -Icon ('Warning')) | Out-Null; return }
...
function Invoke-UpdateCheck {
    ...
    try {
        ...
        if (-not $Silent) { Lz27ea7be04a $true 'Checking for updates...' }
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lz27ea7be04a $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lz4ede88f63a -Title 'About' -Heading 'Datto Workplace to SharePoint Migrator' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lzf2e1f3e538 } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lz38b076c9e5 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lz4b473ebea5 -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lz7c1a40063a {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lz6e4d711239 $false
    Lza48f7ba54f $false
}
$ctrl.MnuJobClose.Add_Click({ Lz7c1a40063a; Lzbf0f0727cf 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lz7c1a40063a
    if (Lza59acb94d5 -Dir $dir) {
        Lzaea6b8cdb1; Lzbf0f0727cf "Deleted job '$nm'"
    } else {
        Lzaea6b8cdb1
        ...
        Lzbf0f0727cf "Could not fully delete job '$nm' (a file is in use)"
    }
})
function Lzfc20ddae68 {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lz27ea7be04a $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lz717205c8f7 'Signing in to Datto and listing projects...'
        $script:Projects = Lzbf5a01d42a
        Lz717205c8f7 "Found $($script:Projects.Count) project(s)."
        Lz9461a3b918
        try { Lzb16e94bab2; Lz717205c8f7 'Connected to Microsoft 365.' } catch { Lz717205c8f7 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lz717205c8f7 "Technical detail: $($_.Exception.Message)" }
        Lz02212aff99
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lzbf0f0727cf 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lz717205c8f7 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lz717205c8f7 "Technical detail: $($_.Exception.Message)"
            Lzbf0f0727cf "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lzbf0f0727cf 'Connect failed'
        }
    }
    finally { Lz27ea7be04a $false }
}
$ctrl.BtnConnect.Add_Click({ Lzfc20ddae68 })
function Lz9bb6374808 {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lzf2e1f3e538 } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lzbf0f0727cf "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lzfc20ddae68 -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lz9fa080d9be
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lz2052b7b77b).Count } catch {}
    ...
    if ($script:Map.ContainsKey($p.Id)) {
        ...
        $ctrl.TxtLoc.Text = Lz03e6b4e1da
        $ctrl.TxtLib.Text = ''; $ctrl.TxtFolder.Text = ''; Lzefde4c697e -Folders @()
...
function Lz6969fdc2df {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lz0986a7cc4a) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lz0986a7cc4a) }
}
$ctrl.BtnApply.Add_Click({
    $p = Lz9fa080d9be
    if (-not $p) { (Show-Msg -Text ('Select a project first.')); return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Remove($p.Id) | Out-Null; Lz02212aff99; [void](Write-MappingQuiet); return }
    $d = Lz6969fdc2df
    if ($d -eq 'ERR') { return }
    ...
    Lzbf0f0727cf "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lzcebc99d149 })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lz68a79ac693 -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Lz02212aff99; [void](Write-MappingQuiet); Lzbf0f0727cf "Set $($sel.Count) project(s) to skip"; return }
    $d = Lz6969fdc2df
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lz78aea41900); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPaths=$keepSrcList; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Lz02212aff99
    [void](Write-MappingQuiet)
    Lzbf0f0727cf "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lz68a79ac693 -Count (@($script:Projects).Count))) { return }
    $d = Lz6969fdc2df
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Lz02212aff99; [void](Write-MappingQuiet); Lzbf0f0727cf 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Lz02212aff99
    [void](Write-MappingQuiet)
    Lzbf0f0727cf "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lz27ea7be04a $true 'Looking up libraries...'
        $libs = Lz1c52c2bea7 -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lz27ea7be04a $false; return }
    finally { Lz27ea7be04a $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz56cfb28f6f -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lzbf0f0727cf "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lzbf0f0727cf "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lz27ea7be04a $true 'Opening destination...'
        Lzb16e94bab2
        $res = Lz022420d3b9
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lz27ea7be04a $false; return }
    finally { Lz27ea7be04a $false }
    $picked = Lzb33786e046 -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lzbf0f0727cf ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnAddSource) { $ctrl.BtnAddSource.Add_Click({
    ...
    try { $picked = Lz341fbcb004 -ProjectId $p.Id -ProjectName $p.Name }
    ...
    Lz028440f867 -Picked $arr
    Lzbf0f0727cf ("Source folders: " + (@(Lzb9bfe183f7) -join ', '))
}) }
if ($ctrl.BtnRemoveSource) { $ctrl.BtnRemoveSource.Add_Click({
    ...
    Lzefde4c697e -Folders @(Lzb9bfe183f7 | Where-Object { $sel -notcontains $_ })
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    $bad = New-Object System.Collections.Generic.List[string]
    try {
        Lz27ea7be04a $true 'Checking the source folder(s) in Datto...'
        foreach ($s in $subs) { try { $null = Lzb86474e3ed -ProjectId $p.Id -SubPath $s } catch { [void]$bad.Add($s) } }
    } catch { $ctrl.LblSourceCheck.Text = "Could not check the source: $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lz27ea7be04a $false; return }
    finally { Lz27ea7be04a $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lz27ea7be04a $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lz00ac19677e -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lz1c52c2bea7 -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lz1a0804ff88 {
    ...
    $srcLabel = Lz7b17ee8283 -Project $p
    $folders  = @(Lzb9bfe183f7)
    $srcSub   = Lz90d598ef4d
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lz0986a7cc4a) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lz2052b7b77b).Count } catch { }
...
function Lz1110c9a56a {
    ...
    $script:Map[$p.Id] = $d
    Lz02212aff99
    Lzbf0f0727cf "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lz717205c8f7 "Saved $n mapping(s)."
    return $true
}
function Lz9461a3b918 {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lz717205c8f7 "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lz717205c8f7 "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lz668d45d490 -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lz668d45d490 -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lzed7d5e9577 {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lzed7d5e9577) }
    ...
    if (-not (Lz4ede88f63a -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lz187fd91592)) { return }
    if (-not (Lza8e0aa4589)) { return }
    Lz668d45d490 -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lz1cfd64acd7 {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lzed7d5e9577) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lz187fd91592)) { return }
    if (-not (Lza8e0aa4589)) { return }
    Lz668d45d490 -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnRerunFailed.Add_Click({
    ...
    if (-not (Lz4ede88f63a -Title 'Rerun failed files only' -Heading 'Rerun failed files only' -Sections $sections -Yes 'Rerun the failed files' -No 'Cancel')) { return }
    if (-not (Lz187fd91592)) { return }
    if (-not (Lza8e0aa4589)) { return }
    Lz668d45d490 -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','NewerWins','-Execute','-FailedOnly') -Label 'Rerun failed files only'
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lz4ede88f63a -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lza8e0aa4589 -For 'verify')) { return }
    Lz668d45d490 -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lz668d45d490 -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lz2e04b2773f {
...
function Lz624b2809a0 {
    try { if ($ctrl.BtnRerunFailed) { $ctrl.BtnRerunFailed.IsEnabled = (Lzbb215021c0) } } catch {}
}
function Lz2f4b155b50 {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lz6814257ac7 {
    param([string]$Name)
...
function Lzfc806007da {
    ...
    foreach ($f in $files) { $l = Lz6814257ac7 $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lzfc806007da -Folder (Lz2e04b2773f) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnCertificate.Add_Click({
    if (Save-Mapping) {
        Lz668d45d490 -EngineArgs @('-Action','Certificate') -Label 'Completion certificate' -OnComplete {
            $rr = Lz2e04b2773f
            $cert = $null
...
$ctrl.BtnOpenAudit.Add_Click({ Lzfc806007da -Folder (Lz2f4b155b50) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lz02403458ae {
    ...
    Lzbf0f0727cf "Speed limit set: up $uTxt, down $dTxt"
    Lz9ae86ff93d "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lz02403458ae })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz02403458ae } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz02403458ae } })
function Lzde01e36b16 {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lzde01e36b16 -ParentId ([int]$_.ProcessId) } } catch {}
...
function Lz56ed005896 {
    $r = ($script:Proc -and -not $script:Proc.HasExited)
    $f = ($script:FinProc -and -not $script:FinProc.HasExited)
    return ($r -or $f)
}
function Lzfc906b8817 { param([string]$T) try { $ctrl.BtnSchedule.Content.Text = $T } catch {} }
function Lzb44e6b8b1f {
    param([string]$Why = '')
    if ($script:SchedTimer) { try { $script:SchedTimer.Stop() } catch {}; $script:SchedTimer = $null }
    $script:Sched = $null
    Lzfc906b8817 'Schedule for later / overnight...'
    if ($Why) { Lzbf0f0727cf $Why }
}
function Lza0b5d76b15 {
    try { $p = Join-Path (Lz2e04b2773f) 'lastrun-outcome.json'; if (Test-Path $p) { return (Get-Content $p -Raw | ConvertFrom-Json) } } catch {}
    return $null
}
function Lz448c7cb58c {
    if (-not $script:Proc -or $script:Proc.HasExited) { return }
    $script:QuietPausing = $true
    $ctrl.BtnStop.IsEnabled = $false
    Lz9ae86ff93d "----- Quiet hours reached at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')): pausing the copy. Files already uploaded are kept. -----"
    Lzbf0f0727cf 'Quiet hours: pausing until the next window...'
    try { Lzde01e36b16 -ParentId ([int]$script:Proc.Id) } catch {}
    Lz85a6fed134 -Status Cancelled -Note 'Finalising for quiet hours...' -OnDone {
        try { Lz9ae86ff93d '----- Paused for quiet hours. It will resume as a Sync at the next window while this app stays open. -----' } catch {}
    }
}
function Lz923cccb457 {
    param([string[]]$RunArgs,[string]$Label)
    Lz9ae86ff93d "----- Scheduled: starting '$Label' at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')) -----"
    Lz668d45d490 -EngineArgs $RunArgs -Label $Label
    return ($script:Proc -and -not $script:Proc.HasExited)
}
function Lz28f2108611 { param([string]$Status)
    ...
    switch ("$($s.State)") {
        'waiting' {
            if ($now -ge $s.StartAt -and -not $busy) {
                ...
                elseif (Lz923cccb457 -RunArgs $s.Args -Label $s.Label) { $s.State = 'running'; $s.Launches++ }
                else { Lzb44e6b8b1f -Why 'The scheduled run could not start (see the message). Schedule cancelled.' }
            } else {
                $left = $s.StartAt - $now; if ($left.Ticks -lt 0) { $left = [TimeSpan]::Zero }
                Lzfc906b8817 ("Scheduled: starts $($s.StartAt.ToString('ddd HH:mm')) ($(Format-Span $left) to go) - click to cancel")
            }
        }
        'running' {
            if (-not $busy) {
                $oc = Lza0b5d76b15; $status = if ($oc) { "$($oc.Status)" } else { '' }
                if ($s.Mode -eq 'once') { Lzb44e6b8b1f -Why 'Scheduled run finished.'; Lz28f2108611 $status }
                elseif ($status -eq 'COMPLETED') { Lzb44e6b8b1f -Why 'Scheduled migration completed.'; Lz28f2108611 $status }
                else { $s.State = 'paused' }
            } elseif ($s.Mode -eq 'window' -and -not (Test-InWindow $m $s.FromMin $s.ToMin)) {
                Lz448c7cb58c; $s.State = 'paused'
            } else {
                $until = if ($s.Mode -eq 'window') { " (until $(Format-MinOfDay $s.ToMin))" } else { '' }
                Lzfc906b8817 ("Scheduled: running$until - click to cancel")
            }
        }
        'paused' {
            if ((Test-InWindow $m $s.FromMin $s.ToMin) -and -not $busy) {
                if (Lz923cccb457 -RunArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','AddMissing','-Execute') -Label 'Sync new and changed') { $s.State = 'running'; $s.Launches++ }
            } else {
                $nf = Get-NextWindowOpen $s.FromMin
                Lzfc906b8817 ("Scheduled: waiting for the next window ($($nf.ToString('ddd HH:mm'))) - click to cancel")
            }
        }
    }
}
function Lza0d89f83af {
...
$ctrl.BtnSchedule.Add_Click({
    if ($script:Sched) {
        if ((Show-Msg -Text ("A schedule is set:" + [Environment]::NewLine + $script:Sched.Summary + [Environment]::NewLine + [Environment]::NewLine + "Cancel it?") -Caption ('Scheduled run') -Buttons ('YesNo') -Icon ('Question')) -eq 'Yes') { Lzb44e6b8b1f -Why 'Schedule cancelled.' }
        return
    }
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')) | Out-Null; return }
    if (Lz56ed005896) { (Show-Msg -Text ('A run is in progress. Wait for it to finish, then set a schedule.') -Caption ('Cannot schedule now') -Icon ('Warning')) | Out-Null; return }
    if (-not (Save-Mapping)) { return }
    $mappedCount = @($script:Projects | Where-Object { $script:Map.ContainsKey($_.Id) }).Count
    ...
    $script:SchedTimer.Add_Tick({ try { Lz314ddc0c5b } catch {} })
    $script:SchedTimer.Start()
    try { Lz314ddc0c5b } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    if ($script:Sched) { Lzb44e6b8b1f -Why 'Schedule cancelled (you stopped the run).' }
    $script:Stopping = $true
    $ctrl.BtnStop.IsEnabled = $false
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz9ae86ff93d "----- STOP requested by user at $stamp -----"
    Lzbf0f0727cf "Stopping $($script:CurLabel)..."
    try { Lzde01e36b16 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lz85a6fed134 -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lz9ae86ff93d "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lzf442e14900 {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lz668d45d490 -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz9ae86ff93d "----- PAUSE requested by user at $stamp -----"
    Lzbf0f0727cf "Pausing $($script:CurLabel)..."
    try { Lzde01e36b16 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lz85a6fed134 -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lz9ae86ff93d "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lza99147bf04 { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lzb079b94dd4 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lzacd8815dfe {
...
function Lz38b076c9e5 {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lz19ddbd4296 -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lzba53e393af {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    ...
    function Lz1f8abe76f3 { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz7a2322c299 { param($Label,$Control,$Info)
    ...
    function Lze84c1fd8de { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lz1f8abe76f3 'Datto Workplace'
    ...
    $parts = Lzb079b94dd4 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz7a2322c299 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lz7a2322c299 'Cell (number)' (Lze84c1fd8de ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lz7a2322c299 'Datto Client ID' (Lze84c1fd8de ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lz7a2322c299 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lz1f8abe76f3 'Microsoft 365'
    $tbTid = Lz7a2322c299 'Tenant ID' (Lze84c1fd8de ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lz7a2322c299 'App (Client) ID' (Lze84c1fd8de ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lz1f8abe76f3 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lz7a2322c299 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lz7a2322c299 '.pfx file' (Lze84c1fd8de '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lz7a2322c299 '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lz1f8abe76f3 'SharePoint and OneDrive'
    ...
    $tbSp  = Lz7a2322c299 'SharePoint root URL' (Lze84c1fd8de ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lz7a2322c299 'OneDrive host URL'   (Lze84c1fd8de ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lz7a2322c299 'Team site base URL'  (Lze84c1fd8de ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lz7a2322c299 'Default site URL'    (Lze84c1fd8de ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lz7a2322c299 'Email / UPN domain'  (Lze84c1fd8de ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lze020a85137 -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lz38b076c9e5 -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz6755573f0d $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lza99147bf04 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lzf2e1f3e538 }
        ...
        try {
            ...
            else { $tok=Lz38b076c9e5 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lza99147bf04 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lze020a85137 -Name 'DattoEndpointUrl' -Value $ep
            Lze020a85137 -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lze020a85137 -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lze020a85137 -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lze020a85137 -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lze020a85137 -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lze020a85137 -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lze020a85137 -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lze020a85137 -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lz9f8e0241b6 -Value $script:AS.pbSec.Password
            }
...
function Lzd6c537cbd1 { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lz3c7b350430 { param($Cfg,[string]$Path,$Value)
...
function Lz638600f225 {
    ...
    $curMax=Lzd6c537cbd1 -Cfg $cfg -Path 'run.tuning.maxFileSizeMB'; if ($null -eq $curMax) { $curMax=0 }; $sizeBox.Text="$curMax"
    ...
    $curInc=@(Lzd6c537cbd1 -Cfg $cfg -Path 'run.tuning.includePatterns' | Where-Object { "$_".Trim() })
    $curExc=@(Lzd6c537cbd1 -Cfg $cfg -Path 'run.tuning.excludePatterns' | Where-Object { "$_".Trim() })
    ...
    $afterPick=New-Object System.Windows.Controls.DatePicker; $afterPick.Margin='0,4,0,4'; $afterPick.Width=150; $afterPick.SelectedDate=(& $toDate (Lzd6c537cbd1 -Cfg $cfg -Path 'run.tuning.modifiedAfter'))
    ...
    $beforePick=New-Object System.Windows.Controls.DatePicker; $beforePick.Margin='0,4,0,4'; $beforePick.Width=150; $beforePick.SelectedDate=(& $toDate (Lzd6c537cbd1 -Cfg $cfg -Path 'run.tuning.modifiedBefore'))
    ...
    $save.Add_Click({
        try {
            ...
            Lz3c7b350430 -Cfg $script:FltCfg -Path 'run.tuning.maxFileSizeMB' -Value $sv
            $pats=@("$($script:FltPat.Text)" -split '[;,\r\n]' | ForEach-Object { $_.Trim() } | Where-Object { $_ })
            if ($script:FltInc.IsChecked) { Lz3c7b350430 -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value $pats; Lz3c7b350430 -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value @() }
            else                          { Lz3c7b350430 -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value $pats; Lz3c7b350430 -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value @() }
            ...
            Lz3c7b350430 -Cfg $script:FltCfg -Path 'run.tuning.modifiedAfter'  -Value $ma
            Lz3c7b350430 -Cfg $script:FltCfg -Path 'run.tuning.modifiedBefore' -Value $mb
...
function Lzec5c7a16bd {
    ...
    function Lz1a24c61a41 { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec,[string]$Type='text')
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lzd6c537cbd1 -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lz1a24c61a41 -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec -Type $s.Type
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lz3c7b350430 -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lz3c7b350430 -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lz3c7b350430 -Cfg $script:SetCfg -Path $s.Path -Value ([bool]$script:SetBoxes[$s.Path].IsChecked) }
                elseif ($s.Type -eq 'date') {
                    ...
                    Lz3c7b350430 -Cfg $script:SetCfg -Path $s.Path -Value $raw
                }
                else { Lz3c7b350430 -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lz7f8e8cf660 {
    ...
    function Lzdb3a04016e { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lzb079b94dd4 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lzdb3a04016e -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lzdb3a04016e -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lzdb3a04016e -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lzdb3a04016e -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lza99147bf04 $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lzf2e1f3e538 }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lze020a85137 -Name 'DattoEndpointUrl' -Value $ep
            Lze020a85137 -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lz9f8e0241b6 -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz0a9f2cb744 {
    ...
    $pre = Lzf53195fa36 -Title 'Prerequisites'
    ...
    $dat = Lzf53195fa36 -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lzd2c59e5929 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lzf53195fa36 -Title 'Microsoft 365' -Buttons @($bMs, (Lzd2c59e5929 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lzf53195fa36 -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lz7f8e8cf660 } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzc556236fe0; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lz6d15d29a2c } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzc556236fe0; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lz5911037d73 } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzc556236fe0; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lzc556236fe0
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lz34ed9b22d6 {
...
function Lz757034f1a7 {
    ...
    $bOpen.Add_Click({ try { Lzd952e1ae06 -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lza4a8e64cb7 {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lzd1bc091e41 {
    ...
    Lz757034f1a7 -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lzd952e1ae06 -Url $pageUrl
    ...
    } finally { Lza4a8e64cb7 }
}
function Lz656d866146 {
    try {
...
function Lza5c982d217 {
    if (Lz089bfc3ecc) { return $false }
...
function Lz8f1d2a9448 {
    ...
    if (-not (Get-Module Microsoft.Graph.Authentication -ListAvailable)) {
        Lz34ed9b22d6 'The Microsoft Graph module is not installed. Re-run the installer (or run: Install-Module Microsoft.Graph.Authentication -Scope CurrentUser) and try again.' '#B42318' -Strong
        return
    }
    Lz34ed9b22d6 'Preparing the Microsoft 365 sign-in. The first time on a new install can take up to a minute while Windows loads the components - please wait, the app has not frozen.' '#475467' -Strong
    try { Import-Module Microsoft.Graph.Authentication -ErrorAction Stop } catch {
        Lz34ed9b22d6 "Could not load the Microsoft Graph module: $($_.Exception.Message)" '#B42318' -Strong
        return
    }
    Lz34ed9b22d6 "Sign in with an account that can manage Microsoft 365 (a Global Administrator is simplest). The first time, Microsoft asks you to accept the permissions this setup itself needs; that is expected.$(if ($ForEmail) { ' Email alerts are included, so expect TWO sign-ins: this one, then one for Exchange Online (a separate service with its own sign-in).' })"
    ...
    if (Lza5c982d217) {
        Lz34ed9b22d6 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B5460F' -Strong
        try {
            Lz92be43c120 -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lz34ed9b22d6 "The browser sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B5460F' -Strong
        }
    } else {
        Lz34ed9b22d6 'Using the code sign-in (the reliable path): a small window will show your code to type at microsoft.com/devicelogin, or on your phone. It stays on top and closes itself when you are done.' '#B5460F' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lzd1bc091e41 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lz34ed9b22d6 $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lzc24787f16e -Token $at -Claim 'tid'
            $wizTokAcct = Lzc24787f16e -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lzc24787f16e -Token $at -Claim 'preferred_username' }
            ...
            Lz34ed9b22d6 "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lz34ed9b22d6 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lz34ed9b22d6 "Signed in to tenant $tid as $wizAcct." '#0E9F6E'
    $newThumb = $null
    try {
        Lz34ed9b22d6 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lz34ed9b22d6 "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B5460F' }
            Lz34ed9b22d6 "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#0E9F6E'
        } else {
            Lz34ed9b22d6 "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lz34ed9b22d6 'App registration created.' '#0E9F6E'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lz34ed9b22d6 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lz34ed9b22d6 "Certificate created in this user's store. Thumbprint $newThumb." '#0E9F6E'
        Lz34ed9b22d6 'Uploading the certificate''s public key to the app registration...'
        ...
        Lz34ed9b22d6 "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#0E9F6E'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lzb36bc916e6 -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lz34ed9b22d6 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lzb36bc916e6 -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lz34ed9b22d6 $consentMsg '#0E9F6E'
        Lze020a85137 -Name 'TenantId'       -Value $tid
        Lze020a85137 -Name 'GraphClientId'  -Value $appId
        Lze020a85137 -Name 'CertThumbprint' -Value $newThumb
        Lz34ed9b22d6 'Connection details saved on this computer.' '#0E9F6E'
    } catch {
        ...
        if ($friendly) { Lz34ed9b22d6 $friendly '#B42318' -Strong; Lz34ed9b22d6 "Technical detail: $m" }
        else { Lz34ed9b22d6 "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lz34ed9b22d6 "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lz34ed9b22d6 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lz38b076c9e5 -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lze020a85137 -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lzacd8815dfe $spUrl
            if ($der) {
                Lze020a85137 -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lze020a85137 -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lze020a85137 -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lz34ed9b22d6 "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#0E9F6E'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lzb36bc916e6 -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lz34ed9b22d6 "The new sign-in has not become active yet: $($_.Exception.Message)" '#B5460F' -Strong
                   Lz34ed9b22d6 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz6755573f0d $org
            if ($u) { Lze020a85137 -Name 'UpnDomain' -Value $u; Lz34ed9b22d6 "Email domain detected: $u" '#0E9F6E' }
        } catch { Lz34ed9b22d6 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lz34ed9b22d6 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#0E9F6E' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lz6d15d29a2c {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lzd717bd5858 $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lz8f1d2a9448 -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lz8eda1a8c62) {
                    Lz34ed9b22d6 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lz34ed9b22d6 $t $c } else { Lz34ed9b22d6 $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lz34ed9b22d6 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B5460F'
                }
            }
            ...
            Lz34ed9b22d6 "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lzd1bc091e41 -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lz92be43c120 -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lzd952e1ae06 -Url 'https://login.microsoft.com/device'
                Lza92d4e25af
                try { Lz92be43c120 -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lz982393ffaf }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (Lza5c982d217) {
            ...
            try {
                Lz92be43c120 -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lzd1bc091e41 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lzc24787f16e -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lzc24787f16e -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lzb36bc916e6 -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lz1520e29f35 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lzb36bc916e6 -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lzb36bc916e6 -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lze020a85137 -Name 'EmailSender' -Value $senderAddr
        Lze020a85137 -Name 'EmailEnabled' -Value '1'
...
function Lz5911037d73 {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lz640547cfb2 { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz023717edf1 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lz023717edf1 -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lz023717edf1 -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lz023717edf1 -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lz640547cfb2 'Subject and attachments'
    ...
    [void](Lz023717edf1 -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz58104f5bd2 -Template $script:ES.tbSubj.Text -Vars (Lz72b0dc7b90)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz58104f5bd2 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz72b0dc7b90)
            $tok = Lz38b076c9e5 -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lze020a85137 -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lze020a85137 -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lze020a85137 -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lze020a85137 -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lze020a85137 -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lze020a85137 -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lze020a85137 -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lze020a85137 -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lze020a85137 -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lze020a85137 -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lze020a85137 -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lze020a85137 -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lze76f183d2f {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lzd1bc091e41 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lz417abd9bf2 {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lze76f183d2f -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lzc556236fe0 } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lz5495feddfc {
    return ((Lzec467f0e55) -and (Lz8eda1a8c62))
}
Lz02d49e79e9
Lz982393ffaf
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lz9ae86ff93d "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lz5495feddfc)) {
        try { Lz0a9f2cb744 } catch {}
        if (Lz5495feddfc) { Lzbf0f0727cf 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lzbf0f0727cf 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lzbf0f0727cf 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ($ctrl.GettingStarted -and $ctrl.GettingStarted.Visibility -eq 'Visible') {
        try { Lzad0cfe1b9e ([bool](Lz5495feddfc)) } catch {}
    }
    if ((Lz5495feddfc) -and -not (Test-Path (Lz2d1329a8ca))) {
        try { Lz54e7f4db3a } catch {}
    }
