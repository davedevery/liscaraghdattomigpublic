...
function Lzf68531c0b8 {
    param([string]$Severity)
    Lz6363291f82 -Severity $Severity
    try {
...
function Lz943a1dad50 { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lzb00faef38c {
...
function Lz50687bf3fe {
    ...
    if (-not $s) { $s = Lzeff1a423af 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lz9e7bebb284 {
    param([string]$Value)
    if (Lzb00faef38c -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lz09f6dc7a64 {
    $f = @(Lz147918ec5d)
    if ($f.Count -eq 1) { return $f[0] }
    return ''
}
function Lz937a3e2ec2 {
...
function Lz0839b98d47 {
    param([string[]]$Picked)
    $result = New-Object System.Collections.Generic.List[string]
    foreach ($e in @(Lz147918ec5d)) { [void]$result.Add($e) }
    $dropped = New-Object System.Collections.Generic.List[string]
    foreach ($p in @($Picked | ForEach-Object { "$_".Trim().Trim('/').Trim('\') } | Where-Object { $_ })) {
        if ($result -ccontains $p) { continue }
        $covered = $false
        foreach ($k in @($result)) { if (Lz937a3e2ec2 -Child $p -Parent $k) { [void]$dropped.Add($p); $covered = $true; break } }
        if ($covered) { continue }
        foreach ($narrow in @($result | Where-Object { Lz937a3e2ec2 -Child $_ -Parent $p -Strict })) { [void]$result.Remove($narrow); [void]$dropped.Add($narrow) }
        [void]$result.Add($p)
    }
    Lz34b3b3c382 -Folders @($result)
    if ($dropped.Count) { Lz3e031df1c0 ("Overlapping folder(s) merged, kept the broader one: " + ((@($dropped) | Select-Object -Unique) -join ', ')) }
}
function Lz1d19a7885f {
    $one = $true; try { $one = (@(Lzd6118b0fdb).Count -le 1) } catch {}
    $n = @(Lz147918ec5d).Count
...
function Lz4f790bb3fc {
    ...
    try {
        ...
        $norm = Lz75518d1a6e -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lz3e031df1c0 "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz7b4bb477e1
    } catch { }
}
function Lz75518d1a6e {
...
function Lzc847fb01fd {
    ...
    return @{ SubfolderCount = @(Lzc3f3554aa2 -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lz9fddf27061 {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lz184f85ae9a -Path "sites/$siteId"
    } catch { return $null }
}
function Lz00d56053c5 {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lz184f85ae9a -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lzfbe5272060 {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lz184f85ae9a -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lza613fef127 {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lz784144fcc1 -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
function Lzda3cd1f9c9 {
    param($State, [string]$Current)
    $out = New-Object System.Collections.Generic.List[string]
    foreach ($s in @($State.Lst.SelectedItems)) {
        $name = "$s"
        if (-not (Lz90066629be -Item $name)) { continue }
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lz00d56053c5 -Search $s.Txt.Text)
...
"@
    ...
    Lza613fef127
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lza613fef127 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lza613fef127 } })
    $new.Add_Click({
        $n = Lz0ef40b6aab -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lzf7fb4020e3 -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lza613fef127; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lzfcdbdae27b -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lz290d37394d -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lzfcdbdae27b -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lz68877563de {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lzc3f3554aa2 -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lz68877563de
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lz68877563de } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lz68877563de } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lzfcdbdae27b -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lzda3cd1f9c9 -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lzfcdbdae27b -State $script:SP
    $r = $w.ShowDialog()
...
function Lz3e031df1c0 { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz9dc7f6ba24 { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lz1e426f4e27 {
    ...
    if ($hasRegime) {
        $etaA = Lzfb98282788 -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lzfb98282788 -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lzfb98282788 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lzfb98282788 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lz613234125d {
    ...
    try {
        ...
        $etaText = Lz1e426f4e27 -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lzb4950d8c68 {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lzfa6ca5829c -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lz91645024cc -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lz613234125d -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lzcba5ac40ad $Line) + "`r`n")
...
function Lzf6cc97f860 { param([string]$Text) Lzb4950d8c68 $Text }
function Lz318f1b9283 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lz3345bae64d $true 'Connecting to Microsoft 365...'
        Lza1f41a8af4
    } catch {
        ...
        Lz3345bae64d $false; return
    } finally { Lz3345bae64d $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lz0dd2a44bae -StartSearch $seed
    if ($picked) {
        ...
        Lz3e031df1c0 "Site set to $picked"
        Lz7b4bb477e1
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lz318f1b9283 }
    Lz7b4bb477e1
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lzec967355d6 }
    Lz7b4bb477e1
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz7b4bb477e1 })
$ctrl.TxtFolder.Add_TextChanged({ Lz7b4bb477e1 })
if ($ctrl.LstSource) { $ctrl.LstSource.Add_SelectionChanged({ Lz1d19a7885f }) }
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz7b4bb477e1 }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz7b4bb477e1 }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lzf771ad66d0 -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lz4f790bb3fc })
$ctrl.TxtLoc.Add_TextChanged({ Lz7b4bb477e1 })
$ctrl.TxtLib.Add_TextChanged({ Lz7b4bb477e1 })
$script:NestSuspend = $false
function Lz58d4e44c24 {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lze618e9779f -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lz3e031df1c0 $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lz58d4e44c24 })
$ctrl.ChkNest.Add_Unchecked({ Lz58d4e44c24 })
$script:ProjectRows = @()
$script:RunStatus = @{}
...
function Lz2c9d961b7c {
    ...
    if (-not $hasProjects) {
        $sd = $false
        if (Get-Command Test-ConnectionConfigured -ErrorAction SilentlyContinue) { try { $sd = [bool](Lz5409d77682) } catch {} }
        Lze75f5e6d76 $sd
    }
...
function Lze62a79053b {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lzc847fb01fd -ProjectId $p.Id -SubPath $sub) }
...
function Lz7be0677eda {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lz184f85ae9a -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lzc0817c5ad4 {
    ...
    if (($EngineArgs -contains 'Transfer') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck') -or ($EngineArgs -contains 'PreFlight')) {
        try { Lz3345bae64d $true 'Checking source folders...'; $srcOk = Lze62a79053b } finally { Lz3345bae64d $false }
        if (-not $srcOk) { return }
    }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lz3345bae64d $true 'Checking destinations...'; $dstOk = Lz7be0677eda } finally { Lz3345bae64d $false }
        if (-not $dstOk) { return }
    }
    ...
    $script:Timer.Add_Tick({
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lzb4950d8c68 $lines[$k] } catch {} }
                }
            }
        } catch {}
        ...
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lzb4950d8c68 $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lzb4950d8c68 $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Format-Span ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lz14207a52e2) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lzf72cf6d7cd -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lzb4950d8c68 "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lzb4950d8c68 "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lz3e031df1c0 ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
            ...
            Lz1f102df75e
            if (-not $script:Paused) { Lzf68531c0b8 -Severity $sev }
            if ($script:RunStatus.Count) { try { Lz2c9d961b7c } catch {} }
            try {
            ...
            try {
                if ($script:CurLabel -eq 'Check readiness' -or $script:CurLabel -eq 'Preview (no upload)') {
                    $newest = Get-ChildItem (Join-Path (Lz14207a52e2) 'report-*.html') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
function Lz3996a937dc { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lze5c70b57db { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lzad976aa1db {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lzc238e4a7cd -ConfigFile $s.Tag; Lzc13870f929 } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lz3e62ff9ca8 { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lz943a1dad50 -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lzad976aa1db
}
function Lzb9086747f6 {
...
function Lzc238e4a7cd {
    param([string]$ConfigFile)
    if ($script:Sched) { Lz8316cc399d -Why 'Schedule cancelled (job changed).' }
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lze5c70b57db $true
    Lz3996a937dc $true
    Lz3e62ff9ca8 $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lzb9086747f6 -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lz3e031df1c0 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
}
function Lzf95040b349 {
    param([string]$Dir)
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lz0ef40b6aab -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Lz19da45c2be -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Open it instead, or choose another name.")); return }
    if ((Test-Path $jobDir) -and -not (Lzf95040b349 -Dir $jobDir)) {
        ...
        Lze618e9779f -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lzc238e4a7cd -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuJobOpen.Add_Click({
    ...
    [void]$w.ShowDialog()
    if ($script:OpenPick) { Lzc238e4a7cd -ConfigFile $script:OpenPick; Lzc13870f929 }
})
$ctrl.MnuJobSave.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')); return }
    if (Save-Mapping) { Lz3e031df1c0 'Migration job saved.' }
})
$ctrl.MnuJobSaveAs.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open a migration job first, then Save As to copy it.')); return }
    try {
        $name = Lz0ef40b6aab -Prompt "New name for the copy:" -Title 'Save job as'
        if (-not $name -or -not $name.Trim()) { return }
        $name = $name.Trim()
        $slug = ($name -replace '[^A-Za-z0-9._-]','_').Trim('_'); if (-not $slug) { $slug = 'job' }
        $jobDir = Join-Path $script:JobsRoot $slug
        if (Lz19da45c2be -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Choose another name.")); return }
        if ((Test-Path $jobDir) -and -not (Lzf95040b349 -Dir $jobDir)) {
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz07f542dfcf })
$ctrl.MnuSettingsApi.Add_Click({ Lzeb821367b2 })
$ctrl.MnuSettingsWizard.Add_Click({ Lz29f406ac8f })
$ctrl.MnuSettingsEmail.Add_Click({ Lz98c1960315 })
$ctrl.MnuSettingsTuning.Add_Click({ Lz14bff391aa })
$ctrl.BtnFilters.Add_Click({ Lz37ac9a27d0 })
$ctrl.MnuSettingsDecommission.Add_Click({ Lzf228c0ff18 })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lz3430b2c05f -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lzf38fbfef0d {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lzeff1a423af 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lzf38fbfef0d "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lzf38fbfef0d (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lz3345bae64d $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lz3345bae64d $false }
    if (-not $b) { return }
...
function Lz48281b00c8 { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lza81d7312b3 {
    $path = Lz48281b00c8
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Datto Workplace to SharePoint Migrator" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lza81d7312b3)) -Caption ('Licence')) | Out-Null })
function Lz78763114b3 {
...
function Lzd7fd432e8f {
    param([string]$For = 'copy')
    if (Test-Path (Lz48281b00c8)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        try {
            ...
            Lz943a1dad50 -Name 'LicenceAppliedId'  -Value "$($lpl.LicenceId)"
            Lz943a1dad50 -Name 'LicenceAppliedUtc' -Value ((Get-Date).ToUniversalTime().ToString('o'))
        } catch {}
        (Show-Msg -Text ("Licence installed.`n`n" + (Lza81d7312b3)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-FullReinstall {
    ...
    Lz3345bae64d $true "Downloading the installer for version $($Payload.version)..."
    ...
    catch { Lz3345bae64d $false; (Show-Msg -Text ("The installer could not be downloaded, so nothing has been changed.`n`n$($_.Exception.Message)") -Caption ('Reinstall') -Icon ('Warning')) | Out-Null; return }
...
function Invoke-UpdateCheck {
    ...
    try {
        ...
        if (-not $Silent) { Lz3345bae64d $true 'Checking for updates...' }
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lz3345bae64d $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lz3430b2c05f -Title 'About' -Heading 'Datto Workplace to SharePoint Migrator' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lz50687bf3fe } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lz1cb0b686e7 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lz0ef40b6aab -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lz7c74623089 {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lze5c70b57db $false
    Lz3996a937dc $false
}
$ctrl.MnuJobClose.Add_Click({ Lz7c74623089; Lz3e031df1c0 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lz7c74623089
    if (Lzf95040b349 -Dir $dir) {
        Lzad976aa1db; Lz3e031df1c0 "Deleted job '$nm'"
    } else {
        Lzad976aa1db
        ...
        Lz3e031df1c0 "Could not fully delete job '$nm' (a file is in use)"
    }
})
function Lz8f3574d4fb {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lz3345bae64d $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lzf6cc97f860 'Signing in to Datto and listing projects...'
        $script:Projects = Lz94da44385f
        Lzf6cc97f860 "Found $($script:Projects.Count) project(s)."
        Lza461db834d
        try { Lza1f41a8af4; Lzf6cc97f860 'Connected to Microsoft 365.' } catch { Lzf6cc97f860 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lzf6cc97f860 "Technical detail: $($_.Exception.Message)" }
        Lz2c9d961b7c
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lz3e031df1c0 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lzf6cc97f860 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lzf6cc97f860 "Technical detail: $($_.Exception.Message)"
            Lz3e031df1c0 "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lz3e031df1c0 'Connect failed'
        }
    }
    finally { Lz3345bae64d $false }
}
$ctrl.BtnConnect.Add_Click({ Lz8f3574d4fb })
function Lzc13870f929 {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lz50687bf3fe } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lz3e031df1c0 "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lz8f3574d4fb -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lz3e71ba38e6
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lzd6118b0fdb).Count } catch {}
    ...
    if ($script:Map.ContainsKey($p.Id)) {
        ...
        $ctrl.TxtLoc.Text = Lzec967355d6
        $ctrl.TxtLib.Text = ''; $ctrl.TxtFolder.Text = ''; Lz34b3b3c382 -Folders @()
...
function Lz5c39b44892 {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lz783eed1096) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lz783eed1096) }
}
$ctrl.BtnApply.Add_Click({
    $p = Lz3e71ba38e6
    if (-not $p) { (Show-Msg -Text ('Select a project first.')); return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Remove($p.Id) | Out-Null; Lz2c9d961b7c; [void](Write-MappingQuiet); return }
    $d = Lz5c39b44892
    if ($d -eq 'ERR') { return }
    ...
    Lz3e031df1c0 "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lzcf9f00c58d })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lzbae073846d -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Lz2c9d961b7c; [void](Write-MappingQuiet); Lz3e031df1c0 "Set $($sel.Count) project(s) to skip"; return }
    $d = Lz5c39b44892
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lz318f1b9283); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPaths=$keepSrcList; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Lz2c9d961b7c
    [void](Write-MappingQuiet)
    Lz3e031df1c0 "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lzbae073846d -Count (@($script:Projects).Count))) { return }
    $d = Lz5c39b44892
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Lz2c9d961b7c; [void](Write-MappingQuiet); Lz3e031df1c0 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Lz2c9d961b7c
    [void](Write-MappingQuiet)
    Lz3e031df1c0 "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lz3345bae64d $true 'Looking up libraries...'
        $libs = Lz375e2aebeb -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lz3345bae64d $false; return }
    finally { Lz3345bae64d $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz7f5f68baf6 -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lz3e031df1c0 "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lz3e031df1c0 "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lz3345bae64d $true 'Opening destination...'
        Lza1f41a8af4
        $res = Lzfbe5272060
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lz3345bae64d $false; return }
    finally { Lz3345bae64d $false }
    $picked = Lz2f9a3dbbe4 -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lz3e031df1c0 ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnAddSource) { $ctrl.BtnAddSource.Add_Click({
    ...
    try { $picked = Lz6e7a896838 -ProjectId $p.Id -ProjectName $p.Name }
    ...
    Lz0839b98d47 -Picked $arr
    Lz3e031df1c0 ("Source folders: " + (@(Lz147918ec5d) -join ', '))
}) }
if ($ctrl.BtnRemoveSource) { $ctrl.BtnRemoveSource.Add_Click({
    ...
    Lz34b3b3c382 -Folders @(Lz147918ec5d | Where-Object { $sel -notcontains $_ })
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    $bad = New-Object System.Collections.Generic.List[string]
    try {
        Lz3345bae64d $true 'Checking the source folder(s) in Datto...'
        foreach ($s in $subs) { try { $null = Lzc847fb01fd -ProjectId $p.Id -SubPath $s } catch { [void]$bad.Add($s) } }
    } catch { $ctrl.LblSourceCheck.Text = "Could not check the source: $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lz3345bae64d $false; return }
    finally { Lz3345bae64d $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lz3345bae64d $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lz184f85ae9a -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lz375e2aebeb -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lz7b4bb477e1 {
    ...
    $srcLabel = Lzf7df236b3b -Project $p
    $folders  = @(Lz147918ec5d)
    $srcSub   = Lz09f6dc7a64
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lz783eed1096) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lzd6118b0fdb).Count } catch { }
...
function Lzd52b7c5ec7 {
    ...
    $script:Map[$p.Id] = $d
    Lz2c9d961b7c
    Lz3e031df1c0 "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lzf6cc97f860 "Saved $n mapping(s)."
    return $true
}
function Lza461db834d {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lzf6cc97f860 "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lzf6cc97f860 "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lzc0817c5ad4 -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lzc0817c5ad4 -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lz50db710030 {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lz50db710030) }
    ...
    if (-not (Lz3430b2c05f -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lzb2389f738e)) { return }
    if (-not (Lzd7fd432e8f)) { return }
    Lzc0817c5ad4 -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lz7b6ad14e0d {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lz50db710030) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lzb2389f738e)) { return }
    if (-not (Lzd7fd432e8f)) { return }
    Lzc0817c5ad4 -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnRerunFailed.Add_Click({
    ...
    if (-not (Lz3430b2c05f -Title 'Rerun failed files only' -Heading 'Rerun failed files only' -Sections $sections -Yes 'Rerun the failed files' -No 'Cancel')) { return }
    if (-not (Lzb2389f738e)) { return }
    if (-not (Lzd7fd432e8f)) { return }
    Lzc0817c5ad4 -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','NewerWins','-Execute','-FailedOnly') -Label 'Rerun failed files only'
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lz3430b2c05f -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lzd7fd432e8f -For 'verify')) { return }
    Lzc0817c5ad4 -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lzc0817c5ad4 -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lz14207a52e2 {
...
function Lz1f102df75e {
    try { if ($ctrl.BtnRerunFailed) { $ctrl.BtnRerunFailed.IsEnabled = (Lz55d25db4d8) } } catch {}
}
function Lz7ef5864820 {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lz04118156fa {
    param([string]$Name)
...
function Lzfedf3bdbf3 {
    ...
    foreach ($f in $files) { $l = Lz04118156fa $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lzfedf3bdbf3 -Folder (Lz14207a52e2) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnCertificate.Add_Click({
    if (Save-Mapping) {
        Lzc0817c5ad4 -EngineArgs @('-Action','Certificate') -Label 'Completion certificate' -OnComplete {
            $rr = Lz14207a52e2
            $cert = $null
...
$ctrl.BtnOpenAudit.Add_Click({ Lzfedf3bdbf3 -Folder (Lz7ef5864820) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lzd64ad1dc59 {
    ...
    Lz3e031df1c0 "Speed limit set: up $uTxt, down $dTxt"
    Lzb4950d8c68 "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lzd64ad1dc59 })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lzd64ad1dc59 } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lzd64ad1dc59 } })
function Lz2c95f04b90 {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lz2c95f04b90 -ParentId ([int]$_.ProcessId) } } catch {}
...
function Lz4ca44992a8 {
    $r = ($script:Proc -and -not $script:Proc.HasExited)
    $f = ($script:FinProc -and -not $script:FinProc.HasExited)
    return ($r -or $f)
}
function Lz1257bb8ee4 { param([string]$T) try { $ctrl.BtnSchedule.Content.Text = $T } catch {} }
function Lz8316cc399d {
    param([string]$Why = '')
    if ($script:SchedTimer) { try { $script:SchedTimer.Stop() } catch {}; $script:SchedTimer = $null }
    $script:Sched = $null
    Lz1257bb8ee4 'Schedule for later / overnight...'
    if ($Why) { Lz3e031df1c0 $Why }
}
function Lz642a8af467 {
    try { $p = Join-Path (Lz14207a52e2) 'lastrun-outcome.json'; if (Test-Path $p) { return (Get-Content $p -Raw | ConvertFrom-Json) } } catch {}
    return $null
}
function Lza61cc50108 {
    if (-not $script:Proc -or $script:Proc.HasExited) { return }
    $script:QuietPausing = $true
    $ctrl.BtnStop.IsEnabled = $false
    Lzb4950d8c68 "----- Quiet hours reached at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')): pausing the copy. Files already uploaded are kept. -----"
    Lz3e031df1c0 'Quiet hours: pausing until the next window...'
    try { Lz2c95f04b90 -ParentId ([int]$script:Proc.Id) } catch {}
    Lzb9086747f6 -Status Cancelled -Note 'Finalising for quiet hours...' -OnDone {
        try { Lzb4950d8c68 '----- Paused for quiet hours. It will resume as a Sync at the next window while this app stays open. -----' } catch {}
    }
}
function Lz8a82a4d585 {
    param([string[]]$RunArgs,[string]$Label)
    Lzb4950d8c68 "----- Scheduled: starting '$Label' at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')) -----"
    Lzc0817c5ad4 -EngineArgs $RunArgs -Label $Label
    return ($script:Proc -and -not $script:Proc.HasExited)
}
function Lz71862298af { param([string]$Status)
    ...
    switch ("$($s.State)") {
        'waiting' {
            if ($now -ge $s.StartAt -and -not $busy) {
                ...
                elseif (Lz8a82a4d585 -RunArgs $s.Args -Label $s.Label) { $s.State = 'running'; $s.Launches++ }
                else { Lz8316cc399d -Why 'The scheduled run could not start (see the message). Schedule cancelled.' }
            } else {
                $left = $s.StartAt - $now; if ($left.Ticks -lt 0) { $left = [TimeSpan]::Zero }
                Lz1257bb8ee4 ("Scheduled: starts $($s.StartAt.ToString('ddd HH:mm')) ($(Format-Span $left) to go) - click to cancel")
            }
        }
        'running' {
            if (-not $busy) {
                $oc = Lz642a8af467; $status = if ($oc) { "$($oc.Status)" } else { '' }
                if ($s.Mode -eq 'once') { Lz8316cc399d -Why 'Scheduled run finished.'; Lz71862298af $status }
                elseif ($status -eq 'COMPLETED') { Lz8316cc399d -Why 'Scheduled migration completed.'; Lz71862298af $status }
                else { $s.State = 'paused' }
            } elseif ($s.Mode -eq 'window' -and -not (Test-InWindow $m $s.FromMin $s.ToMin)) {
                Lza61cc50108; $s.State = 'paused'
            } else {
                $until = if ($s.Mode -eq 'window') { " (until $(Format-MinOfDay $s.ToMin))" } else { '' }
                Lz1257bb8ee4 ("Scheduled: running$until - click to cancel")
            }
        }
        'paused' {
            if ((Test-InWindow $m $s.FromMin $s.ToMin) -and -not $busy) {
                if (Lz8a82a4d585 -RunArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','AddMissing','-Execute') -Label 'Sync new and changed') { $s.State = 'running'; $s.Launches++ }
            } else {
                $nf = Get-NextWindowOpen $s.FromMin
                Lz1257bb8ee4 ("Scheduled: waiting for the next window ($($nf.ToString('ddd HH:mm'))) - click to cancel")
            }
        }
    }
}
function Lz8fc87fa85c {
...
$ctrl.BtnSchedule.Add_Click({
    if ($script:Sched) {
        if ((Show-Msg -Text ("A schedule is set:" + [Environment]::NewLine + $script:Sched.Summary + [Environment]::NewLine + [Environment]::NewLine + "Cancel it?") -Caption ('Scheduled run') -Buttons ('YesNo') -Icon ('Question')) -eq 'Yes') { Lz8316cc399d -Why 'Schedule cancelled.' }
        return
    }
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')) | Out-Null; return }
    if (Lz4ca44992a8) { (Show-Msg -Text ('A run is in progress. Wait for it to finish, then set a schedule.') -Caption ('Cannot schedule now') -Icon ('Warning')) | Out-Null; return }
    if (-not (Save-Mapping)) { return }
    $mappedCount = @($script:Projects | Where-Object { $script:Map.ContainsKey($_.Id) }).Count
    ...
    $script:SchedTimer.Add_Tick({ try { Lz5d3e9dbc56 } catch {} })
    $script:SchedTimer.Start()
    try { Lz5d3e9dbc56 } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    if ($script:Sched) { Lz8316cc399d -Why 'Schedule cancelled (you stopped the run).' }
    $script:Stopping = $true
    $ctrl.BtnStop.IsEnabled = $false
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lzb4950d8c68 "----- STOP requested by user at $stamp -----"
    Lz3e031df1c0 "Stopping $($script:CurLabel)..."
    try { Lz2c95f04b90 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lzb9086747f6 -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lzb4950d8c68 "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lz8cb9a4aa5b {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lzc0817c5ad4 -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lzb4950d8c68 "----- PAUSE requested by user at $stamp -----"
    Lz3e031df1c0 "Pausing $($script:CurLabel)..."
    try { Lz2c95f04b90 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lzb9086747f6 -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lzb4950d8c68 "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lzdcd96c40e0 { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lz825a092477 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lzfe1b629475 {
...
function Lz1cb0b686e7 {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lzaaa7ad509f -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lzeb821367b2 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    ...
    function Lzcd3673647c { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz9ab7ea5d0c { param($Label,$Control,$Info)
    ...
    function Lzd822b2c403 { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lzcd3673647c 'Datto Workplace'
    ...
    $parts = Lz825a092477 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz9ab7ea5d0c 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lz9ab7ea5d0c 'Cell (number)' (Lzd822b2c403 ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lz9ab7ea5d0c 'Datto Client ID' (Lzd822b2c403 ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lz9ab7ea5d0c 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lzcd3673647c 'Microsoft 365'
    $tbTid = Lz9ab7ea5d0c 'Tenant ID' (Lzd822b2c403 ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lz9ab7ea5d0c 'App (Client) ID' (Lzd822b2c403 ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lzcd3673647c 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lz9ab7ea5d0c 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lz9ab7ea5d0c '.pfx file' (Lzd822b2c403 '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lz9ab7ea5d0c '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lzcd3673647c 'SharePoint and OneDrive'
    ...
    $tbSp  = Lz9ab7ea5d0c 'SharePoint root URL' (Lzd822b2c403 ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lz9ab7ea5d0c 'OneDrive host URL'   (Lzd822b2c403 ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lz9ab7ea5d0c 'Team site base URL'  (Lzd822b2c403 ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lz9ab7ea5d0c 'Default site URL'    (Lzd822b2c403 ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lz9ab7ea5d0c 'Email / UPN domain'  (Lzd822b2c403 ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lz943a1dad50 -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lz1cb0b686e7 -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz493e497df6 $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lzdcd96c40e0 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lz50687bf3fe }
        ...
        try {
            ...
            else { $tok=Lz1cb0b686e7 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lzdcd96c40e0 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lz943a1dad50 -Name 'DattoEndpointUrl' -Value $ep
            Lz943a1dad50 -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lz943a1dad50 -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lz943a1dad50 -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lz943a1dad50 -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lz943a1dad50 -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lz943a1dad50 -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lz943a1dad50 -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lz943a1dad50 -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lz9e7bebb284 -Value $script:AS.pbSec.Password
            }
...
function Lz57473dba79 { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lze618e9779f { param($Cfg,[string]$Path,$Value)
...
function Lz37ac9a27d0 {
    ...
    $curMax=Lz57473dba79 -Cfg $cfg -Path 'run.tuning.maxFileSizeMB'; if ($null -eq $curMax) { $curMax=0 }; $sizeBox.Text="$curMax"
    ...
    $curInc=@(Lz57473dba79 -Cfg $cfg -Path 'run.tuning.includePatterns' | Where-Object { "$_".Trim() })
    $curExc=@(Lz57473dba79 -Cfg $cfg -Path 'run.tuning.excludePatterns' | Where-Object { "$_".Trim() })
    ...
    $afterPick=New-Object System.Windows.Controls.DatePicker; $afterPick.Margin='0,4,0,4'; $afterPick.Width=150; $afterPick.SelectedDate=(& $toDate (Lz57473dba79 -Cfg $cfg -Path 'run.tuning.modifiedAfter'))
    ...
    $beforePick=New-Object System.Windows.Controls.DatePicker; $beforePick.Margin='0,4,0,4'; $beforePick.Width=150; $beforePick.SelectedDate=(& $toDate (Lz57473dba79 -Cfg $cfg -Path 'run.tuning.modifiedBefore'))
    ...
    $save.Add_Click({
        try {
            ...
            Lze618e9779f -Cfg $script:FltCfg -Path 'run.tuning.maxFileSizeMB' -Value $sv
            $pats=@("$($script:FltPat.Text)" -split '[;,\r\n]' | ForEach-Object { $_.Trim() } | Where-Object { $_ })
            if ($script:FltInc.IsChecked) { Lze618e9779f -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value $pats; Lze618e9779f -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value @() }
            else                          { Lze618e9779f -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value $pats; Lze618e9779f -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value @() }
            ...
            Lze618e9779f -Cfg $script:FltCfg -Path 'run.tuning.modifiedAfter'  -Value $ma
            Lze618e9779f -Cfg $script:FltCfg -Path 'run.tuning.modifiedBefore' -Value $mb
...
function Lz14bff391aa {
    ...
    function Lz2e592333c6 { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec,[string]$Type='text')
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lz57473dba79 -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lz2e592333c6 -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec -Type $s.Type
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lze618e9779f -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lze618e9779f -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lze618e9779f -Cfg $script:SetCfg -Path $s.Path -Value ([bool]$script:SetBoxes[$s.Path].IsChecked) }
                elseif ($s.Type -eq 'date') {
                    ...
                    Lze618e9779f -Cfg $script:SetCfg -Path $s.Path -Value $raw
                }
                else { Lze618e9779f -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lz728e7f775a {
    ...
    function Lz1990146dd1 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lz825a092477 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz1990146dd1 -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lz1990146dd1 -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lz1990146dd1 -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lz1990146dd1 -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lzdcd96c40e0 $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lz50687bf3fe }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz943a1dad50 -Name 'DattoEndpointUrl' -Value $ep
            Lz943a1dad50 -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lz9e7bebb284 -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz07f542dfcf {
    ...
    $pre = Lz3812636103 -Title 'Prerequisites'
    ...
    $dat = Lz3812636103 -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lz62a0027849 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lz3812636103 -Title 'Microsoft 365' -Buttons @($bMs, (Lz62a0027849 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lz3812636103 -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lz728e7f775a } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzc8737805c0; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lz29f406ac8f } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzc8737805c0; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lz98c1960315 } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzc8737805c0; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lzc8737805c0
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lzfc6508b624 {
...
function Lz6afab30f41 {
    ...
    $bOpen.Add_Click({ try { Lz83a679d8f6 -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lz97a9b39ce7 {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lz07459e48be {
    ...
    Lz6afab30f41 -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lz83a679d8f6 -Url $pageUrl
    ...
    } finally { Lz97a9b39ce7 }
}
function Lz9c45a1e838 {
    try {
...
function Lzc4df292a4a {
    if (Lzd4cb0ddb78) { return $false }
...
function Lz74f842de8e {
    ...
    if (-not (Get-Module Microsoft.Graph.Authentication -ListAvailable)) {
        Lzfc6508b624 'The Microsoft Graph module is not installed. Re-run the installer (or run: Install-Module Microsoft.Graph.Authentication -Scope CurrentUser) and try again.' '#B42318' -Strong
        return
    }
    Lzfc6508b624 'Preparing the Microsoft 365 sign-in. The first time on a new install can take up to a minute while Windows loads the components - please wait, the app has not frozen.' '#475467' -Strong
    try { Import-Module Microsoft.Graph.Authentication -ErrorAction Stop } catch {
        Lzfc6508b624 "Could not load the Microsoft Graph module: $($_.Exception.Message)" '#B42318' -Strong
        return
    }
    Lzfc6508b624 "Sign in with an account that can manage Microsoft 365 (a Global Administrator is simplest). The first time, Microsoft asks you to accept the permissions this setup itself needs; that is expected.$(if ($ForEmail) { ' Email alerts are included, so expect TWO sign-ins: this one, then one for Exchange Online (a separate service with its own sign-in).' })"
    ...
    if (Lzc4df292a4a) {
        Lzfc6508b624 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B5460F' -Strong
        try {
            Lzb57a0f35d3 -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lzfc6508b624 "The browser sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B5460F' -Strong
        }
    } else {
        Lzfc6508b624 'Using the code sign-in (the reliable path): a small window will show your code to type at microsoft.com/devicelogin, or on your phone. It stays on top and closes itself when you are done.' '#B5460F' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lz07459e48be -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lzfc6508b624 $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lzccc9205723 -Token $at -Claim 'tid'
            $wizTokAcct = Lzccc9205723 -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lzccc9205723 -Token $at -Claim 'preferred_username' }
            ...
            Lzfc6508b624 "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lzfc6508b624 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lzfc6508b624 "Signed in to tenant $tid as $wizAcct." '#0E9F6E'
    $newThumb = $null
    try {
        Lzfc6508b624 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lzfc6508b624 "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B5460F' }
            Lzfc6508b624 "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#0E9F6E'
        } else {
            Lzfc6508b624 "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lzfc6508b624 'App registration created.' '#0E9F6E'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lzfc6508b624 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lzfc6508b624 "Certificate created in this user's store. Thumbprint $newThumb." '#0E9F6E'
        Lzfc6508b624 'Uploading the certificate''s public key to the app registration...'
        ...
        Lzfc6508b624 "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#0E9F6E'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lz83eda541b3 -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lzfc6508b624 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lz83eda541b3 -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lzfc6508b624 $consentMsg '#0E9F6E'
        Lz943a1dad50 -Name 'TenantId'       -Value $tid
        Lz943a1dad50 -Name 'GraphClientId'  -Value $appId
        Lz943a1dad50 -Name 'CertThumbprint' -Value $newThumb
        Lzfc6508b624 'Connection details saved on this computer.' '#0E9F6E'
    } catch {
        ...
        if ($friendly) { Lzfc6508b624 $friendly '#B42318' -Strong; Lzfc6508b624 "Technical detail: $m" }
        else { Lzfc6508b624 "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lzfc6508b624 "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lzfc6508b624 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lz1cb0b686e7 -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lz943a1dad50 -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lzfe1b629475 $spUrl
            if ($der) {
                Lz943a1dad50 -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lz943a1dad50 -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lz943a1dad50 -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lzfc6508b624 "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#0E9F6E'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lz83eda541b3 -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lzfc6508b624 "The new sign-in has not become active yet: $($_.Exception.Message)" '#B5460F' -Strong
                   Lzfc6508b624 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz493e497df6 $org
            if ($u) { Lz943a1dad50 -Name 'UpnDomain' -Value $u; Lzfc6508b624 "Email domain detected: $u" '#0E9F6E' }
        } catch { Lzfc6508b624 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lzfc6508b624 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#0E9F6E' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lz29f406ac8f {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lz9c04de8cc0 $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lz74f842de8e -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lzeaf9a3cda5) {
                    Lzfc6508b624 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lzfc6508b624 $t $c } else { Lzfc6508b624 $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lzfc6508b624 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B5460F'
                }
            }
            ...
            Lzfc6508b624 "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lz07459e48be -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lzb57a0f35d3 -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lz83a679d8f6 -Url 'https://login.microsoft.com/device'
                Lzeedd574e10
                try { Lzb57a0f35d3 -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lz5feca07c1b }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (Lzc4df292a4a) {
            ...
            try {
                Lzb57a0f35d3 -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lz07459e48be -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lzccc9205723 -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lzccc9205723 -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lz83eda541b3 -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lz5c71828fbd 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lz83eda541b3 -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lz83eda541b3 -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lz943a1dad50 -Name 'EmailSender' -Value $senderAddr
        Lz943a1dad50 -Name 'EmailEnabled' -Value '1'
...
function Lz98c1960315 {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lz9f151124d8 { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lzf69a384dc4 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lzf69a384dc4 -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lzf69a384dc4 -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lzf69a384dc4 -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lz9f151124d8 'Subject and attachments'
    ...
    [void](Lzf69a384dc4 -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz0117e89e77 -Template $script:ES.tbSubj.Text -Vars (Lz8f78181a17)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz0117e89e77 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz8f78181a17)
            $tok = Lz1cb0b686e7 -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz943a1dad50 -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lz943a1dad50 -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lz943a1dad50 -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lz943a1dad50 -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lz943a1dad50 -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lz943a1dad50 -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lz943a1dad50 -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lz943a1dad50 -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lz943a1dad50 -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lz943a1dad50 -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lz943a1dad50 -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lz943a1dad50 -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lz735e070e57 {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lz07459e48be -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lzf228c0ff18 {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lz735e070e57 -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lzc8737805c0 } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lz5409d77682 {
    return ((Lzeaf3822796) -and (Lzeaf9a3cda5))
}
Lzbba9453c03
Lz5feca07c1b
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lzb4950d8c68 "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lz5409d77682)) {
        try { Lz07f542dfcf } catch {}
        if (Lz5409d77682) { Lz3e031df1c0 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lz3e031df1c0 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lz3e031df1c0 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ($ctrl.GettingStarted -and $ctrl.GettingStarted.Visibility -eq 'Visible') {
        try { Lze75f5e6d76 ([bool](Lz5409d77682)) } catch {}
    }
    if ((Lz5409d77682) -and -not (Test-Path (Lz48281b00c8))) {
        try { Lz7b0d0ad589 } catch {}
    }
