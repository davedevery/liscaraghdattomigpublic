...
function Lz8e4877cd99 { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lz13650d8934 {
...
function Lz1ad5dac711 {
    ...
    if (-not $s) { $s = Lz723cc3bb6c 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lz27b421c8f0 {
    param([string]$Value)
    if (Lz13650d8934 -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lzcd6b4eac4c {
    ...
    return (Lz1ffbe8dbf6 -TargetSubFolder $box -SourceSubPath $src)
}
function Lzb0afd4930b {
    ...
    try {
        ...
        $norm = Lz1ffbe8dbf6 -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lz6cc3accfa7 "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz78d709f4c4
    } catch { }
}
function Lz1ffbe8dbf6 {
...
function Lz35c07ab824 {
    ...
    return @{ SubfolderCount = @(Lza646bf387b -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lzfe14f4347c {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lz828cd09aae -Path "sites/$siteId"
    } catch { return $null }
}
function Lzc2db838e9f {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lz828cd09aae -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lz35eaa800c9 {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lz828cd09aae -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz1f324ca46d {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lz6767fc24f2 -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lzc2db838e9f -Search $s.Txt.Text)
...
"@
    ...
    Lz1f324ca46d
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lz1f324ca46d } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lz1f324ca46d } })
    $new.Add_Click({
        $n = Lz512f35b755 -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lz251fc31687 -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lz1f324ca46d; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lz3550bdf737 -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lz21e5d31103 -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lz3550bdf737 -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lz2f9173503a {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lza646bf387b -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lz2f9173503a
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lz2f9173503a } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lz2f9173503a } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lz3550bdf737 -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lz21e5d31103 -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lz3550bdf737 -State $script:SP
    $r = $w.ShowDialog()
...
function Lz6cc3accfa7 { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz430e7bf7f6 { param([TimeSpan]$ts)
...
function Lz01d4c6aa29 { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lzf767f35ee3 {
    ...
    if ($hasRegime) {
        $etaA = Lz21f4354976 -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz21f4354976 -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lz21f4354976 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz21f4354976 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lz47b197d644 {
    ...
    try {
        ...
        $etaText = Lzf767f35ee3 -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lzbea7c280c0 {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lz048cda969a -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lzef5e631a08 -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lz47b197d644 -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lzdcb15eef2a $Line) + "`r`n")
...
function Lz292dccf12d { param([string]$Text) Lzbea7c280c0 $Text }
function Lz5fee544438 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lzbc5a7d27f0 $true 'Connecting to Microsoft 365...'
        Lza5100a4c12
    } catch {
        ...
        Lzbc5a7d27f0 $false; return
    } finally { Lzbc5a7d27f0 $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lz135b11a95d -StartSearch $seed
    if ($picked) {
        ...
        Lz6cc3accfa7 "Site set to $picked"
        Lz78d709f4c4
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lz5fee544438 }
    Lz78d709f4c4
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lz5a772491ca }
    Lz78d709f4c4
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz78d709f4c4 })
$ctrl.TxtFolder.Add_TextChanged({ Lz78d709f4c4 })
$ctrl.TxtSourceSub.Add_TextChanged({ Lz78d709f4c4 })
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz78d709f4c4 }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz78d709f4c4 }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lzad34ab1363 -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lzb0afd4930b })
$ctrl.TxtSourceSub.Add_LostFocus({ Lzb0afd4930b })
$ctrl.TxtLoc.Add_TextChanged({ Lz78d709f4c4 })
$ctrl.TxtLib.Add_TextChanged({ Lz78d709f4c4 })
$script:NestSuspend = $false
function Lzfe219132aa {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lz4d04fa0c9d -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lz6cc3accfa7 $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lzfe219132aa })
$ctrl.ChkNest.Add_Unchecked({ Lzfe219132aa })
$script:ProjectRows = @()
$script:RunStatus = @{}
function Lz8d797ff200 {
...
function Lz20dbc3e396 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lz35c07ab824 -ProjectId $p.Id -SubPath $sub) }
...
function Lze7397d5bd0 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lz828cd09aae -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lze3f9c7d46d {
    ...
    try { Lzbc5a7d27f0 $true 'Checking source folders...'; $srcOk = Lz20dbc3e396 } finally { Lzbc5a7d27f0 $false }
    if (-not $srcOk) { return }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lzbc5a7d27f0 $true 'Checking destinations...'; $dstOk = Lze7397d5bd0 } finally { Lzbc5a7d27f0 $false }
        if (-not $dstOk) { return }
    }
        ...
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lzbea7c280c0 $lines[$k] } catch {} }
                }
            }
        } catch {}
        $script:SpinIdx = ($script:SpinIdx + 1) % 4
        $spin = @('|','/','-','\')[$script:SpinIdx]
        $ctrl.LblElapsed.Text = "$spin  Elapsed " + (Lz430e7bf7f6 ((Get-Date) - $script:RunStart))
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lzbea7c280c0 $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lzbea7c280c0 $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Lz430e7bf7f6 ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lzcdad0b5cd1) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lz82d990ac58 -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lzbea7c280c0 "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lzbea7c280c0 "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lz6cc3accfa7 ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
...
function Lz9e16a288e9 { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lz19bdea3d74 { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lz9731ca8395 {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lz36147ee895 -ConfigFile $s.Tag; Lz7022bb1720 } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lz9998c74769 { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lz8e4877cd99 -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lz9731ca8395
}
function Lze87c6f93f2 {
...
function Lz36147ee895 {
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lz19bdea3d74 $true
    Lz9e16a288e9 $true
    Lz9998c74769 $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lze87c6f93f2 -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lz6cc3accfa7 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lz512f35b755 -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Test-Path $jobDir) { (Show-Msg -Text ("A job folder '$slug' already exists. Open it instead, or choose another name.")); return }
        ...
        Lz4d04fa0c9d -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lz36147ee895 -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz766affb6e6 })
$ctrl.MnuSettingsApi.Add_Click({ Lza91f7a3ab4 })
$ctrl.MnuSettingsWizard.Add_Click({ Lz8696d839c1 })
$ctrl.MnuSettingsEmail.Add_Click({ Lz78a9159024 })
$ctrl.MnuSettingsTuning.Add_Click({ Lz19a05cd076 })
$ctrl.MnuSettingsDecommission.Add_Click({ Lz0ff3345aae })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lz96bf5867c8 -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lz67d6f684aa {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lz723cc3bb6c 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lz67d6f684aa "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lz67d6f684aa (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lzbc5a7d27f0 $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lzbc5a7d27f0 $false }
    if (-not $b) { return }
...
function Lzd63b986f74 { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lz82e8a6ff53 {
    $path = Lzd63b986f74
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Datto Workplace to SharePoint Migrator" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lz82e8a6ff53)) -Caption ('Licence')) | Out-Null })
function Lz1ce8e43663 {
...
function Lzc02c7a501d {
    param([string]$For = 'copy')
    if (Test-Path (Lzd63b986f74)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        (Show-Msg -Text ("Licence installed.`n`n" + (Lz82e8a6ff53)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-UpdateCheck {
    try {
        ...
        Lzbc5a7d27f0 $true 'Checking for updates...'
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lzbc5a7d27f0 $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lz96bf5867c8 -Title 'About' -Heading 'Datto Workplace to SharePoint Migrator' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lz1ad5dac711 } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lz2ecb46393f -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lz512f35b755 -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lzd4c38cae48 {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lz19bdea3d74 $false
    Lz9e16a288e9 $false
}
$ctrl.MnuJobClose.Add_Click({ Lzd4c38cae48; Lz6cc3accfa7 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lzd4c38cae48
    try { if ($dir -and (Test-Path $dir)) { Remove-Item $dir -Recurse -Force -ErrorAction Stop } ; Lz9731ca8395; Lz6cc3accfa7 "Deleted job '$nm'" }
    catch { (Show-Msg -Text ("Could not fully delete: $($_.Exception.Message)")) }
})
function Lzc75c1df0ea {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lzbc5a7d27f0 $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lz292dccf12d 'Signing in to Datto and listing projects...'
        $script:Projects = Lzb6c7e40c36
        Lz292dccf12d "Found $($script:Projects.Count) project(s)."
        Lz7e60bfdc00
        try { Lza5100a4c12; Lz292dccf12d 'Connected to Microsoft 365.' } catch { Lz292dccf12d 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lz292dccf12d "Technical detail: $($_.Exception.Message)" }
        Update-ProjectList
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lz6cc3accfa7 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lz292dccf12d 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lz292dccf12d "Technical detail: $($_.Exception.Message)"
            Lz6cc3accfa7 "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lz6cc3accfa7 'Connect failed'
        }
    }
    finally { Lzbc5a7d27f0 $false }
}
$ctrl.BtnConnect.Add_Click({ Lzc75c1df0ea })
function Lz7022bb1720 {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lz1ad5dac711 } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lz6cc3accfa7 "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lzc75c1df0ea -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lz284549ab90
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lz65c052361b).Count } catch {}
...
function Lz3b4b124ba6 {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lzcd6b4eac4c) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lzcd6b4eac4c) }
}
$ctrl.BtnApply.Add_Click({
    ...
    Lz6cc3accfa7 "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lz967d7f196d })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lzb7d12b9acf -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Update-ProjectList; [void](Write-MappingQuiet); Lz6cc3accfa7 "Set $($sel.Count) project(s) to skip"; return }
    $d = Lz3b4b124ba6
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lz5fee544438); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Update-ProjectList
    [void](Write-MappingQuiet)
    Lz6cc3accfa7 "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lzb7d12b9acf -Count (@($script:Projects).Count))) { return }
    $d = Lz3b4b124ba6
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Update-ProjectList; [void](Write-MappingQuiet); Lz6cc3accfa7 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Update-ProjectList
    [void](Write-MappingQuiet)
    Lz6cc3accfa7 "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lzbc5a7d27f0 $true 'Looking up libraries...'
        $libs = Lz732664dc0a -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lzbc5a7d27f0 $false; return }
    finally { Lzbc5a7d27f0 $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz0b543ef1e9 -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lz6cc3accfa7 "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lz6cc3accfa7 "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lzbc5a7d27f0 $true 'Opening destination...'
        Lza5100a4c12
        $res = Lz35eaa800c9
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lzbc5a7d27f0 $false; return }
    finally { Lzbc5a7d27f0 $false }
    $picked = Lz2bd42891d5 -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lz6cc3accfa7 ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnBrowseSource) { $ctrl.BtnBrowseSource.Add_Click({
    ...
    try { $picked = Lza3370d2970 -ProjectId $p.Id -ProjectName $p.Name }
    catch { (Show-Msg -Text ("Could not open the project's folders: $($_.Exception.Message)") -Caption ('Browse source')); return }
    if ($null -ne $picked) { $ctrl.TxtSourceSub.Text = $picked; Lz6cc3accfa7 ("Source set to " + $(if ($picked) { "/$picked" } else { 'the whole project' })) }
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    try {
        Lzbc5a7d27f0 $true 'Checking the source folder in Datto...'
        $res = Lz35c07ab824 -ProjectId $p.Id -SubPath $sub
    } catch { $ctrl.LblSourceCheck.Text = "Source NOT found. $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lzbc5a7d27f0 $false; return }
    finally { Lzbc5a7d27f0 $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lzbc5a7d27f0 $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lz828cd09aae -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lz732664dc0a -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lz78d709f4c4 {
    ...
    $srcLabel = Lzd92d4e4d6a -Project $p
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lzcd6b4eac4c) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lz65c052361b).Count } catch { }
...
function Lz6102b43918 {
    ...
    $script:Map[$p.Id] = $d
    Update-ProjectList
    Lz6cc3accfa7 "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lz292dccf12d "Saved $n mapping(s)."
    return $true
}
function Lz7e60bfdc00 {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lz292dccf12d "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lz292dccf12d "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lze3f9c7d46d -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lze3f9c7d46d -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lzefd35c62ab {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lzefd35c62ab) }
    ...
    if (-not (Lz96bf5867c8 -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lzc02c7a501d)) { return }
    Lze3f9c7d46d -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lzd20081a82e {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lzefd35c62ab) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lzc02c7a501d)) { return }
    Lze3f9c7d46d -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lz96bf5867c8 -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lzc02c7a501d -For 'verify')) { return }
    Lze3f9c7d46d -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lze3f9c7d46d -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lzcdad0b5cd1 {
...
function Lzf4638b5f95 {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lzbb66f118ce {
    param([string]$Name)
...
function Lz67086dbea9 {
    ...
    foreach ($f in $files) { $l = Lzbb66f118ce $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lz67086dbea9 -Folder (Lzcdad0b5cd1) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnOpenAudit.Add_Click({ Lz67086dbea9 -Folder (Lzf4638b5f95) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lz9e8a4e124f {
    ...
    Lz6cc3accfa7 "Speed limit set: up $uTxt, down $dTxt"
    Lzbea7c280c0 "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lz9e8a4e124f })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz9e8a4e124f } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz9e8a4e124f } })
function Lz44d26892a7 {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lz44d26892a7 -ParentId ([int]$_.ProcessId) } } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    Lzbea7c280c0 "----- STOP requested by user at $stamp -----"
    Lz6cc3accfa7 "Stopping $($script:CurLabel)..."
    try { Lz44d26892a7 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lze87c6f93f2 -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lzbea7c280c0 "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lzd75d36c232 {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lze3f9c7d46d -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lzbea7c280c0 "----- PAUSE requested by user at $stamp -----"
    Lz6cc3accfa7 "Pausing $($script:CurLabel)..."
    try { Lz44d26892a7 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lze87c6f93f2 -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lzbea7c280c0 "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lz0ae049ce56 { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lz023e907ef6 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lzbe8b4bb946 {
...
function Lz2ecb46393f {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lzfc4f7d6ac5 -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lza91f7a3ab4 {
    ...
    function Lz2061a7619b { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz0b5ce7115f { param($Label,$Control,$Info)
    ...
    function Lz3ce18d92c6 { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lz2061a7619b 'Datto Workplace'
    ...
    $parts = Lz023e907ef6 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz0b5ce7115f 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lz0b5ce7115f 'Cell (number)' (Lz3ce18d92c6 ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lz0b5ce7115f 'Datto Client ID' (Lz3ce18d92c6 ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lz0b5ce7115f 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lz2061a7619b 'Microsoft 365'
    $tbTid = Lz0b5ce7115f 'Tenant ID' (Lz3ce18d92c6 ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lz0b5ce7115f 'App (Client) ID' (Lz3ce18d92c6 ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lz2061a7619b 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lz0b5ce7115f 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lz0b5ce7115f '.pfx file' (Lz3ce18d92c6 '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lz0b5ce7115f '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lz2061a7619b 'SharePoint and OneDrive'
    ...
    $tbSp  = Lz0b5ce7115f 'SharePoint root URL' (Lz3ce18d92c6 ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lz0b5ce7115f 'OneDrive host URL'   (Lz3ce18d92c6 ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lz0b5ce7115f 'Team site base URL'  (Lz3ce18d92c6 ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lz0b5ce7115f 'Default site URL'    (Lz3ce18d92c6 ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lz0b5ce7115f 'Email / UPN domain'  (Lz3ce18d92c6 ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lz8e4877cd99 -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lz2ecb46393f -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz58fce0c840 $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lz0ae049ce56 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lz1ad5dac711 }
        ...
        try {
            ...
            else { $tok=Lz2ecb46393f -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lz0ae049ce56 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lz8e4877cd99 -Name 'DattoEndpointUrl' -Value $ep
            Lz8e4877cd99 -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lz8e4877cd99 -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lz8e4877cd99 -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lz8e4877cd99 -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lz8e4877cd99 -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lz8e4877cd99 -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lz8e4877cd99 -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lz8e4877cd99 -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lz27b421c8f0 -Value $script:AS.pbSec.Password
            }
...
function Lzc2221d4eff { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lz4d04fa0c9d { param($Cfg,[string]$Path,$Value)
...
function Lz19a05cd076 {
    ...
    function Lz67749f1e52 { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec)
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lzc2221d4eff -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lz67749f1e52 -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lz4d04fa0c9d -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lz4d04fa0c9d -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lz4d04fa0c9d -Cfg $script:SetCfg -Path $s.Path -Value ([bool]($raw -match '^(1|true|yes|on)$')) }
                else { Lz4d04fa0c9d -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lzdbcb051591 {
    ...
    function Lz60a91cb6ea { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lz023e907ef6 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz60a91cb6ea -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lz60a91cb6ea -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lz60a91cb6ea -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lz60a91cb6ea -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lz0ae049ce56 $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lz1ad5dac711 }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz8e4877cd99 -Name 'DattoEndpointUrl' -Value $ep
            Lz8e4877cd99 -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lz27b421c8f0 -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz766affb6e6 {
    ...
    $pre = Lzaa44fa68a3 -Title 'Prerequisites'
    ...
    $dat = Lzaa44fa68a3 -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lz132a8511f3 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lzaa44fa68a3 -Title 'Microsoft 365' -Buttons @($bMs, (Lz132a8511f3 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lzaa44fa68a3 -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lzdbcb051591 } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz09c533fc95; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lz8696d839c1 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz09c533fc95; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lz78a9159024 } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz09c533fc95; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lz09c533fc95
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lz8c897eb942 {
...
function Lzb925c6ada8 {
    ...
    $bOpen.Add_Click({ try { Lze4803065e1 -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lza56e28e464 {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lzffeee9ac18 {
    ...
    Lzb925c6ada8 -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lze4803065e1 -Url $pageUrl
    ...
    } finally { Lza56e28e464 }
}
function Lz6e73e3d72e {
    try {
...
function Lzed148ce9d2 {
    ...
    if (-not (Lzac658de9ec)) {
        Lz8c897eb942 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B54708' -Strong
        try {
            Lz89a2fd1dd7 -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lz8c897eb942 "The standard sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B54708' -Strong
        }
    } else {
        Lz8c897eb942 'Using the CODE sign-in (the reliable path on this machine): a small window will show your code, always on top.' '#B54708' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lzffeee9ac18 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lz8c897eb942 $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lzb7ff5cb973 -Token $at -Claim 'tid'
            $wizTokAcct = Lzb7ff5cb973 -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lzb7ff5cb973 -Token $at -Claim 'preferred_username' }
            ...
            Lz8c897eb942 "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lz8c897eb942 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lz8c897eb942 "Signed in to tenant $tid as $wizAcct." '#12B76A'
    $newThumb = $null
    try {
        Lz8c897eb942 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lz8c897eb942 "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B54708' }
            Lz8c897eb942 "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#12B76A'
        } else {
            Lz8c897eb942 "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lz8c897eb942 'App registration created.' '#12B76A'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lz8c897eb942 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lz8c897eb942 "Certificate created in this user's store. Thumbprint $newThumb." '#12B76A'
        Lz8c897eb942 'Uploading the certificate''s public key to the app registration...'
        ...
        Lz8c897eb942 "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#12B76A'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lz24dc6f1d3c -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lz8c897eb942 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lz24dc6f1d3c -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lz8c897eb942 $consentMsg '#12B76A'
        Lz8e4877cd99 -Name 'TenantId'       -Value $tid
        Lz8e4877cd99 -Name 'GraphClientId'  -Value $appId
        Lz8e4877cd99 -Name 'CertThumbprint' -Value $newThumb
        Lz8c897eb942 'Connection details saved on this computer.' '#12B76A'
    } catch {
        ...
        if ($friendly) { Lz8c897eb942 $friendly '#B42318' -Strong; Lz8c897eb942 "Technical detail: $m" }
        else { Lz8c897eb942 "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lz8c897eb942 "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lz8c897eb942 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lz2ecb46393f -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lz8e4877cd99 -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lzbe8b4bb946 $spUrl
            if ($der) {
                Lz8e4877cd99 -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lz8e4877cd99 -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lz8e4877cd99 -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lz8c897eb942 "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#12B76A'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lz24dc6f1d3c -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lz8c897eb942 "The new sign-in has not become active yet: $($_.Exception.Message)" '#B54708' -Strong
                   Lz8c897eb942 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz58fce0c840 $org
            if ($u) { Lz8e4877cd99 -Name 'UpnDomain' -Value $u; Lz8c897eb942 "Email domain detected: $u" '#12B76A' }
        } catch { Lz8c897eb942 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lz8c897eb942 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#12B76A' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lz8696d839c1 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lz6de3316385 $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lzed148ce9d2 -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lze9cae36062) {
                    Lz8c897eb942 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lz8c897eb942 $t $c } else { Lz8c897eb942 $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lz8c897eb942 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B54708'
                }
            }
            ...
            Lz8c897eb942 "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lzffeee9ac18 -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lz89a2fd1dd7 -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lze4803065e1 -Url 'https://login.microsoft.com/device'
                Lz87985f8a4d
                try { Lz89a2fd1dd7 -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lz739f124823 }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (-not (Lzac658de9ec)) {
            ...
            try {
                Lz89a2fd1dd7 -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lzffeee9ac18 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lzb7ff5cb973 -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lzb7ff5cb973 -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lz24dc6f1d3c -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lz7f10dda598 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lz24dc6f1d3c -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lz24dc6f1d3c -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lz8e4877cd99 -Name 'EmailSender' -Value $senderAddr
        Lz8e4877cd99 -Name 'EmailEnabled' -Value '1'
...
function Lz78a9159024 {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lz5a860dc16e { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lzdc395d400f { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lzdc395d400f -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lzdc395d400f -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lzdc395d400f -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lz5a860dc16e 'Subject and attachments'
    ...
    [void](Lzdc395d400f -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz60675acffe -Template $script:ES.tbSubj.Text -Vars (Lza1b36d3ffb)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz60675acffe -Template ("$($script:ES.tbSubj.Text)") -Vars (Lza1b36d3ffb)
            $tok = Lz2ecb46393f -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz8e4877cd99 -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lz8e4877cd99 -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lz8e4877cd99 -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lz8e4877cd99 -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lz8e4877cd99 -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lz8e4877cd99 -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lz8e4877cd99 -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lz8e4877cd99 -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lz8e4877cd99 -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lz8e4877cd99 -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lz8e4877cd99 -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lz8e4877cd99 -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lz8b0a9fa55c {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lzffeee9ac18 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lz0ff3345aae {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lz8b0a9fa55c -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lz09c533fc95 } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lz64c8d1613f {
    return ((Lz7f40ba3f57) -and (Lze9cae36062))
}
Lzabd8102235
Lz739f124823
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lzbea7c280c0 "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lz64c8d1613f)) {
        try { Lz766affb6e6 } catch {}
        if (Lz64c8d1613f) { Lz6cc3accfa7 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lz6cc3accfa7 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lz6cc3accfa7 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ((Lz64c8d1613f) -and -not (Test-Path (Lzd63b986f74))) {
        try { Lzb591254cfa } catch {}
    }
})
try {
    Add-Type -MemberDefinition '
