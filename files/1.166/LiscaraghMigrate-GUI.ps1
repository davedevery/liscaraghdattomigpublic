...
function Lz4d31ab0288 {
    param([string]$Severity)
    Lz430620cb65 -Severity $Severity
    try {
...
function Lzd6ce7b4979 { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lzf050ceb17e {
...
function Lz005eec74b9 {
    ...
    if (-not $s) { $s = Lz177435f608 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lz73b290318e {
    param([string]$Value)
    if (Lzf050ceb17e -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lzb2257d7b33 {
    $f = @(Lz98f895c341)
    if ($f.Count -eq 1) { return $f[0] }
    return ''
}
function Lz1d8e3eb236 {
...
function Lz2411a3b625 {
    param([string[]]$Picked)
    $result = New-Object System.Collections.Generic.List[string]
    foreach ($e in @(Lz98f895c341)) { [void]$result.Add($e) }
    $dropped = New-Object System.Collections.Generic.List[string]
    foreach ($p in @($Picked | ForEach-Object { "$_".Trim().Trim('/').Trim('\') } | Where-Object { $_ })) {
        if ($result -ccontains $p) { continue }
        $covered = $false
        foreach ($k in @($result)) { if (Lz1d8e3eb236 -Child $p -Parent $k) { [void]$dropped.Add($p); $covered = $true; break } }
        if ($covered) { continue }
        foreach ($narrow in @($result | Where-Object { Lz1d8e3eb236 -Child $_ -Parent $p -Strict })) { [void]$result.Remove($narrow); [void]$dropped.Add($narrow) }
        [void]$result.Add($p)
    }
    Lz062fbabf49 -Folders @($result)
    if ($dropped.Count) { Lz96d2c2103d ("Overlapping folder(s) merged, kept the broader one: " + ((@($dropped) | Select-Object -Unique) -join ', ')) }
}
function Lzb326639526 {
    $one = $true; try { $one = (@(Lz409d07dcce).Count -le 1) } catch {}
    $n = @(Lz98f895c341).Count
...
function Lz59566b8ed6 {
    ...
    try {
        ...
        $norm = Lz2d38d716a0 -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lz96d2c2103d "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz205b30abf9
    } catch { }
}
function Lz2d38d716a0 {
...
function Lz36d3ba3982 {
    ...
    return @{ SubfolderCount = @(Lzd6a500e1e5 -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lz443a7f2db6 {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lz08617ac67f -Path "sites/$siteId"
    } catch { return $null }
}
function Lz7741e5630a {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lz08617ac67f -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lz38ec7c1218 {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lz08617ac67f -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz61cd6e5234 {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lzc304675743 -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
function Lz0207f6d9e5 {
    param($State, [string]$Current)
    $out = New-Object System.Collections.Generic.List[string]
    foreach ($s in @($State.Lst.SelectedItems)) {
        $name = "$s"
        if (-not (Lz99b1a42e56 -Item $name)) { continue }
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lz7741e5630a -Search $s.Txt.Text)
...
"@
    ...
    Lz61cd6e5234
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lz61cd6e5234 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lz61cd6e5234 } })
    $new.Add_Click({
        $n = Lzcd8ef0f810 -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lz1ddd4f434f -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lz61cd6e5234; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lz50fca50947 -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lz4f433bc9c6 -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lz50fca50947 -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lzb761aa9a54 {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lzd6a500e1e5 -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lzb761aa9a54
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lzb761aa9a54 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lzb761aa9a54 } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lz50fca50947 -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lz0207f6d9e5 -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lz50fca50947 -State $script:SP
    $r = $w.ShowDialog()
...
function Lz96d2c2103d { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz81ce092f4c { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lz41b789b2f0 {
    ...
    if ($hasRegime) {
        $etaA = Lz0de0ac2d09 -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz0de0ac2d09 -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lz0de0ac2d09 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz0de0ac2d09 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lz70481266f9 {
    ...
    try {
        ...
        $etaText = Lz41b789b2f0 -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lz9424c21995 {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lz5fe7833006 -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lz97527cc1d3 -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lz70481266f9 -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lz53b4bf7ad3 $Line) + "`r`n")
...
function Lz043ca9e63b { param([string]$Text) Lz9424c21995 $Text }
function Lz904828de58 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lzdfb21d5afc $true 'Connecting to Microsoft 365...'
        Lz679eaaf45f
    } catch {
        ...
        Lzdfb21d5afc $false; return
    } finally { Lzdfb21d5afc $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lzf6a7f62567 -StartSearch $seed
    if ($picked) {
        ...
        Lz96d2c2103d "Site set to $picked"
        Lz205b30abf9
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lz904828de58 }
    Lz205b30abf9
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lzaec34a5362 }
    Lz205b30abf9
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz205b30abf9 })
$ctrl.TxtFolder.Add_TextChanged({ Lz205b30abf9 })
if ($ctrl.LstSource) { $ctrl.LstSource.Add_SelectionChanged({ Lzb326639526 }) }
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz205b30abf9 }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz205b30abf9 }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lz95c76e0bb6 -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lz59566b8ed6 })
$ctrl.TxtLoc.Add_TextChanged({ Lz205b30abf9 })
$ctrl.TxtLib.Add_TextChanged({ Lz205b30abf9 })
$script:NestSuspend = $false
function Lz2854033782 {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lz750c905036 -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lz96d2c2103d $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lz2854033782 })
$ctrl.ChkNest.Add_Unchecked({ Lz2854033782 })
$script:ProjectRows = @()
$script:RunStatus = @{}
...
function Lz0dd9d6abf3 {
    ...
    if (-not $hasProjects) {
        $sd = $false
        if (Get-Command Test-ConnectionConfigured -ErrorAction SilentlyContinue) { try { $sd = [bool](Lz07013246ac) } catch {} }
        Lz4b80fc04b2 $sd
    }
...
function Lza82c52830d {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lz36d3ba3982 -ProjectId $p.Id -SubPath $sub) }
...
function Lz8814a2f50b {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lz08617ac67f -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz6d8193594a {
    ...
    if (($EngineArgs -contains 'Transfer') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck') -or ($EngineArgs -contains 'PreFlight')) {
        try { Lzdfb21d5afc $true 'Checking source folders...'; $srcOk = Lza82c52830d } finally { Lzdfb21d5afc $false }
        if (-not $srcOk) { return }
    }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lzdfb21d5afc $true 'Checking destinations...'; $dstOk = Lz8814a2f50b } finally { Lzdfb21d5afc $false }
        if (-not $dstOk) { return }
    }
    ...
    $script:Timer.Add_Tick({
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lz9424c21995 $lines[$k] } catch {} }
                }
            }
        } catch {}
        ...
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lz9424c21995 $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lz9424c21995 $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Format-Span ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lzd946da8580) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lza1a78da460 -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lz9424c21995 "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lz9424c21995 "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lz96d2c2103d ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
            ...
            Lzb816d7e7dc
            if (-not $script:Paused) { Lz4d31ab0288 -Severity $sev }
            if ($script:RunStatus.Count) { try { Lz0dd9d6abf3 } catch {} }
            try {
            ...
            try {
                if ($script:CurLabel -eq 'Check readiness' -or $script:CurLabel -eq 'Preview (no upload)') {
                    $newest = Get-ChildItem (Join-Path (Lzd946da8580) 'report-*.html') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
function Lz63078a65ab { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lz9179326e6d { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lzff1c6664c8 {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lz28f8f25c03 -ConfigFile $s.Tag; Lz1fd1b8e879 } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lz8cdcb82e4a { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lzd6ce7b4979 -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lzff1c6664c8
}
function Lz9cb1a8c796 {
...
function Lz28f8f25c03 {
    param([string]$ConfigFile)
    if ($script:Sched) { Lz8023503067 -Why 'Schedule cancelled (job changed).' }
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lz9179326e6d $true
    Lz63078a65ab $true
    Lz8cdcb82e4a $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lz9cb1a8c796 -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lz96d2c2103d 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
}
function Lzedebc5617e {
    param([string]$Dir)
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lzcd8ef0f810 -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Lz30e83a97db -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Open it instead, or choose another name.")); return }
    if ((Test-Path $jobDir) -and -not (Lzedebc5617e -Dir $jobDir)) {
        ...
        Lz750c905036 -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lz28f8f25c03 -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuJobOpen.Add_Click({
    ...
    [void]$w.ShowDialog()
    if ($script:OpenPick) { Lz28f8f25c03 -ConfigFile $script:OpenPick; Lz1fd1b8e879 }
})
$ctrl.MnuJobSave.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')); return }
    if (Save-Mapping) { Lz96d2c2103d 'Migration job saved.' }
})
$ctrl.MnuJobSaveAs.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open a migration job first, then Save As to copy it.')); return }
    try {
        $name = Lzcd8ef0f810 -Prompt "New name for the copy:" -Title 'Save job as'
        if (-not $name -or -not $name.Trim()) { return }
        $name = $name.Trim()
        $slug = ($name -replace '[^A-Za-z0-9._-]','_').Trim('_'); if (-not $slug) { $slug = 'job' }
        $jobDir = Join-Path $script:JobsRoot $slug
        if (Lz30e83a97db -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Choose another name.")); return }
        if ((Test-Path $jobDir) -and -not (Lzedebc5617e -Dir $jobDir)) {
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz8459b9cc9b })
$ctrl.MnuSettingsApi.Add_Click({ Lz1ae010c39d })
$ctrl.MnuSettingsWizard.Add_Click({ Lz8bddce2ee6 })
$ctrl.MnuSettingsEmail.Add_Click({ Lz726c93159a })
$ctrl.MnuSettingsTuning.Add_Click({ Lz191f9ca02c })
$ctrl.BtnFilters.Add_Click({ Lz67fa6d8dd3 })
$ctrl.MnuSettingsDecommission.Add_Click({ Lzc7cdc5b975 })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lzca63a50f53 -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lzbfbc84a97d {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lz177435f608 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lzbfbc84a97d "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lzbfbc84a97d (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lzdfb21d5afc $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lzdfb21d5afc $false }
    if (-not $b) { return }
...
function Lzba7c2be1ae { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lz1d82e85c13 {
    $path = Lzba7c2be1ae
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Liscaragh Migrate" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lz1d82e85c13)) -Caption ('Licence')) | Out-Null })
function Lz74b6635059 {
...
function Lz631a940945 {
    param([string]$For = 'copy')
    if (Test-Path (Lzba7c2be1ae)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        try {
            ...
            Lzd6ce7b4979 -Name 'LicenceAppliedId'  -Value "$($lpl.LicenceId)"
            Lzd6ce7b4979 -Name 'LicenceAppliedUtc' -Value ((Get-Date).ToUniversalTime().ToString('o'))
        } catch {}
        (Show-Msg -Text ("Licence installed.`n`n" + (Lz1d82e85c13)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-FullReinstall {
    ...
    Lzdfb21d5afc $true "Downloading the installer for version $($Payload.version)..."
    ...
    catch { Lzdfb21d5afc $false; (Show-Msg -Text ("The installer could not be downloaded, so nothing has been changed.`n`n$($_.Exception.Message)") -Caption ('Reinstall') -Icon ('Warning')) | Out-Null; return }
...
function Invoke-UpdateCheck {
    ...
    try {
        ...
        if (-not $Silent) { Lzdfb21d5afc $true 'Checking for updates...' }
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lzdfb21d5afc $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lzca63a50f53 -Title 'About' -Heading 'Liscaragh Migrate' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lz005eec74b9 } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lz307a1c445f -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lzcd8ef0f810 -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lzd94f17be5e {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lz9179326e6d $false
    Lz63078a65ab $false
}
$ctrl.MnuJobClose.Add_Click({ Lzd94f17be5e; Lz96d2c2103d 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lzd94f17be5e
    if (Lzedebc5617e -Dir $dir) {
        Lzff1c6664c8; Lz96d2c2103d "Deleted job '$nm'"
    } else {
        Lzff1c6664c8
        ...
        Lz96d2c2103d "Could not fully delete job '$nm' (a file is in use)"
    }
})
function Lz09947b2c31 {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lzdfb21d5afc $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lz043ca9e63b 'Signing in to Datto and listing projects...'
        $script:Projects = Lze516d5375f
        Lz043ca9e63b "Found $($script:Projects.Count) project(s)."
        Lz1641ce2bd0
        try { Lz679eaaf45f; Lz043ca9e63b 'Connected to Microsoft 365.' } catch { Lz043ca9e63b 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lz043ca9e63b "Technical detail: $($_.Exception.Message)" }
        Lz0dd9d6abf3
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lz96d2c2103d 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lz043ca9e63b 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lz043ca9e63b "Technical detail: $($_.Exception.Message)"
            Lz96d2c2103d "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lz96d2c2103d 'Connect failed'
        }
    }
    finally { Lzdfb21d5afc $false }
}
$ctrl.BtnConnect.Add_Click({ Lz09947b2c31 })
function Lz1fd1b8e879 {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lz005eec74b9 } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lz96d2c2103d "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lz09947b2c31 -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lz194c15dbcb
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lz409d07dcce).Count } catch {}
    ...
    if ($script:Map.ContainsKey($p.Id)) {
        ...
        $ctrl.TxtLoc.Text = Lzaec34a5362
        $ctrl.TxtLib.Text = ''; $ctrl.TxtFolder.Text = ''; Lz062fbabf49 -Folders @()
...
function Lzc05be3bc1e {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lzdef1b04174) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lzdef1b04174) }
}
$ctrl.BtnApply.Add_Click({
    $p = Lz194c15dbcb
    if (-not $p) { (Show-Msg -Text ('Select a project first.')); return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Remove($p.Id) | Out-Null; Lz0dd9d6abf3; [void](Write-MappingQuiet); return }
    $d = Lzc05be3bc1e
    if ($d -eq 'ERR') { return }
    ...
    Lz96d2c2103d "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lz06fc23677e })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lzeb8a61c206 -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Lz0dd9d6abf3; [void](Write-MappingQuiet); Lz96d2c2103d "Set $($sel.Count) project(s) to skip"; return }
    $d = Lzc05be3bc1e
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lz904828de58); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPaths=$keepSrcList; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Lz0dd9d6abf3
    [void](Write-MappingQuiet)
    Lz96d2c2103d "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lzeb8a61c206 -Count (@($script:Projects).Count))) { return }
    $d = Lzc05be3bc1e
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Lz0dd9d6abf3; [void](Write-MappingQuiet); Lz96d2c2103d 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Lz0dd9d6abf3
    [void](Write-MappingQuiet)
    Lz96d2c2103d "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lzdfb21d5afc $true 'Looking up libraries...'
        $libs = Lz92f1ba9f8d -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lzdfb21d5afc $false; return }
    finally { Lzdfb21d5afc $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz59f71d21c7 -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lz96d2c2103d "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lz96d2c2103d "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lzdfb21d5afc $true 'Opening destination...'
        Lz679eaaf45f
        $res = Lz38ec7c1218
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lzdfb21d5afc $false; return }
    finally { Lzdfb21d5afc $false }
    $picked = Lz288bade303 -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lz96d2c2103d ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnAddSource) { $ctrl.BtnAddSource.Add_Click({
    ...
    try { $picked = Lzab5fe7f25a -ProjectId $p.Id -ProjectName $p.Name }
    ...
    Lz2411a3b625 -Picked $arr
    Lz96d2c2103d ("Source folders: " + (@(Lz98f895c341) -join ', '))
}) }
if ($ctrl.BtnRemoveSource) { $ctrl.BtnRemoveSource.Add_Click({
    ...
    Lz062fbabf49 -Folders @(Lz98f895c341 | Where-Object { $sel -notcontains $_ })
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    $bad = New-Object System.Collections.Generic.List[string]
    try {
        Lzdfb21d5afc $true 'Checking the source folder(s) in Datto...'
        foreach ($s in $subs) { try { $null = Lz36d3ba3982 -ProjectId $p.Id -SubPath $s } catch { [void]$bad.Add($s) } }
    } catch { $ctrl.LblSourceCheck.Text = "Could not check the source: $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lzdfb21d5afc $false; return }
    finally { Lzdfb21d5afc $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lzdfb21d5afc $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lz08617ac67f -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lz92f1ba9f8d -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lz205b30abf9 {
    ...
    $srcLabel = Lz045dcf6a27 -Project $p
    $folders  = @(Lz98f895c341)
    $srcSub   = Lzb2257d7b33
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lzdef1b04174) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lz409d07dcce).Count } catch { }
...
function Lzcfe7e520e5 {
    ...
    $script:Map[$p.Id] = $d
    Lz0dd9d6abf3
    Lz96d2c2103d "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lz043ca9e63b "Saved $n mapping(s)."
    return $true
}
function Lz1641ce2bd0 {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lz043ca9e63b "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lz043ca9e63b "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lz6d8193594a -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lz6d8193594a -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lz47571ec71d {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lz47571ec71d) }
    ...
    if (-not (Lzca63a50f53 -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lz44a65b7e6d)) { return }
    if (-not (Lz631a940945)) { return }
    Lz6d8193594a -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lzf31e87f2e2 {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lz47571ec71d) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lz44a65b7e6d)) { return }
    if (-not (Lz631a940945)) { return }
    Lz6d8193594a -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnRerunFailed.Add_Click({
    ...
    if (-not (Lzca63a50f53 -Title 'Rerun failed files only' -Heading 'Rerun failed files only' -Sections $sections -Yes 'Rerun the failed files' -No 'Cancel')) { return }
    if (-not (Lz44a65b7e6d)) { return }
    if (-not (Lz631a940945)) { return }
    Lz6d8193594a -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','NewerWins','-Execute','-FailedOnly') -Label 'Rerun failed files only'
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lzca63a50f53 -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lz631a940945 -For 'verify')) { return }
    Lz6d8193594a -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lz6d8193594a -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lzd946da8580 {
...
function Lzb816d7e7dc {
    try { if ($ctrl.BtnRerunFailed) { $ctrl.BtnRerunFailed.IsEnabled = (Lzb4c2fa6adf) } } catch {}
}
function Lz202a664c3d {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lzec34b97dfd {
    param([string]$Name)
...
function Lz3b6dce793d {
    ...
    foreach ($f in $files) { $l = Lzec34b97dfd $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lz3b6dce793d -Folder (Lzd946da8580) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnCertificate.Add_Click({
    if (Save-Mapping) {
        Lz6d8193594a -EngineArgs @('-Action','Certificate') -Label 'Completion certificate' -OnComplete {
            $rr = Lzd946da8580
            $cert = $null
...
$ctrl.BtnOpenAudit.Add_Click({ Lz3b6dce793d -Folder (Lz202a664c3d) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lz4593e69a4f {
    ...
    Lz96d2c2103d "Speed limit set: up $uTxt, down $dTxt"
    Lz9424c21995 "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lz4593e69a4f })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz4593e69a4f } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz4593e69a4f } })
function Lz5e58ba8600 {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lz5e58ba8600 -ParentId ([int]$_.ProcessId) } } catch {}
...
function Lz4903055ea7 {
    $r = ($script:Proc -and -not $script:Proc.HasExited)
    $f = ($script:FinProc -and -not $script:FinProc.HasExited)
    return ($r -or $f)
}
function Lz5b93e64421 { param([string]$T) try { $ctrl.BtnSchedule.Content.Text = $T } catch {} }
function Lz8023503067 {
    param([string]$Why = '')
    if ($script:SchedTimer) { try { $script:SchedTimer.Stop() } catch {}; $script:SchedTimer = $null }
    $script:Sched = $null
    Lz5b93e64421 'Schedule for later / overnight...'
    if ($Why) { Lz96d2c2103d $Why }
}
function Lz1845f882ce {
    try { $p = Join-Path (Lzd946da8580) 'lastrun-outcome.json'; if (Test-Path $p) { return (Get-Content $p -Raw | ConvertFrom-Json) } } catch {}
    return $null
}
function Lz216c02c206 {
    if (-not $script:Proc -or $script:Proc.HasExited) { return }
    $script:QuietPausing = $true
    $ctrl.BtnStop.IsEnabled = $false
    Lz9424c21995 "----- Quiet hours reached at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')): pausing the copy. Files already uploaded are kept. -----"
    Lz96d2c2103d 'Quiet hours: pausing until the next window...'
    try { Lz5e58ba8600 -ParentId ([int]$script:Proc.Id) } catch {}
    Lz9cb1a8c796 -Status Cancelled -Note 'Finalising for quiet hours...' -OnDone {
        try { Lz9424c21995 '----- Paused for quiet hours. It will resume as a Sync at the next window while this app stays open. -----' } catch {}
    }
}
function Lz1b096b0248 {
    param([string[]]$RunArgs,[string]$Label)
    Lz9424c21995 "----- Scheduled: starting '$Label' at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')) -----"
    Lz6d8193594a -EngineArgs $RunArgs -Label $Label
    return ($script:Proc -and -not $script:Proc.HasExited)
}
function Lza9be710f1d { param([string]$Status)
    ...
    switch ("$($s.State)") {
        'waiting' {
            if ($now -ge $s.StartAt -and -not $busy) {
                ...
                elseif (Lz1b096b0248 -RunArgs $s.Args -Label $s.Label) { $s.State = 'running'; $s.Launches++ }
                else { Lz8023503067 -Why 'The scheduled run could not start (see the message). Schedule cancelled.' }
            } else {
                $left = $s.StartAt - $now; if ($left.Ticks -lt 0) { $left = [TimeSpan]::Zero }
                Lz5b93e64421 ("Scheduled: starts $($s.StartAt.ToString('ddd HH:mm')) ($(Format-Span $left) to go) - click to cancel")
            }
        }
        'running' {
            if (-not $busy) {
                $oc = Lz1845f882ce; $status = if ($oc) { "$($oc.Status)" } else { '' }
                if ($s.Mode -eq 'once') { Lz8023503067 -Why 'Scheduled run finished.'; Lza9be710f1d $status }
                elseif ($status -eq 'COMPLETED') { Lz8023503067 -Why 'Scheduled migration completed.'; Lza9be710f1d $status }
                else { $s.State = 'paused' }
            } elseif ($s.Mode -eq 'window' -and -not (Test-InWindow $m $s.FromMin $s.ToMin)) {
                Lz216c02c206; $s.State = 'paused'
            } else {
                $until = if ($s.Mode -eq 'window') { " (until $(Format-MinOfDay $s.ToMin))" } else { '' }
                Lz5b93e64421 ("Scheduled: running$until - click to cancel")
            }
        }
        'paused' {
            if ((Test-InWindow $m $s.FromMin $s.ToMin) -and -not $busy) {
                if (Lz1b096b0248 -RunArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','AddMissing','-Execute') -Label 'Sync new and changed') { $s.State = 'running'; $s.Launches++ }
            } else {
                $nf = Get-NextWindowOpen $s.FromMin
                Lz5b93e64421 ("Scheduled: waiting for the next window ($($nf.ToString('ddd HH:mm'))) - click to cancel")
            }
        }
    }
}
function Lzc7aa82ac5c {
...
$ctrl.BtnSchedule.Add_Click({
    if ($script:Sched) {
        if ((Show-Msg -Text ("A schedule is set:" + [Environment]::NewLine + $script:Sched.Summary + [Environment]::NewLine + [Environment]::NewLine + "Cancel it?") -Caption ('Scheduled run') -Buttons ('YesNo') -Icon ('Question')) -eq 'Yes') { Lz8023503067 -Why 'Schedule cancelled.' }
        return
    }
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')) | Out-Null; return }
    if (Lz4903055ea7) { (Show-Msg -Text ('A run is in progress. Wait for it to finish, then set a schedule.') -Caption ('Cannot schedule now') -Icon ('Warning')) | Out-Null; return }
    if (-not (Save-Mapping)) { return }
    $mappedCount = @($script:Projects | Where-Object { $script:Map.ContainsKey($_.Id) }).Count
    ...
    $script:SchedTimer.Add_Tick({ try { Lz348aea5df3 } catch {} })
    $script:SchedTimer.Start()
    try { Lz348aea5df3 } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    if ($script:Sched) { Lz8023503067 -Why 'Schedule cancelled (you stopped the run).' }
    $script:Stopping = $true
    $ctrl.BtnStop.IsEnabled = $false
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz9424c21995 "----- STOP requested by user at $stamp -----"
    Lz96d2c2103d "Stopping $($script:CurLabel)..."
    try { Lz5e58ba8600 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lz9cb1a8c796 -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lz9424c21995 "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lz04bbc1fcbc {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lz6d8193594a -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz9424c21995 "----- PAUSE requested by user at $stamp -----"
    Lz96d2c2103d "Pausing $($script:CurLabel)..."
    try { Lz5e58ba8600 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lz9cb1a8c796 -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lz9424c21995 "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lz3d3d6df784 { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lz09282be63e {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lz328a093ea2 {
...
function Lz307a1c445f {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lz6aa2907bfc -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lz1ae010c39d {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    ...
    function Lz4dca3a9fe5 { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz613fef1b32 { param($Label,$Control,$Info)
    ...
    function Lzccb05c300d { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lz4dca3a9fe5 'Datto Workplace'
    ...
    $parts = Lz09282be63e ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz613fef1b32 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lz613fef1b32 'Cell (number)' (Lzccb05c300d ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lz613fef1b32 'Datto Client ID' (Lzccb05c300d ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lz613fef1b32 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lz4dca3a9fe5 'Microsoft 365'
    $tbTid = Lz613fef1b32 'Tenant ID' (Lzccb05c300d ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lz613fef1b32 'App (Client) ID' (Lzccb05c300d ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lz4dca3a9fe5 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lz613fef1b32 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lz613fef1b32 '.pfx file' (Lzccb05c300d '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lz613fef1b32 '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lz4dca3a9fe5 'SharePoint and OneDrive'
    ...
    $tbSp  = Lz613fef1b32 'SharePoint root URL' (Lzccb05c300d ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lz613fef1b32 'OneDrive host URL'   (Lzccb05c300d ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lz613fef1b32 'Team site base URL'  (Lzccb05c300d ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lz613fef1b32 'Default site URL'    (Lzccb05c300d ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lz613fef1b32 'Email / UPN domain'  (Lzccb05c300d ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lzd6ce7b4979 -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lz307a1c445f -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz83c8d863d7 $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lz3d3d6df784 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lz005eec74b9 }
        ...
        try {
            ...
            else { $tok=Lz307a1c445f -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lz3d3d6df784 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lzd6ce7b4979 -Name 'DattoEndpointUrl' -Value $ep
            Lzd6ce7b4979 -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lzd6ce7b4979 -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lzd6ce7b4979 -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lzd6ce7b4979 -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lzd6ce7b4979 -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lzd6ce7b4979 -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lzd6ce7b4979 -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lzd6ce7b4979 -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lz73b290318e -Value $script:AS.pbSec.Password
            }
...
function Lzc9b4f244af { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lz750c905036 { param($Cfg,[string]$Path,$Value)
...
function Lz67fa6d8dd3 {
    ...
    $curMax=Lzc9b4f244af -Cfg $cfg -Path 'run.tuning.maxFileSizeMB'; if ($null -eq $curMax) { $curMax=0 }; $sizeBox.Text="$curMax"
    ...
    $curInc=@(Lzc9b4f244af -Cfg $cfg -Path 'run.tuning.includePatterns' | Where-Object { "$_".Trim() })
    $curExc=@(Lzc9b4f244af -Cfg $cfg -Path 'run.tuning.excludePatterns' | Where-Object { "$_".Trim() })
    ...
    $afterPick=New-Object System.Windows.Controls.DatePicker; $afterPick.Margin='0,4,0,4'; $afterPick.Width=150; $afterPick.SelectedDate=(& $toDate (Lzc9b4f244af -Cfg $cfg -Path 'run.tuning.modifiedAfter'))
    ...
    $beforePick=New-Object System.Windows.Controls.DatePicker; $beforePick.Margin='0,4,0,4'; $beforePick.Width=150; $beforePick.SelectedDate=(& $toDate (Lzc9b4f244af -Cfg $cfg -Path 'run.tuning.modifiedBefore'))
    ...
    $save.Add_Click({
        try {
            ...
            Lz750c905036 -Cfg $script:FltCfg -Path 'run.tuning.maxFileSizeMB' -Value $sv
            $pats=@("$($script:FltPat.Text)" -split '[;,\r\n]' | ForEach-Object { $_.Trim() } | Where-Object { $_ })
            if ($script:FltInc.IsChecked) { Lz750c905036 -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value $pats; Lz750c905036 -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value @() }
            else                          { Lz750c905036 -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value $pats; Lz750c905036 -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value @() }
            ...
            Lz750c905036 -Cfg $script:FltCfg -Path 'run.tuning.modifiedAfter'  -Value $ma
            Lz750c905036 -Cfg $script:FltCfg -Path 'run.tuning.modifiedBefore' -Value $mb
...
function Lz191f9ca02c {
    ...
    function Lz68a26a32ca { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec,[string]$Type='text')
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lzc9b4f244af -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lz68a26a32ca -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec -Type $s.Type
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lz750c905036 -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lz750c905036 -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lz750c905036 -Cfg $script:SetCfg -Path $s.Path -Value ([bool]$script:SetBoxes[$s.Path].IsChecked) }
                elseif ($s.Type -eq 'date') {
                    ...
                    Lz750c905036 -Cfg $script:SetCfg -Path $s.Path -Value $raw
                }
                else { Lz750c905036 -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lz532eeafc98 {
    ...
    function Lzb9c3813f2b { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lz09282be63e ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lzb9c3813f2b -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lzb9c3813f2b -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lzb9c3813f2b -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lzb9c3813f2b -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lz3d3d6df784 $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lz005eec74b9 }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lzd6ce7b4979 -Name 'DattoEndpointUrl' -Value $ep
            Lzd6ce7b4979 -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lz73b290318e -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz8459b9cc9b {
    ...
    $pre = Lz9ba974b538 -Title 'Prerequisites'
    ...
    $dat = Lz9ba974b538 -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lz638c8994e1 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lz9ba974b538 -Title 'Microsoft 365' -Buttons @($bMs, (Lz638c8994e1 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lz9ba974b538 -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lz532eeafc98 } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz6d4b7dd0eb; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lz8bddce2ee6 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz6d4b7dd0eb; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lz726c93159a } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz6d4b7dd0eb; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lz6d4b7dd0eb
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lz0ddcdfa304 {
...
function Lz66b250007b {
    ...
    $bOpen.Add_Click({ try { Lze724b2736d -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lz856f787276 {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lz5fbe3765e8 {
    ...
    Lz66b250007b -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lze724b2736d -Url $pageUrl
    ...
    } finally { Lz856f787276 }
}
function Lzdc8ff2d485 {
    try {
...
function Lzbe8a8df5df {
    if (Lz68e13c8453) { return $false }
...
function Lz4f4d8a5e15 {
    ...
    if (-not (Get-Module Microsoft.Graph.Authentication -ListAvailable)) {
        Lz0ddcdfa304 'The Microsoft Graph module is not installed. Re-run the installer (or run: Install-Module Microsoft.Graph.Authentication -Scope CurrentUser) and try again.' '#B42318' -Strong
        return
    }
    Lz0ddcdfa304 'Preparing the Microsoft 365 sign-in. The first time on a new install can take up to a minute while Windows loads the components - please wait, the app has not frozen.' '#475467' -Strong
    try { Import-Module Microsoft.Graph.Authentication -ErrorAction Stop } catch {
        Lz0ddcdfa304 "Could not load the Microsoft Graph module: $($_.Exception.Message)" '#B42318' -Strong
        return
    }
    Lz0ddcdfa304 "Sign in with an account that can manage Microsoft 365 (a Global Administrator is simplest). The first time, Microsoft asks you to accept the permissions this setup itself needs; that is expected.$(if ($ForEmail) { ' Email alerts are included, so expect TWO sign-ins: this one, then one for Exchange Online (a separate service with its own sign-in).' })"
    ...
    if (Lzbe8a8df5df) {
        Lz0ddcdfa304 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B5460F' -Strong
        try {
            Lzd4330b286a -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lz0ddcdfa304 "The browser sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B5460F' -Strong
        }
    } else {
        Lz0ddcdfa304 'Using the code sign-in (the reliable path): a small window will show your code to type at microsoft.com/devicelogin, or on your phone. It stays on top and closes itself when you are done.' '#B5460F' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lz5fbe3765e8 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lz0ddcdfa304 $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lz86db1913ad -Token $at -Claim 'tid'
            $wizTokAcct = Lz86db1913ad -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lz86db1913ad -Token $at -Claim 'preferred_username' }
            ...
            Lz0ddcdfa304 "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lz0ddcdfa304 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lz0ddcdfa304 "Signed in to tenant $tid as $wizAcct." '#0E9F6E'
    $newThumb = $null
    try {
        Lz0ddcdfa304 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lz0ddcdfa304 "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B5460F' }
            Lz0ddcdfa304 "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#0E9F6E'
        } else {
            Lz0ddcdfa304 "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lz0ddcdfa304 'App registration created.' '#0E9F6E'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lz0ddcdfa304 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lz0ddcdfa304 "Certificate created in this user's store. Thumbprint $newThumb." '#0E9F6E'
        Lz0ddcdfa304 'Uploading the certificate''s public key to the app registration...'
        ...
        Lz0ddcdfa304 "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#0E9F6E'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lz85d353b1eb -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lz0ddcdfa304 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lz85d353b1eb -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lz0ddcdfa304 $consentMsg '#0E9F6E'
        Lzd6ce7b4979 -Name 'TenantId'       -Value $tid
        Lzd6ce7b4979 -Name 'GraphClientId'  -Value $appId
        Lzd6ce7b4979 -Name 'CertThumbprint' -Value $newThumb
        Lz0ddcdfa304 'Connection details saved on this computer.' '#0E9F6E'
    } catch {
        ...
        if ($friendly) { Lz0ddcdfa304 $friendly '#B42318' -Strong; Lz0ddcdfa304 "Technical detail: $m" }
        else { Lz0ddcdfa304 "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lz0ddcdfa304 "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lz0ddcdfa304 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lz307a1c445f -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lzd6ce7b4979 -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lz328a093ea2 $spUrl
            if ($der) {
                Lzd6ce7b4979 -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lzd6ce7b4979 -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lzd6ce7b4979 -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lz0ddcdfa304 "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#0E9F6E'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lz85d353b1eb -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lz0ddcdfa304 "The new sign-in has not become active yet: $($_.Exception.Message)" '#B5460F' -Strong
                   Lz0ddcdfa304 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz83c8d863d7 $org
            if ($u) { Lzd6ce7b4979 -Name 'UpnDomain' -Value $u; Lz0ddcdfa304 "Email domain detected: $u" '#0E9F6E' }
        } catch { Lz0ddcdfa304 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lz0ddcdfa304 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#0E9F6E' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lz8bddce2ee6 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lzbba4e8b397 $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lz4f4d8a5e15 -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lzb318d86a41) {
                    Lz0ddcdfa304 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lz0ddcdfa304 $t $c } else { Lz0ddcdfa304 $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lz0ddcdfa304 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B5460F'
                }
            }
            ...
            Lz0ddcdfa304 "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lz5fbe3765e8 -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lzd4330b286a -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lze724b2736d -Url 'https://login.microsoft.com/device'
                Lz13dace5f11
                try { Lzd4330b286a -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lz170d409691 }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (Lzbe8a8df5df) {
            ...
            try {
                Lzd4330b286a -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lz5fbe3765e8 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lz86db1913ad -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lz86db1913ad -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lz85d353b1eb -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lzd1473e211f 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lz85d353b1eb -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lz85d353b1eb -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lzd6ce7b4979 -Name 'EmailSender' -Value $senderAddr
        Lzd6ce7b4979 -Name 'EmailEnabled' -Value '1'
...
function Lz726c93159a {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lz2be691be3c { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz9d655db5e6 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lz9d655db5e6 -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lz9d655db5e6 -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lz9d655db5e6 -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lz2be691be3c 'Subject and attachments'
    ...
    [void](Lz9d655db5e6 -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lzad2cec8f27 -Template $script:ES.tbSubj.Text -Vars (Lz82a235bbcf)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lzad2cec8f27 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz82a235bbcf)
            $tok = Lz307a1c445f -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lzd6ce7b4979 -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lzd6ce7b4979 -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lzd6ce7b4979 -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lzd6ce7b4979 -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lzd6ce7b4979 -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lzd6ce7b4979 -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lzd6ce7b4979 -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lzd6ce7b4979 -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lzd6ce7b4979 -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lzd6ce7b4979 -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lzd6ce7b4979 -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lzd6ce7b4979 -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lz0a1cb3d872 {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lz5fbe3765e8 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lzc7cdc5b975 {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lz0a1cb3d872 -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lz6d4b7dd0eb } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lz07013246ac {
    return ((Lz92d5a88f36) -and (Lzb318d86a41))
}
Lz38f5ed2c92
Lz170d409691
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lz9424c21995 "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lz07013246ac)) {
        try { Lz8459b9cc9b } catch {}
        if (Lz07013246ac) { Lz96d2c2103d 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lz96d2c2103d 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lz96d2c2103d 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ($ctrl.GettingStarted -and $ctrl.GettingStarted.Visibility -eq 'Visible') {
        try { Lz4b80fc04b2 ([bool](Lz07013246ac)) } catch {}
    }
    if ((Lz07013246ac) -and -not (Test-Path (Lzba7c2be1ae))) {
        try { Lz6dfc68408a } catch {}
    }
