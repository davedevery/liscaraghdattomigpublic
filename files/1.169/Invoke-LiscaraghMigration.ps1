...
function Lzb6d4cf440a { param([int]$Done,[int]$Total,[int64]$BytesDone,[int64]$BytesTotal,[string]$Space,
...
function Lzd11f863f87 { param([int]$Max,[int]$HardMax,[int]$PausedSeconds,[int]$Events,[int]$Code=0)
...
function Lz64d35a764d {
    param($Headers)
    $p = $script:DattoPace
    $rl = Lzcb78b75154 $Headers
    if (-not $rl.HasBudget) {
        [System.Threading.Monitor]::Enter($p.SyncRoot)
        try {
    ...
    if (-not $p.HeadersSeen) { $p.HeadersSeen = $true; Lzb0501403e3 INFO "  Datto rate budget detected ($([string]::Join(', ', $rl.Raw))). Flying close: pacing to stay just under it and avoid forced pauses." }
    ...
    if ($note) {
        $now = [DateTime]::UtcNow
        if ((-not $script:DattoBudgetLogAt) -or (($now - $script:DattoBudgetLogAt).TotalSeconds -ge 10)) { $script:DattoBudgetLogAt = $now; Lzb0501403e3 INFO "  $note" }
    }
}
function Lz2736df1a2c {
    $p = $script:DattoPace
    [System.Threading.Monitor]::Enter($p.SyncRoot)
    try {
        $cur = [int]$p.GapMs
...
function Lz2866944e93 {
    ...
    $tv = Lz0cf006036a
    Lzb0501403e3 INFO ("=== Stage '$Stage' started (pid $PID" + $(if ($tv) { ", v$tv" } else { '' }) + ") ===")
}
...
function Resolve-Secret {
    ...
    if ("$Value" -match '^env:(.+)$') {
        ...
        if (-not $v) { $v = Lz37a7c58bd3 -Name $n }
...
function Lz919f899b36 {
    param([string]$Space, [string]$Original, [string]$Final)
    $script:RenameCount++
    if ($script:RenameFile) { try { [pscustomobject][ordered]@{ Space=$Space; Original=$Original; Final=$Final } | Lz8e2163bc45 | Export-Csv -Path $script:RenameFile -Append -NoTypeInformation -Encoding UTF8 } catch {} }
    $dir = ($Final -replace '/[^/]*$','')
    $key = "$Space|$dir"
    if ($script:RenameSeenDirs -is [hashtable] -and -not $script:RenameSeenDirs.ContainsKey($key)) {
        $script:RenameSeenDirs[$key] = $true
        Lzb0501403e3 INFO "  tidied name(s) to suit SharePoint in: $dir  (every change is listed in the report and the renames CSV)"
    }
}
function Lz6cbeb7eaf7 {
    param([string]$Space, [string]$Final, [string]$FirstSource, [string]$SecondSource)
    $script:CollisionCount++
    if ($script:CollisionFile) { try { [pscustomobject][ordered]@{ Space=$Space; Final=$Final; FirstSource=$FirstSource; SecondSource=$SecondSource } | Lz8e2163bc45 | Export-Csv -Path $script:CollisionFile -Append -NoTypeInformation -Encoding UTF8 } catch {} }
    Lzb0501403e3 WARN "  NAME COLLISION in [$Space]: '$SecondSource' and '$FirstSource' both become '$Final' after tidying. In plain terms: two Datto items would land on the same SharePoint path, so one could overwrite the other. Both are in the collisions CSV and the report."
}
function Lzc7f4602ef7 {
    param([string]$Space, [string]$Rp, $Item)
    $safe = ConvertTo-SafeRelPath $Rp
    if ($safe -ne $Rp) {
        Lz919f899b36 -Space $Space -Original $Rp -Final $safe
    ...
    if ($script:FinalSeen -is [hashtable]) {
        ...
        if ($script:FinalSeen.ContainsKey($k)) { $prev = $script:FinalSeen[$k]; if ($prev -ne $Rp) { Lz6cbeb7eaf7 -Space $Space -Final $safe -FirstSource $prev -SecondSource $Rp } }
        else { $script:FinalSeen[$k] = $Rp }
    }
    return $safe
}
function Lzf9245c2b42 {
...
function Invoke-WithRetry {
    ...
    for ($a = 1; $a -le $throttleMax; $a++) {
        try { return & $Action }
            ...
            $isTransient = Lz300c86af39 $_
            $cap = if ($isThrottle) { $throttleMax } else { [int]$t.maxRetries }
            if ((($retryCodes -notcontains $status) -and (-not $isTransient)) -or $a -ge $cap) {
                Lzb0501403e3 ERROR "$What failed (attempt $a): $($_.Exception.Message)"; throw
            }
            $delay = Lzd251101241 -Attempt $a -Throttle $t -RetryAfter $ra
            if ($isTransient) {
                Lzb0501403e3 WARN "$What hit a network blip and is retrying in $delay s (attempt $a/$cap): $($_.Exception.Message). In plain terms: a brief connection glitch, not a real failure. It retries here so nothing has to wait for a later Sync."
            } elseif ($What -like 'Datto*' -and @(429,503) -contains $status) {
                $learnedTxt = Lz2736df1a2c
                Lzb0501403e3 WARN "  Datto is limiting how fast we can read (HTTP $status). Pausing $delay s as it asked, then settling to the fastest rate your account allows.$learnedTxt This is normal on large projects, not an error, and it gets smarter after the first one or two."
                ...
                Lzb0501403e3 WARN "$What retrying (HTTP $status): $why, waiting $delay s (attempt $a/$cap)."
            }
            $script:RetryEvents++
...
function Test-DestructiveGate { param([switch]$Execute) if (-not $Execute){ Lzb0501403e3 WARN "Preview only: nothing will be uploaded. In plain terms: this is a safe rehearsal. Use 'Upload all files' when you are ready to do it for real." } return [bool]$Execute }
...
function Lze943a6df5b {
    param($Config)
    if ($Config.datto.provider -eq 'LocalSim') { Lzb0501403e3 OK "Datto provider: LocalSim ($($Config.datto.sim.rootPath))"; return }
    ...
    Lzb0501403e3 OK "Datto API authenticated (Basic auth)."
}
function Invoke-DattoApi {
    ...
    Lz64d35a764d -Headers $script:DattoLastHeaders
    return $r
}
...
function Select-IncludedItems {
    ...
    foreach ($it in @($Items)) {
        ...
        if (-not $skip -and -not (Lzd28325b689 $leaf)) { $skip = $true }
...
function Test-ItemIncluded {
    ...
    if (-not (Lzd28325b689 $leaf)) { return $false }
...
function Lz29d8143f9d {
    ...
    Lzb0501403e3 WARN "RERUN FAILED ONLY: re-copying just the $($failed.Count) file(s) that failed in $([System.IO.Path]::GetFileName($AuditPath)). Everything else is left exactly as it is."
    return $set
}
...
function Get-DattoItems {
    ...
    if ($subPath) {
        Lzb0501403e3 WARN "  SOURCE IS SCOPED to [$subPath] inside [$($Space.Name)]. Only that folder is being listed. This is a partial view of the project: do not read the result as a complete migration of it."
        if ($contentsOnly) { Lzb0501403e3 WARN "  CONTENTS ONLY is on for this project: the [$subPath] folder itself is NOT created at the destination; its contents land directly in the target folder. Only runs of this mapping with the setting on understand that layout: a run WITHOUT it would not find these files and would copy them again to [$subPath/...]." }
    }
    ...
    $emitEnum = {
        ...
        Lzb0501403e3 INFO "  SOURCE [$scopeLabel]: $('{0:N0}' -f $prog.Folders) folder(s) read, $('{0:N0}' -f $prog.Files) file(s) found so far.$w"
    ...
    function Lzbdef7e3b37 { param($ParentId, [string]$Prefix)
        ...
        foreach ($it in @($items)) {
            ...
            $isFolder = $false; try { $isFolder = [bool]$it.($f.itemFolder) } catch {}
            if ($isFolder) {
                Lzbdef7e3b37 -ParentId $it.($f.itemId) -Prefix $rel
            } else {
    ...
    Lzbdef7e3b37 -ParentId $startId -Prefix $(if ($contentsOnly) { '' } else { $subPath })
    & $emitEnum -Force
    $script:SrcTotal = $prog.Files
    Lzb0501403e3 OK "  SOURCE [$scopeLabel] FINISHED: $('{0:N0}' -f $prog.Files) file(s) across $('{0:N0}' -f $prog.Folders) folder(s)."
    Lz02ffeda13f -Config $Config -Space $Space -Suspects $suspects
    return $out
}
$script:EnumCacheMaxAgeSec = 86400
function Lz97c3b21f7b {
    param($Config, $Space)
    if (-not $Config.run.stateRoot) { return $null }
    return (Join-Path $Config.run.stateRoot ("enum-" + (ConvertTo-Slug "$($Space.Name)") + ".json"))
}
function Lz40428def97 {
    param($Config, $Space, $Items)
    $p = Lz97c3b21f7b -Config $Config -Space $Space
    if (-not $p) { return }
    try {
        ...
        Lzb0501403e3 INFO "  (could not save the file list for a later Resume: $($_.Exception.Message). Harmless: a Resume would read Datto again instead.)"
    }
}
function Get-SpaceItemsCached {
    ...
    if ($script:UseEnumCache) {
        ...
        Lzb0501403e3 INFO "  reading the file list from Datto again, because $why."
    }
    $items = @(Get-DattoItems -Config $Config -Space $Space)
    Lz40428def97 -Config $Config -Space $Space -Items $items
    return $items
}
function Lz02ffeda13f {
...
function Lzee0692c5be {
    ...
    } catch { Lzb0501403e3 WARN "  permissions unavailable for $($Space.Name)"; return @() }
}
function Lz063c5ae750 {
    ...
    if ($code -eq 5) {
        ...
        if ("$reason".Trim()) { Lzb0501403e3 WARN "Integrity check detail: $reason" }
...
function Connect-Destination {
    param($Config)
    if ($Config.destination.provider -eq 'LocalSim') { Lzb0501403e3 OK "Destination provider: LocalSim ($($Config.destination.sim.rootPath))"; return }
    ...
    $auth = Lz063c5ae750 -Broker $broker -Config $Config
    Connect-MgGraph -AccessToken (ConvertTo-SecureString "$($auth.Token)" -AsPlainText -Force) -NoWelcome
    Lzb0501403e3 OK "Microsoft 365 sign-in via the licence broker (LiscaraAuth): the token is bound to a verified licence."
    $liveTenant = ''
    ...
    if ($script:ScopeRefId -and $liveTenant -and ($script:ScopeRefId -ne $liveTenant) -and ($null -eq $script:TrialCap)) {
        ...
        Lzb0501403e3 WARN "Evaluation limits apply to this run."
    }
    Lzb0501403e3 OK "Connected to Graph (tenant $liveTenant)."
}
...
function Send-FileToDestination {
    ...
    if (($Config.run.PSObject.Properties.Name -contains 'sanitiseNames') -and $Config.run.sanitiseNames) {
        $relPath = Lzc7f4602ef7 -Space $Space -Rp $relPath -Item $Item
    }
    $relPath = Lz6f9dfbca92 $relPath
    if ($Config.destination.provider -eq 'LocalSim') {
        ...
        return @{ Size = (Get-Item $dest).Length; Hash = (Lzeecde4af7e $dest); Path = $relPath; Method = 'LocalSim'; Retries = 0 }
    }
    if ($relPath -match '/') { Lze6e56ae196 -Config $Config -DriveId $DriveId -FolderPath ($relPath -replace '/[^/]*$','') }
    ...
    if ($size -eq 0) {
    ...
    } elseif ($size -le $script:SmallFilePutThreshold -and (Lzf8bfb2b68f) -and -not $script:SpoolKey) {
        ...
        try {
            ...
            Lzb0501403e3 WARN "  direct PUT failed for ${relPath}; falling back to upload session: $($_.Exception.Message). In plain terms: the quick upload did not work for this file, so the tool is switching to the slower resumable method. Usually harmless, the file should still upload."
        }
    }
            ...
            try {
                ...
                if ($size -le $chunk) {
                    if ($script:SpoolKey) {
                        ...
                        $finalResp = Lz248a57f70b -Config $Config -UploadUrl $uploadUrl -Bytes $plain -From 0 -To ($size - 1) -Total $size
                    } else {
                        $finalResp = Lz248a57f70b -Config $Config -UploadUrl $uploadUrl -FilePath $LocalFile -From 0 -To ($size - 1) -Total $size
                    }
                    ...
                    try {
                        ...
                        while ($true) {
                            ...
                            $finalResp = Lz248a57f70b -Config $Config -UploadUrl $uploadUrl -Bytes $plain -From $pos -To ($pos + $plain.Length - 1) -Total $size
                            $pos += $plain.Length
                    ...
                    try {
                        ...
                        while (($read = $fs.Read($buffer, 0, $buffer.Length)) -gt 0) {
                            ...
                            $finalResp = Lz248a57f70b -Config $Config -UploadUrl $uploadUrl -FilePath $chunkTmp -From $pos -To ($pos + $read - 1) -Total $size
                            $pos += $read
                        }
                ...
                if ($st -eq 400 -and $hasFsi -and -not $strippedFsi) {
                    ...
                    if (-not $script:OmitFsi) {
                        $script:OmitFsi = $true
                        Lzb0501403e3 WARN "  destination rejects the modified-date field; omitting it for the rest of this run. In plain terms: SharePoint would not accept the file's last-modified date, so the tool will stop sending it. Files still upload fine."
                    }
                    $sessBody = @{ item = @{ '@microsoft.graph.conflictBehavior' = 'replace' } } | ConvertTo-Json -Depth 5
                    Lzb0501403e3 WARN "  createUploadSession 400 for ${relPath}: retrying without the modified-date field. In plain terms: the first upload attempt was refused, so the tool is retrying without the date. Usually harmless."
                    $script:RetryEvents++; $localRetries++
                    continue
                }
                if ((@(401, 404, 408, 429, 500, 502, 503, 504) -contains $st) -and $attempt -lt $t.maxRetries) {
                    $ra = Lz28e03d056f $_
                    $delay = Lzd251101241 -Attempt $attempt -Throttle $t -RetryAfter $ra
                    ...
                    Lzb0501403e3 WARN "  upload retry for $relPath (HTTP $st): new session, waiting $delay s (attempt $attempt/$($t.maxRetries)). In plain terms: $why, so it will wait $delay seconds and try again. Normal on busy runs."
                    $script:RetryEvents++; $localRetries++
                ...
                if ($gBody) { Lzb0501403e3 ERROR "  Graph reason for $relPath (HTTP $st): $gBody" }
                throw
            }
        }
    }
    ...
    switch ($choice.Trim()) {
        ...
        default {
            ...
            while ($true) {
                ...
                if ($validate) {
                    $siteObj = Lzd472d4c9df -Url $site.Trim()
...
function Lz96d0127fb2 {
    param($Config, [switch]$NonInteractive)
    Lz2866944e93 -Config $Config -Stage 'api-discovery'
    Lze943a6df5b -Config $Config
    ...
    if ($NonInteractive) {
        foreach ($s in $projects) {
            $rows.Add((Lz7cfd1a42d9 -Project $s -Dest @{ Action='MIGRATE'; DestinationType='SharePoint'; DestinationUrl="$($Config.destination.teamSiteBaseUrl)/$((ConvertTo-Slug $s.Name))"; TargetPrincipal=''; TargetLibrary=''; TargetSubFolder=''; Notes='auto (non-interactive)' }))
        }
    } else {
        ...
        while ($remaining.Count -gt 0) {
            ...
            $proj = $remaining[$n - 1]
            $dest = Lz82cb9ed7c1 -Config $Config -Project $proj
            if ($dest.Action -eq 'MIGRATE') {
                $newRow = Lz7cfd1a42d9 -Project $proj -Dest $dest
                $rows.Add($newRow)
    ...
    if ($rows.Count -eq 0) { Lzb0501403e3 WARN "No sources mapped, nothing to migrate. mapping.csv not written."; return }
    $out = Join-Path $Config.run.reportRoot 'mapping.csv'
    ($rows | Sort-Object Space) | Lz8e2163bc45 | Export-Csv -Path $out -NoTypeInformation -Encoding UTF8
    Lzb0501403e3 OK "Discovery complete. $($rows.Count) project(s) mapped. Review: $out"
}
function Lze67cbd6bd5 {
    param($Config,$Summary,$Issues,$Largest,$Duplicates,[int]$DupGroups,[int64]$AllBytes,[int]$FailCount,[int]$RenameCount,[string]$EstimateText)
    ...
    if ($blocked -gt 0 -or $FailCount -gt 0) {
        $bStyle='background:#FFF6EC;border:1px solid #F5C083;color:#B5460F'; $bTitle='Some things to fix before uploading'
    } else {
        $bStyle='background:#E7F7EF;border:1px solid #86D9B4;color:#066A4B'; $bTitle='Ready to upload - no blocking problems found'
    }
    $bExtra=''
    if ($blocked)     { $bExtra += " $blocked destination(s) do not have enough free space." }
    if ($FailCount)   { $bExtra += " $FailCount file(s) would fail on their name or path." }
    if ($RenameCount) { $bExtra += " $RenameCount file(s) will be auto-renamed (they still migrate)." }
    ...
    foreach ($s in $Summary) {
        ...
        $sumRows += "<tr><td>$(_enc $s.Space)</td><td>$(_enc $s.Type)</td><td class='n'>$(_enc $s.RequiredGB) GB</td><td class='n'>$(_enc $s.Free)</td><td class='$vc'>$(_enc $s.Verdict)</td></tr>`n"
    }
    if ($Issues.Count) {
        $iRows=''
        foreach ($x in $Issues) { $iRows += "<tr><td>$(_enc $x.Space)</td><td>$(_enc $x.File)</td><td class='err'>$(_enc $x.Problem)</td></tr>`n" }
        $issueSection = "<h2>Files needing attention ($($Issues.Count))</h2><table><thead><tr><th>Project</th><th>File</th><th>Problem</th></tr></thead><tbody>$iRows</tbody></table>"
    } else { $issueSection = "<p class='okbox'>No file name or path problems. Every file can be uploaded as it is.</p>" }
    $lRows=''
    ...
    $largeSection = if ($lRows) { "<table><thead><tr><th>Project</th><th>File</th><th class='n'>Size</th><th>Modified</th></tr></thead><tbody>$lRows</tbody></table>" } else { "<p class='dim'>No files found.</p>" }
    if ($DupGroups -gt 0) {
        $dRows=''
        ...
        $dupSection = "<details><summary>$DupGroups set(s) of duplicate files (identical content) &middot; click for details</summary><table><thead><tr><th class='n'>Group</th><th>Project</th><th>File</th><th class='n'>Size</th></tr></thead><tbody>$dRows</tbody></table></details><p class='dim'>Nothing is de-duplicated automatically; this is for your review before migrating.</p>"
    ...
    $now = Get-Date
    $tv = Lz0cf006036a; $verSub = if ($tv) { " &middot; v$tv" } else { '' }
    $html = @"
...
<header><div><h1>$(_enc $brand)</h1><div class='sub'>Migration Readiness (before you upload) &middot; generated $($now.ToString('dd MMM yyyy HH:mm'))$verSub</div></div><img class='brandlogo' alt='Liscaragh Software' src='data:image/jpeg;base64,$($script:LiscaraghLogoB64)' /></header>
$banner
<div class='cards'>
 <div class='card'><div class='v'>$projCount</div><div class='l'>Projects checked</div></div>
...
<h2>By project</h2>
<table><thead><tr><th>Project</th><th>Type</th><th class='n'>Required</th><th class='n'>Free at destination</th><th>Verdict</th></tr></thead><tbody>
$sumRows
</tbody></table>
$issueSection
<h2>Largest files</h2>
...
<footer>Source: Datto Workplace &middot; This is a read-only readiness check: nothing was uploaded or changed &middot; Liscaragh Migrate &middot; Liscaragh Software &middot; www.liscaragh.com</footer>
</div>
...
"@
    ...
    Lzb0501403e3 OK "Readiness report written: $outPath"
}
function Lz92f2837a39 {
    param($Config)
    Lz2866944e93 -Config $Config -Stage 'api-preflight'
    Lze943a6df5b -Config $Config
    ...
    $t = $map.Count; $i = 0
    foreach ($row in $map) {
        $i++; Lzf9245c2b42 -Activity 'Pre-flight' -Status $row.Space -Current $i -Total $t | Out-Null
        ...
        if ($verdict -eq 'BLOCKED') { Lzb0501403e3 ERROR "  BLOCKED: $($row.Space) needs $([math]::Round($bytes/1GB,2))GB, free $freeTxt. In plain terms: the destination does not have enough free space for this project. Free up space or increase the storage quota, then run this again." }
    }
    try { Write-Progress -Activity 'Pre-flight' -Completed } catch {}
    $issues  | Lz8e2163bc45 | Export-Csv (Join-Path $Config.run.reportRoot 'api-preflight-issues.csv') -NoTypeInformation -Encoding UTF8
    $summary | Lz8e2163bc45 | Export-Csv (Join-Path $Config.run.reportRoot 'api-preflight-summary.csv') -NoTypeInformation -Encoding UTF8
    $blocked = @($summary | Where-Object Verdict -eq 'BLOCKED').Count
    Lzb0501403e3 OK "Readiness check complete. $blocked destination(s) blocked for lack of storage - 'Upload all files' will skip those."
    if ($renameCount -gt 0) { Lzb0501403e3 INFO "$renameCount file(s) have illegal characters and will be AUTO-RENAMED on upload (they migrate, nothing skipped)." }
    if ($failCount -gt 0)   { Lzb0501403e3 WARN "$failCount file(s) WILL FAIL (path too long, or illegal char with sanitiseNames off). See api-preflight-issues.csv. In plain terms: some files have names or folder paths SharePoint cannot accept. They will be skipped unless shortened or renamed first." }
    if ($issues.Count -eq 0) { Lzb0501403e3 OK "No file name/path issues." }
    ...
    $largestRows | Lz8e2163bc45 | Export-Csv (Join-Path $Config.run.reportRoot 'api-preflight-largest-files.csv') -NoTypeInformation -Encoding UTF8
    if ($largestRows.Count) { Lzb0501403e3 INFO "Largest file: $(Format-Bytes ([int64]$largestRows[0].SizeBytes)) - $($largestRows[0].Space)/$($largestRows[0].RelativePath). The biggest $($largestRows.Count) are in api-preflight-largest-files.csv." }
    ...
    $dupRows | Lz8e2163bc45 | Export-Csv (Join-Path $Config.run.reportRoot 'api-preflight-duplicates.csv') -NoTypeInformation -Encoding UTF8
    if ($gid -gt 0) {
        ...
        Lzb0501403e3 INFO "$gid set(s) of duplicate files (identical content), about $(Format-Bytes $wasted) in redundant copies. See api-preflight-duplicates.csv. Nothing is de-duplicated automatically; this is for review before migrating."
    } else {
        Lzb0501403e3 INFO "No duplicate files detected by content hash."
    }
    $rate = $null; $rateFrom = ''
    ...
    Lzb0501403e3 INFO $estText
    try {
        Lze67cbd6bd5 -Config $Config -Summary $summary -Issues $issues -Largest $largestRows `
            -Duplicates $dupRows -DupGroups $gid -AllBytes $allBytes -FailCount $failCount -RenameCount $renameCount -EstimateText $estText
    } catch { Lzb0501403e3 WARN "Readiness report could not be written: $($_.Exception.Message)" }
}
function Lz3242f06886 {
...
function Lz15648f74b9 { param($Config) return (Join-Path $Config.run.reportRoot 'run-active.json') }
function Lz3ac2d30287 {
    ...
    try { [System.IO.File]::WriteAllText((Lz15648f74b9 $Config), ($o | ConvertTo-Json -Depth 4), [System.Text.UTF8Encoding]::new($false)) } catch {}
}
function Lz3af02e6f1c { param($Config) try { Remove-Item (Lz15648f74b9 $Config) -Force -ErrorAction SilentlyContinue } catch {} }
...
function Get-EmailCommonVars {
    ...
    return @{ JobName = $job; Tenant = $tenant; Version = (Lz0cf006036a) }
}
...
function Send-RunEmail {
    ...
    try {
        ...
        if (-not "$($s.Sender)".Trim() -or -not "$($s.Recipients)".Trim()) {
            Lzb0501403e3 WARN 'Email alerts are switched on but the sender or recipients are missing (Settings > Email alerts). No email was sent; the run itself is unaffected.'
            return
        }
        ...
        foreach ($r in ("$($s.Recipients)" -split '[;,\r\n]+')) {
            ...
            elseif ($a) { Lzb0501403e3 WARN "Email alerts: '$a' does not look like an email address and was skipped." }
        }
        if (-not $to.Count) {
            Lzb0501403e3 WARN 'Email alerts: no valid recipient address is configured. No email was sent; the run itself is unaffected.'
            return
        }
        $atts = @(); $attachNote = ''
        if ("$($s.Attach)" -ne '0') {
            ...
            foreach ($f in @($set.Files)) {
                ...
                } catch { Lzb0501403e3 WARN "Email alerts: could not read attachment $f ($($_.Exception.Message)); sending without it." }
            }
        }
        ...
        Lzb0501403e3 OK "Completion email sent from $emSender to $(@($to).Count) recipient(s)$(if ($atts.Count) { " with $($atts.Count) attachment(s)" })."
    } catch {
        Lzb0501403e3 WARN "The completion email could not be sent: $($_.Exception.Message). The run itself is unaffected; the outcome above stands. Check Settings > Email alerts (sender, recipients, Mail.Send permission)."
    }
}
function Invoke-RunFinalize {
    ...
    try {
        Lz3242f06886 -AuditFile $AuditFile -Status $status -Reason $reason -Processed $processed -Expected $ExpectedFiles `
    ...
    try {
        switch ($status) {
            'COMPLETED'             { Lzb0501403e3 OK    "=== RUN COMPLETE: all $SpacesPlanned project(s) finished. Uploaded $copied file(s) ($szTxt), skipped $skipped unchanged, 0 failures, in $el. ===" }
            'COMPLETED_WITH_ERRORS' { Lzb0501403e3 ERROR "=== RUN COMPLETE WITH ERRORS: reached the end of all $SpacesPlanned project(s). Uploaded $copied ($szTxt), but $errCount file(s) failed or could not be verified and are NOT marked done. Run 'Sync new and changed' to retry just those. ===" }
            'CANCELLED'             { Lzb0501403e3 WARN  "=== RUN CANCELLED by user. Uploaded $copied file(s) ($szTxt) before stopping; those are saved. The rest were NOT uploaded. Click 'Sync new and changed' to carry on from where it stopped: it copies only what is not already there. ===" }
            'INCOMPLETE'            { Lzb0501403e3 ERROR "=== RUN INCOMPLETE: the previous run stopped without finalising (a crash, a forced close, or power loss). $copied file(s) ($szTxt) were recorded as uploaded. Run 'Sync new and changed' to continue and confirm. ===" }
            default                 { Lzb0501403e3 ERROR "=== RUN ENDED EARLY - DID NOT FINISH. Processed $processed file(s) (of ~$ExpectedFiles listed) across $SpacesCompleted of $SpacesPlanned project(s) before stopping. Reason: $reason. The remaining files were NOT uploaded. Fix the cause, then click 'Sync new and changed' to carry on from where it stopped. ===" }
        }
    } catch {}
    ...
    Lz3af02e6f1c -Config $Config
    return $outcome
}
function Lz18e3356160 {
...
function Lz28d83fc621 {
    param($Config,[string]$Status='Incomplete',[string]$AuditPath='')
    Lz2866944e93 -Config $Config -Stage 'api-finalize'
    $act = $null
    $ap = Lz15648f74b9 $Config
    ...
    if (-not $AuditPath -or -not (Test-Path $AuditPath)) {
        Lzb0501403e3 WARN "Record outcome: no audit CSV found, so there is nothing to record."; Lz3af02e6f1c -Config $Config; return
    }
    $sidecar = ($AuditPath -replace '\.csv$','') + '.outcome.json'
    if (Test-Path $sidecar) { Lzb0501403e3 INFO "Record outcome: the run already has an outcome; leaving it as it is."; Lz3af02e6f1c -Config $Config; return }
...
function Lz700d4ba964 {
    ...
    try {
        ...
        if ($days -lt 0) {
            ...
            Lzb0501403e3 WARN "  the sign-in certificate expires in $([int]$days) day(s) (on $($cert.NotAfter.ToString('yyyy-MM-dd'))). Renew it soon to avoid a mid-run authentication failure."
        }
    } catch { throw }
}
function Lz7e0f2d7e3f {
    param($Config)
    if ($Config.datto.provider -eq 'LocalSim') { return }
...
function Lzdd051da073 {
    ...
    try {
        ...
        Lzb0501403e3 INFO "  found $($old.Count) resume record(s) from an older version in [$root]. They are no longer used: the destination is now the source of truth for what has already been copied. They are harmless, and you can delete them whenever you like. Nothing here will delete them for you."
    } catch {}
}
function Lz297ec6e673 {
    param($Config)
    ...
    try {
        ...
        if ($tmpRoot -and (Test-Path $tmpRoot)) {
            ...
            if ($orphans.Count) {
                ...
                Lzb0501403e3 INFO "  cleared $($orphans.Count) leftover temp file(s) from a previous run."
            }
            $freeB = Lzdd2bbdcfd2 -Path $tmpRoot
            if ($freeB -ge 0) {
                Lzb0501403e3 INFO "  temp drive free space: $(Format-Bytes $freeB) (files are staged here then deleted immediately after upload, so only a handful sit on disk at once)."
                if ($freeB -lt 2GB) { Lzb0501403e3 WARN "  the temp drive has under 2 GB free. Everyday files are fine (only a handful stage at a time), but any single file larger than the free space is skipped and reported. Free space, or point run.tempWorkingFolder at a bigger drive." }
            }
        }
    } catch {}
    Lz700d4ba964 -Config $Config
}
function Lz43e2e603c9 {
    param($Config, $Rows)
    if ($Config.destination.provider -eq 'LocalSim') { return }
    $seen = @{}
    foreach ($row in $Rows) {
        ...
        try {
            ...
            if ($null -ne $free -and $free -gt 0) {
                ...
                if ($free -lt 2GB) { Lzb0501403e3 WARN "  destination check: $label is reachable but has only $ftxt free. A large upload may fail part-way; free up space or pick another destination." }
                else { Lzb0501403e3 OK "  destination check: $label reachable, $ftxt free." }
            } else {
                Lzb0501403e3 OK "  destination check: $label reachable."
            }
        } catch {
...
function Invoke-Transfer {
    ...
    if ($Mode -eq 'Resume') {
        $Mode = 'Delta'
        Lzb0501403e3 INFO "Resume now runs as a Sync: it compares Datto against the destination itself, file by file, instead of trusting a local record of the last run. Same intent, and it cannot be fooled by a stale or damaged record."
    }
    Lz2866944e93 -Config $Config -Stage "api-transfer-$Mode" -Friendly $(
    ...
    if ($FailedOnly) {
        ...
        $script:FailedOnlySet = Lz29d8143f9d -Config $Config -AuditPath $FailedFromAudit
    }
    Lzdd051da073 -Config $Config
    Lz7e0f2d7e3f -Config $Config
    if ($Execute -and $Config.destination.provider -ne 'LocalSim' -and $null -eq $script:TrialCap) {
        ...
        if ($lzOk) { $script:ScopeRefId = $lzTen }
            ...
            Lzb0501403e3 WARN "Evaluation limits apply to this run: up to $lzRem file(s) will be copied. Licence this Microsoft tenant at https://www.liscaragh.com to remove the limit."
        }
    }
    $runStart = Get-Date
    ...
    else { Lzb0501403e3 INFO "No readiness check was run for this job (it is optional). The upload checks the destination is reachable before it starts, tidies names as it goes, and flags any problems in the report." }
    $runnable = @($map | Where-Object { $blockedSpaces -notcontains $_.Space })
    if ($MaxParallelSpaces -gt 1 -and $runnable.Count -gt 1) {
        Lz0b1da32a26 -Config $Config -Rows $runnable -Mode $Mode -DeltaMode $DeltaMode -SpoolAhead $SpoolAhead -MaxParallelSpaces $MaxParallelSpaces -Execute:$Execute
        return
    }
    Lzb0501403e3 INFO "This run: $($map.Count) project(s). (Read-ahead: $SpoolAhead file(s).)"
    if ($script:SpoolEncrypt) {
        $script:SpoolKey = New-SpoolKey
        Lzb0501403e3 OK "Files staged on this computer are ENCRYPTED (AES-256) while they wait to upload. The key exists only in this run's memory and dies with it, staged names are random identifiers, and the destination receives the original files unchanged."
    } else {
        $script:SpoolKey = $null
        Lzb0501403e3 INFO "Spool encryption is OFF (run.encryptSpool = false): files staged on this computer are readable while a run is in flight."
    }
    Lze943a6df5b -Config $Config
    Connect-Destination -Config $Config
    $expectedTotal   = 0
    ...
    $spacesCompleted = 0
    $transferError   = $null
    if ($willWrite -and -not $ResultDir) { Lz3ac2d30287 -Config $Config -Mode $Mode -StartUtc $runStart -AuditFile $script:AuditFile -ExpectedFiles 0 -SpacesPlanned $spacesPlanned -SpacesCompleted 0 }
    $summary = New-Object System.Collections.Generic.List[object]
    $sTotal = $map.Count; $sIdx = 0
    try {
    if ($willWrite) { Lz43e2e603c9 -Config $Config -Rows $map; Lz297ec6e673 -Config $Config }
    foreach ($row in $map) {
        $sIdx++
        Lzf9245c2b42 -Activity "Transfer ($Mode)" -Status $row.Space -Current $sIdx -Total $sTotal -Id 1 | Out-Null
        if ($blockedSpaces -contains $row.Space) { Lzb0501403e3 ERROR "SKIPPED '$($row.Space)': the readiness check found too little storage at this destination. Increase the quota or free up space, then run again."; continue }
        Lzb0501403e3 INFO "[$(if (-not $willWrite) { 'Preview' } elseif ($Mode -eq 'Delta') { 'Sync' } else { 'Upload' })] source [$($row.Space)] -> destination $(Format-Destination -Row $row)"
        $space = New-SpaceRef -Row $row
        ...
        if ($streamFP) {
            Lzb0501403e3 INFO "  Upload all: listing and uploading run together (files start moving while the folder list is still being read)."
            $items = @()
        } else {
            $rowScope = "$($row.Space)"
            ...
            Lzb0501403e3 INFO "  listing files in [$rowScope] (large projects can take a few minutes, nothing uploads yet)..."
            $items = @(Get-SpaceItemsCached -Config $Config -Space $space)
            Lzb0501403e3 INFO "  listing complete: $($items.Count) file(s)."
        }
        ...
        if ($effSub) { Lzb0501403e3 INFO "  files land under: [$effSub]$nestNote." }
        elseif ($nestNote) { Lzb0501403e3 INFO "  files land under the project's own folder at the destination root." }
        ...
        if ($restoreSet.Count) {
            ...
            Lzb0501403e3 INFO "  restore: $($toDo.Count) selected file(s) will be overwritten at the destination with the Datto version."
        }
        ...
        elseif ($Mode -eq 'Delta') {
            ...
            $toDo = @($items | Where-Object {
                ...
                if ($null -ne $ds -and -not (Lz1f1bc32c83 -RelPath $rp -SourceSize ([int64]$_.Size) -DestSize $ds)) {
            ...
            Lzb0501403e3 INFO "  sync ($modeTxt): $($toDo.Count) of $($items.Count) file(s) to copy; the rest are already present and are left as they are."
            if ($repair.Count) {
                Lzb0501403e3 WARN "  $($repair.Count) of those file(s) are ALREADY at the destination but are the wrong size, and will be re-copied to repair them. These are uploads that did not complete correctly on an earlier run, not new or changed files. They are recorded as 'repaired' in the audit CSV, not as ordinary copies."
            }
        }
        else {
            $toDo = @($items)
        ...
        if ($willWrite) { Lzb6d4cf440a -Done 0 -Total $fTotal -BytesDone 0 -BytesTotal $spaceBytesTotal -Space $row.Space }
        ...
        if ($willWrite -and $Mode -ne 'FirstPass') {
            ...
            foreach ($sk in @($items | Where-Object { $toDoIds -notcontains $_.Id })) {
                Lzdab48dc84f -Space $row.Space -Item $sk -Status 'Skipped' -Reason 'already at the destination, intact and not older than the source'
            }
        }
        if (-not $willWrite) {
            $copied = $fTotal; $bytes = $spaceBytesTotal
            foreach ($it in $toDo) { Lzdab48dc84f -Space $row.Space -Item $it -Status 'WouldCopy' -Reason 'dry-run' }
        }
        elseif ($UploadWorkers -gt 1) {
            ...
            foreach ($it in $toDo) {
                ...
                $rp = $rp -replace '\\','/'
                if ($sanitise) { $rp = Lzc7f4602ef7 -Space $row.Space -Rp $rp -Item $it }
            ...
            if ($Config.destination.provider -ne 'LocalSim') { $tk = Lzafdefedc3c -Config $Config; $tokenHolder.Token = $tk.Token; $tokenHolder.Exp = $tk.Exp }
            $ensured = [hashtable]::Synchronized(@{})
            Lzb0501403e3 INFO "  copying up to $downloadThreads file(s) at a time from Datto, and up to $UploadWorkers at a time into Microsoft 365."
            ...
            if ($maxUpMbps -gt 0 -or $maxDownMbps -gt 0) {
                ...
                Lzb0501403e3 INFO "  bandwidth: $upTxt, $dnTxt (so the client's line is not saturated)."
            }
            ...
            $process = {
                ...
                switch ($res.Status) {
                    'Copied'   { $script:copiedRef.Value++; $script:bytesRef.Value+=$it.Size; if($script:VerboseFiles){Lzb0501403e3 INFO "  + [$($it.RelativePath)] $($res.Reason)"} }
                    'ZeroByte' { $script:copiedRef.Value++; $script:zeroRef.Value++; $script:bytesRef.Value+=$it.Size; Lzb0501403e3 WARN "  empty file (0 bytes), uploaded as-is: $($it.RelativePath)" }
                    'VerifyFail'{ $script:verifyFailRef.Value++; Lzb0501403e3 ERROR "  VERIFY FAILED $($it.RelativePath): $($res.Reason). In plain terms: the uploaded copy did not match the original, so it was not marked done. It will be retried on the next Sync." }
                    'DownloadError'{ $script:failedRef.Value++; Lzb0501403e3 ERROR "  DOWNLOAD FAILED $($it.RelativePath): $($res.Error). In plain terms: the file could not be downloaded from Datto. It will be retried on the next Sync." }
                    'SkippedTooLarge'{ $script:failedRef.Value++; Lzb0501403e3 ERROR "  SKIPPED (too large for temp drive) $($it.RelativePath): $($res.Error). In plain terms: this one file will not fit in the temp drive's free space, so it was left for a later run. Free up space (or point the temp folder at a bigger drive) and Sync to pick it up." }
                    default    { $script:failedRef.Value++; Lzb0501403e3 ERROR "  UPLOAD FAILED $($it.RelativePath): $($res.Error). In plain terms: the file could not be uploaded to Microsoft 365. It will be retried on the next Sync." }
                }
                $rDl = 0.0; try { $rDl = [double]$res.DlMs } catch {}
                $rsn = Lze4d69b4479 -Repair $repair -Item $it -Status $res.Status -Reason $res.Reason
                Lzdab48dc84f -Space $row.Space -Item $it -Status $res.Status -Reason $rsn -DestSize $res.DestSize -DestPath $res.DestPath -DestHash $res.DestHash -Method $res.Method -Retries $res.Retries -DurationMs $res.Ms -DownloadMs $rDl -ErrorMsg $res.Error
                Lzb6d4cf440a -Done $script:fIdxRef.Value -Total $fTotal -BytesDone $script:bytesRef.Value -BytesTotal $spaceBytesTotal -Space $row.Space `
            ...
            if ($streamFP) {
                ...
                Get-DattoItems -Config $Config -Space $space -OnItem {
                    ...
                    if ($Config.destination.provider -ne 'LocalSim' -and [DateTimeOffset]::UtcNow -gt $tokenHolder.Exp) {
                        try { $tk = Lzafdefedc3c -Config $Config; $tokenHolder.Token = $tk.Token; $tokenHolder.Exp = $tk.Exp } catch {}
                    }
                    $rp = if ($effSub) { ($effSub.Trim('/') + '/' + $it.RelativePath) } else { $it.RelativePath }
                    $rp = $rp -replace '\\','/'
                    if ($sanitise) { $rp = Lzc7f4602ef7 -Space $row.Space -Rp $rp -Item $it }
                ...
                $expectedTotal += $fTotal
                Lzb0501403e3 INFO "  listing finished: $fTotal file(s), $([math]::Round($spaceBytesTotal/1MB,1)) MB to move."
            ...
            while (@($workers | Where-Object { -not $_.Handle.IsCompleted }).Count -gt 0 -or $resQ.Count -gt 0) {
                $res=$null
                while ($resQ.TryDequeue([ref]$res)) { & $process $res; Lzf9245c2b42 -Activity "Files: $($row.Space)" -Status $res.DestPath -Current $script:fIdxRef.Value -Total $fTotal -Id 2 | Out-Null }
                ...
                if ($Config.destination.provider -ne 'LocalSim' -and [DateTimeOffset]::UtcNow -gt $tokenHolder.Exp) { try { $tk=Lzafdefedc3c -Config $Config; $tokenHolder.Token=$tk.Token; $tokenHolder.Exp=$tk.Exp } catch {} }
                $nowT=[DateTime]::UtcNow.Ticks
                [System.Threading.Monitor]::Enter($thr.SyncRoot)
                try {
                ...
                if ($curMax -ne $script:LastThrMax -or $isPaused -ne $script:LastThrPaused) {
                    Lzd11f863f87 -Max $curMax -HardMax $thr.HardMax -PausedSeconds $pausedSecs -Events $curEvents -Code $curCode
                    ...
                    if ($curMax -lt $thr.HardMax -or $isPaused) {
                        ...
                        Lzb0501403e3 WARN "  $msg In plain terms: this is normal protection, the job keeps running and speeds back up on its own."
                    } elseif ($script:LastThrMax -ne -1) {
                        Lzb0501403e3 OK "  Throttling cleared: back to full speed ($curMax of $($thr.HardMax) uploaders)."
                    }
                    ...
                    if (Test-Path $bwControlPath) {
                        ...
                        if ($st -ne $bwCtrlStamp) {
                            ...
                            if ($nu -ne $bwLastUp -or $nd -ne $bwLastDown) {
                                ...
                                Lzb0501403e3 INFO "  bandwidth changed on the fly: upload $ut, download $dt. Applied immediately."
                            }
                        }
                    }
                } catch {}
            ...
            if ($script:LastThrMax -ne -1 -and $script:LastThrMax -lt $thr.HardMax) { Lzd11f863f87 -Max $thr.HardMax -HardMax $thr.HardMax -PausedSeconds 0 -Events $thr.Events }
        ...
        elseif ($SpoolAhead -le 1) {
            ...
            foreach ($it in $toDo) {
                ...
                Lzf9245c2b42 -Activity "Files: $($row.Space)" -Status $it.RelativePath -Current $fIdx -Total $fTotal -Id 2 | Out-Null
                ...
                try {
                    Lz0d74a11445 -Config $Config -Item $it -OutFile $tempFile | Out-Null
                    ...
                    $v = Lzb2993dd71c -Item $it -LocalFile $tempFile -DestInfo $destInfo
                    $upSw.Stop()
                    if ($v.Ok) {
                        if ($v.Zero) { Lzb0501403e3 WARN "  empty file (0 bytes), uploaded as-is: $($it.RelativePath)"; $zero++ }
                        $copied++; $bytes += $it.Size
                        if ($script:VerboseFiles) { Lzb0501403e3 INFO "  + [$($it.RelativePath)] $($v.Reason)" }
                        $stat = if($v.Zero){'ZeroByte'}else{'Copied'}
                        Lzdab48dc84f -Space $row.Space -Item $it -Status $stat -Reason (Lze4d69b4479 -Repair $repair -Item $it -Status $stat -Reason $v.Reason) -DestSize $destInfo.Size -DestPath $destInfo.Path -DestHash $destInfo.Hash -Method $destInfo.Method -Retries $destInfo.Retries -DurationMs $upSw.Elapsed.TotalMilliseconds -DownloadMs $dlSw.Elapsed.TotalMilliseconds
                    } else {
                        $verifyFail++; Lzb0501403e3 ERROR "  VERIFY FAILED $($it.RelativePath): $($v.Reason). In plain terms: the uploaded copy did not match the original, so it was not marked done. It will be retried on the next Sync."
                        Lzdab48dc84f -Space $row.Space -Item $it -Status 'VerifyFail' -Reason $v.Reason -DestSize $destInfo.Size -DestPath $destInfo.Path -DestHash $destInfo.Hash -Method $destInfo.Method -Retries $destInfo.Retries -DurationMs $upSw.Elapsed.TotalMilliseconds -DownloadMs $dlSw.Elapsed.TotalMilliseconds
                    }
                } catch { if ($dlSw.IsRunning){$dlSw.Stop()}; if($upSw.IsRunning){$upSw.Stop()}; $failed++; Lzb0501403e3 ERROR "  FAILED $($it.RelativePath): $($_.Exception.Message). In plain terms: this file could not be copied. It will be retried on the next Sync."; Lzdab48dc84f -Space $row.Space -Item $it -Status 'Error' -DurationMs $upSw.Elapsed.TotalMilliseconds -DownloadMs $dlSw.Elapsed.TotalMilliseconds -ErrorMsg $_.Exception.Message }
                finally { if (Test-Path $tempFile) { Remove-Item $tempFile -Force -ErrorAction SilentlyContinue } }
                Lzb6d4cf440a -Done $fIdx -Total $fTotal -BytesDone $bytes -BytesTotal $spaceBytesTotal -Space $row.Space -SmallDone $etaSmallDone -SmallTotal $etaSmallTotal -LargeBytesDone $etaLargeDone -LargeBytesTotal $etaLargeTotal -Final 1
            }
        }
        else {
            ...
            foreach ($ready in $spool.GetConsumingEnumerable()) {
                ...
                if (-not $ready.Ok) { $failed++; Lzb0501403e3 ERROR "  DOWNLOAD FAILED $($it.RelativePath): $($ready.Error). In plain terms: the file could not be downloaded from Datto. It will be retried on the next Sync."; Lzdab48dc84f -Space $row.Space -Item $it -Status 'DownloadError' -DownloadMs $dlMs -ErrorMsg $ready.Error; if ($ready.TempFile) { $delQ.Enqueue($ready.TempFile) }; continue }
                ...
                try {
                    ...
                    $v = Lzb2993dd71c -Item $it -LocalFile $ready.TempFile -DestInfo $destInfo
                    $sw.Stop()
                    if ($v.Ok) {
                        if ($v.Zero) { Lzb0501403e3 WARN "  empty file (0 bytes), uploaded as-is: $($it.RelativePath)"; $zero++ }
                        $copied++; $bytes += $it.Size
                        if ($script:VerboseFiles) { Lzb0501403e3 INFO "  + [$($it.RelativePath)] $($v.Reason)" }
                        $stat = if($v.Zero){'ZeroByte'}else{'Copied'}
                        Lzdab48dc84f -Space $row.Space -Item $it -Status $stat -Reason (Lze4d69b4479 -Repair $repair -Item $it -Status $stat -Reason $v.Reason) -DestSize $destInfo.Size -DestPath $destInfo.Path -DestHash $destInfo.Hash -Method $destInfo.Method -Retries $destInfo.Retries -DurationMs $sw.Elapsed.TotalMilliseconds -DownloadMs $dlMs
                    } else {
                        $verifyFail++; Lzb0501403e3 ERROR "  VERIFY FAILED $($it.RelativePath): $($v.Reason). In plain terms: the uploaded copy did not match the original, so it was not marked done. It will be retried on the next Sync."
                        Lzdab48dc84f -Space $row.Space -Item $it -Status 'VerifyFail' -Reason $v.Reason -DestSize $destInfo.Size -DestPath $destInfo.Path -DestHash $destInfo.Hash -Method $destInfo.Method -Retries $destInfo.Retries -DurationMs $sw.Elapsed.TotalMilliseconds -DownloadMs $dlMs
                    }
                } catch { $sw.Stop(); $failed++; Lzb0501403e3 ERROR "  UPLOAD FAILED $($it.RelativePath): $($_.Exception.Message). In plain terms: the file could not be uploaded to Microsoft 365. It will be retried on the next Sync."; Lzdab48dc84f -Space $row.Space -Item $it -Status 'Error' -DurationMs $sw.Elapsed.TotalMilliseconds -DownloadMs $dlMs -ErrorMsg $_.Exception.Message }
        ...
        if ($willWrite) {
            Lzb0501403e3 $lvl "  $($row.Space): copied $copied, skipped $skipped, failed $failed, failed verification $verifyFail, empty files $zero ($szTxt)"
        } else {
            Lzb0501403e3 INFO "  $($row.Space): PREVIEW - would copy $copied file(s) ($szTxt) and skip $skipped unchanged. Nothing was uploaded."
        }
        ...
        $spacesCompleted++
        if ($willWrite -and -not $ResultDir) { Lz3ac2d30287 -Config $Config -Mode $Mode -StartUtc $runStart -AuditFile $script:AuditFile -ExpectedFiles $expectedTotal -SpacesPlanned $spacesPlanned -SpacesCompleted $spacesCompleted }
    }
    ...
    if ($ResultDir) {
        ...
        $summary | Lz8e2163bc45 | Export-Csv -Path (Join-Path $Config.run.reportRoot "api-transfer-$Mode-$(Get-Date -Format 'yyyyMMdd-HHmmss').csv") -NoTypeInformation -Encoding UTF8
    }
    ...
    if ($willWrite) {
        ...
        Lzb0501403e3 $lvl "$(if ($Mode -eq 'Delta') { 'Sync' } else { 'Upload' }) finished in $(Format-Duration $elapsed) of copying. Totals: copied $tc, skipped $ts, failed $tf, failed verification $tv, $szTxt.$rate  Rate-limit pauses and retries: $script:RetryEvents."
        if ($script:RenameCount -gt 0) { Lzb0501403e3 INFO "Tidied $script:RenameCount name(s) to suit SharePoint (trailing spaces, illegal characters). Every change is listed in the report and in: $script:RenameFile" }
        if ($script:CollisionCount -gt 0) { Lzb0501403e3 WARN "$script:CollisionCount name collision(s): two Datto items would land on the same SharePoint path. Review before relying on this migration. Details in the report and in: $script:CollisionFile" }
        if (($tf + $tv) -gt 0) {
            Lzb0501403e3 WARN "In plain terms: uploaded $tc file(s) ($szTxt), but $($tf + $tv) did not make it ($tf failed, $tv could not be verified). Next step: click 'Sync new and changed' to retry the ones that did not upload, then 'Verify files arrived' to confirm everything is there."
            Lzb0501403e3 WARN "Review this log for [ERROR] lines (files that failed or failed verification). Failed files are not recorded as done; 'Sync new and changed' will retry them."
        } else {
            Lzb0501403e3 OK "In plain terms: all done. Uploaded $tc file(s) ($szTxt) with no failures. Next step (optional): click 'Verify files arrived' to double-check, or 'Compare sizes' for a quick overview."
        }
        if (-not $ResultDir -and $script:AuditFile -and (Test-Path $script:AuditFile)) {
            Lzb0501403e3 OK "Audit trail written: $script:AuditFile"
            try {
                $diag = Lzd584848127 -Rows @(Import-Csv $script:AuditFile) -Elapsed $elapsed -ConfiguredWorkers $UploadWorkers -ThrottleEvents $script:RetryEvents
                if ($diag) {
                    Lzb0501403e3 INFO "What limited this run: $($diag.Verdict). $($diag.Why)"
                    Lzb0501403e3 INFO "  $($diag.Action)"
                    Lzb0501403e3 INFO "  Numbers: moved $(Format-Bytes $diag.Bytes) at about $($diag.AggMbps) Mb/s overall; single file about $($diag.PerStreamMbps) Mb/s; $($diag.UpSec)s uploading vs $($diag.DlSec)s downloading."
                    if ($diag.Concurrency) { Lzb0501403e3 INFO "  Concurrency: $($diag.Concurrency)" }
                    if ($diag.RetryDetail) { Lzb0501403e3 INFO "  Retries: $($diag.RetryDetail)" }
                    if ($diag.Projection)  { Lzb0501403e3 INFO "  Planning: $($diag.Projection)" }
                    if ($diag.Headroom)    { Lzb0501403e3 INFO "  Headroom: $($diag.Headroom)" }
                }
            } catch {}
        }
    }
    else { Lzb0501403e3 OK "Preview finished in $el. This was a rehearsal, nothing was uploaded. It would copy $tc file(s), $szTxt. In plain terms: if this looks right, click 'Upload all files' to do it for real." }
    }
    catch {
        $transferError = $_
        Lzb0501403e3 ERROR "RUN ABORTED before all files were processed: $($_.Exception.Message)"
    }
    finally {
        Lz18e3356160 -Config $Config -Mode $Mode -RunStart $runStart -ExpectedFiles $expectedTotal `
...
function Lz0b1da32a26 {
    ...
    Lzb0501403e3 INFO "Parallel projects: $($Rows.Count) project(s), $MaxParallelSpaces at a time."
    $parallelStart = Get-Date
    $childExitCodes = New-Object System.Collections.Generic.List[int]
    if ($willWrite) { Lz3ac2d30287 -Config $Config -Mode $Mode -StartUtc $parallelStart -AuditFile '' -ExpectedFiles 0 -SpacesPlanned $Rows.Count -SpacesCompleted 0 }
    ...
    if ($auditRows.Count) {
        ...
        Lzb0501403e3 OK "Audit trail written: $auditOut ($($auditRows.Count) file actions)"
        if ($willWrite) {
    ...
    Lzb0501403e3 OK "Parallel projects finished. Copied $tc, failed $tf, failed verification $tv. Record: $combined"
}
...
function Get-DestinationInventory {
    ...
    $emitDest = {
        ...
        Lzb0501403e3 INFO "  DESTINATION: $('{0:N0}' -f $folders) folder(s) read, $('{0:N0}' -f $count) file(s) found so far.$w"
    ...
    while ($stack.Count -gt 0) {
        ...
        try {
            ...
            if ($cur.Root -and $st -eq 404) {
                Lzb0501403e3 INFO "  destination inventory: the folder does not exist at the destination yet, so there is nothing to compare against and every file will be copied. This is normal the first time you copy into a new folder."
    ...
    $extra = $pages - $folders
    Lzb0501403e3 INFO "  DESTINATION FINISHED: $('{0:N0}' -f $count) file(s) across $('{0:N0}' -f $folders) folder(s), $pages page(s) followed."
    if ($extra -gt 0) {
        Lzb0501403e3 WARN "  $extra destination folder(s) held more than one page of children. Before the paging fix everything past the first page was invisible here, so a Sync would have judged those files missing and re-uploaded over the top of them. They are now enumerated in full."
    }
...
function Lzd9e38b7ce4 {
    ...
    if (-not $agg.Count) { Lzb0501403e3 OK "  [$($Row.Space)] every file matches by path, both ways. Nothing extra, nothing missing."; return ([pscustomobject]@{ Missing = 0; Extra = 0 }) }
    $rows = New-Object System.Collections.Generic.List[object]
    ...
    try {
        $rows | Lz8e2163bc45 | Export-Csv -Path $out -NoTypeInformation -Encoding UTF8
        Lzb0501403e3 OK "  [$($Row.Space)] difference written (one row per file): $out"
        if ($script:GuiMode) { Write-Host "##CHECKFILES##|$nMissing|$nExtra|0|$out" }
    } catch {
        Lzb0501403e3 WARN "  [$($Row.Space)] could not write the difference CSV: $($_.Exception.Message)"
    }
    if ($nMissing) {
        Lzb0501403e3 WARN "  [$($Row.Space)] $('{0:N0}' -f $nMissing) file(s) in Datto that are NOT at the destination:"
        foreach ($p in @($missingPaths | Select-Object -First 15)) { Lzb0501403e3 WARN "      missing: /$p" }
        if ($nMissing -gt 15) { Lzb0501403e3 WARN "      ... and $('{0:N0}' -f ($nMissing - 15)) more. Every one is named in the CSV above." }
    }
    if ($nExtra)   { Lzb0501403e3 INFO "  [$($Row.Space)] $('{0:N0}' -f $nExtra) file(s) at the destination that are NOT in Datto (usually pre-existing content). Biggest folders:" }
    foreach ($r in @($sortedAgg | Where-Object { $_.Side -eq 'AtDestinationOnly' } | Select-Object -First 8)) {
        Lzb0501403e3 INFO ("      {0,8:N0}  /{1}" -f $r.Files, $r.Folder)
    }
    return ([pscustomobject]@{ Missing = $nMissing; Extra = $nExtra })
}
function Lzb580882364 {
    param($Config, [string]$SpaceName, [string]$ResultDir)
    Lz2866944e93 -Config $Config -Stage 'api-destinv'
    ...
    Lzb0501403e3 OK "  DESTINATION inventory written: $('{0:N0}' -f [int]$inv.Count) file(s). $out"
}
function Lz3a6ccb8a80 {
    param($Config, $Row)
    try {
        $pwshExe = [System.Diagnostics.Process]::GetCurrentProcess().MainModule.FileName
        $self = $PSCommandPath
        ...
        Lzb0501403e3 INFO "  Reading the DESTINATION in the background (helper process $($proc.Id)) while the Datto list is read."
        return @{ Process = $proc; File = (Join-Path $dir "destinv-$slug.json"); OutLog = $out; ErrLog = $err; Pos = [long]0; Partial = '' }
    } catch {
        Lzb0501403e3 WARN "  could not start the background destination read ($($_.Exception.Message)). The destination will be read after the source instead - slower, same answer."
        return $null
    }
}
function Lz0e5f4e90ae {
    param($Job)
...
function Receive-DestInventoryJob {
    ...
    try {
        ...
        Lzb0501403e3 OK "  DESTINATION list received from the background helper: $('{0:N0}' -f [int]$doc.Count) file(s)."
        return @{ Count = [int]$doc.Count; Bytes = [int64]$doc.Bytes; Paths = $paths; Times = $times; Sizes = $sizes }
    } catch {
        Lzb0501403e3 WARN "  the background destination list could not be used ($($_.Exception.Message)). Reading the destination directly instead - slower, same answer."
...
function Invoke-SizeCheck {
    ...
    Lz2866944e93 -Config $Config -Stage 'api-sizecheck'
    $ckStart = Get-Date
    Lze943a6df5b -Config $Config
    Connect-Destination -Config $Config
    $mapPath = Join-Path $Config.run.reportRoot 'mapping.csv'
    if (-not (Test-Path $mapPath)) { Lzb0501403e3 ERROR "No mapping.csv found. Set a destination and save the mapping first."; return }
    $map = @(Import-Csv $mapPath | Where-Object Action -eq 'MIGRATE')
    if ($OnlySpace) { $map = @($map | Where-Object { $_.Space -eq $OnlySpace }) }
    if (-not $map.Count) { Lzb0501403e3 WARN "No mapped projects to check."; return }
    ...
    foreach ($row in $map) {
        $i++; Lzb6d4cf440a -Done $i -Total $t -BytesDone 0 -BytesTotal 0 -Space $row.Space
        try {
            $null = Resolve-DestinationDriveId -Config $Config -Row $row
            Lzb0501403e3 OK "  destination reachable: $(Get-RowDestLabel $row). Reading the source now."
        } catch {
            Lzb0501403e3 ERROR "  destination NOT reachable for [$($row.Space)], so the source was not read: $($_.Exception.Message)"
            continue
        }
        $space = New-SpaceRef -Row $row
        $dj = Lz3a6ccb8a80 -Config $Config -Row $row
        $items = @(Get-DattoItems -Config $Config -Space $space)
        $sc = $items.Count
        ...
        $d = Lzd9e38b7ce4 -Config $Config -Row $row -Items $items -Inv $inv
        ...
        Lzb0501403e3 $lvl ("  [$($row.Space)]  source $sc files / $(Format-Bytes $sb)   ->   destination $($inv.Count) files / $(Format-Bytes $inv.Bytes)   [$tag]")
    }
    Lzb0501403e3 OK ("TOTAL  source $tSrcC files / $(Format-Bytes $tSrcB)    destination $tDstC files / $(Format-Bytes $tDstB)")
    if ($tMissing -gt 0) {
        ...
        Lzb0501403e3 INFO "The destination has more files than the source. That is normal if the destination already held content before the migration. To confirm every source file actually arrived, click 'Verify files arrived': it checks file by file and ignores pre-existing extras."
    }
    Lzb0501403e3 INFO "Bytes differ from the source because Microsoft 365 rewrites Office files on upload, so treat totals as indicative. 'Verify files arrived' is the definitive per-file check."
    try { Lz5d2ed58c0f -Config $Config -Rows $scRows -SrcCount $tSrcC -SrcBytes $tSrcB -DstCount $tDstC -DstBytes $tDstB -Missing $tMissing -Extra $tExtra } catch { Lzb0501403e3 WARN "Compare report could not be written: $($_.Exception.Message)" }
    if ($script:GuiMode) {
...
<header><div><h1>$(_encS $brand)</h1><div class='sub'>$(_encS $Subtitle) &middot; generated $($now.ToString('dd MMM yyyy HH:mm'))$verSub</div></div><img class='brandlogo' alt='Liscaragh Software' src='data:image/jpeg;base64,$($script:LiscaraghLogoB64)' /></header>
$Banner
$Body
<footer>Source: Datto Workplace &middot; This is a read-only check: nothing was uploaded or changed &middot; Liscaragh Migrate &middot; Liscaragh Software &middot; www.liscaragh.com</footer>
</div></body></html>
"@
}
function Lz2f211917c7 {
    ...
    if ($fail -gt 0)       { $bStyle='background:#FEF3F2;border:1px solid #FDA29B;color:#B42318'; $bTitle='Some files are missing'; $bExtra=" $missTotal file(s) in Datto are not at the destination across $fail project(s). Run 'Sync new and changed', then verify again." }
    elseif ($stalep -gt 0) { $bStyle='background:#FFFAEB;border:1px solid #FEDF89;color:#B5460F'; $bTitle='Everything is present, some are out of date'; $bExtra=" $staleTotal file(s) are newer in Datto than at the destination. Run 'Sync new and changed' with 'Update where Datto is newer' to refresh them." }
    ...
    if ($missTotal) {
        ...
        $missSec = "<h2>Missing files ($missTotal)</h2><table><thead><tr><th>Project</th><th>File</th></tr></thead><tbody>$mr</tbody></table>$cap"
    ...
    if ($staleTotal) {
        ...
        $staleSec = "<h2>Out-of-date files ($staleTotal)</h2><table><thead><tr><th>Project</th><th>File</th></tr></thead><tbody>$sr</tbody></table>$cap2"
    } else { $staleSec = '' }
    $body = @"
<div class='cards'>
 <div class='card'><div class='v'>$projCount</div><div class='l'>Projects checked</div></div>
...
<h2>By project</h2>
<table><thead><tr><th>Project</th><th class='n'>Source</th><th class='n'>Present</th><th class='n'>Missing</th><th class='n'>Out of date</th><th class='n'>Extra at dest</th><th>Verdict</th></tr></thead><tbody>
$rows
</tbody></table>
$missSec
$staleSec
"@
    $html = Lza9940065c4 -Config $Config -Subtitle 'Verify files arrived (per-file check)' -Banner $banner -Body $body
    $outPath = Join-Path $Config.run.reportRoot ("report-validation-" + (Get-Date).ToString('yyyyMMdd-HHmmss') + ".html")
    Set-Content -Path $outPath -Value $html -Encoding UTF8
    Lzb0501403e3 OK "Verify report written: $outPath"
}
function Lz5d2ed58c0f {
    ...
    if ($Missing -gt 0)   { $bStyle='background:#FFFAEB;border:1px solid #FEDF89;color:#B5460F'; $bTitle='Some files are missing at the destination'; $bExtra=" $Missing file(s) in Datto are not at the destination. Run 'Sync new and changed', then compare again.$(if($Extra){" The destination also holds $Extra extra file(s), usually pre-existing content."})" }
    elseif ($Extra -gt 0) { $bStyle='background:#FFFAEB;border:1px solid #FEDF89;color:#B5460F'; $bTitle='The destination holds extra files'; $bExtra=" $Extra file(s) at the destination are not in Datto, usually content that was there before the migration. 'Verify files arrived' checks file by file and ignores extras." }
    ...
    $body = @"
<div class='cards'>
 <div class='card'><div class='v'>$SrcCount</div><div class='l'>In Datto</div></div>
...
<h2>By project</h2>
<table><thead><tr><th>Project</th><th class='n'>Datto files</th><th class='n'>Datto data</th><th class='n'>Dest files</th><th class='n'>Dest data</th><th class='n'>Missing</th><th class='n'>Extra</th></tr></thead><tbody>
$rowHtml
</tbody></table>
...
"@
    $html = Lza9940065c4 -Config $Config -Subtitle 'Compare sizes (count and byte overview)' -Banner $banner -Body $body
    $outPath = Join-Path $Config.run.reportRoot ("report-sizecheck-" + (Get-Date).ToString('yyyyMMdd-HHmmss') + ".html")
    Set-Content -Path $outPath -Value $html -Encoding UTF8
    Lzb0501403e3 OK "Compare report written: $outPath"
}
...
function Invoke-HtmlReport {
    ...
    if (-not $NoStageLog) { Lz2866944e93 -Config $Config -Stage 'api-report' }
    if (-not $AuditPath) {
        $latest = Get-ChildItem (Join-Path $Config.run.reportRoot 'audit-*.csv') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if ($latest) { $AuditPath = $latest.FullName }
    }
    if (-not $AuditPath -or -not (Test-Path $AuditPath)) { Lzb0501403e3 ERROR "No audit CSV found in $($Config.run.reportRoot). In plain terms: there is no record to build a report from yet. Run an upload first, then build the report."; return }
    ...
    if (-not $rows.Count) { Lzb0501403e3 WARN "Audit CSV is empty: $AuditPath"; return }
    function _enc { param($s) [System.Net.WebUtility]::HtmlEncode("$s") }
    function Lz50bbe59cbc { param($st) @($rows | Where-Object { $_.Status -eq $st }).Count }
    ...
    $total = $rows.Count
    $copied = (Lz50bbe59cbc 'Copied') + (Lz50bbe59cbc 'ZeroByte')
    $skipped = Lz50bbe59cbc 'Skipped'
    $errors = (Lz50bbe59cbc 'Error') + (Lz50bbe59cbc 'DownloadError') + (Lz50bbe59cbc 'SkippedTooLarge')
    $verifyFail = Lz50bbe59cbc 'VerifyFail'
    $wouldCopy = Lz50bbe59cbc 'WouldCopy'
    ...
    $diagSection = ''
    if (-not $dryRun) {
        $diag = $null; try { $diag = Lzd584848127 -Rows $rows -Elapsed $elapsed } catch {}
        if ($diag) {
            $diagSection = @"
...
"@
    ...
    if ($dryRun) {
        ...
        if ($tooBig.Count) {
            ...
            $bigBits += "<details open><summary style='color:#B42318'><strong>$($tooBig.Count) file(s) over the SharePoint 250 GB per-file limit</strong>: these cannot be uploaded and need another home &middot; click for details</summary><table><thead><tr><th>Project</th><th>File</th><th class='n'>Size</th></tr></thead><tbody>$tr</tbody></table></details>"
        }
        if ($bigSlow.Count) {
...
<footer>Source: Datto Workplace &middot; Full per-file detail: $(_enc ([System.IO.Path]::GetFileName($AuditPath))) &middot; Generated from the migration audit trail by Liscaragh Migrate &middot; Liscaragh Software &middot; www.liscaragh.com</footer>
</div>
<script>window.addEventListener('beforeprint',function(){document.querySelectorAll('details').forEach(function(d){d.open=true;});});</script>
</body></html>
"@
    ...
    Set-Content -Path $outPath -Value $html -Encoding UTF8
    Lzb0501403e3 OK "HTML report written: $outPath"
    return $outPath
}
function Invoke-CompletionCertificate {
    param($Config, [string]$AuditPath, [switch]$NoStageLog)
    if (-not $NoStageLog) { Lz2866944e93 -Config $Config -Stage 'api-certificate' }
    if (-not $AuditPath) {
        $latest = Get-ChildItem (Join-Path $Config.run.reportRoot 'audit-*.csv') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if ($latest) { $AuditPath = $latest.FullName }
    }
    if (-not $AuditPath -or -not (Test-Path $AuditPath)) { Lzb0501403e3 ERROR "No audit CSV found in $($Config.run.reportRoot). A certificate is built from a completed run's audit trail; run a migration to completion first."; return }
    $rows = @(Import-Csv $AuditPath)
    $endRow = @($rows | Where-Object { "$($_.Status)" -like 'RUN_END:*' }) | Select-Object -Last 1
    $rows   = @($rows | Where-Object { "$($_.Status)" -notlike 'RUN_END:*' })
    if (-not $rows.Count) { Lzb0501403e3 WARN "Audit CSV is empty: $AuditPath"; return }
    ...
    if ($dryRun -or @('PREVIEW','INCOMPLETE','ENDED_EARLY','CANCELLED') -contains $ocStatus) {
        Lzb0501403e3 ERROR ("A completion certificate is only issued for a completed migration. This run is '" + $ocStatus + "'. Run a full sync through to completion (then a verify), and issue the certificate from that run.")
        return
    }
...
"@
    ...
    Set-Content -Path $outPath -Value $html -Encoding UTF8
    Lzb0501403e3 OK "Completion certificate written: $outPath"
    return $outPath
}
function Invoke-Validate {
    param($Config)
    Lz2866944e93 -Config $Config -Stage 'api-validation'
    $ckStart = Get-Date
    Lze943a6df5b -Config $Config
    Connect-Destination -Config $Config
    $map = @(Import-Csv (Join-Path $Config.run.reportRoot 'mapping.csv') | Where-Object Action -eq 'MIGRATE')
    ...
    $t = $map.Count; $i = 0
    foreach ($row in $map) {
        $i++; Lzf9245c2b42 -Activity 'Verify' -Status $row.Space -Current $i -Total $t | Out-Null
        ...
        if ($verdict -eq 'PASS') {
            Lzb0501403e3 OK "  $($row.Space): PASS - all $srcCount file(s) are present and up to date at the destination.$extraNote"
        } elseif ($verdict -eq 'OUT OF DATE') {
            Lzb0501403e3 WARN "  $($row.Space): OUT OF DATE - all $srcCount file(s) are present, but $($stale.Count) have a newer version in Datto than at the destination (see api-validation-stale.csv). In plain terms: those files were changed in Datto since they were copied. Run 'Sync new and changed' with 'Update where Datto is newer' to bring them up to date."
        } else {
            Lzb0501403e3 ERROR "  $($row.Space): FAIL - $($missing.Count) of $srcCount file(s) were NOT found at the destination (see api-validation-missing.csv).$(if($stale.Count){" $($stale.Count) present file(s) are also out of date."}) In plain terms: some files did not make it across. Run 'Sync new and changed' to copy them, then verify again."
            foreach ($m in @($missing | Select-Object -First 15)) { Lzb0501403e3 ERROR "      missing: /$m" }
            if ($missing.Count -gt 15) { Lzb0501403e3 ERROR "      ... and $('{0:N0}' -f ($missing.Count - 15)) more, all named in api-validation-missing.csv." }
        }
    }
    try { Write-Progress -Activity 'Verify' -Completed } catch {}
    $report | Lz8e2163bc45 | Export-Csv -Path (Join-Path $Config.run.reportRoot 'api-validation-report.csv') -NoTypeInformation -Encoding UTF8
    try { Lz2f211917c7 -Config $Config -Report $report -Missing $missingAll -Stale $staleAll } catch { Lzb0501403e3 WARN "Verify report could not be written: $($_.Exception.Message)" }
    if ($missingAll.Count) {
        $missingAll | Lz8e2163bc45 | Export-Csv -Path (Join-Path $Config.run.reportRoot 'api-validation-missing.csv') -NoTypeInformation -Encoding UTF8
        if ($script:GuiMode) { Write-Host "##CHECKFILES##|$($missingAll.Count)|0|0|$(Join-Path $Config.run.reportRoot 'api-validation-missing.csv')" }
    }
    if ($staleAll.Count) {
        $staleAll | Lz8e2163bc45 | Export-Csv -Path (Join-Path $Config.run.reportRoot 'api-validation-stale.csv') -NoTypeInformation -Encoding UTF8
        if ($script:GuiMode) { Write-Host "##CHECKFILES##|0|0|$($staleAll.Count)|$(Join-Path $Config.run.reportRoot 'api-validation-stale.csv')" }
    }
    $fail  = @($report | Where-Object Verdict -eq 'FAIL').Count
    $stalep = @($report | Where-Object Verdict -eq 'OUT OF DATE').Count
    if ($fail -gt 0) { Lzb0501403e3 ERROR "Verify complete. $fail project(s) have missing files (see api-validation-missing.csv). Run 'Sync new and changed' to re-copy them, then verify again." }
    elseif ($stalep -gt 0) { Lzb0501403e3 WARN "Verify complete. Everything is present, but $stalep project(s) have file(s) that are newer in Datto than at the destination (see api-validation-stale.csv). Run 'Sync new and changed' with 'Update where Datto is newer' to refresh them." }
    else { Lzb0501403e3 OK "Verify complete. Every file is present at the destination and up to date (no file is newer in Datto). Extra files already in the destination are expected and were ignored." }
    if ($script:GuiMode) {
...
function Lzd28374f0ab {
    param($Config)
    Lz2866944e93 -Config $Config -Stage 'api-permissions'
    Lze943a6df5b -Config $Config
    $map = @(Import-Csv (Join-Path $Config.run.reportRoot 'mapping.csv') | Where-Object Action -eq 'MIGRATE')
    function Lz3df9534b63 { param([string]$r) switch -Regex ($r.ToLower()) { 'owner|manage|admin'{'Site Owner (Full Control)';break} 'edit|write|contrib'{'Member (Edit)';break} 'view|read'{'Visitor (Read)';break} default{'REVIEW'} } }
    $plan = New-Object System.Collections.Generic.List[object]
    $t = $map.Count; $i = 0
    foreach ($row in $map) {
        $i++; Lzf9245c2b42 -Activity 'Permissions' -Status $row.Space -Current $i -Total $t | Out-Null
        $space = New-SpaceRef -Row $row
        foreach ($p in @(Lzee0692c5be -Config $Config -Space $space)) {
            $plan.Add([pscustomobject]@{ Space=$row.Space; Principal=$p.Principal; DattoRole=$p.Role; DestinationUrl=$row.DestinationUrl; ProposedM365Role=(Lz3df9534b63 $p.Role) })
        }
    }
    try { Write-Progress -Activity 'Permissions' -Completed } catch {}
    $out = Join-Path $Config.run.reportRoot 'api-permissions-plan.csv'
    $plan | Lz8e2163bc45 | Export-Csv -Path $out -NoTypeInformation -Encoding UTF8
    if ($plan.Count -eq 0) { Lzb0501403e3 WARN "No permissions returned by the API (endpoint may not be configured). $out is empty." }
    else { Lzb0501403e3 OK "Permissions plan written (NOT applied): $($plan.Count) entries. $out" }
}
function Lz0c09a5cf6a {
    param($Config)
    Lz2866944e93 -Config $Config -Stage 'api-test'
    Lze943a6df5b -Config $Config
    Connect-Destination -Config $Config
    $spaces = @(Get-DattoSpaces -Config $Config)
    Lzb0501403e3 OK "Datto returned $($spaces.Count) project(s)."
    if ($spaces.Count) {
        $s = $spaces[0]
        Lzb0501403e3 INFO "First project: $($s.Name) ($($s.Type))"
        $items = @(Get-DattoItems -Config $Config -Space $s | Select-Object -First 5)
        foreach ($it in $items) { Lzb0501403e3 INFO "  file: $($it.RelativePath) ($([math]::Round($it.Size/1KB,1)) KB, mod $($it.ModifiedUtc))" }
        Lzb0501403e3 OK "If the project and files above look right, the Datto connection is reading real data correctly."
    }
}
function Lz6933502a3d {
    param($Config)
    while ($true) {
        ...
        switch ((Read-Host 'Select').ToUpper()) {
            '1' { Lz0c09a5cf6a -Config $Config }
            '2' { Lz96d0127fb2 -Config $Config }
            '3' { Lz92f2837a39 -Config $Config }
            ...
            '8' {
                Lz96d0127fb2 -Config $Config
                $mapPath = Join-Path $Config.run.reportRoot 'mapping.csv'
                if (Test-Path $mapPath) {
                    Lz92f2837a39 -Config $Config
...
if (-not $parallelEnabled -and $MaxParallelSpaces -gt 1) {
    Lzb0501403e3 INFO "Parallel projects are off (run.parallel.enabled is not set); running one project at a time."
    $MaxParallelSpaces = 1
}
...
try {
    ...
    if ($needsLicence -and ($cfg.destination.provider -ne 'LocalSim')) {
        ...
        if ($lic.Licensed) {
            ...
            Lzb0501403e3 OK "Licence accepted: $($lic.Customer), Microsoft tenant $($lic.TenantId) (licence $($lic.LicenceId))."
        }
        elseif ($lic.Reason -match '^no licence file') {
            if ($Action -eq 'Validate') {
                if ($script:GuiMode) { Write-Host "##TRIAL##|START|0|Validate" }
                Lzb0501403e3 WARN "No licence: EVALUATION MODE. Verify will still check whatever is at the destination, so you can confirm the trial copy landed correctly. Licence this Microsoft tenant at https://www.liscaragh.com to remove the limit."
            }
            else {
                ...
                Lzb0501403e3 WARN "No licence: EVALUATION MODE. Up to $remaining $what file(s) will be copied for real so you can prove the migration works end to end, then it stops. This is a sample, not a full migration: this tenant's free $what allowance is $($script:TrialFileLimit) file(s) in total, and $spent have been used. Licence this Microsoft tenant at https://www.liscaragh.com, then Help > Install licence file, to migrate everything."
            }
        }
        else {
            if ($script:GuiMode) { Write-Host "##LICENCE##|$($lic.Reason)" }
            Lzb0501403e3 ERROR "This action needs a valid licence: $($lic.Reason). Without a valid licence you can still test the connection, list projects, Preview (see what would copy, without copying), and run 'Compare sizes'. Obtain a licence at https://www.liscaragh.com, then install it via Help > Install licence file."
            exit 1
        }
    }
    switch ($Action) {
        'TestApi'     { Lz0c09a5cf6a        -Config $cfg }
        'Discover'    { Lz96d0127fb2 -Config $cfg -NonInteractive:$NonInteractive }
        'PreFlight'   { Lz92f2837a39 -Config $cfg }
        ...
        'DestInventory' { Lzb580882364 -Config $cfg -SpaceName $Spaces -ResultDir $ResultDir }
        ...
        'Finalize'    { Lz28d83fc621 -Config $cfg -Status $FinalizeStatus -AuditPath $AuditPath; exit $script:ExitCode }
        default       { Lz6933502a3d       -Config $cfg }
    }
}
catch {
    $e = $_
    $emit = {
        ...
        try { Lzb0501403e3 $Level $Text } catch { Write-Host "[$Level] $Text" }
    }
