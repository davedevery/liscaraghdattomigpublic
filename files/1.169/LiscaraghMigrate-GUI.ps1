...
function Lz43339df778 {
    param([string]$Severity)
    Lz82a0a98721 -Severity $Severity
    try {
...
function Lz5552fa5ad2 { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lz1623024dca {
...
function Lz85bf18b0b4 {
    ...
    if (-not $s) { $s = Lzcc5fd8d01f 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lz9cc75f7c85 {
    param([string]$Value)
    if (Lz1623024dca -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lzbf68b88727 {
    $f = @(Lz2d612467b0)
    if ($f.Count -eq 1) { return $f[0] }
    return ''
}
function Lzf2f796633f {
...
function Lz24c1a33dad {
    param([string[]]$Picked)
    $result = New-Object System.Collections.Generic.List[string]
    foreach ($e in @(Lz2d612467b0)) { [void]$result.Add($e) }
    $dropped = New-Object System.Collections.Generic.List[string]
    foreach ($p in @($Picked | ForEach-Object { "$_".Trim().Trim('/').Trim('\') } | Where-Object { $_ })) {
        if ($result -ccontains $p) { continue }
        $covered = $false
        foreach ($k in @($result)) { if (Lzf2f796633f -Child $p -Parent $k) { [void]$dropped.Add($p); $covered = $true; break } }
        if ($covered) { continue }
        foreach ($narrow in @($result | Where-Object { Lzf2f796633f -Child $_ -Parent $p -Strict })) { [void]$result.Remove($narrow); [void]$dropped.Add($narrow) }
        [void]$result.Add($p)
    }
    Lz0cac7eb3ce -Folders @($result)
    if ($dropped.Count) { Lzf194790344 ("Overlapping folder(s) merged, kept the broader one: " + ((@($dropped) | Select-Object -Unique) -join ', ')) }
}
function Lz5a4d48d5e4 {
    $one = $true; try { $one = (@(Lze4e5bbb936).Count -le 1) } catch {}
    $n = @(Lz2d612467b0).Count
...
function Lz29c0cd0cab {
    ...
    try {
        ...
        $norm = Lz3eb744cf4e -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lzf194790344 "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz0155212fcd
    } catch { }
}
function Lz3eb744cf4e {
...
function Lza3abb20e90 {
    ...
    return @{ SubfolderCount = @(Lz0ca1ba7c08 -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lz541fa5bcf1 {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lzb665da5837 -Path "sites/$siteId"
    } catch { return $null }
}
function Lz441a7910aa {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lzb665da5837 -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lzd763859e20 {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lzb665da5837 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lza238b84bc8 {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lz4b8134a5fa -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
function Lz8f01b8ac5b {
    param($State, [string]$Current)
    $out = New-Object System.Collections.Generic.List[string]
    foreach ($s in @($State.Lst.SelectedItems)) {
        $name = "$s"
        if (-not (Lzf77082ee91 -Item $name)) { continue }
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lz441a7910aa -Search $s.Txt.Text)
...
"@
    ...
    Lza238b84bc8
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lza238b84bc8 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lza238b84bc8 } })
    $new.Add_Click({
        $n = Lz1ae4d3e40c -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lz73dca31daa -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lza238b84bc8; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lzf285f5fbd4 -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lzead8a811b1 -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lzf285f5fbd4 -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lz7c79a9d466 {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lz0ca1ba7c08 -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lz7c79a9d466
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lz7c79a9d466 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lz7c79a9d466 } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lzf285f5fbd4 -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lz8f01b8ac5b -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lzf285f5fbd4 -State $script:SP
    $r = $w.ShowDialog()
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        FontFamily="Segoe UI" FontSize="14" Foreground="#111827" Background="#EBEDF1" WindowStartupLocation="CenterScreen">
  <Window.Resources>
    <!-- Palette -->
  ...
  <DockPanel LastChildFill="True">
    ...
    <Grid Margin="10">
      ...
      <Border Grid.Row="0" Grid.RowSpan="2" Panel.ZIndex="100" HorizontalAlignment="Right" VerticalAlignment="Top"
        ...
        <Image x:Name="ImgLogoTop" Height="75" Stretch="Uniform" RenderOptions.BitmapScalingMode="HighQuality" ToolTip="Liscaragh Software"/>
      </Border>
      ...
      <Grid Grid.Row="1">
        ...
        <Grid Grid.Column="0" Margin="0,0,8,0">
                    ...
                    <DataGridTemplateColumn.CellTemplate>
                      <DataTemplate>
                        ...
                        <DataTemplate.Triggers>
                          ...
                          <DataTrigger Binding="{Binding Status}" Value="Queued"><Setter TargetName="dot" Property="Fill" Value="#B5460F"/><Setter TargetName="txt" Property="Foreground" Value="#B5460F"/></DataTrigger>
                          ...
                          <!-- Selected row wins LAST: the status colours above are dark (green #066A4B,
                               steel #1C6091) and vanish against the selected-row highlight (#1C6091). This
                               trigger is declared after them, so when the row is selected the dot and text
                               go white and stay legible; when it is not selected, the status colour applies.
                               The word is still shown beside the dot, so colour is never the only cue. -->
                          <DataTrigger Binding="{Binding IsSelected, RelativeSource={RelativeSource AncestorType=DataGridRow}}" Value="True"><Setter TargetName="dot" Property="Fill" Value="White"/><Setter TargetName="txt" Property="Foreground" Value="White"/></DataTrigger>
                        </DataTemplate.Triggers>
                      </DataTemplate>
                    </DataGridTemplateColumn.CellTemplate>
          ...
          <GroupBox Grid.Row="1" Margin="0,8,8,0" >
            <GroupBox.Header><TextBlock Text="Run the migration" Foreground="#1C6091" FontWeight="SemiBold"/></GroupBox.Header>
        ...
        <GroupBox Grid.Row="0" >
          <GroupBox.Header><TextBlock Text="Source and Destination mapping for the selected project" Foreground="#1C6091" FontWeight="SemiBold"/></GroupBox.Header>
          <StackPanel Margin="8,5,8,6">
            ...
            <Grid Margin="0,0,0,6">
              ...
              <StackPanel Grid.Column="2" Orientation="Vertical">
                <Button x:Name="BtnBrowseFolder" Content="Browse..." Margin="6,0,0,0" Padding="8,2" ToolTip="List the folders that already exist at this destination, pick one to drill into, or create a new folder."/>
                <Button x:Name="BtnTestDest" Content="Test" Margin="6,4,0,0" Padding="8,2" ToolTip="Check this destination is reachable right now (the user's OneDrive exists, or the SharePoint site and library exist). Changes nothing."/>
              </StackPanel>
            </Grid>
        ...
        <GroupBox Grid.Row="1" Margin="0,8,0,0" >
          <GroupBox.Header><TextBlock Text="Check and review  -  nothing is changed" Foreground="#1C6091" FontWeight="SemiBold"/></GroupBox.Header>
          <ScrollViewer VerticalScrollBarVisibility="Auto" Padding="6,4">
            <StackPanel>
              <WrapPanel>
...
function Lzf194790344 { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz2421524f6a { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lzbcf164d9fd {
    ...
    if ($hasRegime) {
        $etaA = Lzd067205ad9 -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lzd067205ad9 -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lzd067205ad9 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lzd067205ad9 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lzcb50daf727 {
    ...
    try {
        ...
        $etaText = Lzbcf164d9fd -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lz6fb5d60c50 {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lz56f1b28268 -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lzf53c48c6fb -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lzcb50daf727 -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lz1546c0345d $Line) + "`r`n")
...
function Lz9ae38a5a06 { param([string]$Text) Lz6fb5d60c50 $Text }
function Lz545cd69d17 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lz5cf70985ba $true 'Connecting to Microsoft 365...'
        Lz7cc574e6b0
    } catch {
        ...
        Lz5cf70985ba $false; return
    } finally { Lz5cf70985ba $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lz774fa148ba -StartSearch $seed
    if ($picked) {
        ...
        Lzf194790344 "Site set to $picked"
        Lz0155212fcd
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lz545cd69d17 }
    Lz0155212fcd
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lzd14d5562cc }
    Lz0155212fcd
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz0155212fcd })
$ctrl.TxtFolder.Add_TextChanged({ Lz0155212fcd })
if ($ctrl.LstSource) { $ctrl.LstSource.Add_SelectionChanged({ Lz5a4d48d5e4 }) }
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz0155212fcd }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz0155212fcd }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lz16bade74b5 -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lz29c0cd0cab })
$ctrl.TxtLoc.Add_TextChanged({ Lz0155212fcd })
$ctrl.TxtLib.Add_TextChanged({ Lz0155212fcd })
$script:NestSuspend = $false
function Lzd867af81a8 {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lzccb6a8408c -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lzf194790344 $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lzd867af81a8 })
$ctrl.ChkNest.Add_Unchecked({ Lzd867af81a8 })
$script:ProjectRows = @()
$script:RunStatus = @{}
...
function Lzd86249bd4b {
    ...
    if (-not $hasProjects) {
        $sd = $false
        if (Get-Command Test-ConnectionConfigured -ErrorAction SilentlyContinue) { try { $sd = [bool](Lzf4d6142cdf) } catch {} }
        Lzea96387076 $sd
    }
...
function Lz0a46420ff5 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lza3abb20e90 -ProjectId $p.Id -SubPath $sub) }
...
function Lz717936ca9d {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lzb665da5837 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lze347bd2ac5 {
    ...
    if (($EngineArgs -contains 'Transfer') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck') -or ($EngineArgs -contains 'PreFlight')) {
        try { Lz5cf70985ba $true 'Checking source folders...'; $srcOk = Lz0a46420ff5 } finally { Lz5cf70985ba $false }
        if (-not $srcOk) { return }
    }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lz5cf70985ba $true 'Checking destinations...'; $dstOk = Lz717936ca9d } finally { Lz5cf70985ba $false }
        if (-not $dstOk) { return }
    }
    elseif (($EngineArgs -contains 'PreFlight') -or ($EngineArgs -contains 'Transfer')) {
        try { Lz5cf70985ba $true 'Checking destinations...'; [void](Lz717936ca9d -WarnOnly) } finally { Lz5cf70985ba $false }
    }
    ...
    $script:Timer.Add_Tick({
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lz6fb5d60c50 $lines[$k] } catch {} }
                }
            }
        } catch {}
        ...
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lz6fb5d60c50 $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lz6fb5d60c50 $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Format-Span ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lz5b69ad4286) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lz8dfa438001 -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lz6fb5d60c50 "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lz6fb5d60c50 "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lzf194790344 ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
            ...
            Lzded2c4b170
            if (-not $script:Paused) { Lz43339df778 -Severity $sev }
            if ($script:RunStatus.Count) { try { Lzd86249bd4b } catch {} }
            try {
            ...
            try {
                $newest = Get-ChildItem (Join-Path (Lz5b69ad4286) 'report-*.html') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
function Lz67e43e7d5d { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lza576454479 { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lzdb804e4c1f {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lz5f4ce37bb0 -ConfigFile $s.Tag; Lz67f47e0508 } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lz1b725e7e92 { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lz5552fa5ad2 -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lzdb804e4c1f
}
function Lz7946ae5ae9 {
...
function Lz5f4ce37bb0 {
    param([string]$ConfigFile)
    if ($script:Sched) { Lzc840fce5ce -Why 'Schedule cancelled (job changed).' }
    ...
    $win.Title = "Liscaragh Migrate  -  $name"
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lza576454479 $true
    Lz67e43e7d5d $true
    Lz1b725e7e92 $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lz7946ae5ae9 -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lzf194790344 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
}
function Lzdef067f578 {
    param([string]$Dir)
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lz1ae4d3e40c -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Lzebd52eb072 -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Open it instead, or choose another name.")); return }
    if ((Test-Path $jobDir) -and -not (Lzdef067f578 -Dir $jobDir)) {
        ...
        Lzccb6a8408c -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lz5f4ce37bb0 -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuJobOpen.Add_Click({
    ...
    [void]$w.ShowDialog()
    if ($script:OpenPick) { Lz5f4ce37bb0 -ConfigFile $script:OpenPick; Lz67f47e0508 }
})
$ctrl.MnuJobSave.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')); return }
    if (Save-Mapping) { Lzf194790344 'Migration job saved.' }
})
$ctrl.MnuJobSaveAs.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open a migration job first, then Save As to copy it.')); return }
    try {
        $name = Lz1ae4d3e40c -Prompt "New name for the copy:" -Title 'Save job as'
        if (-not $name -or -not $name.Trim()) { return }
        $name = $name.Trim()
        $slug = ($name -replace '[^A-Za-z0-9._-]','_').Trim('_'); if (-not $slug) { $slug = 'job' }
        $jobDir = Join-Path $script:JobsRoot $slug
        if (Lzebd52eb072 -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Choose another name.")); return }
        if ((Test-Path $jobDir) -and -not (Lzdef067f578 -Dir $jobDir)) {
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz3af793d285 })
$ctrl.MnuSettingsApi.Add_Click({ Lz069c99d7b7 })
$ctrl.MnuSettingsWizard.Add_Click({ Lzcd018f0ac3 })
$ctrl.MnuSettingsEmail.Add_Click({ Lz3ef476ce45 })
$ctrl.MnuSettingsTuning.Add_Click({ Lz94600ec941 })
$ctrl.BtnFilters.Add_Click({ Lz7a94e9c972 })
$ctrl.MnuSettingsDecommission.Add_Click({ Lz4929007517 })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lz940cec8402 -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lza7926c9a0f {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lzcc5fd8d01f 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lza7926c9a0f "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lza7926c9a0f (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lz5cf70985ba $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lz5cf70985ba $false }
    if (-not $b) { return }
...
function Lz90929cc04d { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lz06bbae85d6 {
    $path = Lz90929cc04d
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Liscaragh Migrate" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lz06bbae85d6)) -Caption ('Licence')) | Out-Null })
function Lz9f7205ebd6 {
...
function Lz1dac6ef764 {
    param([string]$For = 'copy')
    if (Test-Path (Lz90929cc04d)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        try {
            ...
            Lz5552fa5ad2 -Name 'LicenceAppliedId'  -Value "$($lpl.LicenceId)"
            Lz5552fa5ad2 -Name 'LicenceAppliedUtc' -Value ((Get-Date).ToUniversalTime().ToString('o'))
        } catch {}
        (Show-Msg -Text ("Licence installed.`n`n" + (Lz06bbae85d6)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-FullReinstall {
    ...
    Lz5cf70985ba $true "Downloading the installer for version $($Payload.version)..."
    ...
    catch { Lz5cf70985ba $false; (Show-Msg -Text ("The installer could not be downloaded, so nothing has been changed.`n`n$($_.Exception.Message)") -Caption ('Reinstall') -Icon ('Warning')) | Out-Null; return }
...
function Invoke-UpdateCheck {
    ...
    try {
        ...
        if (-not $Silent) { Lz5cf70985ba $true 'Checking for updates...' }
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lz5cf70985ba $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lz940cec8402 -Title 'About' -Heading 'Liscaragh Migrate' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lz85bf18b0b4 } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lzf96144b178 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lz1ae4d3e40c -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
    ...
    try {
        ...
        $ctrl.LblJob.Text = $new; $win.Title = "Liscaragh Migrate  -  $new"
        Lzf194790344 "Renamed to $new"
    } catch { (Show-Msg -Text ("Could not rename: $($_.Exception.Message)")) }
})
function Lzc73b3f0855 {
    $script:JobOpen = $false; $script:Cfg = $null; $script:Projects = @(); $script:Map = @{}; $script:GraphReady = $false
    $script:RunStatus = @{}; Lzd86249bd4b
    if ($ctrl.RunSummaryBanner) { $ctrl.RunSummaryBanner.Visibility = 'Collapsed' }
    $ctrl.LblProject.Text = '(no project selected)'
    $ctrl.LblConn.Text = 'Not connected'; $ctrl.LblConn.Foreground = 'Gray'
    $ctrl.LblJob.Text = '(no job open)'; $win.Title = 'Liscaragh Migrate'
    $ctrl.BtnConnect.IsEnabled = $false; Lza576454479 $false
    Lz67e43e7d5d $false
}
$ctrl.MnuJobClose.Add_Click({ Lzc73b3f0855; Lzf194790344 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lzc73b3f0855
    if (Lzdef067f578 -Dir $dir) {
        Lzdb804e4c1f; Lzf194790344 "Deleted job '$nm'"
    } else {
        Lzdb804e4c1f
        ...
        Lzf194790344 "Could not fully delete job '$nm' (a file is in use)"
    }
})
function Lzcc79e4a9e8 {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lz5cf70985ba $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lz9ae38a5a06 'Signing in to Datto and listing projects...'
        $script:Projects = Lz62bbd92e18
        Lz9ae38a5a06 "Found $($script:Projects.Count) project(s)."
        Lz7d0535c4d0
        try { Lz7cc574e6b0; Lz9ae38a5a06 'Connected to Microsoft 365.' } catch { Lz9ae38a5a06 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lz9ae38a5a06 "Technical detail: $($_.Exception.Message)" }
        Lzd86249bd4b
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lzf194790344 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lz9ae38a5a06 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lz9ae38a5a06 "Technical detail: $($_.Exception.Message)"
            Lzf194790344 "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lzf194790344 'Connect failed'
        }
    }
    finally { Lz5cf70985ba $false }
}
$ctrl.BtnConnect.Add_Click({ Lzcc79e4a9e8 })
function Lz67f47e0508 {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lz85bf18b0b4 } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lzf194790344 "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lzcc79e4a9e8 -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lz4cb3811fbf
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lze4e5bbb936).Count } catch {}
    ...
    if ($script:Map.ContainsKey($p.Id)) {
        ...
        $ctrl.TxtLoc.Text = Lzd14d5562cc
        $ctrl.TxtLib.Text = ''; $ctrl.TxtFolder.Text = ''; Lz0cac7eb3ce -Folders @()
...
function Lzbb728cffe8 {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lz2067d87295) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lz2067d87295) }
}
$ctrl.BtnApply.Add_Click({
    $p = Lz4cb3811fbf
    if (-not $p) { (Show-Msg -Text ('Select a project first.')); return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Remove($p.Id) | Out-Null; Lzd86249bd4b; [void](Write-MappingQuiet); return }
    $d = Lzbb728cffe8
    if ($d -eq 'ERR') { return }
    ...
    Lzf194790344 "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lz73f399c828 })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lz28f8384d38 -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Lzd86249bd4b; [void](Write-MappingQuiet); Lzf194790344 "Set $($sel.Count) project(s) to skip"; return }
    $d = Lzbb728cffe8
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lz545cd69d17); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPaths=$keepSrcList; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Lzd86249bd4b
    [void](Write-MappingQuiet)
    Lzf194790344 "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lz28f8384d38 -Count (@($script:Projects).Count))) { return }
    $d = Lzbb728cffe8
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Lzd86249bd4b; [void](Write-MappingQuiet); Lzf194790344 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Lzd86249bd4b
    [void](Write-MappingQuiet)
    Lzf194790344 "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lz5cf70985ba $true 'Looking up libraries...'
        $libs = Lz80c84e3cd1 -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lz5cf70985ba $false; return }
    finally { Lz5cf70985ba $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz8f4c05271d -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lzf194790344 "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lzf194790344 "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lz5cf70985ba $true 'Opening destination...'
        Lz7cc574e6b0
        $res = Lzd763859e20
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lz5cf70985ba $false; return }
    finally { Lz5cf70985ba $false }
    $picked = Lz97e1761bf5 -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lzf194790344 ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnAddSource) { $ctrl.BtnAddSource.Add_Click({
    ...
    try { $picked = Lza4c752151b -ProjectId $p.Id -ProjectName $p.Name }
    ...
    Lz24c1a33dad -Picked $arr
    Lzf194790344 ("Source folders: " + (@(Lz2d612467b0) -join ', '))
}) }
if ($ctrl.BtnRemoveSource) { $ctrl.BtnRemoveSource.Add_Click({
    ...
    Lz0cac7eb3ce -Folders @(Lz2d612467b0 | Where-Object { $sel -notcontains $_ })
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    $bad = New-Object System.Collections.Generic.List[string]
    try {
        Lz5cf70985ba $true 'Checking the source folder(s) in Datto...'
        foreach ($s in $subs) { try { $null = Lza3abb20e90 -ProjectId $p.Id -SubPath $s } catch { [void]$bad.Add($s) } }
    } catch { $ctrl.LblSourceCheck.Text = "Could not check the source: $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lz5cf70985ba $false; return }
    finally { Lz5cf70985ba $false }
...
function Lzafd4ada937 {
    ...
    try {
        Lz5cf70985ba $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lzb665da5837 -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lz80c84e3cd1 -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
$ctrl.BtnCheck.Add_Click({ Lzafd4ada937 })
if ($ctrl.BtnTestDest) { $ctrl.BtnTestDest.Add_Click({ Lzafd4ada937 }) }
...
function Lz0155212fcd {
    ...
    $srcLabel = Lz15f849205c -Project $p
    $folders  = @(Lz2d612467b0)
    $srcSub   = Lzbf68b88727
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lz2067d87295) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lze4e5bbb936).Count } catch { }
...
function Lzf947f42f23 {
    ...
    $script:Map[$p.Id] = $d
    Lzd86249bd4b
    Lzf194790344 "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lz9ae38a5a06 "Saved $n mapping(s)."
    return $true
}
function Lz7d0535c4d0 {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lz9ae38a5a06 "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lz9ae38a5a06 "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lze347bd2ac5 -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lze347bd2ac5 -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lz9a566182ae {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lz9a566182ae) }
    ...
    if (-not (Lz940cec8402 -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lz7d54a52108)) { return }
    if (-not (Lz1dac6ef764)) { return }
    Lze347bd2ac5 -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lz23166e496b {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lz9a566182ae) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lz7d54a52108)) { return }
    if (-not (Lz1dac6ef764)) { return }
    Lze347bd2ac5 -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnRerunFailed.Add_Click({
    ...
    if (-not (Lz940cec8402 -Title 'Rerun failed files only' -Heading 'Rerun failed files only' -Sections $sections -Yes 'Rerun the failed files' -No 'Cancel')) { return }
    if (-not (Lz7d54a52108)) { return }
    if (-not (Lz1dac6ef764)) { return }
    Lze347bd2ac5 -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','NewerWins','-Execute','-FailedOnly') -Label 'Rerun failed files only'
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lz940cec8402 -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lz1dac6ef764 -For 'verify')) { return }
    Lze347bd2ac5 -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lze347bd2ac5 -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lz5b69ad4286 {
...
function Lzded2c4b170 {
    try { if ($ctrl.BtnRerunFailed) { $ctrl.BtnRerunFailed.IsEnabled = (Lz816d9d2af3) } } catch {}
}
function Lz68191e9437 {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lz3d816196db {
    param([string]$Name)
...
function Lz895013f16a {
    ...
    foreach ($f in $files) { $l = Lz3d816196db $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lz895013f16a -Folder (Lz5b69ad4286) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnCertificate.Add_Click({
    if (Save-Mapping) {
        Lze347bd2ac5 -EngineArgs @('-Action','Certificate') -Label 'Completion certificate' -OnComplete {
            $rr = Lz5b69ad4286
            $cert = $null
...
$ctrl.BtnOpenAudit.Add_Click({ Lz895013f16a -Folder (Lz68191e9437) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lz20e5777b86 {
    ...
    Lzf194790344 "Speed limit set: up $uTxt, down $dTxt"
    Lz6fb5d60c50 "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lz20e5777b86 })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz20e5777b86 } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz20e5777b86 } })
function Lzb014ed733b {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lzb014ed733b -ParentId ([int]$_.ProcessId) } } catch {}
...
function Lz9670c36fe6 {
    $r = ($script:Proc -and -not $script:Proc.HasExited)
    $f = ($script:FinProc -and -not $script:FinProc.HasExited)
    return ($r -or $f)
}
function Lz5a63c47078 { param([string]$T) try { $ctrl.BtnSchedule.Content.Text = $T } catch {} }
function Lzc840fce5ce {
    param([string]$Why = '')
    if ($script:SchedTimer) { try { $script:SchedTimer.Stop() } catch {}; $script:SchedTimer = $null }
    $script:Sched = $null
    Lz5a63c47078 'Schedule for later / overnight...'
    if ($Why) { Lzf194790344 $Why }
}
function Lz50bc4a06bc {
    try { $p = Join-Path (Lz5b69ad4286) 'lastrun-outcome.json'; if (Test-Path $p) { return (Get-Content $p -Raw | ConvertFrom-Json) } } catch {}
    return $null
}
function Lz9c101b032d {
    if (-not $script:Proc -or $script:Proc.HasExited) { return }
    $script:QuietPausing = $true
    $ctrl.BtnStop.IsEnabled = $false
    Lz6fb5d60c50 "----- Quiet hours reached at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')): pausing the copy. Files already uploaded are kept. -----"
    Lzf194790344 'Quiet hours: pausing until the next window...'
    try { Lzb014ed733b -ParentId ([int]$script:Proc.Id) } catch {}
    Lz7946ae5ae9 -Status Cancelled -Note 'Finalising for quiet hours...' -OnDone {
        try { Lz6fb5d60c50 '----- Paused for quiet hours. It will resume as a Sync at the next window while this app stays open. -----' } catch {}
    }
}
function Lz889a2c6c44 {
    param([string[]]$RunArgs,[string]$Label)
    Lz6fb5d60c50 "----- Scheduled: starting '$Label' at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')) -----"
    Lze347bd2ac5 -EngineArgs $RunArgs -Label $Label
    return ($script:Proc -and -not $script:Proc.HasExited)
}
function Lz7d82934f41 { param([string]$Status)
    ...
    switch ("$($s.State)") {
        'waiting' {
            if ($now -ge $s.StartAt -and -not $busy) {
                ...
                elseif (Lz889a2c6c44 -RunArgs $s.Args -Label $s.Label) { $s.State = 'running'; $s.Launches++ }
                else { Lzc840fce5ce -Why 'The scheduled run could not start (see the message). Schedule cancelled.' }
            } else {
                $left = $s.StartAt - $now; if ($left.Ticks -lt 0) { $left = [TimeSpan]::Zero }
                Lz5a63c47078 ("Scheduled: starts $($s.StartAt.ToString('ddd HH:mm')) ($(Format-Span $left) to go) - click to cancel")
            }
        }
        'running' {
            if (-not $busy) {
                $oc = Lz50bc4a06bc; $status = if ($oc) { "$($oc.Status)" } else { '' }
                if ($s.Mode -eq 'once') { Lzc840fce5ce -Why 'Scheduled run finished.'; Lz7d82934f41 $status }
                elseif ($status -eq 'COMPLETED') { Lzc840fce5ce -Why 'Scheduled migration completed.'; Lz7d82934f41 $status }
                else { $s.State = 'paused' }
            } elseif ($s.Mode -eq 'window' -and -not (Test-InWindow $m $s.FromMin $s.ToMin)) {
                Lz9c101b032d; $s.State = 'paused'
            } else {
                $until = if ($s.Mode -eq 'window') { " (until $(Format-MinOfDay $s.ToMin))" } else { '' }
                Lz5a63c47078 ("Scheduled: running$until - click to cancel")
            }
        }
        'paused' {
            if ((Test-InWindow $m $s.FromMin $s.ToMin) -and -not $busy) {
                if (Lz889a2c6c44 -RunArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','AddMissing','-Execute') -Label 'Sync new and changed') { $s.State = 'running'; $s.Launches++ }
            } else {
                $nf = Get-NextWindowOpen $s.FromMin
                Lz5a63c47078 ("Scheduled: waiting for the next window ($($nf.ToString('ddd HH:mm'))) - click to cancel")
            }
        }
    }
}
function Lzce3f829e0f {
...
$ctrl.BtnSchedule.Add_Click({
    if ($script:Sched) {
        if ((Show-Msg -Text ("A schedule is set:" + [Environment]::NewLine + $script:Sched.Summary + [Environment]::NewLine + [Environment]::NewLine + "Cancel it?") -Caption ('Scheduled run') -Buttons ('YesNo') -Icon ('Question')) -eq 'Yes') { Lzc840fce5ce -Why 'Schedule cancelled.' }
        return
    }
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')) | Out-Null; return }
    if (Lz9670c36fe6) { (Show-Msg -Text ('A run is in progress. Wait for it to finish, then set a schedule.') -Caption ('Cannot schedule now') -Icon ('Warning')) | Out-Null; return }
    if (-not (Save-Mapping)) { return }
    $mappedCount = @($script:Projects | Where-Object { $script:Map.ContainsKey($_.Id) }).Count
    ...
    $script:SchedTimer.Add_Tick({ try { Lzce12b7d6be } catch {} })
    $script:SchedTimer.Start()
    try { Lzce12b7d6be } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    if ($script:Sched) { Lzc840fce5ce -Why 'Schedule cancelled (you stopped the run).' }
    $script:Stopping = $true
    $ctrl.BtnStop.IsEnabled = $false
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz6fb5d60c50 "----- STOP requested by user at $stamp -----"
    Lzf194790344 "Stopping $($script:CurLabel)..."
    try { Lzb014ed733b -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lz7946ae5ae9 -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lz6fb5d60c50 "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lz20915f0229 {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lze347bd2ac5 -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz6fb5d60c50 "----- PAUSE requested by user at $stamp -----"
    Lzf194790344 "Pausing $($script:CurLabel)..."
    try { Lzb014ed733b -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lz7946ae5ae9 -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lz6fb5d60c50 "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lz4c434a9c5f { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lz128256e2f3 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lzef01eb27c1 {
...
function Lzf96144b178 {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lza7b4597ed3 -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lz069c99d7b7 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    ...
    function Lze64c41eb9e { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz762bb327ad { param($Label,$Control,$Info)
    ...
    function Lz0830f241c2 { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lze64c41eb9e 'Datto Workplace'
    ...
    $parts = Lz128256e2f3 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz762bb327ad 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lz762bb327ad 'Cell (number)' (Lz0830f241c2 ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lz762bb327ad 'Datto Client ID' (Lz0830f241c2 ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lz762bb327ad 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lze64c41eb9e 'Microsoft 365'
    $tbTid = Lz762bb327ad 'Tenant ID' (Lz0830f241c2 ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lz762bb327ad 'App (Client) ID' (Lz0830f241c2 ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lze64c41eb9e 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lz762bb327ad 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lz762bb327ad '.pfx file' (Lz0830f241c2 '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lz762bb327ad '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lze64c41eb9e 'SharePoint and OneDrive'
    ...
    $tbSp  = Lz762bb327ad 'SharePoint root URL' (Lz0830f241c2 ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lz762bb327ad 'OneDrive host URL'   (Lz0830f241c2 ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lz762bb327ad 'Team site base URL'  (Lz0830f241c2 ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lz762bb327ad 'Default site URL'    (Lz0830f241c2 ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lz762bb327ad 'Email / UPN domain'  (Lz0830f241c2 ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lz5552fa5ad2 -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lzf96144b178 -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz93cee3b3a2 $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lz4c434a9c5f $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lz85bf18b0b4 }
        ...
        try {
            ...
            else { $tok=Lzf96144b178 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lz4c434a9c5f $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lz5552fa5ad2 -Name 'DattoEndpointUrl' -Value $ep
            Lz5552fa5ad2 -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lz5552fa5ad2 -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lz5552fa5ad2 -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lz5552fa5ad2 -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lz5552fa5ad2 -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lz5552fa5ad2 -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lz5552fa5ad2 -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lz5552fa5ad2 -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lz9cc75f7c85 -Value $script:AS.pbSec.Password
            }
...
function Lzcf5429e0bc { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lzccb6a8408c { param($Cfg,[string]$Path,$Value)
...
function Lz7a94e9c972 {
    ...
    $curMax=Lzcf5429e0bc -Cfg $cfg -Path 'run.tuning.maxFileSizeMB'; if ($null -eq $curMax) { $curMax=0 }; $sizeBox.Text="$curMax"
    ...
    $curInc=@(Lzcf5429e0bc -Cfg $cfg -Path 'run.tuning.includePatterns' | Where-Object { "$_".Trim() })
    $curExc=@(Lzcf5429e0bc -Cfg $cfg -Path 'run.tuning.excludePatterns' | Where-Object { "$_".Trim() })
    ...
    $afterPick=New-Object System.Windows.Controls.DatePicker; $afterPick.Margin='0,4,0,4'; $afterPick.Width=150; $afterPick.SelectedDate=(& $toDate (Lzcf5429e0bc -Cfg $cfg -Path 'run.tuning.modifiedAfter'))
    ...
    $beforePick=New-Object System.Windows.Controls.DatePicker; $beforePick.Margin='0,4,0,4'; $beforePick.Width=150; $beforePick.SelectedDate=(& $toDate (Lzcf5429e0bc -Cfg $cfg -Path 'run.tuning.modifiedBefore'))
    ...
    $save.Add_Click({
        try {
            ...
            Lzccb6a8408c -Cfg $script:FltCfg -Path 'run.tuning.maxFileSizeMB' -Value $sv
            $pats=@("$($script:FltPat.Text)" -split '[;,\r\n]' | ForEach-Object { $_.Trim() } | Where-Object { $_ })
            if ($script:FltInc.IsChecked) { Lzccb6a8408c -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value $pats; Lzccb6a8408c -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value @() }
            else                          { Lzccb6a8408c -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value $pats; Lzccb6a8408c -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value @() }
            ...
            Lzccb6a8408c -Cfg $script:FltCfg -Path 'run.tuning.modifiedAfter'  -Value $ma
            Lzccb6a8408c -Cfg $script:FltCfg -Path 'run.tuning.modifiedBefore' -Value $mb
...
function Lz94600ec941 {
    ...
    function Lzec00d07e0d { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec,[string]$Type='text')
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lzcf5429e0bc -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lzec00d07e0d -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec -Type $s.Type
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lzccb6a8408c -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lzccb6a8408c -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lzccb6a8408c -Cfg $script:SetCfg -Path $s.Path -Value ([bool]$script:SetBoxes[$s.Path].IsChecked) }
                elseif ($s.Type -eq 'date') {
                    ...
                    Lzccb6a8408c -Cfg $script:SetCfg -Path $s.Path -Value $raw
                }
                else { Lzccb6a8408c -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lz98743514ea {
    ...
    function Lzbc94a54808 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lz128256e2f3 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lzbc94a54808 -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lzbc94a54808 -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lzbc94a54808 -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lzbc94a54808 -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lz4c434a9c5f $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lz85bf18b0b4 }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz5552fa5ad2 -Name 'DattoEndpointUrl' -Value $ep
            Lz5552fa5ad2 -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lz9cc75f7c85 -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz3af793d285 {
    ...
    $pre = Lz3a496f862e -Title 'Prerequisites'
    ...
    $dat = Lz3a496f862e -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lzb1163461b0 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lz3a496f862e -Title 'Microsoft 365' -Buttons @($bMs, (Lzb1163461b0 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lz3a496f862e -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lz98743514ea } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz01db4bc807; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lzcd018f0ac3 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz01db4bc807; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lz3ef476ce45 } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz01db4bc807; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lz01db4bc807
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lz1950a86418 {
...
function Lz2b47e1851c {
    ...
    $bOpen.Add_Click({ try { Lz73129cc7b3 -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lze862e99a4d {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lz41934fdf67 {
    ...
    Lz2b47e1851c -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lz73129cc7b3 -Url $pageUrl
    ...
    } finally { Lze862e99a4d }
}
function Lz62ac5ca686 {
    try {
...
function Lz5c17ff499d {
    if (Lz8c8a3226a1) { return $false }
...
function Lz978a453f3f {
    ...
    if (-not (Get-Module Microsoft.Graph.Authentication -ListAvailable)) {
        Lz1950a86418 'The Microsoft Graph module is not installed. Re-run the installer (or run: Install-Module Microsoft.Graph.Authentication -Scope CurrentUser) and try again.' '#B42318' -Strong
        return
    }
    Lz1950a86418 'Preparing the Microsoft 365 sign-in. The first time on a new install can take up to a minute while Windows loads the components - please wait, the app has not frozen.' '#475467' -Strong
    try { Import-Module Microsoft.Graph.Authentication -ErrorAction Stop } catch {
        Lz1950a86418 "Could not load the Microsoft Graph module: $($_.Exception.Message)" '#B42318' -Strong
        return
    }
    Lz1950a86418 "Sign in with an account that can manage Microsoft 365 (a Global Administrator is simplest). The first time, Microsoft asks you to accept the permissions this setup itself needs; that is expected.$(if ($ForEmail) { ' Email alerts are included, so expect TWO sign-ins: this one, then one for Exchange Online (a separate service with its own sign-in).' })"
    ...
    if (Lz5c17ff499d) {
        Lz1950a86418 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B5460F' -Strong
        try {
            Lz74a03d67ce -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lz1950a86418 "The browser sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B5460F' -Strong
        }
    } else {
        Lz1950a86418 'Using the code sign-in (the reliable path): a small window will show your code to type at microsoft.com/devicelogin, or on your phone. It stays on top and closes itself when you are done.' '#B5460F' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lz41934fdf67 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lz1950a86418 $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lz8cb2c5a9dd -Token $at -Claim 'tid'
            $wizTokAcct = Lz8cb2c5a9dd -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lz8cb2c5a9dd -Token $at -Claim 'preferred_username' }
            ...
            Lz1950a86418 "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lz1950a86418 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lz1950a86418 "Signed in to tenant $tid as $wizAcct." '#0E9F6E'
    $newThumb = $null
    try {
        Lz1950a86418 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lz1950a86418 "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B5460F' }
            Lz1950a86418 "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#0E9F6E'
        } else {
            Lz1950a86418 "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lz1950a86418 'App registration created.' '#0E9F6E'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lz1950a86418 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lz1950a86418 "Certificate created in this user's store. Thumbprint $newThumb." '#0E9F6E'
        Lz1950a86418 'Uploading the certificate''s public key to the app registration...'
        ...
        Lz1950a86418 "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#0E9F6E'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lza5223a25a7 -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lz1950a86418 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lza5223a25a7 -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lz1950a86418 $consentMsg '#0E9F6E'
        Lz5552fa5ad2 -Name 'TenantId'       -Value $tid
        Lz5552fa5ad2 -Name 'GraphClientId'  -Value $appId
        Lz5552fa5ad2 -Name 'CertThumbprint' -Value $newThumb
        Lz1950a86418 'Connection details saved on this computer.' '#0E9F6E'
    } catch {
        ...
        if ($friendly) { Lz1950a86418 $friendly '#B42318' -Strong; Lz1950a86418 "Technical detail: $m" }
        else { Lz1950a86418 "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lz1950a86418 "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lz1950a86418 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lzf96144b178 -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lz5552fa5ad2 -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lzef01eb27c1 $spUrl
            if ($der) {
                Lz5552fa5ad2 -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lz5552fa5ad2 -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lz5552fa5ad2 -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lz1950a86418 "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#0E9F6E'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lza5223a25a7 -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lz1950a86418 "The new sign-in has not become active yet: $($_.Exception.Message)" '#B5460F' -Strong
                   Lz1950a86418 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz93cee3b3a2 $org
            if ($u) { Lz5552fa5ad2 -Name 'UpnDomain' -Value $u; Lz1950a86418 "Email domain detected: $u" '#0E9F6E' }
        } catch { Lz1950a86418 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lz1950a86418 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#0E9F6E' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lzcd018f0ac3 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lz06f2cd8f69 $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lz978a453f3f -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lz4653755542) {
                    Lz1950a86418 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lz1950a86418 $t $c } else { Lz1950a86418 $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lz1950a86418 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B5460F'
                }
            }
            ...
            Lz1950a86418 "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lz41934fdf67 -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lz74a03d67ce -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lz73129cc7b3 -Url 'https://login.microsoft.com/device'
                Lzbc8a175702
                try { Lz74a03d67ce -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lzbdb3b2ff3d }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (Lz5c17ff499d) {
            ...
            try {
                Lz74a03d67ce -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lz41934fdf67 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lz8cb2c5a9dd -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lz8cb2c5a9dd -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lza5223a25a7 -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lz992dc1c9c7 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lza5223a25a7 -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lza5223a25a7 -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lz5552fa5ad2 -Name 'EmailSender' -Value $senderAddr
        Lz5552fa5ad2 -Name 'EmailEnabled' -Value '1'
...
function Lz3ef476ce45 {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lz7518389689 { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lzc12ee25e30 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lzc12ee25e30 -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lzc12ee25e30 -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lzc12ee25e30 -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lz7518389689 'Subject and attachments'
    ...
    [void](Lzc12ee25e30 -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz3cf8c54560 -Template $script:ES.tbSubj.Text -Vars (Lz0fc055b862)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz3cf8c54560 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz0fc055b862)
            $tok = Lzf96144b178 -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz5552fa5ad2 -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lz5552fa5ad2 -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lz5552fa5ad2 -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lz5552fa5ad2 -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lz5552fa5ad2 -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lz5552fa5ad2 -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lz5552fa5ad2 -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lz5552fa5ad2 -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lz5552fa5ad2 -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lz5552fa5ad2 -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lz5552fa5ad2 -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lz5552fa5ad2 -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lz3456b8eb67 {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lz41934fdf67 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
            ...
            if (-not $regSender) {
                & $say 'The sender mailbox address is unknown; if a datto-migration@ mailbox exists, remove it by hand.' '#B5460F'
            } elseif ($localPart -notmatch '^datto-?migration$') {
                & $say "The sender ($regSender) is not the tool's own datto-migration@ mailbox, so it was NOT deleted (it may be a mailbox you chose or already had). Remove it by hand if you want it gone." '#B5460F'
            } else {
                $u = $null; $uq = $false
...
function Lz4929007517 {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lz3456b8eb67 -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lz01db4bc807 } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lzf4d6142cdf {
    return ((Lz6f09eb1a34) -and (Lz4653755542))
}
Lzb2ac636b87
Lzbdb3b2ff3d
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lz6fb5d60c50 "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lzf4d6142cdf)) {
        try { Lz3af793d285 } catch {}
        if (Lzf4d6142cdf) { Lzf194790344 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lzf194790344 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lzf194790344 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ($ctrl.GettingStarted -and $ctrl.GettingStarted.Visibility -eq 'Visible') {
        try { Lzea96387076 ([bool](Lzf4d6142cdf)) } catch {}
    }
    if ((Lzf4d6142cdf) -and -not (Test-Path (Lz90929cc04d))) {
        try { Lz852644c444 } catch {}
    }
...
try {
    ...
    $script:ActivateWait = [System.Threading.ThreadPool]::RegisterWaitForSingleObject(
        $script:ActivateEvent,
        [System.Threading.WaitOrTimerCallback]{ param($state, $timedOut)
            try { $win.Dispatcher.BeginInvoke([action]{
                try {
    ...
    $win.Add_Closed({ try { if ($script:ActivateWait) { [void]$script:ActivateWait.Unregister($null) } } catch {} })
} catch {}
if ($script:Splash) { $win.Add_ContentRendered({ try { if ($script:Splash) { $script:Splash.Close(); $script:Splash = $null } } catch {} }) }
Lz0cc99c4fe3
$win.Add_Closed({ Lzc8d64cd662 })
[void]$win.ShowDialog()
