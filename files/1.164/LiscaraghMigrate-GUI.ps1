...
function Lzdc42dd4528 {
    param([string]$Severity)
    Lzc2f91e6345 -Severity $Severity
    try {
...
function Lz4c11770abc { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lzf0779d138f {
...
function Lzefa5f28243 {
    ...
    if (-not $s) { $s = Lz91a5710cdc 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lz394a5299da {
    param([string]$Value)
    if (Lzf0779d138f -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lz60a2cdf441 {
    $f = @(Lz048eaf50d9)
    if ($f.Count -eq 1) { return $f[0] }
    return ''
}
function Lzb5145dabd5 {
...
function Lz7b83686662 {
    param([string[]]$Picked)
    $result = New-Object System.Collections.Generic.List[string]
    foreach ($e in @(Lz048eaf50d9)) { [void]$result.Add($e) }
    $dropped = New-Object System.Collections.Generic.List[string]
    foreach ($p in @($Picked | ForEach-Object { "$_".Trim().Trim('/').Trim('\') } | Where-Object { $_ })) {
        if ($result -ccontains $p) { continue }
        $covered = $false
        foreach ($k in @($result)) { if (Lzb5145dabd5 -Child $p -Parent $k) { [void]$dropped.Add($p); $covered = $true; break } }
        if ($covered) { continue }
        foreach ($narrow in @($result | Where-Object { Lzb5145dabd5 -Child $_ -Parent $p -Strict })) { [void]$result.Remove($narrow); [void]$dropped.Add($narrow) }
        [void]$result.Add($p)
    }
    Lz38e6da83b9 -Folders @($result)
    if ($dropped.Count) { Lz497ef04340 ("Overlapping folder(s) merged, kept the broader one: " + ((@($dropped) | Select-Object -Unique) -join ', ')) }
}
function Lz1b91a80259 {
    $one = $true; try { $one = (@(Lz7bdf7fba80).Count -le 1) } catch {}
    $n = @(Lz048eaf50d9).Count
...
function Lz658d807032 {
    ...
    try {
        ...
        $norm = Lzcd5f2c9f6e -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lz497ef04340 "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz3ef9a0eaf0
    } catch { }
}
function Lzcd5f2c9f6e {
...
function Lz408c467eef {
    ...
    return @{ SubfolderCount = @(Lz5265c37866 -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lze23236c693 {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lz39db3ab5fb -Path "sites/$siteId"
    } catch { return $null }
}
function Lz49bbc71bf3 {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lz39db3ab5fb -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lzbabce09b7b {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lz39db3ab5fb -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lzee35e206e9 {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lz69e1ab7278 -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
function Lz4b2c6e8b8f {
    param($State, [string]$Current)
    $out = New-Object System.Collections.Generic.List[string]
    foreach ($s in @($State.Lst.SelectedItems)) {
        $name = "$s"
        if (-not (Lzd8bde2ba9c -Item $name)) { continue }
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lz49bbc71bf3 -Search $s.Txt.Text)
...
"@
    ...
    Lzee35e206e9
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lzee35e206e9 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lzee35e206e9 } })
    $new.Add_Click({
        $n = Lz93694723aa -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lz20352a1db0 -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lzee35e206e9; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lz306188e49d -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lz92fb6cb113 -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lz306188e49d -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lz38083de1b1 {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lz5265c37866 -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lz38083de1b1
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lz38083de1b1 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lz38083de1b1 } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lz306188e49d -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lz4b2c6e8b8f -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lz306188e49d -State $script:SP
    $r = $w.ShowDialog()
...
function Lz497ef04340 { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz1a40b1b887 { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lz8a92f454d4 {
    ...
    if ($hasRegime) {
        $etaA = Lzde64a907ba -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lzde64a907ba -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lzde64a907ba -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lzde64a907ba -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lz689525d299 {
    ...
    try {
        ...
        $etaText = Lz8a92f454d4 -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lz0280ab2844 {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lz48a043253e -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lz745ab84851 -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lz689525d299 -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lze7221c5f1a $Line) + "`r`n")
...
function Lzbce494d68f { param([string]$Text) Lz0280ab2844 $Text }
function Lz29abd215e3 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lzfb64e7def2 $true 'Connecting to Microsoft 365...'
        Lz3614a5e4e2
    } catch {
        ...
        Lzfb64e7def2 $false; return
    } finally { Lzfb64e7def2 $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lzf71008e7ed -StartSearch $seed
    if ($picked) {
        ...
        Lz497ef04340 "Site set to $picked"
        Lz3ef9a0eaf0
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lz29abd215e3 }
    Lz3ef9a0eaf0
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lz68ef14d110 }
    Lz3ef9a0eaf0
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz3ef9a0eaf0 })
$ctrl.TxtFolder.Add_TextChanged({ Lz3ef9a0eaf0 })
if ($ctrl.LstSource) { $ctrl.LstSource.Add_SelectionChanged({ Lz1b91a80259 }) }
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz3ef9a0eaf0 }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz3ef9a0eaf0 }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lza22af54dc9 -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lz658d807032 })
$ctrl.TxtLoc.Add_TextChanged({ Lz3ef9a0eaf0 })
$ctrl.TxtLib.Add_TextChanged({ Lz3ef9a0eaf0 })
$script:NestSuspend = $false
function Lze31b7f3fcd {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lzec6d28f01e -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lz497ef04340 $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lze31b7f3fcd })
$ctrl.ChkNest.Add_Unchecked({ Lze31b7f3fcd })
$script:ProjectRows = @()
$script:RunStatus = @{}
...
function Lz980e6c7c8d {
    ...
    if (-not $hasProjects) {
        $sd = $false
        if (Get-Command Test-ConnectionConfigured -ErrorAction SilentlyContinue) { try { $sd = [bool](Lz3a80a5280c) } catch {} }
        Lzf7de4d2b65 $sd
    }
...
function Lz4a7adcd112 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lz408c467eef -ProjectId $p.Id -SubPath $sub) }
...
function Lz399c5175c8 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lz39db3ab5fb -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz61b4e8f469 {
    ...
    if (($EngineArgs -contains 'Transfer') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck') -or ($EngineArgs -contains 'PreFlight')) {
        try { Lzfb64e7def2 $true 'Checking source folders...'; $srcOk = Lz4a7adcd112 } finally { Lzfb64e7def2 $false }
        if (-not $srcOk) { return }
    }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lzfb64e7def2 $true 'Checking destinations...'; $dstOk = Lz399c5175c8 } finally { Lzfb64e7def2 $false }
        if (-not $dstOk) { return }
    }
    ...
    $script:Timer.Add_Tick({
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lz0280ab2844 $lines[$k] } catch {} }
                }
            }
        } catch {}
        ...
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lz0280ab2844 $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lz0280ab2844 $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Format-Span ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lz7344f06e6c) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lze4d6a61702 -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lz0280ab2844 "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lz0280ab2844 "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lz497ef04340 ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
            ...
            Lz9c3375a6af
            if (-not $script:Paused) { Lzdc42dd4528 -Severity $sev }
            if ($script:RunStatus.Count) { try { Lz980e6c7c8d } catch {} }
            try {
            ...
            try {
                if ($script:CurLabel -eq 'Check readiness' -or $script:CurLabel -eq 'Preview (no upload)') {
                    $newest = Get-ChildItem (Join-Path (Lz7344f06e6c) 'report-*.html') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
function Lzca4b3eb27e { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lzdc59afd2e9 { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lzd0483dc7f3 {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lzdcd4cc73b4 -ConfigFile $s.Tag; Lz253eb4e070 } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lza34cbd29f6 { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lz4c11770abc -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lzd0483dc7f3
}
function Lzca9fff028f {
...
function Lzdcd4cc73b4 {
    param([string]$ConfigFile)
    if ($script:Sched) { Lz54e8c77a0f -Why 'Schedule cancelled (job changed).' }
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lzdc59afd2e9 $true
    Lzca4b3eb27e $true
    Lza34cbd29f6 $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lzca9fff028f -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lz497ef04340 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
}
function Lzaac0657ff6 {
    param([string]$Dir)
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lz93694723aa -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Lzf05c5668fe -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Open it instead, or choose another name.")); return }
    if ((Test-Path $jobDir) -and -not (Lzaac0657ff6 -Dir $jobDir)) {
        ...
        Lzec6d28f01e -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lzdcd4cc73b4 -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuJobOpen.Add_Click({
    ...
    [void]$w.ShowDialog()
    if ($script:OpenPick) { Lzdcd4cc73b4 -ConfigFile $script:OpenPick; Lz253eb4e070 }
})
$ctrl.MnuJobSave.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')); return }
    if (Save-Mapping) { Lz497ef04340 'Migration job saved.' }
})
$ctrl.MnuJobSaveAs.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open a migration job first, then Save As to copy it.')); return }
    try {
        $name = Lz93694723aa -Prompt "New name for the copy:" -Title 'Save job as'
        if (-not $name -or -not $name.Trim()) { return }
        $name = $name.Trim()
        $slug = ($name -replace '[^A-Za-z0-9._-]','_').Trim('_'); if (-not $slug) { $slug = 'job' }
        $jobDir = Join-Path $script:JobsRoot $slug
        if (Lzf05c5668fe -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Choose another name.")); return }
        if ((Test-Path $jobDir) -and -not (Lzaac0657ff6 -Dir $jobDir)) {
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lzd7e6d57d1e })
$ctrl.MnuSettingsApi.Add_Click({ Lza65661e99b })
$ctrl.MnuSettingsWizard.Add_Click({ Lz46047d6f52 })
$ctrl.MnuSettingsEmail.Add_Click({ Lzbc7f73b951 })
$ctrl.MnuSettingsTuning.Add_Click({ Lzd9617138f3 })
$ctrl.BtnFilters.Add_Click({ Lz9f7506ddee })
$ctrl.MnuSettingsDecommission.Add_Click({ Lz22afc2b5a5 })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lzd474b70955 -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lz889f7b3018 {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lz91a5710cdc 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lz889f7b3018 "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lz889f7b3018 (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lzfb64e7def2 $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lzfb64e7def2 $false }
    if (-not $b) { return }
...
function Lzd13272c134 { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lz00e5e501e6 {
    $path = Lzd13272c134
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Liscaragh Migrate" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lz00e5e501e6)) -Caption ('Licence')) | Out-Null })
function Lz9104d105b7 {
...
function Lz9409385af1 {
    param([string]$For = 'copy')
    if (Test-Path (Lzd13272c134)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        try {
            ...
            Lz4c11770abc -Name 'LicenceAppliedId'  -Value "$($lpl.LicenceId)"
            Lz4c11770abc -Name 'LicenceAppliedUtc' -Value ((Get-Date).ToUniversalTime().ToString('o'))
        } catch {}
        (Show-Msg -Text ("Licence installed.`n`n" + (Lz00e5e501e6)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-FullReinstall {
    ...
    Lzfb64e7def2 $true "Downloading the installer for version $($Payload.version)..."
    ...
    catch { Lzfb64e7def2 $false; (Show-Msg -Text ("The installer could not be downloaded, so nothing has been changed.`n`n$($_.Exception.Message)") -Caption ('Reinstall') -Icon ('Warning')) | Out-Null; return }
...
function Invoke-UpdateCheck {
    ...
    try {
        ...
        if (-not $Silent) { Lzfb64e7def2 $true 'Checking for updates...' }
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lzfb64e7def2 $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lzd474b70955 -Title 'About' -Heading 'Liscaragh Migrate' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lzefa5f28243 } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lz587665a355 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lz93694723aa -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lzefdb176682 {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lzdc59afd2e9 $false
    Lzca4b3eb27e $false
}
$ctrl.MnuJobClose.Add_Click({ Lzefdb176682; Lz497ef04340 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lzefdb176682
    if (Lzaac0657ff6 -Dir $dir) {
        Lzd0483dc7f3; Lz497ef04340 "Deleted job '$nm'"
    } else {
        Lzd0483dc7f3
        ...
        Lz497ef04340 "Could not fully delete job '$nm' (a file is in use)"
    }
})
function Lz228ccfb657 {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lzfb64e7def2 $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lzbce494d68f 'Signing in to Datto and listing projects...'
        $script:Projects = Lzaa3d82ef22
        Lzbce494d68f "Found $($script:Projects.Count) project(s)."
        Lz5125715030
        try { Lz3614a5e4e2; Lzbce494d68f 'Connected to Microsoft 365.' } catch { Lzbce494d68f 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lzbce494d68f "Technical detail: $($_.Exception.Message)" }
        Lz980e6c7c8d
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lz497ef04340 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lzbce494d68f 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lzbce494d68f "Technical detail: $($_.Exception.Message)"
            Lz497ef04340 "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lz497ef04340 'Connect failed'
        }
    }
    finally { Lzfb64e7def2 $false }
}
$ctrl.BtnConnect.Add_Click({ Lz228ccfb657 })
function Lz253eb4e070 {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lzefa5f28243 } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lz497ef04340 "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lz228ccfb657 -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lzbfe35e7abc
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lz7bdf7fba80).Count } catch {}
    ...
    if ($script:Map.ContainsKey($p.Id)) {
        ...
        $ctrl.TxtLoc.Text = Lz68ef14d110
        $ctrl.TxtLib.Text = ''; $ctrl.TxtFolder.Text = ''; Lz38e6da83b9 -Folders @()
...
function Lz0184ad2e5b {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lz2e7f0c81fb) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lz2e7f0c81fb) }
}
$ctrl.BtnApply.Add_Click({
    $p = Lzbfe35e7abc
    if (-not $p) { (Show-Msg -Text ('Select a project first.')); return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Remove($p.Id) | Out-Null; Lz980e6c7c8d; [void](Write-MappingQuiet); return }
    $d = Lz0184ad2e5b
    if ($d -eq 'ERR') { return }
    ...
    Lz497ef04340 "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lz5350f479f3 })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lza921aec0a1 -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Lz980e6c7c8d; [void](Write-MappingQuiet); Lz497ef04340 "Set $($sel.Count) project(s) to skip"; return }
    $d = Lz0184ad2e5b
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lz29abd215e3); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPaths=$keepSrcList; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Lz980e6c7c8d
    [void](Write-MappingQuiet)
    Lz497ef04340 "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lza921aec0a1 -Count (@($script:Projects).Count))) { return }
    $d = Lz0184ad2e5b
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Lz980e6c7c8d; [void](Write-MappingQuiet); Lz497ef04340 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Lz980e6c7c8d
    [void](Write-MappingQuiet)
    Lz497ef04340 "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lzfb64e7def2 $true 'Looking up libraries...'
        $libs = Lz47c5f4a7b5 -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lzfb64e7def2 $false; return }
    finally { Lzfb64e7def2 $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz9706bbc311 -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lz497ef04340 "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lz497ef04340 "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lzfb64e7def2 $true 'Opening destination...'
        Lz3614a5e4e2
        $res = Lzbabce09b7b
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lzfb64e7def2 $false; return }
    finally { Lzfb64e7def2 $false }
    $picked = Lzd77f135b76 -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lz497ef04340 ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnAddSource) { $ctrl.BtnAddSource.Add_Click({
    ...
    try { $picked = Lz620ef91a0d -ProjectId $p.Id -ProjectName $p.Name }
    ...
    Lz7b83686662 -Picked $arr
    Lz497ef04340 ("Source folders: " + (@(Lz048eaf50d9) -join ', '))
}) }
if ($ctrl.BtnRemoveSource) { $ctrl.BtnRemoveSource.Add_Click({
    ...
    Lz38e6da83b9 -Folders @(Lz048eaf50d9 | Where-Object { $sel -notcontains $_ })
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    $bad = New-Object System.Collections.Generic.List[string]
    try {
        Lzfb64e7def2 $true 'Checking the source folder(s) in Datto...'
        foreach ($s in $subs) { try { $null = Lz408c467eef -ProjectId $p.Id -SubPath $s } catch { [void]$bad.Add($s) } }
    } catch { $ctrl.LblSourceCheck.Text = "Could not check the source: $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lzfb64e7def2 $false; return }
    finally { Lzfb64e7def2 $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lzfb64e7def2 $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lz39db3ab5fb -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lz47c5f4a7b5 -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lz3ef9a0eaf0 {
    ...
    $srcLabel = Lzdaf552f18e -Project $p
    $folders  = @(Lz048eaf50d9)
    $srcSub   = Lz60a2cdf441
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lz2e7f0c81fb) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lz7bdf7fba80).Count } catch { }
...
function Lz88b4fb1173 {
    ...
    $script:Map[$p.Id] = $d
    Lz980e6c7c8d
    Lz497ef04340 "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lzbce494d68f "Saved $n mapping(s)."
    return $true
}
function Lz5125715030 {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lzbce494d68f "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lzbce494d68f "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lz61b4e8f469 -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lz61b4e8f469 -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lz69d668df72 {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lz69d668df72) }
    ...
    if (-not (Lzd474b70955 -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lzcbaf08d31b)) { return }
    if (-not (Lz9409385af1)) { return }
    Lz61b4e8f469 -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lzd19e3ed28f {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lz69d668df72) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lzcbaf08d31b)) { return }
    if (-not (Lz9409385af1)) { return }
    Lz61b4e8f469 -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnRerunFailed.Add_Click({
    ...
    if (-not (Lzd474b70955 -Title 'Rerun failed files only' -Heading 'Rerun failed files only' -Sections $sections -Yes 'Rerun the failed files' -No 'Cancel')) { return }
    if (-not (Lzcbaf08d31b)) { return }
    if (-not (Lz9409385af1)) { return }
    Lz61b4e8f469 -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','NewerWins','-Execute','-FailedOnly') -Label 'Rerun failed files only'
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lzd474b70955 -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lz9409385af1 -For 'verify')) { return }
    Lz61b4e8f469 -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lz61b4e8f469 -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lz7344f06e6c {
...
function Lz9c3375a6af {
    try { if ($ctrl.BtnRerunFailed) { $ctrl.BtnRerunFailed.IsEnabled = (Lzc70166ec6d) } } catch {}
}
function Lz48cb259d49 {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lz9884a835ce {
    param([string]$Name)
...
function Lze402de0f40 {
    ...
    foreach ($f in $files) { $l = Lz9884a835ce $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lze402de0f40 -Folder (Lz7344f06e6c) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnCertificate.Add_Click({
    if (Save-Mapping) {
        Lz61b4e8f469 -EngineArgs @('-Action','Certificate') -Label 'Completion certificate' -OnComplete {
            $rr = Lz7344f06e6c
            $cert = $null
...
$ctrl.BtnOpenAudit.Add_Click({ Lze402de0f40 -Folder (Lz48cb259d49) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lzabc7692c6a {
    ...
    Lz497ef04340 "Speed limit set: up $uTxt, down $dTxt"
    Lz0280ab2844 "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lzabc7692c6a })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lzabc7692c6a } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lzabc7692c6a } })
function Lz255d59a1fe {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lz255d59a1fe -ParentId ([int]$_.ProcessId) } } catch {}
...
function Lz4e94f4ced3 {
    $r = ($script:Proc -and -not $script:Proc.HasExited)
    $f = ($script:FinProc -and -not $script:FinProc.HasExited)
    return ($r -or $f)
}
function Lz08d9c33d64 { param([string]$T) try { $ctrl.BtnSchedule.Content.Text = $T } catch {} }
function Lz54e8c77a0f {
    param([string]$Why = '')
    if ($script:SchedTimer) { try { $script:SchedTimer.Stop() } catch {}; $script:SchedTimer = $null }
    $script:Sched = $null
    Lz08d9c33d64 'Schedule for later / overnight...'
    if ($Why) { Lz497ef04340 $Why }
}
function Lzf0cdc088b4 {
    try { $p = Join-Path (Lz7344f06e6c) 'lastrun-outcome.json'; if (Test-Path $p) { return (Get-Content $p -Raw | ConvertFrom-Json) } } catch {}
    return $null
}
function Lz602eaa3c76 {
    if (-not $script:Proc -or $script:Proc.HasExited) { return }
    $script:QuietPausing = $true
    $ctrl.BtnStop.IsEnabled = $false
    Lz0280ab2844 "----- Quiet hours reached at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')): pausing the copy. Files already uploaded are kept. -----"
    Lz497ef04340 'Quiet hours: pausing until the next window...'
    try { Lz255d59a1fe -ParentId ([int]$script:Proc.Id) } catch {}
    Lzca9fff028f -Status Cancelled -Note 'Finalising for quiet hours...' -OnDone {
        try { Lz0280ab2844 '----- Paused for quiet hours. It will resume as a Sync at the next window while this app stays open. -----' } catch {}
    }
}
function Lzff2593d7b2 {
    param([string[]]$RunArgs,[string]$Label)
    Lz0280ab2844 "----- Scheduled: starting '$Label' at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')) -----"
    Lz61b4e8f469 -EngineArgs $RunArgs -Label $Label
    return ($script:Proc -and -not $script:Proc.HasExited)
}
function Lz123eca84e4 { param([string]$Status)
    ...
    switch ("$($s.State)") {
        'waiting' {
            if ($now -ge $s.StartAt -and -not $busy) {
                ...
                elseif (Lzff2593d7b2 -RunArgs $s.Args -Label $s.Label) { $s.State = 'running'; $s.Launches++ }
                else { Lz54e8c77a0f -Why 'The scheduled run could not start (see the message). Schedule cancelled.' }
            } else {
                $left = $s.StartAt - $now; if ($left.Ticks -lt 0) { $left = [TimeSpan]::Zero }
                Lz08d9c33d64 ("Scheduled: starts $($s.StartAt.ToString('ddd HH:mm')) ($(Format-Span $left) to go) - click to cancel")
            }
        }
        'running' {
            if (-not $busy) {
                $oc = Lzf0cdc088b4; $status = if ($oc) { "$($oc.Status)" } else { '' }
                if ($s.Mode -eq 'once') { Lz54e8c77a0f -Why 'Scheduled run finished.'; Lz123eca84e4 $status }
                elseif ($status -eq 'COMPLETED') { Lz54e8c77a0f -Why 'Scheduled migration completed.'; Lz123eca84e4 $status }
                else { $s.State = 'paused' }
            } elseif ($s.Mode -eq 'window' -and -not (Test-InWindow $m $s.FromMin $s.ToMin)) {
                Lz602eaa3c76; $s.State = 'paused'
            } else {
                $until = if ($s.Mode -eq 'window') { " (until $(Format-MinOfDay $s.ToMin))" } else { '' }
                Lz08d9c33d64 ("Scheduled: running$until - click to cancel")
            }
        }
        'paused' {
            if ((Test-InWindow $m $s.FromMin $s.ToMin) -and -not $busy) {
                if (Lzff2593d7b2 -RunArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','AddMissing','-Execute') -Label 'Sync new and changed') { $s.State = 'running'; $s.Launches++ }
            } else {
                $nf = Get-NextWindowOpen $s.FromMin
                Lz08d9c33d64 ("Scheduled: waiting for the next window ($($nf.ToString('ddd HH:mm'))) - click to cancel")
            }
        }
    }
}
function Lz0fc094e51d {
...
$ctrl.BtnSchedule.Add_Click({
    if ($script:Sched) {
        if ((Show-Msg -Text ("A schedule is set:" + [Environment]::NewLine + $script:Sched.Summary + [Environment]::NewLine + [Environment]::NewLine + "Cancel it?") -Caption ('Scheduled run') -Buttons ('YesNo') -Icon ('Question')) -eq 'Yes') { Lz54e8c77a0f -Why 'Schedule cancelled.' }
        return
    }
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')) | Out-Null; return }
    if (Lz4e94f4ced3) { (Show-Msg -Text ('A run is in progress. Wait for it to finish, then set a schedule.') -Caption ('Cannot schedule now') -Icon ('Warning')) | Out-Null; return }
    if (-not (Save-Mapping)) { return }
    $mappedCount = @($script:Projects | Where-Object { $script:Map.ContainsKey($_.Id) }).Count
    ...
    $script:SchedTimer.Add_Tick({ try { Lz0119f25dbb } catch {} })
    $script:SchedTimer.Start()
    try { Lz0119f25dbb } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    if ($script:Sched) { Lz54e8c77a0f -Why 'Schedule cancelled (you stopped the run).' }
    $script:Stopping = $true
    $ctrl.BtnStop.IsEnabled = $false
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz0280ab2844 "----- STOP requested by user at $stamp -----"
    Lz497ef04340 "Stopping $($script:CurLabel)..."
    try { Lz255d59a1fe -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lzca9fff028f -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lz0280ab2844 "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lz43d349cec7 {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lz61b4e8f469 -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz0280ab2844 "----- PAUSE requested by user at $stamp -----"
    Lz497ef04340 "Pausing $($script:CurLabel)..."
    try { Lz255d59a1fe -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lzca9fff028f -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lz0280ab2844 "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lz06ce4bb604 { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lz145394bd12 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lz92e258d864 {
...
function Lz587665a355 {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lz4abea80481 -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lza65661e99b {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    ...
    function Lzfb1b74e2de { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz6bb2b70801 { param($Label,$Control,$Info)
    ...
    function Lz75b009d1ba { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lzfb1b74e2de 'Datto Workplace'
    ...
    $parts = Lz145394bd12 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz6bb2b70801 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lz6bb2b70801 'Cell (number)' (Lz75b009d1ba ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lz6bb2b70801 'Datto Client ID' (Lz75b009d1ba ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lz6bb2b70801 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lzfb1b74e2de 'Microsoft 365'
    $tbTid = Lz6bb2b70801 'Tenant ID' (Lz75b009d1ba ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lz6bb2b70801 'App (Client) ID' (Lz75b009d1ba ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lzfb1b74e2de 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lz6bb2b70801 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lz6bb2b70801 '.pfx file' (Lz75b009d1ba '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lz6bb2b70801 '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lzfb1b74e2de 'SharePoint and OneDrive'
    ...
    $tbSp  = Lz6bb2b70801 'SharePoint root URL' (Lz75b009d1ba ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lz6bb2b70801 'OneDrive host URL'   (Lz75b009d1ba ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lz6bb2b70801 'Team site base URL'  (Lz75b009d1ba ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lz6bb2b70801 'Default site URL'    (Lz75b009d1ba ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lz6bb2b70801 'Email / UPN domain'  (Lz75b009d1ba ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lz4c11770abc -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lz587665a355 -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz0ace70846d $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lz06ce4bb604 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lzefa5f28243 }
        ...
        try {
            ...
            else { $tok=Lz587665a355 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lz06ce4bb604 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lz4c11770abc -Name 'DattoEndpointUrl' -Value $ep
            Lz4c11770abc -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lz4c11770abc -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lz4c11770abc -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lz4c11770abc -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lz4c11770abc -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lz4c11770abc -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lz4c11770abc -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lz4c11770abc -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lz394a5299da -Value $script:AS.pbSec.Password
            }
...
function Lz8538b4a2c7 { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lzec6d28f01e { param($Cfg,[string]$Path,$Value)
...
function Lz9f7506ddee {
    ...
    $curMax=Lz8538b4a2c7 -Cfg $cfg -Path 'run.tuning.maxFileSizeMB'; if ($null -eq $curMax) { $curMax=0 }; $sizeBox.Text="$curMax"
    ...
    $curInc=@(Lz8538b4a2c7 -Cfg $cfg -Path 'run.tuning.includePatterns' | Where-Object { "$_".Trim() })
    $curExc=@(Lz8538b4a2c7 -Cfg $cfg -Path 'run.tuning.excludePatterns' | Where-Object { "$_".Trim() })
    ...
    $afterPick=New-Object System.Windows.Controls.DatePicker; $afterPick.Margin='0,4,0,4'; $afterPick.Width=150; $afterPick.SelectedDate=(& $toDate (Lz8538b4a2c7 -Cfg $cfg -Path 'run.tuning.modifiedAfter'))
    ...
    $beforePick=New-Object System.Windows.Controls.DatePicker; $beforePick.Margin='0,4,0,4'; $beforePick.Width=150; $beforePick.SelectedDate=(& $toDate (Lz8538b4a2c7 -Cfg $cfg -Path 'run.tuning.modifiedBefore'))
    ...
    $save.Add_Click({
        try {
            ...
            Lzec6d28f01e -Cfg $script:FltCfg -Path 'run.tuning.maxFileSizeMB' -Value $sv
            $pats=@("$($script:FltPat.Text)" -split '[;,\r\n]' | ForEach-Object { $_.Trim() } | Where-Object { $_ })
            if ($script:FltInc.IsChecked) { Lzec6d28f01e -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value $pats; Lzec6d28f01e -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value @() }
            else                          { Lzec6d28f01e -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value $pats; Lzec6d28f01e -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value @() }
            ...
            Lzec6d28f01e -Cfg $script:FltCfg -Path 'run.tuning.modifiedAfter'  -Value $ma
            Lzec6d28f01e -Cfg $script:FltCfg -Path 'run.tuning.modifiedBefore' -Value $mb
...
function Lzd9617138f3 {
    ...
    function Lz7605cc8b58 { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec,[string]$Type='text')
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lz8538b4a2c7 -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lz7605cc8b58 -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec -Type $s.Type
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lzec6d28f01e -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lzec6d28f01e -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lzec6d28f01e -Cfg $script:SetCfg -Path $s.Path -Value ([bool]$script:SetBoxes[$s.Path].IsChecked) }
                elseif ($s.Type -eq 'date') {
                    ...
                    Lzec6d28f01e -Cfg $script:SetCfg -Path $s.Path -Value $raw
                }
                else { Lzec6d28f01e -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lz62b4eac4db {
    ...
    function Lzef05f42b6f { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lz145394bd12 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lzef05f42b6f -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lzef05f42b6f -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lzef05f42b6f -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lzef05f42b6f -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lz06ce4bb604 $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lzefa5f28243 }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz4c11770abc -Name 'DattoEndpointUrl' -Value $ep
            Lz4c11770abc -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lz394a5299da -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lzd7e6d57d1e {
    ...
    $pre = Lzdcadcd8d67 -Title 'Prerequisites'
    ...
    $dat = Lzdcadcd8d67 -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lzd2ea9861c6 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lzdcadcd8d67 -Title 'Microsoft 365' -Buttons @($bMs, (Lzd2ea9861c6 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lzdcadcd8d67 -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lz62b4eac4db } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz57c3732fbf; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lz46047d6f52 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz57c3732fbf; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lzbc7f73b951 } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz57c3732fbf; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lz57c3732fbf
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lz036d6adb92 {
...
function Lz02a89f4193 {
    ...
    $bOpen.Add_Click({ try { Lz735c86b429 -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lzaa6712b839 {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lzedbba33fe4 {
    ...
    Lz02a89f4193 -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lz735c86b429 -Url $pageUrl
    ...
    } finally { Lzaa6712b839 }
}
function Lz9eaed81a2a {
    try {
...
function Lzff4766da38 {
    if (Lz94d19d9b7b) { return $false }
...
function Lzbccedc9061 {
    ...
    if (-not (Get-Module Microsoft.Graph.Authentication -ListAvailable)) {
        Lz036d6adb92 'The Microsoft Graph module is not installed. Re-run the installer (or run: Install-Module Microsoft.Graph.Authentication -Scope CurrentUser) and try again.' '#B42318' -Strong
        return
    }
    Lz036d6adb92 'Preparing the Microsoft 365 sign-in. The first time on a new install can take up to a minute while Windows loads the components - please wait, the app has not frozen.' '#475467' -Strong
    try { Import-Module Microsoft.Graph.Authentication -ErrorAction Stop } catch {
        Lz036d6adb92 "Could not load the Microsoft Graph module: $($_.Exception.Message)" '#B42318' -Strong
        return
    }
    ...
    if (Lzff4766da38) {
        Lz036d6adb92 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B5460F' -Strong
        try {
            Lza3c9f57d84 -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lz036d6adb92 "The browser sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B5460F' -Strong
        }
    } else {
        Lz036d6adb92 'Using the code sign-in (the reliable path): a small window will show your code to type at microsoft.com/devicelogin, or on your phone. It stays on top and closes itself when you are done.' '#B5460F' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lzedbba33fe4 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lz036d6adb92 $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lzd54e19bf18 -Token $at -Claim 'tid'
            $wizTokAcct = Lzd54e19bf18 -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lzd54e19bf18 -Token $at -Claim 'preferred_username' }
            ...
            Lz036d6adb92 "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lz036d6adb92 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lz036d6adb92 "Signed in to tenant $tid as $wizAcct." '#0E9F6E'
    $newThumb = $null
    try {
        Lz036d6adb92 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lz036d6adb92 "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B5460F' }
            Lz036d6adb92 "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#0E9F6E'
        } else {
            Lz036d6adb92 "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lz036d6adb92 'App registration created.' '#0E9F6E'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lz036d6adb92 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lz036d6adb92 "Certificate created in this user's store. Thumbprint $newThumb." '#0E9F6E'
        Lz036d6adb92 'Uploading the certificate''s public key to the app registration...'
        ...
        Lz036d6adb92 "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#0E9F6E'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lzbb0c766923 -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lz036d6adb92 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lzbb0c766923 -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lz036d6adb92 $consentMsg '#0E9F6E'
        Lz4c11770abc -Name 'TenantId'       -Value $tid
        Lz4c11770abc -Name 'GraphClientId'  -Value $appId
        Lz4c11770abc -Name 'CertThumbprint' -Value $newThumb
        Lz036d6adb92 'Connection details saved on this computer.' '#0E9F6E'
    } catch {
        ...
        if ($friendly) { Lz036d6adb92 $friendly '#B42318' -Strong; Lz036d6adb92 "Technical detail: $m" }
        else { Lz036d6adb92 "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lz036d6adb92 "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lz036d6adb92 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lz587665a355 -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lz4c11770abc -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lz92e258d864 $spUrl
            if ($der) {
                Lz4c11770abc -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lz4c11770abc -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lz4c11770abc -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lz036d6adb92 "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#0E9F6E'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lzbb0c766923 -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lz036d6adb92 "The new sign-in has not become active yet: $($_.Exception.Message)" '#B5460F' -Strong
                   Lz036d6adb92 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz0ace70846d $org
            if ($u) { Lz4c11770abc -Name 'UpnDomain' -Value $u; Lz036d6adb92 "Email domain detected: $u" '#0E9F6E' }
        } catch { Lz036d6adb92 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lz036d6adb92 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#0E9F6E' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lz46047d6f52 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lz9f71f5a94d $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lzbccedc9061 -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lz788b63b21d) {
                    Lz036d6adb92 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lz036d6adb92 $t $c } else { Lz036d6adb92 $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lz036d6adb92 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B5460F'
                }
            }
            ...
            Lz036d6adb92 "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lzedbba33fe4 -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lza3c9f57d84 -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lz735c86b429 -Url 'https://login.microsoft.com/device'
                Lz63cc3afde5
                try { Lza3c9f57d84 -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lzacd6eba03b }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (Lzff4766da38) {
            ...
            try {
                Lza3c9f57d84 -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lzedbba33fe4 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lzd54e19bf18 -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lzd54e19bf18 -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lzbb0c766923 -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lzc151a6e10d 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lzbb0c766923 -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lzbb0c766923 -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lz4c11770abc -Name 'EmailSender' -Value $senderAddr
        Lz4c11770abc -Name 'EmailEnabled' -Value '1'
...
function Lzbc7f73b951 {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lz1ea796e78f { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz7b72e676e9 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lz7b72e676e9 -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lz7b72e676e9 -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lz7b72e676e9 -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lz1ea796e78f 'Subject and attachments'
    ...
    [void](Lz7b72e676e9 -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lzde717a89d7 -Template $script:ES.tbSubj.Text -Vars (Lz4e00d2ce6f)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lzde717a89d7 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz4e00d2ce6f)
            $tok = Lz587665a355 -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz4c11770abc -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lz4c11770abc -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lz4c11770abc -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lz4c11770abc -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lz4c11770abc -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lz4c11770abc -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lz4c11770abc -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lz4c11770abc -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lz4c11770abc -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lz4c11770abc -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lz4c11770abc -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lz4c11770abc -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lz74b849452b {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lzedbba33fe4 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lz22afc2b5a5 {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lz74b849452b -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lz57c3732fbf } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lz3a80a5280c {
    return ((Lz9e0a642599) -and (Lz788b63b21d))
}
Lz25a4189051
Lzacd6eba03b
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lz0280ab2844 "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lz3a80a5280c)) {
        try { Lzd7e6d57d1e } catch {}
        if (Lz3a80a5280c) { Lz497ef04340 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lz497ef04340 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lz497ef04340 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ($ctrl.GettingStarted -and $ctrl.GettingStarted.Visibility -eq 'Visible') {
        try { Lzf7de4d2b65 ([bool](Lz3a80a5280c)) } catch {}
    }
    if ((Lz3a80a5280c) -and -not (Test-Path (Lzd13272c134))) {
        try { Lzf1a254be6c } catch {}
    }
