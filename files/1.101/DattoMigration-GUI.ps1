...
function Lz9f443a5408 { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lz6d8df20419 {
...
function Lz2e57a5e41f {
    ...
    if (-not $s) { $s = Lz89acaab10c 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lz5d70abf859 {
    param([string]$Value)
    if (Lz6d8df20419 -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lz5121072565 {
    ...
    return (Lz35af6ae6f7 -TargetSubFolder $box -SourceSubPath $src)
}
function Lz3a04b3d827 {
    ...
    try {
        ...
        $norm = Lz35af6ae6f7 -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lz2b16ae1d2e "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz8070cb5907
    } catch { }
}
function Lz35af6ae6f7 {
...
function Lz60baf977a5 {
    ...
    return @{ SubfolderCount = @(Lze263302b55 -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lzcb98289680 {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lzfee3f979a8 -Path "sites/$siteId"
    } catch { return $null }
}
function Lz3051c7da8c {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lzfee3f979a8 -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lz7df0f77826 {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lzfee3f979a8 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz6a0e3724c2 {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lze66d5ead1d -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lz3051c7da8c -Search $s.Txt.Text)
...
"@
    ...
    Lz6a0e3724c2
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lz6a0e3724c2 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lz6a0e3724c2 } })
    $new.Add_Click({
        $n = Lz71b5a571e4 -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lz7b957bd45f -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lz6a0e3724c2; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lz11c5725608 -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lz19b2377ed0 -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lz11c5725608 -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lzb95aa7d172 {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lze263302b55 -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lzb95aa7d172
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lzb95aa7d172 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lzb95aa7d172 } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lz11c5725608 -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lz19b2377ed0 -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lz11c5725608 -State $script:SP
    $r = $w.ShowDialog()
...
function Lz2b16ae1d2e { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz15baa42515 { param([TimeSpan]$ts)
...
function Lz01241eb976 { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lz47e5de67fb {
    ...
    if ($hasRegime) {
        $etaA = Lz0464e333ad -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz0464e333ad -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lz0464e333ad -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz0464e333ad -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lz02bc2c827a {
    ...
    try {
        ...
        $etaText = Lz47e5de67fb -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lza8f3161219 {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lzcce78e66bd -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lz912984911d -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lz02bc2c827a -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lz588418dc96 $Line) + "`r`n")
...
function Lz9dac24dd6f { param([string]$Text) Lza8f3161219 $Text }
function Lz9cd29a2f7e {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lz657d4df07e $true 'Connecting to Microsoft 365...'
        Lz3901b83cca
    } catch {
        ...
        Lz657d4df07e $false; return
    } finally { Lz657d4df07e $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lze9612b6dea -StartSearch $seed
    if ($picked) {
        ...
        Lz2b16ae1d2e "Site set to $picked"
        Lz8070cb5907
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lz9cd29a2f7e }
    Lz8070cb5907
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lz25351780d1 }
    Lz8070cb5907
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz8070cb5907 })
$ctrl.TxtFolder.Add_TextChanged({ Lz8070cb5907 })
$ctrl.TxtSourceSub.Add_TextChanged({ Lz8070cb5907 })
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz8070cb5907 }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz8070cb5907 }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lz949bd3f66c -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lz3a04b3d827 })
$ctrl.TxtSourceSub.Add_LostFocus({ Lz3a04b3d827 })
$ctrl.TxtLoc.Add_TextChanged({ Lz8070cb5907 })
$ctrl.TxtLib.Add_TextChanged({ Lz8070cb5907 })
$script:NestSuspend = $false
function Lz7a66db5ef1 {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lz80be481f55 -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lz2b16ae1d2e $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lz7a66db5ef1 })
$ctrl.ChkNest.Add_Unchecked({ Lz7a66db5ef1 })
$script:ProjectRows = @()
$script:RunStatus = @{}
function Lz694a979ead {
...
function Lza72f5fe55c {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lz60baf977a5 -ProjectId $p.Id -SubPath $sub) }
...
function Lz05e925d865 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lzfee3f979a8 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz1755873769 {
    ...
    try { Lz657d4df07e $true 'Checking source folders...'; $srcOk = Lza72f5fe55c } finally { Lz657d4df07e $false }
    if (-not $srcOk) { return }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lz657d4df07e $true 'Checking destinations...'; $dstOk = Lz05e925d865 } finally { Lz657d4df07e $false }
        if (-not $dstOk) { return }
    }
        ...
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lza8f3161219 $lines[$k] } catch {} }
                }
            }
        } catch {}
        $script:SpinIdx = ($script:SpinIdx + 1) % 4
        $spin = @('|','/','-','\')[$script:SpinIdx]
        $ctrl.LblElapsed.Text = "$spin  Elapsed " + (Lz15baa42515 ((Get-Date) - $script:RunStart))
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lza8f3161219 $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lza8f3161219 $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Lz15baa42515 ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lz8ec73657de) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lz50e8d21ef5 -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lza8f3161219 "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lza8f3161219 "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lz2b16ae1d2e ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
...
function Lz44f4126790 { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lz08f72da5d6 { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lz97137fd951 {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lz0ad3f143c9 -ConfigFile $s.Tag; Lzcba0acafd2 } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lz3c21c544f8 { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lz9f443a5408 -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lz97137fd951
}
function Lz1bbfa67641 {
...
function Lz0ad3f143c9 {
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lz08f72da5d6 $true
    Lz44f4126790 $true
    Lz3c21c544f8 $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lz1bbfa67641 -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lz2b16ae1d2e 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lz71b5a571e4 -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Test-Path $jobDir) { (Show-Msg -Text ("A job folder '$slug' already exists. Open it instead, or choose another name.")); return }
        ...
        Lz80be481f55 -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lz0ad3f143c9 -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz7fce0237c6 })
$ctrl.MnuSettingsApi.Add_Click({ Lz8a40a0ac68 })
$ctrl.MnuSettingsWizard.Add_Click({ Lza5aa589442 })
$ctrl.MnuSettingsEmail.Add_Click({ Lz077fbbe22f })
$ctrl.MnuSettingsTuning.Add_Click({ Lz4651b8a5a1 })
$ctrl.MnuSettingsDecommission.Add_Click({ Lzd159592379 })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lzec6e99ebf2 -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lzbdcea457b5 {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lz89acaab10c 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lzbdcea457b5 "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lzbdcea457b5 (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lz657d4df07e $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lz657d4df07e $false }
    if (-not $b) { return }
...
function Lz82eebd7e38 { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lzff960e552a {
    $path = Lz82eebd7e38
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Datto Workplace to SharePoint Migrator" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lzff960e552a)) -Caption ('Licence')) | Out-Null })
function Lz9ba52b9422 {
...
function Lz3b6a17a3f0 {
    param([string]$For = 'copy')
    if (Test-Path (Lz82eebd7e38)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        (Show-Msg -Text ("Licence installed.`n`n" + (Lzff960e552a)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-UpdateCheck {
    try {
        ...
        Lz657d4df07e $true 'Checking for updates...'
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lz657d4df07e $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lzec6e99ebf2 -Title 'About' -Heading 'Datto Workplace to SharePoint Migrator' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lz2e57a5e41f } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lzac670728de -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lz71b5a571e4 -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lz348f431bd2 {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lz08f72da5d6 $false
    Lz44f4126790 $false
}
$ctrl.MnuJobClose.Add_Click({ Lz348f431bd2; Lz2b16ae1d2e 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lz348f431bd2
    try { if ($dir -and (Test-Path $dir)) { Remove-Item $dir -Recurse -Force -ErrorAction Stop } ; Lz97137fd951; Lz2b16ae1d2e "Deleted job '$nm'" }
    catch { (Show-Msg -Text ("Could not fully delete: $($_.Exception.Message)")) }
})
function Lzeea7c9c0af {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lz657d4df07e $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lz9dac24dd6f 'Signing in to Datto and listing projects...'
        $script:Projects = Lz268d81f39b
        Lz9dac24dd6f "Found $($script:Projects.Count) project(s)."
        Lzbb0cbe107f
        try { Lz3901b83cca; Lz9dac24dd6f 'Connected to Microsoft 365.' } catch { Lz9dac24dd6f 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lz9dac24dd6f "Technical detail: $($_.Exception.Message)" }
        Update-ProjectList
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lz2b16ae1d2e 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lz9dac24dd6f 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lz9dac24dd6f "Technical detail: $($_.Exception.Message)"
            Lz2b16ae1d2e "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lz2b16ae1d2e 'Connect failed'
        }
    }
    finally { Lz657d4df07e $false }
}
$ctrl.BtnConnect.Add_Click({ Lzeea7c9c0af })
function Lzcba0acafd2 {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lz2e57a5e41f } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lz2b16ae1d2e "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lzeea7c9c0af -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lz0d29e67a77
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lz9e6a4818f2).Count } catch {}
...
function Lz0cb123d640 {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lz5121072565) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lz5121072565) }
}
$ctrl.BtnApply.Add_Click({
    ...
    Lz2b16ae1d2e "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lz20f1284d33 })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lz55858dbe89 -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Update-ProjectList; [void](Write-MappingQuiet); Lz2b16ae1d2e "Set $($sel.Count) project(s) to skip"; return }
    $d = Lz0cb123d640
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lz9cd29a2f7e); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Update-ProjectList
    [void](Write-MappingQuiet)
    Lz2b16ae1d2e "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lz55858dbe89 -Count (@($script:Projects).Count))) { return }
    $d = Lz0cb123d640
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Update-ProjectList; [void](Write-MappingQuiet); Lz2b16ae1d2e 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Update-ProjectList
    [void](Write-MappingQuiet)
    Lz2b16ae1d2e "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lz657d4df07e $true 'Looking up libraries...'
        $libs = Lz8a82d0998d -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lz657d4df07e $false; return }
    finally { Lz657d4df07e $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz8a4e47058d -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lz2b16ae1d2e "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lz2b16ae1d2e "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lz657d4df07e $true 'Opening destination...'
        Lz3901b83cca
        $res = Lz7df0f77826
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lz657d4df07e $false; return }
    finally { Lz657d4df07e $false }
    $picked = Lz93e3ee40f0 -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lz2b16ae1d2e ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnBrowseSource) { $ctrl.BtnBrowseSource.Add_Click({
    ...
    try { $picked = Lz3676339f6a -ProjectId $p.Id -ProjectName $p.Name }
    catch { (Show-Msg -Text ("Could not open the project's folders: $($_.Exception.Message)") -Caption ('Browse source')); return }
    if ($null -ne $picked) { $ctrl.TxtSourceSub.Text = $picked; Lz2b16ae1d2e ("Source set to " + $(if ($picked) { "/$picked" } else { 'the whole project' })) }
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    try {
        Lz657d4df07e $true 'Checking the source folder in Datto...'
        $res = Lz60baf977a5 -ProjectId $p.Id -SubPath $sub
    } catch { $ctrl.LblSourceCheck.Text = "Source NOT found. $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lz657d4df07e $false; return }
    finally { Lz657d4df07e $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lz657d4df07e $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lzfee3f979a8 -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lz8a82d0998d -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lz8070cb5907 {
    ...
    $srcLabel = Lz97512cd94f -Project $p
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lz5121072565) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lz9e6a4818f2).Count } catch { }
...
function Lz4bac2dc43e {
    ...
    $script:Map[$p.Id] = $d
    Update-ProjectList
    Lz2b16ae1d2e "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lz9dac24dd6f "Saved $n mapping(s)."
    return $true
}
function Lzbb0cbe107f {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lz9dac24dd6f "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lz9dac24dd6f "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lz1755873769 -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lz1755873769 -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lz134ba082fc {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lz134ba082fc) }
    ...
    if (-not (Lzec6e99ebf2 -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lz3b6a17a3f0)) { return }
    Lz1755873769 -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lza0c921b226 {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lz134ba082fc) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lz3b6a17a3f0)) { return }
    Lz1755873769 -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lzec6e99ebf2 -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lz3b6a17a3f0 -For 'verify')) { return }
    Lz1755873769 -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lz1755873769 -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lz8ec73657de {
...
function Lz0c8eee41c9 {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lz83f83937d1 {
    param([string]$Name)
...
function Lzb745f106ee {
    ...
    foreach ($f in $files) { $l = Lz83f83937d1 $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lzb745f106ee -Folder (Lz8ec73657de) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnOpenAudit.Add_Click({ Lzb745f106ee -Folder (Lz0c8eee41c9) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lzec82a3253e {
    ...
    Lz2b16ae1d2e "Speed limit set: up $uTxt, down $dTxt"
    Lza8f3161219 "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lzec82a3253e })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lzec82a3253e } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lzec82a3253e } })
function Lzedaf3ce0ac {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lzedaf3ce0ac -ParentId ([int]$_.ProcessId) } } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    Lza8f3161219 "----- STOP requested by user at $stamp -----"
    Lz2b16ae1d2e "Stopping $($script:CurLabel)..."
    try { Lzedaf3ce0ac -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lz1bbfa67641 -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lza8f3161219 "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lz24a495873e {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lz1755873769 -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lza8f3161219 "----- PAUSE requested by user at $stamp -----"
    Lz2b16ae1d2e "Pausing $($script:CurLabel)..."
    try { Lzedaf3ce0ac -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lz1bbfa67641 -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lza8f3161219 "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lzd8fca5375e { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lz5ef0bc3fc6 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lz8a4e61f96b {
...
function Lzac670728de {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lzfd88f120a6 -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lz8a40a0ac68 {
    ...
    function Lzf174dc08ae { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lzc26ec1f8f2 { param($Label,$Control,$Info)
    ...
    function Lzfec299b14c { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lzf174dc08ae 'Datto Workplace'
    ...
    $parts = Lz5ef0bc3fc6 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lzc26ec1f8f2 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lzc26ec1f8f2 'Cell (number)' (Lzfec299b14c ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lzc26ec1f8f2 'Datto Client ID' (Lzfec299b14c ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lzc26ec1f8f2 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lzf174dc08ae 'Microsoft 365'
    $tbTid = Lzc26ec1f8f2 'Tenant ID' (Lzfec299b14c ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lzc26ec1f8f2 'App (Client) ID' (Lzfec299b14c ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lzf174dc08ae 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lzc26ec1f8f2 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lzc26ec1f8f2 '.pfx file' (Lzfec299b14c '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lzc26ec1f8f2 '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lzf174dc08ae 'SharePoint and OneDrive'
    ...
    $tbSp  = Lzc26ec1f8f2 'SharePoint root URL' (Lzfec299b14c ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lzc26ec1f8f2 'OneDrive host URL'   (Lzfec299b14c ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lzc26ec1f8f2 'Team site base URL'  (Lzfec299b14c ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lzc26ec1f8f2 'Default site URL'    (Lzfec299b14c ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lzc26ec1f8f2 'Email / UPN domain'  (Lzfec299b14c ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lz9f443a5408 -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lzac670728de -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz27e1213e65 $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lzd8fca5375e $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lz2e57a5e41f }
        ...
        try {
            ...
            else { $tok=Lzac670728de -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lzd8fca5375e $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lz9f443a5408 -Name 'DattoEndpointUrl' -Value $ep
            Lz9f443a5408 -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lz9f443a5408 -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lz9f443a5408 -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lz9f443a5408 -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lz9f443a5408 -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lz9f443a5408 -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lz9f443a5408 -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lz9f443a5408 -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lz5d70abf859 -Value $script:AS.pbSec.Password
            }
...
function Lz57dc1130a0 { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lz80be481f55 { param($Cfg,[string]$Path,$Value)
...
function Lz4651b8a5a1 {
    ...
    function Lz8da78b76b9 { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec)
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lz57dc1130a0 -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lz8da78b76b9 -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lz80be481f55 -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lz80be481f55 -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lz80be481f55 -Cfg $script:SetCfg -Path $s.Path -Value ([bool]($raw -match '^(1|true|yes|on)$')) }
                else { Lz80be481f55 -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lzd73bb659f2 {
    ...
    function Lz87ccd1306e { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lz5ef0bc3fc6 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz87ccd1306e -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lz87ccd1306e -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lz87ccd1306e -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lz87ccd1306e -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lzd8fca5375e $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lz2e57a5e41f }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz9f443a5408 -Name 'DattoEndpointUrl' -Value $ep
            Lz9f443a5408 -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lz5d70abf859 -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz7fce0237c6 {
    ...
    $pre = Lz40b1115f1c -Title 'Prerequisites'
    ...
    $dat = Lz40b1115f1c -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lzab04cdd26a 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lz40b1115f1c -Title 'Microsoft 365' -Buttons @($bMs, (Lzab04cdd26a 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lz40b1115f1c -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lzd73bb659f2 } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz581b883337; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lza5aa589442 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz581b883337; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lz077fbbe22f } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz581b883337; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lz581b883337
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lz1b0774f843 {
...
function Lz978b25248a {
    ...
    $bOpen.Add_Click({ try { Lz4261f83427 -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lzc27f071bd3 {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lz7eea765f48 {
    ...
    Lz978b25248a -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lz4261f83427 -Url $pageUrl
    ...
    } finally { Lzc27f071bd3 }
}
function Lz3733fce504 {
    try {
...
function Lzc4dff6b60f {
    ...
    if (-not (Lz86568a9e7f)) {
        Lz1b0774f843 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B54708' -Strong
        try {
            Lze3d6c5e980 -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lz1b0774f843 "The standard sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B54708' -Strong
        }
    } else {
        Lz1b0774f843 'Using the CODE sign-in (the reliable path on this machine): a small window will show your code, always on top.' '#B54708' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lz7eea765f48 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lz1b0774f843 $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lz2144e827c3 -Token $at -Claim 'tid'
            $wizTokAcct = Lz2144e827c3 -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lz2144e827c3 -Token $at -Claim 'preferred_username' }
            ...
            Lz1b0774f843 "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lz1b0774f843 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lz1b0774f843 "Signed in to tenant $tid as $wizAcct." '#12B76A'
    $newThumb = $null
    try {
        Lz1b0774f843 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lz1b0774f843 "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B54708' }
            Lz1b0774f843 "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#12B76A'
        } else {
            Lz1b0774f843 "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lz1b0774f843 'App registration created.' '#12B76A'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lz1b0774f843 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lz1b0774f843 "Certificate created in this user's store. Thumbprint $newThumb." '#12B76A'
        Lz1b0774f843 'Uploading the certificate''s public key to the app registration...'
        ...
        Lz1b0774f843 "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#12B76A'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lzbd999acb3c -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lz1b0774f843 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lzbd999acb3c -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lz1b0774f843 $consentMsg '#12B76A'
        Lz9f443a5408 -Name 'TenantId'       -Value $tid
        Lz9f443a5408 -Name 'GraphClientId'  -Value $appId
        Lz9f443a5408 -Name 'CertThumbprint' -Value $newThumb
        Lz1b0774f843 'Connection details saved on this computer.' '#12B76A'
    } catch {
        ...
        if ($friendly) { Lz1b0774f843 $friendly '#B42318' -Strong; Lz1b0774f843 "Technical detail: $m" }
        else { Lz1b0774f843 "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lz1b0774f843 "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lz1b0774f843 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lzac670728de -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lz9f443a5408 -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lz8a4e61f96b $spUrl
            if ($der) {
                Lz9f443a5408 -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lz9f443a5408 -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lz9f443a5408 -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lz1b0774f843 "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#12B76A'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lzbd999acb3c -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lz1b0774f843 "The new sign-in has not become active yet: $($_.Exception.Message)" '#B54708' -Strong
                   Lz1b0774f843 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz27e1213e65 $org
            if ($u) { Lz9f443a5408 -Name 'UpnDomain' -Value $u; Lz1b0774f843 "Email domain detected: $u" '#12B76A' }
        } catch { Lz1b0774f843 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lz1b0774f843 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#12B76A' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lza5aa589442 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lz0241c529d8 $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lzc4dff6b60f -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lz7a022854e8) {
                    Lz1b0774f843 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lz1b0774f843 $t $c } else { Lz1b0774f843 $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lz1b0774f843 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B54708'
                }
            }
            ...
            Lz1b0774f843 "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lz7eea765f48 -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lze3d6c5e980 -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lz4261f83427 -Url 'https://login.microsoft.com/device'
                Lz563aef49f2
                try { Lze3d6c5e980 -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lz5dea0701ec }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (-not (Lz86568a9e7f)) {
            ...
            try {
                Lze3d6c5e980 -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lz7eea765f48 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lz2144e827c3 -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lz2144e827c3 -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lzbd999acb3c -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lz7739626746 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lzbd999acb3c -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lzbd999acb3c -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lz9f443a5408 -Name 'EmailSender' -Value $senderAddr
        Lz9f443a5408 -Name 'EmailEnabled' -Value '1'
...
function Lz077fbbe22f {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lz820628211e { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lze9c68098f5 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lze9c68098f5 -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lze9c68098f5 -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lze9c68098f5 -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lz820628211e 'Subject and attachments'
    ...
    [void](Lze9c68098f5 -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz79a871b5d1 -Template $script:ES.tbSubj.Text -Vars (Lz3bcd0f4f0f)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz79a871b5d1 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz3bcd0f4f0f)
            $tok = Lzac670728de -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz9f443a5408 -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lz9f443a5408 -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lz9f443a5408 -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lz9f443a5408 -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lz9f443a5408 -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lz9f443a5408 -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lz9f443a5408 -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lz9f443a5408 -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lz9f443a5408 -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lz9f443a5408 -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lz9f443a5408 -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lz9f443a5408 -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lza28cff2796 {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lz7eea765f48 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lzd159592379 {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lza28cff2796 -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
...
function Lz53c638cdc0 {
    return ((Lza1b3578da2) -and (Lz7a022854e8))
}
Lzb1c9429dc3
Lz5dea0701ec
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lza8f3161219 "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lz53c638cdc0)) {
        try { Lz7fce0237c6 } catch {}
        if (Lz53c638cdc0) { Lz2b16ae1d2e 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lz2b16ae1d2e 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lz2b16ae1d2e 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ((Lz53c638cdc0) -and -not (Test-Path (Lz82eebd7e38))) {
        try { Lzf5567d447d } catch {}
    }
})
try {
