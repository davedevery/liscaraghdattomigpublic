...
function Lz70bc5f9b70 { param([int]$Done,[int]$Total,[int64]$BytesDone,[int64]$BytesTotal,[string]$Space,
...
function Lz480cfbe69a { param([int]$Max,[int]$HardMax,[int]$PausedSeconds,[int]$Events,[int]$Code=0)
...
function Lzad17439b17 {
    param($Headers)
    $p = $script:DattoPace
    $rl = Lz4fab4f4b7f $Headers
    if (-not $rl.HasBudget) {
        [System.Threading.Monitor]::Enter($p.SyncRoot)
        try {
...
$script:ProvOwnerAvailable = $false
...
function Import-MigrationConfig {
    ...
    if ((Lz9a9b8e46d2 $cfg 'run') -and $null -ne $cfg.run) {
        $liscBase = Join-Path ([Environment]::GetFolderPath('LocalApplicationData')) 'Liscaragh'
        $rootDefaults = [ordered]@{ tempWorkingFolder = 'temp'; logRoot = 'logs'; reportRoot = 'reports'; stateRoot = 'state' }
        foreach ($rk in @($rootDefaults.Keys)) {
            $cur = ''
            if (Lz9a9b8e46d2 $cfg.run $rk) { $cur = "$($cfg.run.$rk)" }
            if (-not $cur.Trim()) {
                $rv = Join-Path $liscBase $rootDefaults[$rk]
                if (Lz9a9b8e46d2 $cfg.run $rk) { $cfg.run.$rk = $rv }
                else { $cfg.run | Add-Member -NotePropertyName $rk -NotePropertyValue $rv }
            }
        }
        if ((Lz9a9b8e46d2 $cfg 'datto') -and $null -ne $cfg.datto -and (Lz9a9b8e46d2 $cfg.datto 'sim') -and $null -ne $cfg.datto.sim) {
            if (-not (Lz9a9b8e46d2 $cfg.datto.sim 'rootPath')) { $cfg.datto.sim | Add-Member -NotePropertyName rootPath -NotePropertyValue '' }
            if (-not "$($cfg.datto.sim.rootPath)".Trim()) { $cfg.datto.sim.rootPath = (Join-Path $liscBase 'sim-source') }
        }
        if ((Lz9a9b8e46d2 $cfg 'destination') -and $null -ne $cfg.destination -and (Lz9a9b8e46d2 $cfg.destination 'sim') -and $null -ne $cfg.destination.sim) {
            if (-not (Lz9a9b8e46d2 $cfg.destination.sim 'rootPath')) { $cfg.destination.sim | Add-Member -NotePropertyName rootPath -NotePropertyValue '' }
            if (-not "$($cfg.destination.sim.rootPath)".Trim()) { $cfg.destination.sim.rootPath = (Join-Path $liscBase 'sim-dest') }
        }
    }
    if (-not ((Lz9a9b8e46d2 $cfg 'source')) -or $null -eq $cfg.source) {
        $cfg | Add-Member -NotePropertyName source -NotePropertyValue ([pscustomobject]@{ provider = 'Datto'; localRoots = @() }) -Force
    }
    if (-not ((Lz9a9b8e46d2 $cfg.source 'provider')) -or -not "$($cfg.source.provider)") { $cfg.source | Add-Member -NotePropertyName provider -NotePropertyValue 'Datto' -Force }
    if (-not ((Lz9a9b8e46d2 $cfg.source 'localRoots')) -or $null -eq $cfg.source.localRoots) { $cfg.source | Add-Member -NotePropertyName localRoots -NotePropertyValue @() -Force }
    if ("$($cfg.source.provider)" -eq 'LocalFs') {
        ...
        if (((Lz9a9b8e46d2 $cfg.source 'posix')) -and $cfg.source.posix) { $script:PosixSrc = $true }
    }
    if (-not (Lz9a9b8e46d2 $cfg.run 'throttle') -or $null -eq $cfg.run.throttle) {
        $cfg.run | Add-Member -NotePropertyName throttle -NotePropertyValue ([pscustomobject]@{}) -Force
    }
    $__throttleDefaults = @{ maxRetries = 6; baseDelaySeconds = 5; honorRetryAfter = $true; timeoutSec = 120; transferTimeoutSec = 3600 }
    foreach ($__k in $__throttleDefaults.Keys) {
        if (-not (Lz9a9b8e46d2 $cfg.run.throttle $__k)) {
            $cfg.run.throttle | Add-Member -NotePropertyName $__k -NotePropertyValue $__throttleDefaults[$__k] -Force
        }
    }
    if (-not (Lz9a9b8e46d2 $cfg.run.throttle 'adaptive')) {
        $cfg.run.throttle | Add-Member -NotePropertyName adaptive -NotePropertyValue ([pscustomobject]@{ enabled = $true; minWorkers = 1; growAfterSeconds = 30 }) -Force
    }
...
function Unprotect-ConfigText {
    ...
    try {
        ...
        throw "This job's settings file is encrypted for a different Windows account or machine, or the stored data is damaged and cannot be read here. Run the tool under the account that installed it, or re-run setup."
    }
}
function Lzf051de9d76 {
    param([string]$Name)
    try {
        Add-Type -AssemblyName System.Security -ErrorAction Stop | Out-Null
...
function Resolve-Secret {
    ...
    if ("$Value" -match '^env:(.+)$') {
        ...
        if (-not $v) { $v = Lzf051de9d76 -Name $n }
...
function Get-SafeSum {
    ...
    if ($null -eq $Items) { $Items = @() } else { $Items = [object[]]$Items }
    if ($Property) {
        $m = $Items | Measure-Object -Property $Property -Sum -ErrorAction SilentlyContinue
    } else {
        $m = $Items | Measure-Object -Sum -ErrorAction SilentlyContinue
    }
    if ($m -and $null -ne $m.Sum) { [int64]$m.Sum } else { [int64]0 }
}
function Lz9a9b8e46d2 {
    param($Obj, [string]$Name)
    if ($null -eq $Obj) { return $false }
    return (@($Obj.PSObject.Properties) | Select-Object -ExpandProperty Name) -contains $Name
}
function Lz551c5b804a {
...
function Write-AuditRecord {
    ...
    $rec = [pscustomobject][ordered]@{
        ...
        DestPath          = $DestPath
        Renamed           = [bool](((Lz9a9b8e46d2 $Item 'Renamed')) -and $Item.Renamed)
        RenamedFrom       = if ((Lz9a9b8e46d2 $Item 'RenamedFrom')) { "$($Item.RenamedFrom)" } else { '' }
        Status            = $Status
...
function Lz41ef0b0f16 {
    param([string]$Space, [string]$Original, [string]$Final)
    $script:RenameCount++
    if ($script:RenameFile) { try { [pscustomobject][ordered]@{ Space=$Space; Original=$Original; Final=$Final } | Lz551c5b804a | Export-Csv -Path $script:RenameFile -Append -NoTypeInformation -Encoding UTF8 } catch {} }
    $dir = ($Final -replace '/[^/]*$','')
    $key = "$Space|$dir"
...
function Lz1749194205 {
    param([string]$Space, [string]$Final, [string]$FirstSource, [string]$SecondSource)
    $script:CollisionCount++
    if ($script:CollisionFile) { try { [pscustomobject][ordered]@{ Space=$Space; Final=$Final; FirstSource=$FirstSource; SecondSource=$SecondSource } | Lz551c5b804a | Export-Csv -Path $script:CollisionFile -Append -NoTypeInformation -Encoding UTF8 } catch {} }
...
function Lzc13255adcc {
    param([string]$Space, [string]$Rp, $Item)
    $safe = ConvertTo-SafeRelPath $Rp
    if ($safe -ne $Rp) {
        Lz41ef0b0f16 -Space $Space -Original $Rp -Final $safe
    ...
    if ($script:FinalSeen -is [hashtable]) {
        ...
        if ($script:FinalSeen.ContainsKey($k)) { $prev = $script:FinalSeen[$k]; if ($prev -ne $Rp) { Lz1749194205 -Space $Space -Final $safe -FirstSource $prev -SecondSource $Rp } }
        else { $script:FinalSeen[$k] = $Rp }
    }
    return $safe
}
function Lzccc361cfae {
...
function Invoke-WithRetry {
    param([scriptblock] $Action, $Config, [string] $What = 'operation', [switch] $RetryNotFound, [switch] $QuietNotFound)
    ...
    for ($a = 1; $a -le $throttleMax; $a++) {
        try { return & $Action }
            ...
            $isTransient = Lz0f2aac4437 $_
            $cap = if ($isThrottle) { $throttleMax } else { [int]$t.maxRetries }
            if ((($retryCodes -notcontains $status) -and (-not $isTransient)) -or $a -ge $cap) {
                if (-not ($QuietNotFound -and $status -eq 404)) {
                    $__edetail2 = $null; try { $__edetail2 = $_.ErrorDetails.Message } catch {}; Write-MigrationLog ERROR "$What failed (attempt $a): $($_.Exception.Message)$(if ($__edetail2) { ' [DIAG] ' + $__edetail2 })"
                }
                throw
            }
            $delay = Lzd20b0dfc60 -Attempt $a -Throttle $t -RetryAfter $ra
            if ($isTransient) {
...
function Invoke-DattoApi {
    ...
    Lzad17439b17 -Headers $script:DattoLastHeaders
    return $r
}
function Get-DattoSpaces {
    ...
    $items = if ($resp -is [System.Array]) { $resp } elseif ($f.collection -and (Lz9a9b8e46d2 $resp $f.collection)) { $resp.($f.collection) } else { $resp }
...
function New-SpaceRef {
    ...
    $sub = ''
    try { if ((Lz9a9b8e46d2 $Row 'SourceSubPath')) { $sub = "$($Row.SourceSubPath)".Trim().Trim('/').Trim('\') } } catch {}
    $co = $false
    try { if ((Lz9a9b8e46d2 $Row 'SourceContentsOnly')) { $co = ("$($Row.SourceContentsOnly)".Trim() -match '^(?i)(true|1|yes)$') } } catch {}
    $isLocalFs = $false
...
function Get-DattoItems {
    ...
    $subPath = ''
    try { if ((Lz9a9b8e46d2 $Space 'SubPath')) { $subPath = "$($Space.SubPath)" } } catch {}
    $contentsOnly = $false
    try { if ((Lz9a9b8e46d2 $Space 'ContentsOnly')) { $contentsOnly = [bool]$Space.ContentsOnly } } catch {}
    if ($subPath) {
    ...
    if ($Config.datto.provider -eq 'LocalSim') {
        ...
        if ($isLocalFs -and -not $contentsOnly) {
            $rootSeed = "$($Space.Name)"
            if (((Lz9a9b8e46d2 $Config.run 'sanitiseNames')) -and $Config.run.sanitiseNames) { $rootSeed = ConvertTo-SafeRelPath $rootSeed }
        }
        $walkErrs = $null
        $phc = [ref]0
        ...
        if (@($walkErrs).Count) {
            ...
            $script:UnreadableCount += @($badPaths).Count
            if (@($script:UnreadableSpaces) -notcontains $scopeLabel) { $script:UnreadableSpaces += $scopeLabel }
    ...
    function Lz0ebc2480d6 { param($ParentId, [string]$Prefix)
        ...
        foreach ($it in @($items)) {
            ...
            $isFolder = $false; try { $isFolder = [bool]$it.($f.itemFolder) } catch {}
            if ($isFolder) {
                Lz0ebc2480d6 -ParentId $it.($f.itemId) -Prefix $rel
            } else {
    ...
    Lz0ebc2480d6 -ParentId $startId -Prefix $(if ($contentsOnly) { '' } else { $subPath })
    & $emitEnum -Force
    ...
    Lz69ec7897f1 -Config $Config -Space $Space -Suspects $suspects
    return $out
}
$script:EnumCacheMaxAgeSec = 86400
function Lz5cca1bf197 {
    param($Config, $Space)
    if (-not $Config.run.stateRoot) { return $null }
    return (Join-Path $Config.run.stateRoot ("enum-" + (ConvertTo-Slug "$($Space.Name)") + ".json"))
}
function Lz62caca2c13 {
    param($Config, $Space, $Items)
    $p = Lz5cca1bf197 -Config $Config -Space $Space
    if (-not $p) { return }
    try {
        if (-not (Test-Path $Config.run.stateRoot)) { New-Item -ItemType Directory -Path $Config.run.stateRoot -Force | Out-Null }
        $sub = ''; try { if ((Lz9a9b8e46d2 $Space 'SubPath')) { $sub = "$($Space.SubPath)" } } catch {}
        $co = $false; try { if ((Lz9a9b8e46d2 $Space 'ContentsOnly')) { $co = [bool]$Space.ContentsOnly } } catch {}
        $now = [DateTime]::UtcNow
        [pscustomobject]@{
            Version        = 2
            SpaceId        = "$($Space.Id)"
...
function Get-SpaceItemsCached {
    ...
    if ($script:UseEnumCache) {
        ...
        if (-not $p -or -not (Test-Path $p)) { $why = 'there is no saved file list for this project' }
            ...
            try {
                ...
                elseif ($(((Lz9a9b8e46d2 $c 'ContentsOnly')) -and [bool]$c.ContentsOnly) -ne $co) { $why = 'the contents-only setting has changed since the list was saved' }
    ...
    Lz62caca2c13 -Config $Config -Space $Space -Items $items
    $__clean = New-Object System.Collections.Generic.List[object]
    for ($__i = 0; $__i -lt $items.Count; $__i++) {
        $__x = $items[$__i]
        if ($null -eq $__x) {
...
function Lzd0689f99ad {
    param([string]$Broker, $Config)
    $lic = ''
    try { if (((Lz9a9b8e46d2 $Config.run 'licenceFile')) -and $Config.run.licenceFile) { $lic = "$($Config.run.licenceFile)" } } catch {}
    ...
    $tokenOk = $obj -and ((Lz9a9b8e46d2 $obj 'token')) -and $obj.token
    if ($code -eq 0 -and $tokenOk) {
        $exp = [DateTimeOffset]::UtcNow.AddMinutes(45)
        try { if (((Lz9a9b8e46d2 $obj 'expiresOn')) -and $obj.expiresOn) { $exp = ([DateTimeOffset]::Parse("$($obj.expiresOn)", [System.Globalization.CultureInfo]::InvariantCulture, [System.Globalization.DateTimeStyles]::AssumeUniversal)).AddMinutes(-4) } } catch {}
        return [pscustomobject]@{ Token = "$($obj.token)"; Exp = $exp; Mode = "$($obj.mode)"; Customer = "$($obj.customer)" }
    }
    $reason = if ($obj -and ((Lz9a9b8e46d2 $obj 'reason'))) { "$($obj.reason)" } else { $raw.Trim() }
    if ($code -eq 2) {
...
function Connect-Destination {
    ...
    $auth = Lzd0689f99ad -Broker $broker -Config $Config
...
function Get-NestFolder {
    param($Config, $Row)
    $isLocalFs = $false
    try { if (((Lz9a9b8e46d2 $Config 'source')) -and $Config.source -and ("$($Config.source.provider)" -eq 'LocalFs')) { $isLocalFs = $true } } catch {}
    if ($isLocalFs) { return '' }
    $co = $false
    try { if ((Lz9a9b8e46d2 $Row 'SourceContentsOnly')) { $co = ("$($Row.SourceContentsOnly)".Trim() -match '^(?i)(true|1|yes)$') } } catch {}
    if ($co) { return '' }
    $name = "$($Row.Space)"; if (-not $name) { return '' }
    if (((Lz9a9b8e46d2 $Config.run 'sanitiseNames')) -and $Config.run.sanitiseNames) { $name = ConvertTo-SafeRelPath $name }
    return $name
}
function Get-GraphCollection {
    param($Config, [string]$Uri, [string]$What = 'list', [switch]$QuietNotFound)
    $values = [System.Collections.Generic.List[object]]::new()
    $pages = 0
    $next = $Uri
    while ($next) {
        $u = $next
        $resp = Invoke-WithRetry -Config $Config -What $What -QuietNotFound:$QuietNotFound -Action { Invoke-MgGraphRequest -Method GET -Uri $u }
        $pages++
...
function Resolve-DestinationDriveId {
    ...
    $lib = ''; if ((Lz9a9b8e46d2 $Row 'TargetLibrary')) { $lib = "$($Row.TargetLibrary)".Trim() }
    if ($lib) {
...
function Send-FileToDestination {
    ...
    if (((Lz9a9b8e46d2 $Config.run 'sanitiseNames')) -and $Config.run.sanitiseNames) {
        $relPath = Lzc13255adcc -Space $Space -Rp $relPath -Item $Item
    }
    $relPath = Lzf26d666b91 $relPath
    if ($Config.destination.provider -eq 'LocalSim') {
        ...
        return @{ Size = (Get-Item $dest).Length; Hash = (Lza0e16f1376 $dest); Path = $relPath; Method = 'LocalSim'; Retries = 0 }
    }
    if ($relPath -match '/') { Lz48aff0152e -Config $Config -DriveId $DriveId -FolderPath ($relPath -replace '/[^/]*$','') }
    $size = if ($script:SpoolKey) { Get-SpoolLength $LocalFile } else { (Get-Item $LocalFile).Length }
    $mtime = $Item.ModifiedUtc
    $ctime = ''; try { if (((Lz9a9b8e46d2 $Item 'CreatedUtc')) -and $Item.CreatedUtc) { $ctime = "$($Item.CreatedUtc)" } } catch {}
    ...
    } elseif ($size -le $script:SmallFilePutThreshold -and (Lz59ac9b7002) -and -not $script:SpoolKey) {
            ...
            try {
                ...
                if ($size -le $chunk) {
                    if ($script:SpoolKey) {
                        ...
                        $finalResp = Lz3a4c57220c -Config $Config -UploadUrl $uploadUrl -Bytes $plain -From 0 -To ($size - 1) -Total $size
                    } else {
                        $finalResp = Lz3a4c57220c -Config $Config -UploadUrl $uploadUrl -FilePath $LocalFile -From 0 -To ($size - 1) -Total $size
                    }
                    ...
                    try {
                        ...
                        while ($true) {
                            ...
                            $finalResp = Lz3a4c57220c -Config $Config -UploadUrl $uploadUrl -Bytes $plain -From $pos -To ($pos + $plain.Length - 1) -Total $size
                            $pos += $plain.Length
                    ...
                    try {
                        ...
                        while (($read = $fs.Read($buffer, 0, $buffer.Length)) -gt 0) {
                            ...
                            $finalResp = Lz3a4c57220c -Config $Config -UploadUrl $uploadUrl -FilePath $chunkTmp -From $pos -To ($pos + $read - 1) -Total $size
                            $pos += $read
                        }
                ...
                if ((@(401, 404, 408, 429, 500, 502, 503, 504) -contains $st) -and $attempt -lt $t.maxRetries) {
                    $ra = Lzc216c38f0c $_
                    $delay = Lzd20b0dfc60 -Attempt $attempt -Throttle $t -RetryAfter $ra
...
function Lz38d86e77f6 {
    ...
    if ($dsz -eq $localSize) { return @{ Ok = $true; Reason = 'verified (size match)'; Zero = $false } }
    ...
    switch ($choice.Trim()) {
        ...
        default {
            ...
            while ($true) {
                ...
                if ($validate) {
                    $siteObj = Lz134a8c6f24 -Url $site.Trim()
...
function Format-Destination {
    param($Row)
    $sub = if (((Lz9a9b8e46d2 $Row 'TargetSubFolder')) -and $Row.TargetSubFolder) { $Row.TargetSubFolder } else { 'root' }
    if ($Row.DestinationType -eq 'OneDrive') {
        return "OneDrive [$($Row.TargetPrincipal)] folder [$sub]"
    }
    $lib = if (((Lz9a9b8e46d2 $Row 'TargetLibrary')) -and $Row.TargetLibrary) { $Row.TargetLibrary } else { 'Documents' }
    return "SharePoint [$($Row.DestinationUrl)] library [$lib] folder [$sub]"
}
function Lz0b069fad38 {
    param($Project, $Dest)
...
function Lzc1e7fe7adf {
    param($Config, [switch]$NonInteractive)
    Lz1cc9bde2a4 -Config $Config -Stage 'api-discovery'
    Lz81c8264516 -Config $Config
    ...
    if ($NonInteractive) {
        foreach ($s in $projects) {
            $rows.Add((Lz0b069fad38 -Project $s -Dest @{ Action='MIGRATE'; DestinationType='SharePoint'; DestinationUrl="$($Config.destination.teamSiteBaseUrl)/$((ConvertTo-Slug $s.Name))"; TargetPrincipal=''; TargetLibrary=''; TargetSubFolder=''; Notes='auto (non-interactive)' }))
        }
    } else {
        ...
        while ($remaining.Count -gt 0) {
            ...
            $proj = $remaining[$n - 1]
            $dest = Lz695dba8506 -Config $Config -Project $proj
            if ($dest.Action -eq 'MIGRATE') {
                $newRow = Lz0b069fad38 -Project $proj -Dest $dest
                $rows.Add($newRow)
    ...
    ($rows | Sort-Object Space) | Lz551c5b804a | Export-Csv -Path $out -NoTypeInformation -Encoding UTF8
...
function Write-PreFlightReport {
    ...
    $actionIssues = @($Issues | Where-Object { ((Lz9a9b8e46d2 $_ 'Severity')) -and "$($_.Severity)" -eq 'ACTION' })
    $autoIssues   = @($Issues | Where-Object { -not (((Lz9a9b8e46d2 $_ 'Severity')) -and "$($_.Severity)" -eq 'ACTION') })
    ...
    if ($reviewCount) { $bExtra += " $reviewCount destination(s) could not be checked (sign-in, access, or the destination was not found) - see the table below and verify before migrating." }
    ...
    foreach ($s in $Summary) {
        ...
        $spNote = ''
        try { if (((Lz9a9b8e46d2 $s 'SrcPath')) -and "$($s.SrcPath)") { $spNote = "<div class='dim' style='font-size:11px'>$(_enc $s.SrcPath)</div>" } } catch {}
    ...
    if (((Lz9a9b8e46d2 $Config.run 'report')) -and ((Lz9a9b8e46d2 $Config.run.report 'brand')) -and $Config.run.report.brand) { $brand = "$($Config.run.report.brand)" }
    $now = Get-Date
    $tv = Lz6a2d694fde; $verSub = if ($tv) { " &middot; v$tv" } else { '' }
    $html = @"
...
function Invoke-PreFlight {
    param($Config)
    Lz1cc9bde2a4 -Config $Config -Stage 'api-preflight'
    Lz81c8264516 -Config $Config
    ...
    $maxPath = 400
    $sanitise = (((Lz9a9b8e46d2 $Config.run 'sanitiseNames')) -and $Config.run.sanitiseNames)
    ...
    $t = $map.Count; $i = 0
    foreach ($row in $map) {
        $i++; Lzccc361cfae -Activity 'Pre-flight' -Status $row.Space -Current $i -Total $t | Out-Null
        $space = New-SpaceRef -Row $row -Config $Config
        try {
            $items = @(Get-DattoItems -Config $Config -Space $space)
        } catch {
            Write-MigrationLog ERROR "  could not read files for '$($row.Space)': $($_.Exception.Message). In plain terms: the source may be empty, moved, or unreachable. Skipping it in pre-flight checks."
            $issues.Add([pscustomobject]@{ Space = $row.Space; File = ''; Problem = "source could not be read ($($_.Exception.Message)) - skipped"; Severity = 'ACTION' })
            continue
        }
        ...
        $rowCo = $false
        try { if ((Lz9a9b8e46d2 $row 'SourceContentsOnly')) { $rowCo = ("$($row.SourceContentsOnly)".Trim() -match '^(?i)(true|1|yes)$') } } catch {}
        $destKey = @(
        ...
        if ("$($Config.source.provider)" -eq 'LocalFs') {
            foreach ($it2 in @($items | Where-Object { ((Lz9a9b8e46d2 $_ 'NfdRenamed')) -and $_.NfdRenamed })) {
        ...
        if ($Config.destination.provider -eq 'LocalSim') { $verdict = 'SIM'; $freeTxt = 'SIM' }
            ...
            try {
                $drive = $null
                $destRes = Resolve-DestinationDriveId -Config $Config -Row $row
                if ($destRes -and $destRes.DriveId) {
                    $drive = Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/v1.0/drives/$($destRes.DriveId)"
                }
    ...
    if ($destCollideGroupCount -gt 0) {
        $destCollisionRows | Lz551c5b804a | Export-Csv (Join-Path $Config.run.reportRoot 'api-preflight-destination-collisions.csv') -NoTypeInformation -Encoding UTF8
        foreach ($g in $collisionGroups) {
            $spacesHere = @($g.Group | Select-Object -ExpandProperty Space -Unique | Sort-Object)
    ...
    $issues  | Lz551c5b804a | Export-Csv (Join-Path $Config.run.reportRoot 'api-preflight-issues.csv') -NoTypeInformation -Encoding UTF8
    $summary | Lz551c5b804a | Export-Csv (Join-Path $Config.run.reportRoot 'api-preflight-summary.csv') -NoTypeInformation -Encoding UTF8
    $blocked = @($summary | Where-Object Verdict -eq 'BLOCKED').Count
    $reviewCount = @($summary | Where-Object Verdict -eq 'REVIEW').Count
    ...
    if ($reviewCount -gt 0) { Write-MigrationLog WARN "$reviewCount destination(s) could not be checked (sign-in, access, or the destination was not found) - see api-preflight-summary.csv. In plain terms: the tool could not confirm this destination is reachable, so it is NOT counted as ready. Fix sign-in or access, or confirm the destination exists, then run this check again." }
    ...
    $largestRows | Lz551c5b804a | Export-Csv (Join-Path $Config.run.reportRoot 'api-preflight-largest-files.csv') -NoTypeInformation -Encoding UTF8
    ...
    $dupRows | Lz551c5b804a | Export-Csv (Join-Path $Config.run.reportRoot 'api-preflight-duplicates.csv') -NoTypeInformation -Encoding UTF8
    $wasted = [int64]0
    if ($gid -gt 0) {
        $wasted = Get-SafeSum -Items (@($dupRows | Group-Object DuplicateGroupId | ForEach-Object { [int64]$_.Group[0].SizeBytes * ($_.Count - 1) }))
    ...
    foreach ($a in @(Get-ChildItem (Join-Path $Config.run.reportRoot 'audit-*.csv') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending)) {
        try {
            ...
            $cb  = Get-SafeSum -Items $cp -Property SourceSizeBytes
...
function Write-AuditRunEnd {
    param([string]$AuditFile,[string]$Status,[string]$Reason,[int]$Processed,[int]$Expected,[int]$Copied,[int]$Skipped,
          [int]$Failed,[int]$VerifyFail,[int]$ZeroByte,[int64]$Bytes,[int]$SpacesPlanned,[int]$SpacesCompleted,[string]$ElapsedText,[int]$NotAttempted=0)
    ...
    $rec = [pscustomobject][ordered]@{
        ...
        Reason=("$Status | $Reason | processed $Processed of $Expected | copied $Copied skipped $Skipped notAttempted $NotAttempted failed $Failed verifyFail $VerifyFail zero $ZeroByte | spaces $SpacesCompleted/$SpacesPlanned | elapsed $ElapsedText")
        Error=''
    }
...
function Lzcca33cb883 { param($Config) try { Remove-Item (Get-RunActivePath $Config) -Force -ErrorAction SilentlyContinue } catch {} }
...
function Get-EmailCommonVars {
    ...
    return @{ JobName = $job; Tenant = $tenant; Version = (Lz6a2d694fde) }
}
...
function Get-EmailAttachmentSet {
    ...
    try {
        ...
        $tot = Get-SafeSum -Items $ex -Property Length
    ...
    try {
        ...
        if ($listId -and $siteId) {
            ...
            if ($script:ProvColsOn) {
                ...
                if ($script:ProvOwnerAvailable) {
                    $defs += ,@{ name = 'SourceOwner'; displayName = 'Source owner'; description = 'Owner at the migration source'; text = @{} }
                }
                $defs += @(
...
function Set-ProvenanceFields {
    ...
    $bodyH = @{}
    if ("$MigratedOnIso") {
        $bodyH['SourcePath'] = "$SourcePath"
        $bodyH['MigratedOn'] = "$MigratedOnIso"
        if ("$SourceOwner") { $bodyH['SourceOwner'] = "$SourceOwner" }
    }
...
function Invoke-ProvenanceColumnSweep {
    ...
    try {
        ...
        $script:ProvOwnerAvailable = (@(@($map.Values) | Where-Object { $o = "$($_.OwnerResolved)"; $o -and $o -ne 'n/a' }).Count -gt 0)
        $onIso = (Get-Date).ToUniversalTime().ToString('o')
        Write-MigrationLog INFO "Stamping provenance columns (Source path$(if ($script:ProvOwnerAvailable) { ' / Source owner' } else { '' }) / Migrated on) onto $($rows.Count) migrated file(s)..."
        $done = 0; $i = 0
        foreach ($r in $rows) {
            $i++
            if (($i % 25) -eq 0) { Lzccc361cfae -Activity 'Provenance columns' -Status "$i of $($rows.Count)" -Current $i -Total $rows.Count | Out-Null }
            $mrow = $map[$r.Space]
            ...
            $extra = @{}
            $relRaw   = ("$($r.SourcePath)" -replace '\\','/').TrimStart('/')
            $spaceSeg = "$($r.Space)"
            $relUnder = $relRaw
            if ($spaceSeg -and ($relRaw -eq $spaceSeg -or $relRaw.StartsWith(($spaceSeg + '/'), [System.StringComparison]::Ordinal))) {
                $relUnder = $relRaw.Substring([Math]::Min($spaceSeg.Length + 1, $relRaw.Length))
            }
            $relParts = @($relUnder -split '/')
            ...
            try {
                ...
                Set-ProvenanceFields -Config $Config -DriveId $driveId -EncPath $enc `
                    -SourcePath $(if ($trioOn) { $(if ($spaceSeg) { "$spaceSeg/$relUnder" } else { $relUnder }) } else { '' }) `
...
function Invoke-RunFinalize {
    ...
    $copied=0;$skipped=0;$failed=0;$verify=0;$zero=0;$notAttempted=0;$processed=0;[int64]$bytes=0;$auditWriteFails=0
    $auditRows = @()
    ...
    foreach ($r in $auditRows) {
        switch ("$($r.Status)") {
            ...
            'NotAttempted'  { $notAttempted++; $processed++ }
    ...
    $unreadable = 0; $unreadableWhere = 'the source'
    try {
        $unreadable = [int]$script:UnreadableCount
        $ul = @(@($script:UnreadableSpaces) | Where-Object { $_ })
        if ($ul.Count -eq 1) { $unreadableWhere = "[$($ul[0])]" } elseif ($ul.Count) { $unreadableWhere = "$($ul.Count) source(s)" }
    } catch {}
    if ($ForcedStatus) {
        $status = $ForcedStatus; $reason = $ForcedReason
    } elseif ($script:TrialCapped -or $notAttempted -gt 0) {
        $status = 'CAPPED'
        $reason = "evaluation limit reached: $copied file(s) copied for real, $notAttempted file(s) NOT attempted (never checked or copied) - a licence is required to migrate the rest"
        if ($errCount -gt 0) { $reason += "; also $errCount file(s) failed or could not be verified" }
    } else {
        ...
        elseif ($errCount -gt 0 -or $unreadable -gt 0) {
            $status = 'COMPLETED_WITH_ERRORS'
            $rParts = @()
            if ($errCount -gt 0)   { $rParts += "$errCount file(s) failed or could not be verified" }
            if ($unreadable -gt 0) { $rParts += "$unreadable folder(s)/file(s) under $unreadableWhere could NOT be read, so they were never copied - this run is NOT a complete copy of the source" }
            $reason = ($rParts -join '; ')
        }
    ...
    try {
        Write-AuditRunEnd -AuditFile $AuditFile -Status $status -Reason $reason -Processed $processed -Expected $ExpectedFiles `
            ...
            -Bytes $bytes -SpacesPlanned $SpacesPlanned -SpacesCompleted $SpacesCompleted -ElapsedText $el -NotAttempted $notAttempted
    } catch {}
    $outcome = [ordered]@{
        ...
        Copied=$copied; Skipped=$skipped; NotAttempted=$notAttempted; Failed=$failed; VerifyFail=$verify; ZeroByte=$zero
        Unreadable=$unreadable
        UnreadableSpaces=(@(@($script:UnreadableSpaces) | Where-Object { $_ }) -join '; ')
    ...
    try {
        switch ($status) {
            ...
            'CAPPED'                { Write-MigrationLog WARN  "=== RUN STOPPED: evaluation limit reached. Uploaded $copied file(s) ($szTxt) for real; $notAttempted file(s) were NOT attempted (never checked or copied) because the free evaluation cap was hit. Licence this tenant to migrate the rest, then run 'Sync new and changed' to pick up where it stopped. ===" }
    ...
    try {
        $emBucket = switch ($status) { 'COMPLETED' { 'Success' } 'COMPLETED_WITH_ERRORS' { 'Warning' } 'CAPPED' { 'Warning' } default { 'Failure' } }
    ...
    Lzcca33cb883 -Config $Config
    return $outcome
}
function Lzf9c949f85f {
...
function Invoke-FinalizeAction {
    ...
    Lz1cc9bde2a4 -Config $Config -Stage 'api-finalize'
    $act = $null
    ...
    if (-not $AuditPath -or -not (Test-Path $AuditPath)) {
        Write-MigrationLog WARN "Record outcome: no audit CSV found, so there is nothing to record."; Lzcca33cb883 -Config $Config; return
    }
    $sidecar = ($AuditPath -replace '\.csv$','') + '.outcome.json'
    if (Test-Path $sidecar) { Write-MigrationLog INFO "Record outcome: the run already has an outcome; leaving it as it is."; Lzcca33cb883 -Config $Config; return }
...
function Lza56e77c7fd {
    param($Config)
    try {
        $thumb = "$($Config.auth.certThumbprint)".Trim()
        if (-not $thumb -or $thumb -like 'reg:*') { return }
        $store = 'Cert:\CurrentUser\My'
        try { if (((Lz9a9b8e46d2 $Config.auth 'certStore')) -and $Config.auth.certStore) { $store = "$($Config.auth.certStore)" } } catch {}
...
function Lz5ae1a37821 {
    ...
    try {
        ...
        if ($tmpRoot -and (Test-Path $tmpRoot)) {
            ...
            $freeB = Lz0a762041da -Path $tmpRoot
            if ($freeB -ge 0) {
    ...
    Lza56e77c7fd -Config $Config
}
function Lz39083596db {
    param($Config, $Rows)
    if ($Config.destination.provider -eq 'LocalSim') { return }
    $seen = @{}
    foreach ($row in $Rows) {
...
function Invoke-Transfer {
    ...
    Lz1cc9bde2a4 -Config $Config -Stage "api-transfer-$Mode" -Friendly $(
    ...
    if ($FailedOnly) {
        ...
        $script:FailedOnlySet = Lze8b6a94c7a -Config $Config -AuditPath $FailedFromAudit
    }
    Lze324547335 -Config $Config
    Test-MappingSpaceIds -Config $Config
    if ($Execute -and $Config.destination.provider -ne 'LocalSim' -and $null -eq $script:TrialCap) {
        $lzOk = $false; $lzTen = ''
        try {
            $lzP = ''
            try { if (((Lz9a9b8e46d2 $Config.run 'licenceFile')) -and $Config.run.licenceFile) { $lzP = "$($Config.run.licenceFile)" } } catch {}
    ...
    if ($MaxParallelSpaces -gt 1 -and $runnable.Count -gt 1) {
        Lzf746bc2b4a -Config $Config -Rows $runnable -Mode $Mode -DeltaMode $DeltaMode -SpoolAhead $SpoolAhead -MaxParallelSpaces $MaxParallelSpaces -Execute:$Execute
        return
    }
    ...
    $spacesCompleted = 0
    $transferError   = $null
    if ($willWrite -and -not $ResultDir) { Lz375616067b -Config $Config -Mode $Mode -StartUtc $runStart -AuditFile $script:AuditFile -ExpectedFiles 0 -SpacesPlanned $spacesPlanned -SpacesCompleted 0 }
    $summary = New-Object System.Collections.Generic.List[object]
    $sTotal = $map.Count; $sIdx = 0
    try {
    if ($willWrite) { Lz39083596db -Config $Config -Rows $map; Lz5ae1a37821 -Config $Config }
    foreach ($row in $map) {
        $sIdx++
        Lzccc361cfae -Activity "Transfer ($Mode)" -Status $row.Space -Current $sIdx -Total $sTotal -Id 1 | Out-Null
        ...
        try { if (((Lz9a9b8e46d2 $Config.run.upload 'overlapListing')) -and $Config.run.upload.overlapListing) { $overlapListing = $true } } catch {}
        ...
        if ($streamFP) {
            ...
            $rowScope = "$($row.Space)"
            try { if (((Lz9a9b8e46d2 $row 'SourceSubPath')) -and "$($row.SourceSubPath)".Trim()) { $rowScope = "$($row.Space) / $("$($row.SourceSubPath)".Trim().Trim('/'))" } } catch {}
            ...
            try {
                ...
                Write-MigrationLog ERROR "  could not read files for '$($row.Space)': $($_.Exception.Message). In plain terms: the source may be empty, moved, or unreachable. Skipping it in this run."
                continue
            }
        ...
        $presenceMode = $false
        $repair = @{}
        $san = (((Lz9a9b8e46d2 $Config.run 'sanitiseNames')) -and $Config.run.sanitiseNames)
        if ($restoreSet.Count) {
        ...
        elseif ($Mode -eq 'Delta') {
            ...
            $toDo = @($items | Where-Object {
                ...
                if ($null -ne $ds -and -not (Lza8a3882917 -RelPath $rp -SourceSize ([int64]$_.Size) -DestSize $ds)) {
        ...
        $toDo = @($toDo)
        $toDoPreCapIds = New-Object 'System.Collections.Generic.HashSet[string]'
        foreach ($it in $toDo) { [void]$toDoPreCapIds.Add("$($it.Id)") }
        $capCutIds = New-Object 'System.Collections.Generic.HashSet[string]'
        if ($null -ne $script:TrialCap) {
            if ($script:TrialFilesRemaining -le 0) {
                if ($toDo.Count -gt 0) { $script:TrialCapped = $true; foreach ($it in $toDo) { [void]$capCutIds.Add("$($it.Id)") } }
                $toDo = @()
            } elseif ($toDo.Count -gt $script:TrialFilesRemaining) {
                $script:TrialCapped = $true
                $kept = @($toDo | Select-Object -First $script:TrialFilesRemaining)
                $keptIds = New-Object 'System.Collections.Generic.HashSet[string]'
                foreach ($it in $kept) { [void]$keptIds.Add("$($it.Id)") }
                foreach ($it in $toDo) { if (-not $keptIds.Contains("$($it.Id)")) { [void]$capCutIds.Add("$($it.Id)") } }
                $toDo = $kept
            }
            $script:TrialFilesRemaining -= $toDo.Count
        }
        $skipped = $items.Count - $toDo.Count - $capCutIds.Count
        $script:TrialNotAttempted = ([int]$script:TrialNotAttempted) + $capCutIds.Count
        $fTotal = $toDo.Count; $fIdx = 0
        $spaceBytesTotal = Get-SafeSum -Items $toDo -Property Size
        $expectedTotal += $fTotal
        if ($willWrite) { Lz70bc5f9b70 -Done 0 -Total $fTotal -BytesDone 0 -BytesTotal $spaceBytesTotal -Space $row.Space }
        if ($script:GuiMode) { Write-Host "##SCOPE##|$fTotal|$($items.Count)|$skipped" }
        if ($willWrite -and $Mode -ne 'FirstPass') {
            foreach ($sk in @($items | Where-Object { -not $toDoPreCapIds.Contains("$($_.Id)") })) {
        ...
        if ($willWrite -and $capCutIds.Count -gt 0) {
            foreach ($sk in @($items | Where-Object { $capCutIds.Contains("$($_.Id)") })) {
                Write-AuditRecord -Space $row.Space -Item $sk -Status 'NotAttempted' -Reason 'evaluation limit reached before this file could be attempted: never checked or copied. A licence is required to migrate it.'
            }
        }
        if (-not $willWrite) {
        ...
        elseif ($UploadWorkers -gt 1) {
            ...
            foreach ($it in $toDo) {
                ...
                $rp = $rp -replace '\\','/'
                if ($sanitise) { $rp = Lzc13255adcc -Space $row.Space -Rp $rp -Item $it }
                $it | Add-Member -NotePropertyName RelPath -NotePropertyValue $rp -Force
            }
            $downloadThreads = 4
            if ("$($Config.source.provider)" -eq 'LocalFs') { $downloadThreads = 8 }
            if (((Lz9a9b8e46d2 $Config.run 'download')) -and ((Lz9a9b8e46d2 $Config.run.download 'threads')) -and $Config.run.download.threads) { $downloadThreads = [int]$Config.run.download.threads }
            $simDestBase = ''
            ...
            if ($Config.destination.provider -ne 'LocalSim') { $tk = Lz6148a4ec50 -Config $Config; $tokenHolder.Token = $tk.Token; $tokenHolder.Exp = $tk.Exp }
            ...
            if ((Lz9a9b8e46d2 $Config.run.throttle 'adaptive')) {
                $ad = $Config.run.throttle.adaptive
                if ((Lz9a9b8e46d2 $ad 'enabled')) { $adaptOn = [bool]$ad.enabled }
                if ((Lz9a9b8e46d2 $ad 'minWorkers') -and [int]$ad.minWorkers -ge 1) { $minWorkers = [int]$ad.minWorkers }
                if ((Lz9a9b8e46d2 $ad 'growAfterSeconds') -and [int]$ad.growAfterSeconds -ge 1) { $growAfter = [int]$ad.growAfterSeconds }
            }
            ...
            if ((Lz9a9b8e46d2 $Config.run 'bandwidth')) {
                $bwc = $Config.run.bandwidth
                if ((Lz9a9b8e46d2 $bwc 'maxUploadMbps'))   { $maxUpMbps   = [double]$bwc.maxUploadMbps }
                if ((Lz9a9b8e46d2 $bwc 'maxDownloadMbps')) { $maxDownMbps = [double]$bwc.maxDownloadMbps }
                if (((Lz9a9b8e46d2 $bwc 'burstSeconds')) -and ([double]$bwc.burstSeconds -gt 0)) { $burstSec = [double]$bwc.burstSeconds }
            }
                ...
                foreach ($ready in $spool.GetConsumingEnumerable()) {
                    ...
                    try {
                        ...
                        if ($preserveDates) {
                            ...
                            $fsiCreated=$fsiIso
                            try { if (((Lz9a9b8e46d2 $it 'CreatedUtc')) -and $it.CreatedUtc) { $fsiCreated=([datetimeoffset]::Parse("$($it.CreatedUtc)",[System.Globalization.CultureInfo]::InvariantCulture)).UtcDateTime.ToString('o') } } catch {}
                        }
                        ...
                        elseif ($destSize -eq $localSize) { $status='Copied';$reason='verified (size match)' }
            ...
            $process = {
                ...
                switch ($res.Status) {
                    ...
                    'SkippedTooLarge'{ $script:failedRef.Value++; Write-MigrationLog ERROR "  NOT COPIED (too large for the temp drive) $($it.RelativePath): $($res.Error). In plain terms: this one file will not fit in the temp drive's free space, so it has NOT been copied. It counts as a failure until it is. Free up space (or point the temp folder at a bigger drive) and Sync to pick it up." }
                ...
                if ($script:PreserveDates -and $null -ne $script:SweepList -and $Config.destination.provider -ne 'LocalSim' -and
                    ...
                    $swCt = ''; try { if (((Lz9a9b8e46d2 $it 'CreatedUtc')) -and $it.CreatedUtc) { $swCt = "$($it.CreatedUtc)" } } catch {}
                ...
                $rsn = Lz25230be3b4 -Repair $repair -Item $it -Status $res.Status -Reason $res.Reason
                ...
                Lz70bc5f9b70 -Done $script:fIdxRef.Value -Total $fTotal -BytesDone $script:bytesRef.Value -BytesTotal $spaceBytesTotal -Space $row.Space `
            ...
            if ($streamFP) {
                ...
                $streamEnumFailed = $false; $streamEnumErr = ''
                try {
                Get-DattoItems -Config $Config -Space $space -OnItem {
                    ...
                    if ($Config.destination.provider -ne 'LocalSim' -and [DateTimeOffset]::UtcNow -gt $tokenHolder.Exp) {
                        try { $tk = Lz6148a4ec50 -Config $Config; $tokenHolder.Token = $tk.Token; $tokenHolder.Exp = $tk.Exp } catch {}
                    }
                    $rp = if ($effSub) { ($effSub.Trim('/') + '/' + $it.RelativePath) } else { $it.RelativePath }
                    $rp = $rp -replace '\\','/'
                    if ($sanitise) { $rp = Lzc13255adcc -Space $row.Space -Rp $rp -Item $it }
                    ...
                    $streamEnumFailed = $true; $streamEnumErr = $_.Exception.Message
                }
                $enumFlag.Done = $true
                ...
                $expectedTotal += $fTotal
                if ($streamEnumFailed) {
                    Write-MigrationLog ERROR "  could not read files for '$($row.Space)': $streamEnumErr. In plain terms: the source may be empty, moved, or unreachable. $fTotal file(s) already queued will still upload; nothing further will be read for this project."
                    if ($script:GuiMode) { Write-Host "##STATUS##|Could not read [$($row.Space)] from $($script:SrcNoun): $streamEnumErr" }
                } else {
            ...
            while (@($workers | Where-Object { -not $_.Handle.IsCompleted }).Count -gt 0 -or $resQ.Count -gt 0) {
                $res=$null
                while ($resQ.TryDequeue([ref]$res)) { & $process $res; Lzccc361cfae -Activity "Files: $($row.Space)" -Status $res.DestPath -Current $script:fIdxRef.Value -Total $fTotal -Id 2 | Out-Null }
                ...
                if ($Config.destination.provider -ne 'LocalSim' -and [DateTimeOffset]::UtcNow -gt $tokenHolder.Exp) { try { $tk=Lz6148a4ec50 -Config $Config; $tokenHolder.Token=$tk.Token; $tokenHolder.Exp=$tk.Exp } catch {} }
                $nowT=[DateTime]::UtcNow.Ticks
                [System.Threading.Monitor]::Enter($thr.SyncRoot)
                try {
                ...
                if ($curMax -ne $script:LastThrMax -or $isPaused -ne $script:LastThrPaused) {
                    Lz480cfbe69a -Max $curMax -HardMax $thr.HardMax -PausedSeconds $pausedSecs -Events $curEvents -Code $curCode
            ...
            if ($script:LastThrMax -ne -1 -and $script:LastThrMax -lt $thr.HardMax) { Lz480cfbe69a -Max $thr.HardMax -HardMax $thr.HardMax -PausedSeconds 0 -Events $thr.Events }
        ...
        elseif ($SpoolAhead -le 1) {
            ...
            foreach ($it in $toDo) {
                ...
                Lzccc361cfae -Activity "Files: $($row.Space)" -Status $it.RelativePath -Current $fIdx -Total $fTotal -Id 2 | Out-Null
                ...
                try {
                    Lz280e72082c -Config $Config -Item $it -OutFile $tempFile | Out-Null
                    ...
                    $v = Lz38d86e77f6 -Item $it -LocalFile $tempFile -DestInfo $destInfo
                    $upSw.Stop()
                    if ($v.Ok) {
                ...
                Lz70bc5f9b70 -Done $fIdx -Total $fTotal -BytesDone $bytes -BytesTotal $spaceBytesTotal -Space $row.Space -SmallDone $etaSmallDone -SmallTotal $etaSmallTotal -LargeBytesDone $etaLargeDone -LargeBytesTotal $etaLargeTotal -Final 1
            }
        }
        else {
            ...
            foreach ($ready in $spool.GetConsumingEnumerable()) {
                ...
                try {
                    ...
                    $v = Lz38d86e77f6 -Item $it -LocalFile $ready.TempFile -DestInfo $destInfo
                    $sw.Stop()
                    if ($v.Ok) {
                        if ($v.Zero) { Write-MigrationLog WARN "  empty file (0 bytes), uploaded as-is: $($it.RelativePath)"; $zero++ }
                        $copied++; $bytes += $it.Size
                        if ($script:VerboseFiles) { Write-MigrationLog INFO "  + [$($it.RelativePath)] $($v.Reason)" }
                        $stat = if($v.Zero){'ZeroByte'}else{'Copied'}
                        Write-AuditRecord -Space $row.Space -Item $it -Status $stat -Reason (Lz25230be3b4 -Repair $repair -Item $it -Status $stat -Reason $v.Reason) -DestSize $destInfo.Size -DestPath $destInfo.Path -DestHash $destInfo.Hash -Method $destInfo.Method -Retries $destInfo.Retries -DurationMs $sw.Elapsed.TotalMilliseconds -DownloadMs $dlMs
                    } else {
                        $verifyFail++; Write-MigrationLog ERROR "  VERIFY FAILED $($it.RelativePath): $($v.Reason). In plain terms: the uploaded copy did not match the original, so it was not marked done. It will be retried on the next Sync."
        ...
        if ($willWrite) {
            ...
            $previewCaveat =
                if ($Mode -eq 'FirstPass') { " This counts every file as if none of it existed at the destination yet (a Full-migration-shaped count); Sync would very likely copy far fewer, since it only touches files that are new or changed there." }
                elseif ($Mode -eq 'Delta') { " NOTE: a Sync preview does not read the real destination either (Preview never does), so this cannot show what is already there - treat it as an upper bound, not a Sync forecast." }
                else { '' }
            Write-MigrationLog INFO "  $($row.Space): PREVIEW - would copy $copied file(s) ($szTxt) and skip $skipped unchanged. Nothing was uploaded.$previewCaveat"
        }
        ...
        $spacesCompleted++
        if ($willWrite -and -not $ResultDir) { Lz375616067b -Config $Config -Mode $Mode -StartUtc $runStart -AuditFile $script:AuditFile -ExpectedFiles $expectedTotal -SpacesPlanned $spacesPlanned -SpacesCompleted $spacesCompleted }
    }
    ...
    if ($ResultDir) {
        ...
        $summary | Lz551c5b804a | Export-Csv -Path (Join-Path $Config.run.reportRoot "api-transfer-$Mode-$(Get-Date -Format 'yyyyMMdd-HHmmss').csv") -NoTypeInformation -Encoding UTF8
    }
        ...
        try {
            ...
            foreach ($sw in $script:SweepList) {
                try {
                    ...
                    $destLm = $null
                    try { $destLm = Lz520b3311f9 $di['fileSystemInfo']['lastModifiedDateTime'] } catch { try { $destLm = Lz520b3311f9 $di.fileSystemInfo.lastModifiedDateTime } catch {} }
                    $destCr = $null
                    try { $destCr = Lz520b3311f9 $di['fileSystemInfo']['createdDateTime'] } catch { try { $destCr = Lz520b3311f9 $di.fileSystemInfo.createdDateTime } catch {} }
    ...
    if ($willWrite -and -not $ResultDir -and $Config.destination.provider -ne 'LocalSim' -and $script:AuditFile -and (Test-Path $script:AuditFile)) {
        $provOn = $true
        try { if (((Lz9a9b8e46d2 $Config.run 'provenance')) -and ((Lz9a9b8e46d2 $Config.run.provenance 'enabled'))) { $provOn = [bool]$Config.run.provenance.enabled } } catch {}
    ...
    $tc = Get-SafeSum -Items $summary -Property Copied
    $ts = Get-SafeSum -Items $summary -Property Skipped
    $tf = Get-SafeSum -Items $summary -Property Failed
    $tv = Get-SafeSum -Items $summary -Property VerifyFail
    $tb = Get-SafeSum -Items $summary -Property Bytes
    ...
    if ($willWrite) {
        ...
        if (-not $ResultDir -and $script:AuditFile -and (Test-Path $script:AuditFile)) {
            ...
            try {
                $diag = Lzbbdf04187d -Rows @(Import-Csv $script:AuditFile) -Elapsed $elapsed -ConfiguredWorkers $UploadWorkers -ThrottleEvents $script:RetryEvents -TrialCapped:$script:TrialCapped
                if ($diag) {
        ...
        Write-MigrationLog OK "Preview finished in $el. This was a rehearsal, nothing was uploaded. It would copy $tc file(s), $szTxt, counting every one of them as if starting fresh - this preview does not check what is already at the destination, so it cannot forecast a Sync. In plain terms: click 'Full migration' only if you want every file re-copied from $($script:SrcNounBare) regardless of what is already there; for an ordinary top-up that copies only what is new or changed, use 'Sync new and changed' instead - it will very likely touch far fewer files than the number above."
    }
    }
    catch {
        $transferError = $_
        ...
        Lzf9c949f85f -Config $Config -Mode $Mode -RunStart $runStart -ExpectedFiles $expectedTotal `
...
function Lzf746bc2b4a {
    ...
    if ($willWrite) { Lz375616067b -Config $Config -Mode $Mode -StartUtc $parallelStart -AuditFile '' -ExpectedFiles 0 -SpacesPlanned $Rows.Count -SpacesCompleted 0 }
    ...
    Lzcca33cb883 -Config $Config
    $tc = Get-SafeSum -Items $all -Property Copied
    $tf = Get-SafeSum -Items $all -Property Failed
    $tv = Get-SafeSum -Items $all -Property VerifyFail
...
function Get-RowDestLabel {
    ...
    $lib = ''
    try { if ((Lz9a9b8e46d2 $Row 'TargetLibrary')) { $lib = "$($Row.TargetLibrary)".Trim() } } catch {}
    if (-not $lib) { $lib = 'the default library' }
    return "$($Row.DestinationUrl) > $lib"
}
function Get-LocalFsRootSeed {
    param($Config, $Row)
    $isLocalFs = $false
    try { if ($Config -and ($Config.PSObject.Properties.Name -contains 'source') -and $Config.source -and ("$($Config.source.provider)" -eq 'LocalFs')) { $isLocalFs = $true } } catch {}
    if (-not $isLocalFs) { return '' }
    $scope = ''
    ...
    if ($co) { return '' }
    $seed = "$($Row.Space)"
    try { if (($Config.run.PSObject.Properties.Name -contains 'sanitiseNames') -and $Config.run.sanitiseNames) { $seed = ConvertTo-SafeRelPath $seed } } catch {}
    return $seed
}
function Get-DestinationInventory {
    ...
    if ($Config.destination.provider -eq 'LocalSim') {
        ...
        $destSub = Join-SubPath $Row.TargetSubFolder (Get-NestFolder $Config $Row)
        $simScope = ''
        try { if ((Lz9a9b8e46d2 $Row 'SourceSubPath')) { $simScope = "$($Row.SourceSubPath)".Trim().Trim('/').Trim('\') } } catch {}
        $simCo = $false
        try { if ((Lz9a9b8e46d2 $Row 'SourceContentsOnly')) { $simCo = ("$($Row.SourceContentsOnly)".Trim() -match '^(?i)(true|1|yes)$') } } catch {}
        $rootSeed = Get-LocalFsRootSeed -Config $Config -Row $Row
        $seedSeg = if ($simScope -and -not $simCo) { $simScope } elseif ($rootSeed) { $rootSeed } else { '' }
        $sub = if ($simCo) { $destSub } else { Join-SubPath $destSub $seedSeg }
        ...
        foreach ($f in $files) {
            $relBare = ($f.FullName.Substring($root.Length).TrimStart('\','/') -replace '\\','/')
            $rel = if ($seedSeg -and -not $simCo) { "$seedSeg/$relBare" } else { $relBare }
            [void]$paths.Add($rel)
    ...
    $driveId = $res.DriveId
    $scope = ''
    try { if ((Lz9a9b8e46d2 $Row 'SourceSubPath')) { $scope = "$($Row.SourceSubPath)".Trim().Trim('/').Trim('\') } } catch {}
    $co = $false
    try { if ((Lz9a9b8e46d2 $Row 'SourceContentsOnly')) { $co = ("$($Row.SourceContentsOnly)".Trim() -match '^(?i)(true|1|yes)$') } } catch {}
    $isLocalFs = $false
    ...
    if (-not $scope -and -not $isLocalFs) { $co = $false }
    $rootSeed = Get-LocalFsRootSeed -Config $Config -Row $Row
    $seedSeg = if ($co) { '' } elseif ($scope) { $scope } else { $rootSeed }
    $startSub = if ($co) { $res.SubFolder } else { Join-SubPath $res.SubFolder $seedSeg }
    ...
    $stack.Push(@{ Node = $startNode; Prefix = $(if ($seedSeg) { "$seedSeg/" } else { '' }); Root = $true })
    while ($stack.Count -gt 0) {
        $cur = $stack.Pop()
        $uri = "https://graph.microsoft.com/v1.0/drives/$driveId/$($cur.Node)/children"
        $page = $null
        try {
            $page = Get-GraphCollection -Config $Config -Uri $uri -What "list $($cur.Node)" -QuietNotFound:([bool]$cur.Root)
        } catch {
            $st = $null; try { $st = $_.Exception.Response.StatusCode.value__ } catch {}
...
function Test-MigrationLicence {
    ...
    $path = ''
    try { if (((Lz9a9b8e46d2 $Config.run 'licenceFile')) -and $Config.run.licenceFile) { $path = "$($Config.run.licenceFile)" } } catch {}
...
function Lz1203286ccb {
    ...
    if (-not $agg.Count) {
        ...
        Write-MigrationLog OK "  [$($Row.Space)] every file matches by path, both ways. Nothing extra, nothing missing.$alsoNote"; return ([pscustomobject]@{ Missing = 0; Extra = 0; OtherSource = $nOtherSource })
    }
    $rows = New-Object System.Collections.Generic.List[object]
    ...
    try {
        $rows | Lz551c5b804a | Export-Csv -Path $out -NoTypeInformation -Encoding UTF8
    ...
    return ([pscustomobject]@{ Missing = $nMissing; Extra = $nExtra; OtherSource = $nOtherSource })
}
function Lz7e4db560c9 {
    param($Config, [string]$SpaceName, [string]$ResultDir)
    Lz1cc9bde2a4 -Config $Config -Stage 'api-destinv'
...
function Receive-DestInventoryJob {
    ...
    try {
        ...
        $co = $false
        try { if ((Lz9a9b8e46d2 $Row 'SourceContentsOnly')) { $co = ("$($Row.SourceContentsOnly)".Trim() -match '^(?i)(true|1|yes)$') } } catch {}
        $docCo = $false; if ((Lz9a9b8e46d2 $doc 'ContentsOnly')) { $docCo = [bool]$doc.ContentsOnly }
...
function Invoke-SizeCheck {
    ...
    Lz1cc9bde2a4 -Config $Config -Stage 'api-sizecheck'
    $ckStart = Get-Date
    Lz81c8264516 -Config $Config
    Connect-Destination -Config $Config
    $mapPath = Join-Path $Config.run.reportRoot 'mapping.csv'
    ...
    foreach ($row in $map) {
        $i++; Lz70bc5f9b70 -Done $i -Total $t -BytesDone 0 -BytesTotal 0 -Space $row.Space
        try {
        ...
        $space = New-SpaceRef -Row $row -Config $Config
        $dj = Lzc2a09b194a -Config $Config -Row $row
        try {
            $items = @(Get-DattoItems -Config $Config -Space $space)
        } catch {
            Write-MigrationLog ERROR "  could not read files for '$($row.Space)': $($_.Exception.Message). In plain terms: the source may be empty, moved, or unreachable. Skipping it in Compare sizes."
            $null = Receive-DestInventoryJob -Config $Config -Row $row -Job $dj
            continue
        }
        $sc = $items.Count
        ...
        $d = Lz1203286ccb -Config $Config -Row $row -Items $items -Inv $inv -EffectivePrefix $effD -JobAuditIndex $jobAuditIndex
        $nMiss = [int]$d.Missing; $nExt = [int]$d.Extra; $nOther = [int]$d.OtherSource
        $dstCountOwn = [Math]::Max(0, [int]$inv.Count - $nOther)
        $dstBytesOwn = $inv.Bytes
        $tSrcC += $sc; $tSrcB += $sb; $tDstC += $dstCountOwn; $tDstB += $dstBytesOwn
        $tMissing += $nMiss; $tExtra += $nExt
        [void]$scRows.Add([pscustomobject]@{ Space = $row.Space; SrcCount = $sc; SrcBytes = $sb; DstCount = $dstCountOwn; DstBytes = $dstBytesOwn; Missing = $nMiss; Extra = $nExt })
        ...
        $bytesNote = if ($nOther -gt 0) { " (bytes still include $('{0:N0}' -f $nOther) file(s)' worth from another source sharing this folder - count above is corrected, bytes are not)" } else { '' }
        Write-MigrationLog $lvl ("  [$($row.Space)]  source $sc files / $(Format-Bytes $sb)   ->   destination $dstCountOwn files / $(Format-Bytes $dstBytesOwn)   [$tag]$bytesNote")
    }
    ...
    try { Lzb6cd3afde6 -Config $Config -Rows $scRows -SrcCount $tSrcC -SrcBytes $tSrcB -DstCount $tDstC -DstBytes $tDstB -Missing $tMissing -Extra $tExtra } catch { Write-MigrationLog WARN "Compare report could not be written: $($_.Exception.Message)" }
    if ($script:GuiMode) {
...
function Lz3f212c0686 {
    ...
    $srcTotal  = Get-SafeSum -Items (@($Report | ForEach-Object { [int64]$_.SourceFiles }))
...
$staleSec
"@
    $html = Lz276740bda0 -Config $Config -Subtitle 'Verify files arrived (per-file check)' -Banner $banner -Body $body
...
"@
    $html = Lz276740bda0 -Config $Config -Subtitle 'Compare sizes (count and byte overview)' -Banner $banner -Body $body
...
$script:BrandLogoWarned = $false
function Get-BrandLogoTag {
    param($Config, [string]$Class = 'logo')
    try {
        if (-not (Lz9a9b8e46d2 $Config.run 'report')) { return '' }
        if (-not (Lz9a9b8e46d2 $Config.run.report 'logo')) { return '' }
        $p = "$($Config.run.report.logo)".Trim()
        if (-not $p) { return '' }
        if (-not (Test-Path -LiteralPath $p)) {
            if (-not $script:BrandLogoWarned) {
                $script:BrandLogoWarned = $true
                Write-MigrationLog WARN "White-label logo not found at [$p], so this report carries no client logo. Set a valid path under Settings > Reports, or clear the setting."
            }
            return ''
        }
        $lb = [System.IO.File]::ReadAllBytes($p)
        $mime = Get-ImageMimeType -Bytes $lb
        if (-not $mime) {
            if (-not $script:BrandLogoWarned) {
                $script:BrandLogoWarned = $true
                Write-MigrationLog WARN "White-label logo [$p] is not an image file (its first bytes match no known image format), so it was NOT embedded. Embedding it would put a broken-image icon on the report."
            }
            return ''
        }
        $altName = ''
        if ((Lz9a9b8e46d2 $Config.run.report 'client')) { $altName = "$($Config.run.report.client)".Trim() }
        if (-not $altName -and (Lz9a9b8e46d2 $Config.run.report 'brand')) { $altName = "$($Config.run.report.brand)".Trim() }
        $alt = if ($altName) { "$altName logo" } else { 'logo' }
        return "<img class='$Class' alt='$([System.Net.WebUtility]::HtmlEncode($alt))' src='data:$mime;base64,$([Convert]::ToBase64String($lb))' />"
    } catch {
        if (-not $script:BrandLogoWarned) {
            $script:BrandLogoWarned = $true
            try { Write-MigrationLog WARN "White-label logo could not be read: $($_.Exception.Message). The report carries no client logo." } catch {}
        }
        return ''
    }
}
...
function Invoke-HtmlReport {
    ...
    if (-not $NoStageLog) { Lz1cc9bde2a4 -Config $Config -Stage 'api-report' }
    if (-not $AuditPath) {
        $latest = Get-ChildItem (Join-Path $Config.run.reportRoot 'audit-*.csv') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if ($latest) { $AuditPath = $latest.FullName }
    }
    ...
    if (-not $rows.Count) { Write-MigrationLog WARN "Audit CSV is empty: $AuditPath"; Write-NothingProducedOutcome "no report was written: the last run's audit trail is empty, so there is nothing to report on."; return }
    function _enc { param($s) [System.Net.WebUtility]::HtmlEncode("$s") }
    function Lzdc113475f8 { param($st) @($rows | Where-Object { $_.Status -eq $st }).Count }
    ...
    $brand = 'Liscaragh Migrate'
    $logoTag = ''
    if ((Lz9a9b8e46d2 $Config.run 'report')) {
        if (((Lz9a9b8e46d2 $Config.run.report 'brand')) -and $Config.run.report.brand) { $brand = "$($Config.run.report.brand)" }
        $logoTag = Get-BrandLogoTag -Config $Config
    }
    $total = $rows.Count
    $copied = (Lzdc113475f8 'Copied') + (Lzdc113475f8 'ZeroByte')
    $skipped = Lzdc113475f8 'Skipped'
    $notAttempted = Lzdc113475f8 'NotAttempted'
    $errors = (Lzdc113475f8 'Error') + (Lzdc113475f8 'DownloadError') + (Lzdc113475f8 'SkippedTooLarge')
    $verifyFail = Lzdc113475f8 'VerifyFail'
    $wouldCopy = Lzdc113475f8 'WouldCopy'
    ...
    $diagSection = ''
    if (-not $dryRun) {
        $diag = $null; try { $diag = Lzbbdf04187d -Rows $rows -Elapsed $elapsed -TrialCapped:($notAttempted -gt 0) } catch {}
        if ($diag) {
            $diagSection = @"
...
"@
    ...
    foreach ($g in ($rows | Group-Object Space | Sort-Object Name)) {
        ...
        $dest = ''
        if ($destBySpace.ContainsKey($g.Name)) { $d = $destBySpace[$g.Name]; $dest = if ($d.DestinationType -eq 'OneDrive') { ((@("OneDrive: $($d.TargetPrincipal)", "$($d.TargetSubFolder)") | Where-Object { "$_".Trim() }) -join ' / ') } else { (@("$($d.DestinationUrl)", "$($d.TargetLibrary)", "$($d.TargetSubFolder)") | Where-Object { "$_".Trim() }) -join ' / ' } }
        ...
        $destActual = ''
        $dps = @($g.Group | ForEach-Object { ("$($_.DestPath)" -replace '\\','/').Trim('/') } | Where-Object { $_ })
        if ($dps.Count) {
            $segsD = @($dps[0] -split '/')
            $candD = @(if ($segsD.Count -gt 1) { $segsD[0..($segsD.Count - 2)] } else { @() })
            while ($candD.Count -gt 0) {
                $tryD = ($candD -join '/') + '/'
                if (@($dps | Where-Object { -not $_.StartsWith($tryD, [System.StringComparison]::OrdinalIgnoreCase) }).Count -eq 0) { $destActual = ($candD -join '/'); break }
                if ($candD.Count -ge 2) { $candD = @($candD[0..($candD.Count - 2)]) } else { $candD = @() }
            }
        }
        $destNote = ''
        if ($destActual -and $dest.IndexOf($destActual, [System.StringComparison]::OrdinalIgnoreCase) -lt 0) {
            $destNote = "<div class='dim' style='font-size:11px'>$(_enc $destActual)</div>"
        }
        $spaceRows += "<tr><td>$(_enc $g.Name)$srcPathNote</td><td class='dim'>$(_enc $dest)$destNote</td><td class='n'>$sc</td><td class='n'>$ss</td>$seCell<td class='n'>$(Format-Bytes $sb)</td></tr>`n"
    }
    $detailSections = ''
    foreach ($g in ($rows | Group-Object Space | Sort-Object Name)) {
    ...
    $dlSecTotal = [math]::Round((Get-SafeSum -Items (@($rows | ForEach-Object { [double]$_.DownloadMs }))) / 1000, 0)
    $upSecTotal = [math]::Round((Get-SafeSum -Items (@($rows | ForEach-Object { [double]$_.UploadMs }))) / 1000, 0)
    $extRows = ''
    ...
    $hasFlag = ($rows.Count -and ((Lz9a9b8e46d2 $rows[0] 'Renamed')))
    ...
    if ($renamed.Count) {
        $rr=''; foreach ($x in ($renamed | Select-Object -First 300)) { $of = if (((Lz9a9b8e46d2 $x 'RenamedFrom')) -and $x.RenamedFrom) { $x.RenamedFrom } else { $x.SourcePath }; $rr += "<tr><td>$(_enc $of)</td><td>$(_enc $x.DestPath)</td></tr>`n" }
    ...
    if (((Lz9a9b8e46d2 $Config.run 'report')) -and ((Lz9a9b8e46d2 $Config.run.report 'helpUrl')) -and $Config.run.report.helpUrl) { $helpUrl = "$($Config.run.report.helpUrl)" }
    ...
    if (Test-Path $sidecar) {
        try {
            ...
            try {
                $rv = $oc.Resumed
                $ocResumed = if ($null -ne $rv -and (Lz9a9b8e46d2 $rv 'IsPresent')) { [bool]$rv.IsPresent } else { [bool]$rv }
            } catch {}
            try { if ((Lz9a9b8e46d2 $oc 'SpoolEncrypted')) { $ocSpoolEnc = [bool]$oc.SpoolEncrypted } } catch {}
        } catch {}
    }
    ...
    $bannerStyle = switch ($ocStatus) {
        ...
        'CAPPED'                { 'background:#FFF6EC;border:1px solid #F5C083;color:#B5460F' }
    ...
    $bannerTitle = switch ($ocStatus) {
        ...
        'CAPPED'                { 'Stopped: evaluation limit reached - a licence is needed to finish' }
    ...
    $bannerExtra = ''
    if ($ocStatus -eq 'CAPPED') {
        $bannerExtra += " $copied file(s) copied for real"
        if ($skipped -gt 0) { $bannerExtra += ", $skipped already present and left alone" }
        $bannerExtra += ", and $notAttempted file(s) NOT attempted because the evaluation limit was reached (a licence is required to migrate the rest): $total file(s) accounted for."
    }
    elseif ($skipped -gt 0)  { $bannerExtra += " $copied file(s) copied, $skipped already present and left alone: $total file(s) accounted for." }
    ...
    function Lz671be36350 { param([string]$k, [string]$v) if (-not "$v".Trim()) { $v = '&mdash;' }; return "<tr><td>$k</td><td>$v</td></tr>`n" }
    $rdRows = ''
    ...
    $rdRows += Lz671be36350 'Run type' (_enc $runTypeLabel)
    $rdRows += Lz671be36350 'Source' (_enc $script:SrcSysName)
    $rdRows += Lz671be36350 "$($script:SrcItemCap)s in this run" ("$((@($rows | Group-Object Space)).Count)")
    try { if ($times.Count -ge 2) {
        $rdRows += Lz671be36350 'Started'  (_enc (($times | Measure-Object -Minimum).Minimum.ToLocalTime().ToString('dd MMM yyyy HH:mm:ss')))
        $rdRows += Lz671be36350 'Finished' (_enc (($times | Measure-Object -Maximum).Maximum.ToLocalTime().ToString('dd MMM yyyy HH:mm:ss')))
        $rdRows += Lz671be36350 'Copying window' (_enc $elapsedStr)
    } } catch {}
    try { if ("$env:USERNAME".Trim()) { $rdRows += Lz671be36350 'Run by' (_enc ("$env:USERNAME" + $(if ("$env:COMPUTERNAME".Trim()) { " on $env:COMPUTERNAME" }))) } } catch {}
    $rdRows += Lz671be36350 'App version' (_enc "$tv")
    try {
        $w = 4; if (((Lz9a9b8e46d2 $Config.run 'upload')) -and ((Lz9a9b8e46d2 $Config.run.upload 'workers'))) { $w = [int]$Config.run.upload.workers }
        $rdRows += Lz671be36350 'Parallel uploads' ("$w")
    } catch {}
    try {
        $bwU = 0; $bwD = 0
        if ((Lz9a9b8e46d2 $Config.run 'bandwidth')) {
            if ((Lz9a9b8e46d2 $Config.run.bandwidth 'maxUploadMbps'))   { $bwU = [int]$Config.run.bandwidth.maxUploadMbps }
            if ((Lz9a9b8e46d2 $Config.run.bandwidth 'maxDownloadMbps')) { $bwD = [int]$Config.run.bandwidth.maxDownloadMbps }
        }
        $rdRows += Lz671be36350 'Speed limits' $(if ($bwU -or $bwD) { "upload $(if($bwU){"$bwU Mb/s"}else{'unlimited'}), download $(if($bwD){"$bwD Mb/s"}else{'unlimited'})" } else { 'none (unlimited)' })
    } catch {}
    try {
        $fBits = @()
        if (((Lz9a9b8e46d2 $Config.run 'tuning'))) {
            $tn = $Config.run.tuning
            if (((Lz9a9b8e46d2 $tn 'maxFileSizeMB')) -and [int]$tn.maxFileSizeMB -gt 0) { $fBits += "files over $([int]$tn.maxFileSizeMB) MB skipped" }
            if (((Lz9a9b8e46d2 $tn 'excludePatterns')) -and @($tn.excludePatterns).Count) { $fBits += "excluded: $((@($tn.excludePatterns) -join ', '))" }
            if (((Lz9a9b8e46d2 $tn 'modifiedAfter'))  -and "$($tn.modifiedAfter)".Trim())  { $fBits += "modified after $($tn.modifiedAfter)" }
            if (((Lz9a9b8e46d2 $tn 'modifiedBefore')) -and "$($tn.modifiedBefore)".Trim()) { $fBits += "modified before $($tn.modifiedBefore)" }
        }
        $rdRows += Lz671be36350 'Filters' $(if ($fBits.Count) { _enc ($fBits -join '; ') } else { 'none (everything included)' })
    } catch {}
    try { $rdRows += Lz671be36350 'Name tidying' $(if (((Lz9a9b8e46d2 $Config.run 'sanitiseNames')) -and $Config.run.sanitiseNames) { 'on (M365-illegal names corrected and recorded)' } else { 'off' }) } catch {}
    $rdRows += Lz671be36350 'Audit record' (_enc ([System.IO.Path]::GetFileName($AuditPath)))
...
$(if ($notAttempted -gt 0) { "<div class='card'><div class='v'>$notAttempted</div><div class='l'>Not attempted (evaluation limit)</div></div>" })
...
$(if ($dryRun) {
 <#B1: 'Would copy' above used to look like a forecast for whichever real run came next, when in
   fact it always counts every file as if none of it existed at the destination yet - a
   Full-migration-shaped count, not a Sync one. This preview does not read the destination
   (deliberately, to stay fast and source-only), so it cannot tell what Sync would actually skip.#>
 "<p class='dim' style='margin-top:-6px'>This is a rehearsal only. The 'Would copy' figure above counts every file as if starting fresh; it is not a forecast for 'Sync new and changed', which only copies what is new or changed at the destination and will very likely touch far fewer files than this number.</p>"
})
$runDetailsSection
$diagSection
$readySection
...
function Write-NothingProducedOutcome {
    param([string]$Text)
    if ($script:GuiMode) { Write-Host ("##CHECKOUTCOME##|WARN|" + $Text) }
}
function Invoke-CompletionCertificate {
    param($Config, [string]$AuditPath, [switch]$NoStageLog)
    if (-not $NoStageLog) { Lz1cc9bde2a4 -Config $Config -Stage 'api-certificate' }
    if (-not $AuditPath) {
        $latest = Get-ChildItem (Join-Path $Config.run.reportRoot 'audit-*.csv') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if ($latest) { $AuditPath = $latest.FullName }
    }
    if (-not $AuditPath -or -not (Test-Path $AuditPath)) { Write-MigrationLog ERROR "No audit CSV found in $($Config.run.reportRoot). A certificate is built from a completed run's audit trail; run a migration to completion first."; Write-NothingProducedOutcome "no certificate was issued: there is no run on record to certify. Run a migration through to completion first."; return }
    $rows = @(Import-Csv $AuditPath)
    $endRow = @($rows | Where-Object { "$($_.Status)" -like 'RUN_END:*' }) | Select-Object -Last 1
    $rows   = @($rows | Where-Object { "$($_.Status)" -notlike 'RUN_END:*' })
    if (-not $rows.Count) { Write-MigrationLog WARN "Audit CSV is empty: $AuditPath"; Write-NothingProducedOutcome "no certificate was issued: the last run's audit trail is empty, so there is nothing to certify."; return }
    function _enc { param($s) [System.Net.WebUtility]::HtmlEncode("$s") }
    function Lzdc113475f8 { param($st) @($rows | Where-Object { $_.Status -eq $st }).Count }
    function _sumBytes { param($items) Get-SafeSum -Items $items -Property SourceSizeBytes }
    $copied  = (Lzdc113475f8 'Copied') + (Lzdc113475f8 'ZeroByte')
    ...
    if (-not $ocStatus) { $ocStatus = if ($dryRun) { 'PREVIEW' } else { 'INCOMPLETE' } }
    if ($dryRun -or @('PREVIEW','INCOMPLETE','ENDED_EARLY','CANCELLED','CAPPED') -contains $ocStatus) {
        ...
        Write-NothingProducedOutcome ("no certificate was issued: the last run is '" + $ocStatus + "', and a certificate is only issued for a completed migration. Run a full sync through to completion, then a verify, and issue it from that run.")
        return
    }
    ...
    if ((Lz9a9b8e46d2 $Config.run 'report')) {
        if (((Lz9a9b8e46d2 $Config.run.report 'brand')) -and $Config.run.report.brand) { $brand = "$($Config.run.report.brand)" }
        if (((Lz9a9b8e46d2 $Config.run.report 'client')) -and $Config.run.report.client) { $client = "$($Config.run.report.client)" }
        $logoTag = Get-BrandLogoTag -Config $Config
    }
    $caveat = ''
    if ($withErrors) {
...
"@
    ...
    if ($script:GuiMode) { Write-Host ("##CHECKOUTCOME##|OK|completion certificate issued: " + [System.IO.Path]::GetFileName($outPath)) }
    return $outPath
}
function Invoke-DriftCheck {
    param($Config)
    Lz1cc9bde2a4 -Config $Config -Stage 'api-drift'
    Connect-Destination -Config $Config
    $map = @(Import-Csv (Join-Path $Config.run.reportRoot 'mapping.csv') | Where-Object Action -eq 'MIGRATE')
    $base = @{}
    foreach ($af in @(Get-ChildItem (Join-Path $Config.run.reportRoot 'audit-*.csv') -ErrorAction SilentlyContinue)) {
    ...
    $jobAuditIndex = @{}
    foreach ($bv in $base.Values) { $jobAuditIndex["$($bv.DestPath)".ToLowerInvariant()] = "$($bv.Space)" }
    $rows = New-Object System.Collections.Generic.List[object]
    $chg = 0; $del = 0; $add = 0; $addOther = 0
    $tol = [TimeSpan]::FromSeconds(5)
    $t = $map.Count; $i = 0
    foreach ($row in $map) {
        $i++; Lzccc361cfae -Activity 'Drift check' -Status $row.Space -Current $i -Total $t | Out-Null
        ...
        foreach ($p in $inv.Paths) {
            if (-not $seen.Contains($p)) {
                $absP = (Join-SubPath $effD $p) -replace '\\','/'
                $owner = $jobAuditIndex["$absP".ToLowerInvariant()]
                if ($owner -and $owner -ne "$($row.Space)") { $addOther++; continue }
    ...
    $rows | Lz551c5b804a | Export-Csv $csvPath -NoTypeInformation -Encoding UTF8
    ...
    if ($addOther -gt 0) { Write-MigrationLog INFO "$addOther file(s) at the destination belong to another source mapped in this same job (a shared destination folder) - not counted as Added." }
    ...
    Lz46a0c7d32e -Config $Config -Rows $rows -Changed $chg -Deleted $del -Added $add -BaselineCount $base.Count
}
function Lz46a0c7d32e {
...
"@
    $html = Lz276740bda0 -Config $Config -Subtitle 'Destination drift (changes since migration)' -Banner $banner -Body $body
...
function Invoke-Validate {
    param($Config)
    Lz1cc9bde2a4 -Config $Config -Stage 'api-validation'
    $ckStart = Get-Date
    Lz81c8264516 -Config $Config
    Connect-Destination -Config $Config
    $map = @(Import-Csv (Join-Path $Config.run.reportRoot 'mapping.csv') | Where-Object Action -eq 'MIGRATE')
    ...
    $t = $map.Count; $i = 0
    foreach ($row in $map) {
        $i++; Lzccc361cfae -Activity 'Verify' -Status $row.Space -Current $i -Total $t | Out-Null
        ...
        try {
            ...
            Write-MigrationLog ERROR "  could not read files for '$($row.Space)': $($_.Exception.Message). In plain terms: the source may be empty, moved, or unreachable. Skipping it in Verify files arrived."
            $null = Receive-DestInventoryJob -Config $Config -Row $row -Job $dj -WithTimes
            continue
        }
    ...
    $report | Lz551c5b804a | Export-Csv -Path (Join-Path $Config.run.reportRoot 'api-validation-report.csv') -NoTypeInformation -Encoding UTF8
    try { Lz3f212c0686 -Config $Config -Report $report -Missing $missingAll -Stale $staleAll } catch { Write-MigrationLog WARN "Verify report could not be written: $($_.Exception.Message)" }
    if ($missingAll.Count) {
        $missingAll | Lz551c5b804a | Export-Csv -Path (Join-Path $Config.run.reportRoot 'api-validation-missing.csv') -NoTypeInformation -Encoding UTF8
        if ($script:GuiMode) { Write-Host "##CHECKFILES##|$($missingAll.Count)|0|0|$(Join-Path $Config.run.reportRoot 'api-validation-missing.csv')" }
    }
    if ($staleAll.Count) {
        $staleAll | Lz551c5b804a | Export-Csv -Path (Join-Path $Config.run.reportRoot 'api-validation-stale.csv') -NoTypeInformation -Encoding UTF8
...
function Lz0dd8767341 {
    param($Config)
    Lz1cc9bde2a4 -Config $Config -Stage 'api-permissions'
    Lz81c8264516 -Config $Config
    $map = @(Import-Csv (Join-Path $Config.run.reportRoot 'mapping.csv') | Where-Object Action -eq 'MIGRATE')
    function Lzbe75442909 { param([string]$r) switch -Regex ($r.ToLower()) { 'owner|manage|admin'{'Site Owner (Full Control)';break} 'edit|write|contrib'{'Member (Edit)';break} 'view|read'{'Visitor (Read)';break} default{'REVIEW'} } }
    $plan = New-Object System.Collections.Generic.List[object]
    $t = $map.Count; $i = 0
    foreach ($row in $map) {
        $i++; Lzccc361cfae -Activity 'Permissions' -Status $row.Space -Current $i -Total $t | Out-Null
        $space = New-SpaceRef -Row $row -Config $Config
        foreach ($p in @(Lza7c4f0dbe3 -Config $Config -Space $space)) {
            $plan.Add([pscustomobject]@{ Space=$row.Space; Principal=$p.Principal; DattoRole=$p.Role; DestinationUrl=$row.DestinationUrl; ProposedM365Role=(Lzbe75442909 $p.Role) })
        }
    }
    try { Write-Progress -Activity 'Permissions' -Completed } catch {}
    $out = Join-Path $Config.run.reportRoot 'api-permissions-plan.csv'
    $plan | Lz551c5b804a | Export-Csv -Path $out -NoTypeInformation -Encoding UTF8
...
function Invoke-SourceReview {
    ...
    if (-not $NoStageLog) { Lz1cc9bde2a4 -Config $Config -Stage 'api-sourcereview' }
    $started = Get-Date
    $isSim = ($Config.datto.provider -eq 'LocalSim')
    if (-not $isSim) { Lz81c8264516 -Config $Config }
    function _raw { param($o, [string]$n) if ($null -ne $o -and ((Lz9a9b8e46d2 $o $n))) { return $o.$n }; return $null }
    function Lz3f2c94eb10 { param($ms) try { $v = [int64]$ms; if ($v -gt 0) { $x = if ($v -gt 100000000000) { $v } else { $v * 1000 }; return [DateTimeOffset]::FromUnixTimeMilliseconds($x).UtcDateTime.ToString('dd MMM yyyy HH:mm') } } catch {}; return '' }
    ...
    function Lzb02bcb7979 { param([string]$Space, [string]$Path, [string]$Kind, $Pub, $Link, $By, [int64]$Size, [string]$ModUtc, $Locked, [string]$LockName, [string]$LockEmail, [string]$LockSince, $NotReady, [string]$ReadyReason)
    ...
    if ($isSim) {
        foreach ($sp in @(Get-DattoSpaces -Config $Config)) {
            $stats.Projects++
            Lzb02bcb7979 -Space $sp.Name -Path '' -Kind 'Project' -Pub $false -Link '' -By '' -Size 0 -ModUtc '' -Locked $false -LockName '' -LockEmail '' -LockSince '' -NotReady $false -ReadyReason ''
            foreach ($e in @(Get-ChildItem -LiteralPath $sp.Id -Recurse -Force -ErrorAction SilentlyContinue)) {
                ...
                if ($e.PSIsContainer) {
                    $stats.Folders++
                    Lzb02bcb7979 -Space $sp.Name -Path $rel -Kind 'Folder' -Pub $pub -Link '' -By '' -Size 0 -ModUtc '' -Locked $false -LockName '' -LockEmail '' -LockSince '' -NotReady $false -ReadyReason ''
                } else {
                    $stats.Files++
                    if ($lck) { $stats.Locked++ }
                    if ($off) { $stats.NotReady++ }
                    Lzb02bcb7979 -Space $sp.Name -Path $rel -Kind 'File' -Pub $pub -Link '' -By 'Sim User' -Size ([int64]$e.Length) -ModUtc ($e.LastWriteTimeUtc.ToString('o')) `
        ...
        $f = $Config.datto.fields
        $q = @{ metadata = 'sjGOS' }
        $unwrap = { param($resp) if ($resp -is [System.Array]) { $resp } elseif ($f.collection -and ((Lz9a9b8e46d2 $resp $f.collection))) { $resp.($f.collection) } else { $resp } }
        $mapPath = Join-Path $Config.run.reportRoot 'mapping.csv'
        $mapRows = @()
        ...
        function Lz4820c480a9 { param($it, [string]$SpaceName, [string]$Rel, [bool]$IsFolder)
            ...
            $ln = ''; $le = ''; $ls = ''
            if ($locked) { $ln = "$(_raw $lockObj 'fullname')"; $le = "$(_raw $lockObj 'email')"; $ls = Lz3f2c94eb10 (_raw $lockObj 'lockedSince') }
            ...
            Lzb02bcb7979 -Space $SpaceName -Path $Rel -Kind $(if ($IsFolder) { 'Folder' } else { 'File' }) -Pub $pub -Link $link -By $by -Size $size -ModUtc $mod `
                -Locked $locked -LockName $ln -LockEmail $le -LockSince $ls -NotReady $notReady -ReadyReason $reason
        }
        function Lz7fd2c27a31 { param($ParentId, [string]$SpaceName, [string]$Prefix)
            $childPath = ($Config.datto.apiPaths.listChildren -replace '\{parentID\}', [uri]::EscapeDataString("$ParentId"))
            $resp = Invoke-DattoApi -Config $Config -Path $childPath -Query $q
            $stats.Calls++
            $items = @(& $unwrap $resp)
            ...
            foreach ($it in $items) {
                ...
                if ($isFolder) {
                    $stats.Folders++
                    Lz4820c480a9 -it $it -SpaceName $SpaceName -Rel $rel -IsFolder $true
                    Lz7fd2c27a31 -ParentId $it.($f.itemId) -SpaceName $SpaceName -Prefix $rel
                } else {
                    $stats.Files++
                    Lz4820c480a9 -it $it -SpaceName $SpaceName -Rel $rel -IsFolder $false
                }
        ...
        foreach ($row in $mapRows) {
            ...
            Lzccc361cfae -Activity 'Source review' -Status $sp.Name -Current $si -Total $mapRows.Count | Out-Null
            ...
            Lz7fd2c27a31 -ParentId $startId -SpaceName $sp.Name -Prefix $seedPrefix
        }
    ...
    if ($csvRows.Count) { $csvRows | Lz551c5b804a | Export-Csv -Path $csvPath -NoTypeInformation -Encoding UTF8 }
    ...
    function Lz27987d4f45 { param($v, $l, [bool]$hot) "<div class='tile$(if ($hot) { ' hot' })'><div class='v'>$v</div><div class='l'>$l</div></div>" }
    $tiles = (Lz27987d4f45 $stats.Projects 'Folders in job' $false) + (Lz27987d4f45 ('{0:N0}' -f $stats.Folders) 'Subfolders scanned' $false) + (Lz27987d4f45 ('{0:N0}' -f $stats.Files) 'Files scanned' $false) + (Lz27987d4f45 $nPub 'Public links' ($nPub -gt 0)) + (Lz27987d4f45 $nLock 'Locked files' ($nLock -gt 0)) + (Lz27987d4f45 $nNR 'Not cloud-ready' ($nNR -gt 0)) + (Lz27987d4f45 $dur 'Scan time' $false)
    $plRows = ''
...
function Invoke-ProvenancePack {
    ...
    if (-not $NoStageLog) { Lz1cc9bde2a4 -Config $Config -Stage 'api-provenance' }
    if (-not $AuditPath) {
    ...
    if ((Lz9a9b8e46d2 $Config.run 'provenance')) {
        if ((Lz9a9b8e46d2 $Config.run.provenance 'enabled')) { $tsaEnabled = [bool]$Config.run.provenance.enabled }
        if (((Lz9a9b8e46d2 $Config.run.provenance 'tsaUrl')) -and $Config.run.provenance.tsaUrl) { $tsaUrl = "$($Config.run.provenance.tsaUrl)" }
    }
...
function Test-Api {
    param($Config)
    Lz1cc9bde2a4 -Config $Config -Stage 'api-test'
    Lz81c8264516 -Config $Config
    Connect-Destination -Config $Config
...
function Lz8b91a37666 {
    ...
    while ($true) {
        ...
        switch ((Read-Host 'Select').ToUpper()) {
            '1' { Test-Api -Config $Config }
            '2' { Lzc1e7fe7adf -Config $Config }
...
if (-not $PSBoundParameters.ContainsKey('SpoolAhead') -and
    ((Lz9a9b8e46d2 $cfg.run 'parallel')) -and
    ((Lz9a9b8e46d2 $cfg.run.parallel 'spoolAhead'))) {
    $SpoolAhead = [int]$cfg.run.parallel.spoolAhead
}
if (-not $PSBoundParameters.ContainsKey('MaxParallelSpaces') -and
    ((Lz9a9b8e46d2 $cfg.run 'parallel')) -and
    ((Lz9a9b8e46d2 $cfg.run.parallel 'maxParallelSpaces'))) {
    $MaxParallelSpaces = [int]$cfg.run.parallel.maxParallelSpaces
}
$parallelEnabled = $false
try { if (((Lz9a9b8e46d2 $cfg.run 'parallel')) -and ((Lz9a9b8e46d2 $cfg.run.parallel 'enabled')) -and $cfg.run.parallel.enabled) { $parallelEnabled = $true } } catch {}
...
if ((Lz9a9b8e46d2 $cfg.run 'throttle')) {
    if (((Lz9a9b8e46d2 $cfg.run.throttle 'timeoutSec')) -and $cfg.run.throttle.timeoutSec) {
        $script:HttpTimeoutSec = [int]$cfg.run.throttle.timeoutSec
    }
    if (((Lz9a9b8e46d2 $cfg.run.throttle 'transferTimeoutSec')) -and $cfg.run.throttle.transferTimeoutSec) {
        $script:TransferTimeoutSec = [int]$cfg.run.throttle.transferTimeoutSec
    }
}
$UploadWorkers = 4
$workersExplicit = $false
if ((Lz9a9b8e46d2 $cfg.run 'upload')) {
    if (((Lz9a9b8e46d2 $cfg.run.upload 'directPutMaxMB')) -and $cfg.run.upload.directPutMaxMB) {
        $script:SmallFilePutThreshold = [int64]$cfg.run.upload.directPutMaxMB * 1MB
    }
    if (((Lz9a9b8e46d2 $cfg.run.upload 'workers')) -and $cfg.run.upload.workers) {
        $UploadWorkers = [int]$cfg.run.upload.workers
        $workersExplicit = $true
    }
    if ((Lz9a9b8e46d2 $cfg.run.upload 'preserveDates')) { $script:PreserveDates = [bool]$cfg.run.upload.preserveDates }
    if (((Lz9a9b8e46d2 $cfg.run 'metadata')) -and $cfg.run.metadata -and
        ((Lz9a9b8e46d2 $cfg.run.metadata 'provenanceColumns'))) {
        $script:ProvColsOn = [bool]$cfg.run.metadata.provenanceColumns
    }
    if (((Lz9a9b8e46d2 $cfg.run 'metadata')) -and $cfg.run.metadata -and
        ((Lz9a9b8e46d2 $cfg.run.metadata 'folderColumns'))) {
        $script:FolderColRules = ConvertTo-FolderColRules "$($cfg.run.metadata.folderColumns)"
    }
    if (((Lz9a9b8e46d2 $cfg.run 'metadata')) -and $cfg.run.metadata -and
        ((Lz9a9b8e46d2 $cfg.run.metadata 'filenameColumns'))) {
        $script:FilenameColRules = ConvertTo-FilenameColRules "$($cfg.run.metadata.filenameColumns)"
    }
    if (((Lz9a9b8e46d2 $cfg.run 'metadata')) -and $cfg.run.metadata) {
        if ((Lz9a9b8e46d2 $cfg.run.metadata 'extensionCategory')) {
            $script:ExtCatRules = ConvertTo-ExtensionCategoryRules "$($cfg.run.metadata.extensionCategory)"
        }
        if ((Lz9a9b8e46d2 $cfg.run.metadata 'retentionYears')) {
...
if ((Lz9a9b8e46d2 $cfg.run 'encryptSpool')) { $script:SpoolEncrypt = [bool]$cfg.run.encryptSpool }
if ((Lz9a9b8e46d2 $cfg.run 'trialFileLimit') -and $cfg.run.trialFileLimit) { $script:TrialFileLimit = [Math]::Min([Math]::Max(1, [int]$cfg.run.trialFileLimit), $script:TrialFileLimit) }
...
if ((Lz9a9b8e46d2 $cfg.run 'tuning')) {
    $tn = $cfg.run.tuning
    if (((Lz9a9b8e46d2 $tn 'chunkSizeMB'))  -and $tn.chunkSizeMB)  { $script:ChunkSize    = [int64]$tn.chunkSizeMB * 1MB }
    if (((Lz9a9b8e46d2 $tn 'excludePatterns')))                    { $script:ExcludePatterns = @($tn.excludePatterns | ForEach-Object { "$_" -split '[;,\r\n]' } | ForEach-Object { $_.Trim() } | Where-Object { $_ }) }
    if (((Lz9a9b8e46d2 $tn 'includePatterns')))                    { $script:IncludePatterns = @($tn.includePatterns | ForEach-Object { "$_" -split '[;,\r\n]' } | ForEach-Object { $_.Trim() } | Where-Object { $_ }) }
    if (((Lz9a9b8e46d2 $tn 'maxFileSizeMB')) -and $tn.maxFileSizeMB){ $script:MaxFileBytes  = [int64]$tn.maxFileSizeMB * 1MB }
    if (((Lz9a9b8e46d2 $tn 'largeFileMB'))   -and $tn.largeFileMB)  { $script:LargeFileBytes = [int64]$tn.largeFileMB * 1MB }
    if (((Lz9a9b8e46d2 $tn 'modifiedAfter')))  { $script:ModifiedAfterUtc  = ConvertTo-ConfigUtcBound $tn.modifiedAfter }
    if (((Lz9a9b8e46d2 $tn 'modifiedBefore'))) { $script:ModifiedBeforeUtc = ConvertTo-ConfigUtcBound $tn.modifiedBefore }
    ...
    if (((Lz9a9b8e46d2 $tn 'assessmentTopFiles')) -and $tn.assessmentTopFiles) { $script:AssessmentTopFiles = [Math]::Max(1, [int]$tn.assessmentTopFiles) }
}
try {
    ...
    switch ($Action) {
        'TestApi'     { Test-Api        -Config $cfg }
        'Discover'    { Lzc1e7fe7adf -Config $cfg -NonInteractive:$NonInteractive }
        ...
        'Drift'       { Invoke-DriftCheck -Config $cfg }
        'SizeCheck'   { Invoke-SizeCheck -Config $cfg -OnlySpace $OnlySpace }
        'DestInventory' { Lz7e4db560c9 -Config $cfg -SpaceName $Spaces -ResultDir $ResultDir }
        ...
        default       { Lz8b91a37666       -Config $cfg }
    }
}
catch {
    $e = $_
