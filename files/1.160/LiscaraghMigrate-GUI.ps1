...
function Lzbb25f777e4 {
    param([string]$Severity)
    Lzfb65fffbda -Severity $Severity
    try {
...
function Lzf5c3b16832 { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lz1f898705eb {
...
function Lzf3dcc88e74 {
    ...
    if (-not $s) { $s = Lz571a1a86af 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lz4fb840b232 {
    param([string]$Value)
    if (Lz1f898705eb -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lz198cdefbef {
    $f = @(Lz18ab8a6a63)
    if ($f.Count -eq 1) { return $f[0] }
    return ''
}
function Lz1e47f02622 {
...
function Lz4cad1f4201 {
    param([string[]]$Picked)
    $result = New-Object System.Collections.Generic.List[string]
    foreach ($e in @(Lz18ab8a6a63)) { [void]$result.Add($e) }
    $dropped = New-Object System.Collections.Generic.List[string]
    foreach ($p in @($Picked | ForEach-Object { "$_".Trim().Trim('/').Trim('\') } | Where-Object { $_ })) {
        if ($result -ccontains $p) { continue }
        $covered = $false
        foreach ($k in @($result)) { if (Lz1e47f02622 -Child $p -Parent $k) { [void]$dropped.Add($p); $covered = $true; break } }
        if ($covered) { continue }
        foreach ($narrow in @($result | Where-Object { Lz1e47f02622 -Child $_ -Parent $p -Strict })) { [void]$result.Remove($narrow); [void]$dropped.Add($narrow) }
        [void]$result.Add($p)
    }
    Lz00e22bb3ae -Folders @($result)
    if ($dropped.Count) { Lz522bd82a8b ("Overlapping folder(s) merged, kept the broader one: " + ((@($dropped) | Select-Object -Unique) -join ', ')) }
}
function Lz1515c84db5 {
    $one = $true; try { $one = (@(Lz903fcbd0b7).Count -le 1) } catch {}
    $n = @(Lz18ab8a6a63).Count
...
function Lz13fa107a1c {
    ...
    try {
        ...
        $norm = Lz6ce83a60e8 -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lz522bd82a8b "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz39a4783897
    } catch { }
}
function Lz6ce83a60e8 {
...
function Lz562e229460 {
    ...
    return @{ SubfolderCount = @(Lze7fe63a302 -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lz9f9262b5fe {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lz9aa5fd21e2 -Path "sites/$siteId"
    } catch { return $null }
}
function Lz595a83ce8c {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lz9aa5fd21e2 -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lzf23fda3a43 {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lz9aa5fd21e2 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lze81ddb080d {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lzac8e432a8e -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
function Lz501f34d5db {
    param($State, [string]$Current)
    $out = New-Object System.Collections.Generic.List[string]
    foreach ($s in @($State.Lst.SelectedItems)) {
        $name = "$s"
        if (-not (Lzabd81e293d -Item $name)) { continue }
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lz595a83ce8c -Search $s.Txt.Text)
...
"@
    ...
    Lze81ddb080d
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lze81ddb080d } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lze81ddb080d } })
    $new.Add_Click({
        $n = Lze059ea6816 -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lz9c8548e735 -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lze81ddb080d; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lz5b58868721 -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lzf0f478c143 -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lz5b58868721 -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lz3703882b43 {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lze7fe63a302 -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lz3703882b43
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lz3703882b43 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lz3703882b43 } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lz5b58868721 -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lz501f34d5db -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lz5b58868721 -State $script:SP
    $r = $w.ShowDialog()
...
function Lz522bd82a8b { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz21c015a5cf { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lzc579ce34e6 {
    ...
    if ($hasRegime) {
        $etaA = Lz4548f7b69c -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz4548f7b69c -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lz4548f7b69c -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz4548f7b69c -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lze7f25c9460 {
    ...
    try {
        ...
        $etaText = Lzc579ce34e6 -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lz83a1296516 {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lzc1af652996 -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lze80e8c19a4 -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lze7f25c9460 -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lz498c0fbc48 $Line) + "`r`n")
...
function Lz75db8837f7 { param([string]$Text) Lz83a1296516 $Text }
function Lzb240ecbe36 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lz5575993e78 $true 'Connecting to Microsoft 365...'
        Lz308a9a536b
    } catch {
        ...
        Lz5575993e78 $false; return
    } finally { Lz5575993e78 $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lzfccee7a28c -StartSearch $seed
    if ($picked) {
        ...
        Lz522bd82a8b "Site set to $picked"
        Lz39a4783897
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lzb240ecbe36 }
    Lz39a4783897
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lzf176986ddc }
    Lz39a4783897
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz39a4783897 })
$ctrl.TxtFolder.Add_TextChanged({ Lz39a4783897 })
if ($ctrl.LstSource) { $ctrl.LstSource.Add_SelectionChanged({ Lz1515c84db5 }) }
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz39a4783897 }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz39a4783897 }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lzf7a568e56e -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lz13fa107a1c })
$ctrl.TxtLoc.Add_TextChanged({ Lz39a4783897 })
$ctrl.TxtLib.Add_TextChanged({ Lz39a4783897 })
$script:NestSuspend = $false
function Lz1b13e0a243 {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lzfc3d66abb4 -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lz522bd82a8b $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lz1b13e0a243 })
$ctrl.ChkNest.Add_Unchecked({ Lz1b13e0a243 })
$script:ProjectRows = @()
$script:RunStatus = @{}
...
function Lz11b0657119 {
    ...
    if (-not $hasProjects) {
        $sd = $false
        if (Get-Command Test-ConnectionConfigured -ErrorAction SilentlyContinue) { try { $sd = [bool](Lz35ab417bc7) } catch {} }
        Lz5d22deca64 $sd
    }
...
function Lzd6277c7f85 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lz562e229460 -ProjectId $p.Id -SubPath $sub) }
...
function Lze1ae29f233 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lz9aa5fd21e2 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz7bd6016d3b {
    ...
    if (($EngineArgs -contains 'Transfer') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck') -or ($EngineArgs -contains 'PreFlight')) {
        try { Lz5575993e78 $true 'Checking source folders...'; $srcOk = Lzd6277c7f85 } finally { Lz5575993e78 $false }
        if (-not $srcOk) { return }
    }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lz5575993e78 $true 'Checking destinations...'; $dstOk = Lze1ae29f233 } finally { Lz5575993e78 $false }
        if (-not $dstOk) { return }
    }
    ...
    $script:Timer.Add_Tick({
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lz83a1296516 $lines[$k] } catch {} }
                }
            }
        } catch {}
        ...
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lz83a1296516 $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lz83a1296516 $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Format-Span ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lz33eb9d7959) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lz3d0e9ed3a8 -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lz83a1296516 "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lz83a1296516 "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lz522bd82a8b ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
            ...
            Lzeff8cdd8e7
            if (-not $script:Paused) { Lzbb25f777e4 -Severity $sev }
            if ($script:RunStatus.Count) { try { Lz11b0657119 } catch {} }
            try {
            ...
            try {
                if ($script:CurLabel -eq 'Check readiness' -or $script:CurLabel -eq 'Preview (no upload)') {
                    $newest = Get-ChildItem (Join-Path (Lz33eb9d7959) 'report-*.html') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
function Lzfaaff863ce { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lz0512955e55 { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lz6635466bee {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lz789274813a -ConfigFile $s.Tag; Lz1ccd981c2f } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lzf246970cfb { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lzf5c3b16832 -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lz6635466bee
}
function Lzcacad11e53 {
...
function Lz789274813a {
    param([string]$ConfigFile)
    if ($script:Sched) { Lz335943c829 -Why 'Schedule cancelled (job changed).' }
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lz0512955e55 $true
    Lzfaaff863ce $true
    Lzf246970cfb $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lzcacad11e53 -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lz522bd82a8b 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
}
function Lzeb478a4697 {
    param([string]$Dir)
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lze059ea6816 -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Lz003aa1f038 -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Open it instead, or choose another name.")); return }
    if ((Test-Path $jobDir) -and -not (Lzeb478a4697 -Dir $jobDir)) {
        ...
        Lzfc3d66abb4 -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lz789274813a -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuJobOpen.Add_Click({
    ...
    [void]$w.ShowDialog()
    if ($script:OpenPick) { Lz789274813a -ConfigFile $script:OpenPick; Lz1ccd981c2f }
})
$ctrl.MnuJobSave.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')); return }
    if (Save-Mapping) { Lz522bd82a8b 'Migration job saved.' }
})
$ctrl.MnuJobSaveAs.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open a migration job first, then Save As to copy it.')); return }
    try {
        $name = Lze059ea6816 -Prompt "New name for the copy:" -Title 'Save job as'
        if (-not $name -or -not $name.Trim()) { return }
        $name = $name.Trim()
        $slug = ($name -replace '[^A-Za-z0-9._-]','_').Trim('_'); if (-not $slug) { $slug = 'job' }
        $jobDir = Join-Path $script:JobsRoot $slug
        if (Lz003aa1f038 -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Choose another name.")); return }
        if ((Test-Path $jobDir) -and -not (Lzeb478a4697 -Dir $jobDir)) {
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lza91ccc4b83 })
$ctrl.MnuSettingsApi.Add_Click({ Lz25f2968f86 })
$ctrl.MnuSettingsWizard.Add_Click({ Lz29f33bfa10 })
$ctrl.MnuSettingsEmail.Add_Click({ Lzf81fd497b8 })
$ctrl.MnuSettingsTuning.Add_Click({ Lz860cdd4051 })
$ctrl.BtnFilters.Add_Click({ Lz117e575e5e })
$ctrl.MnuSettingsDecommission.Add_Click({ Lz9f4e6793b3 })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lz603c66341f -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lzdd859232b6 {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lz571a1a86af 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lzdd859232b6 "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lzdd859232b6 (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lz5575993e78 $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lz5575993e78 $false }
    if (-not $b) { return }
...
function Lz4f9f4f2597 { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lze0e37dec33 {
    $path = Lz4f9f4f2597
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Liscaragh Migrate" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lze0e37dec33)) -Caption ('Licence')) | Out-Null })
function Lz94c8b2f114 {
...
function Lze8b69da749 {
    param([string]$For = 'copy')
    if (Test-Path (Lz4f9f4f2597)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        try {
            ...
            Lzf5c3b16832 -Name 'LicenceAppliedId'  -Value "$($lpl.LicenceId)"
            Lzf5c3b16832 -Name 'LicenceAppliedUtc' -Value ((Get-Date).ToUniversalTime().ToString('o'))
        } catch {}
        (Show-Msg -Text ("Licence installed.`n`n" + (Lze0e37dec33)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-FullReinstall {
    ...
    Lz5575993e78 $true "Downloading the installer for version $($Payload.version)..."
    ...
    catch { Lz5575993e78 $false; (Show-Msg -Text ("The installer could not be downloaded, so nothing has been changed.`n`n$($_.Exception.Message)") -Caption ('Reinstall') -Icon ('Warning')) | Out-Null; return }
...
function Invoke-UpdateCheck {
    ...
    try {
        ...
        if (-not $Silent) { Lz5575993e78 $true 'Checking for updates...' }
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lz5575993e78 $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lz603c66341f -Title 'About' -Heading 'Liscaragh Migrate' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lzf3dcc88e74 } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lz8c1949ff1a -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lze059ea6816 -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lz49154dd0e9 {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lz0512955e55 $false
    Lzfaaff863ce $false
}
$ctrl.MnuJobClose.Add_Click({ Lz49154dd0e9; Lz522bd82a8b 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lz49154dd0e9
    if (Lzeb478a4697 -Dir $dir) {
        Lz6635466bee; Lz522bd82a8b "Deleted job '$nm'"
    } else {
        Lz6635466bee
        ...
        Lz522bd82a8b "Could not fully delete job '$nm' (a file is in use)"
    }
})
function Lz612d80c01e {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lz5575993e78 $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lz75db8837f7 'Signing in to Datto and listing projects...'
        $script:Projects = Lze4f574aad0
        Lz75db8837f7 "Found $($script:Projects.Count) project(s)."
        Lz1be2a6c307
        try { Lz308a9a536b; Lz75db8837f7 'Connected to Microsoft 365.' } catch { Lz75db8837f7 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lz75db8837f7 "Technical detail: $($_.Exception.Message)" }
        Lz11b0657119
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lz522bd82a8b 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lz75db8837f7 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lz75db8837f7 "Technical detail: $($_.Exception.Message)"
            Lz522bd82a8b "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lz522bd82a8b 'Connect failed'
        }
    }
    finally { Lz5575993e78 $false }
}
$ctrl.BtnConnect.Add_Click({ Lz612d80c01e })
function Lz1ccd981c2f {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lzf3dcc88e74 } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lz522bd82a8b "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lz612d80c01e -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lz57a76767c3
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lz903fcbd0b7).Count } catch {}
    ...
    if ($script:Map.ContainsKey($p.Id)) {
        ...
        $ctrl.TxtLoc.Text = Lzf176986ddc
        $ctrl.TxtLib.Text = ''; $ctrl.TxtFolder.Text = ''; Lz00e22bb3ae -Folders @()
...
function Lz0b0917a908 {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lze849142fdd) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lze849142fdd) }
}
$ctrl.BtnApply.Add_Click({
    $p = Lz57a76767c3
    if (-not $p) { (Show-Msg -Text ('Select a project first.')); return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Remove($p.Id) | Out-Null; Lz11b0657119; [void](Write-MappingQuiet); return }
    $d = Lz0b0917a908
    if ($d -eq 'ERR') { return }
    ...
    Lz522bd82a8b "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lz29dab6ff0a })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lz0ea8c8e1af -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Lz11b0657119; [void](Write-MappingQuiet); Lz522bd82a8b "Set $($sel.Count) project(s) to skip"; return }
    $d = Lz0b0917a908
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lzb240ecbe36); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPaths=$keepSrcList; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Lz11b0657119
    [void](Write-MappingQuiet)
    Lz522bd82a8b "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lz0ea8c8e1af -Count (@($script:Projects).Count))) { return }
    $d = Lz0b0917a908
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Lz11b0657119; [void](Write-MappingQuiet); Lz522bd82a8b 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Lz11b0657119
    [void](Write-MappingQuiet)
    Lz522bd82a8b "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lz5575993e78 $true 'Looking up libraries...'
        $libs = Lzbc4c9f1fc6 -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lz5575993e78 $false; return }
    finally { Lz5575993e78 $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lze5ced34ff5 -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lz522bd82a8b "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lz522bd82a8b "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lz5575993e78 $true 'Opening destination...'
        Lz308a9a536b
        $res = Lzf23fda3a43
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lz5575993e78 $false; return }
    finally { Lz5575993e78 $false }
    $picked = Lzc19b01f83f -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lz522bd82a8b ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnAddSource) { $ctrl.BtnAddSource.Add_Click({
    ...
    try { $picked = Lzfd6dab84a7 -ProjectId $p.Id -ProjectName $p.Name }
    ...
    Lz4cad1f4201 -Picked $arr
    Lz522bd82a8b ("Source folders: " + (@(Lz18ab8a6a63) -join ', '))
}) }
if ($ctrl.BtnRemoveSource) { $ctrl.BtnRemoveSource.Add_Click({
    ...
    Lz00e22bb3ae -Folders @(Lz18ab8a6a63 | Where-Object { $sel -notcontains $_ })
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    $bad = New-Object System.Collections.Generic.List[string]
    try {
        Lz5575993e78 $true 'Checking the source folder(s) in Datto...'
        foreach ($s in $subs) { try { $null = Lz562e229460 -ProjectId $p.Id -SubPath $s } catch { [void]$bad.Add($s) } }
    } catch { $ctrl.LblSourceCheck.Text = "Could not check the source: $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lz5575993e78 $false; return }
    finally { Lz5575993e78 $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lz5575993e78 $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lz9aa5fd21e2 -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lzbc4c9f1fc6 -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lz39a4783897 {
    ...
    $srcLabel = Lzab0e3f617d -Project $p
    $folders  = @(Lz18ab8a6a63)
    $srcSub   = Lz198cdefbef
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lze849142fdd) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lz903fcbd0b7).Count } catch { }
...
function Lz8d493241d4 {
    ...
    $script:Map[$p.Id] = $d
    Lz11b0657119
    Lz522bd82a8b "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lz75db8837f7 "Saved $n mapping(s)."
    return $true
}
function Lz1be2a6c307 {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lz75db8837f7 "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lz75db8837f7 "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lz7bd6016d3b -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lz7bd6016d3b -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lzc46bceb8af {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lzc46bceb8af) }
    ...
    if (-not (Lz603c66341f -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lz8717d3976b)) { return }
    if (-not (Lze8b69da749)) { return }
    Lz7bd6016d3b -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lz77ab06bcf4 {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lzc46bceb8af) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lz8717d3976b)) { return }
    if (-not (Lze8b69da749)) { return }
    Lz7bd6016d3b -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnRerunFailed.Add_Click({
    ...
    if (-not (Lz603c66341f -Title 'Rerun failed files only' -Heading 'Rerun failed files only' -Sections $sections -Yes 'Rerun the failed files' -No 'Cancel')) { return }
    if (-not (Lz8717d3976b)) { return }
    if (-not (Lze8b69da749)) { return }
    Lz7bd6016d3b -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','NewerWins','-Execute','-FailedOnly') -Label 'Rerun failed files only'
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lz603c66341f -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lze8b69da749 -For 'verify')) { return }
    Lz7bd6016d3b -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lz7bd6016d3b -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lz33eb9d7959 {
...
function Lzeff8cdd8e7 {
    try { if ($ctrl.BtnRerunFailed) { $ctrl.BtnRerunFailed.IsEnabled = (Lzd38d4c2c95) } } catch {}
}
function Lz0a9e348290 {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lz0df67d3eb2 {
    param([string]$Name)
...
function Lz674fa57a8b {
    ...
    foreach ($f in $files) { $l = Lz0df67d3eb2 $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lz674fa57a8b -Folder (Lz33eb9d7959) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnCertificate.Add_Click({
    if (Save-Mapping) {
        Lz7bd6016d3b -EngineArgs @('-Action','Certificate') -Label 'Completion certificate' -OnComplete {
            $rr = Lz33eb9d7959
            $cert = $null
...
$ctrl.BtnOpenAudit.Add_Click({ Lz674fa57a8b -Folder (Lz0a9e348290) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lz3301443c60 {
    ...
    Lz522bd82a8b "Speed limit set: up $uTxt, down $dTxt"
    Lz83a1296516 "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lz3301443c60 })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz3301443c60 } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz3301443c60 } })
function Lze1cb0431f3 {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lze1cb0431f3 -ParentId ([int]$_.ProcessId) } } catch {}
...
function Lz81f1acca19 {
    $r = ($script:Proc -and -not $script:Proc.HasExited)
    $f = ($script:FinProc -and -not $script:FinProc.HasExited)
    return ($r -or $f)
}
function Lz2a54e04e15 { param([string]$T) try { $ctrl.BtnSchedule.Content.Text = $T } catch {} }
function Lz335943c829 {
    param([string]$Why = '')
    if ($script:SchedTimer) { try { $script:SchedTimer.Stop() } catch {}; $script:SchedTimer = $null }
    $script:Sched = $null
    Lz2a54e04e15 'Schedule for later / overnight...'
    if ($Why) { Lz522bd82a8b $Why }
}
function Lzd19732868b {
    try { $p = Join-Path (Lz33eb9d7959) 'lastrun-outcome.json'; if (Test-Path $p) { return (Get-Content $p -Raw | ConvertFrom-Json) } } catch {}
    return $null
}
function Lzc7127caeb9 {
    if (-not $script:Proc -or $script:Proc.HasExited) { return }
    $script:QuietPausing = $true
    $ctrl.BtnStop.IsEnabled = $false
    Lz83a1296516 "----- Quiet hours reached at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')): pausing the copy. Files already uploaded are kept. -----"
    Lz522bd82a8b 'Quiet hours: pausing until the next window...'
    try { Lze1cb0431f3 -ParentId ([int]$script:Proc.Id) } catch {}
    Lzcacad11e53 -Status Cancelled -Note 'Finalising for quiet hours...' -OnDone {
        try { Lz83a1296516 '----- Paused for quiet hours. It will resume as a Sync at the next window while this app stays open. -----' } catch {}
    }
}
function Lz99bb84a952 {
    param([string[]]$RunArgs,[string]$Label)
    Lz83a1296516 "----- Scheduled: starting '$Label' at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')) -----"
    Lz7bd6016d3b -EngineArgs $RunArgs -Label $Label
    return ($script:Proc -and -not $script:Proc.HasExited)
}
function Lz690c84bcfd { param([string]$Status)
    ...
    switch ("$($s.State)") {
        'waiting' {
            if ($now -ge $s.StartAt -and -not $busy) {
                ...
                elseif (Lz99bb84a952 -RunArgs $s.Args -Label $s.Label) { $s.State = 'running'; $s.Launches++ }
                else { Lz335943c829 -Why 'The scheduled run could not start (see the message). Schedule cancelled.' }
            } else {
                $left = $s.StartAt - $now; if ($left.Ticks -lt 0) { $left = [TimeSpan]::Zero }
                Lz2a54e04e15 ("Scheduled: starts $($s.StartAt.ToString('ddd HH:mm')) ($(Format-Span $left) to go) - click to cancel")
            }
        }
        'running' {
            if (-not $busy) {
                $oc = Lzd19732868b; $status = if ($oc) { "$($oc.Status)" } else { '' }
                if ($s.Mode -eq 'once') { Lz335943c829 -Why 'Scheduled run finished.'; Lz690c84bcfd $status }
                elseif ($status -eq 'COMPLETED') { Lz335943c829 -Why 'Scheduled migration completed.'; Lz690c84bcfd $status }
                else { $s.State = 'paused' }
            } elseif ($s.Mode -eq 'window' -and -not (Test-InWindow $m $s.FromMin $s.ToMin)) {
                Lzc7127caeb9; $s.State = 'paused'
            } else {
                $until = if ($s.Mode -eq 'window') { " (until $(Format-MinOfDay $s.ToMin))" } else { '' }
                Lz2a54e04e15 ("Scheduled: running$until - click to cancel")
            }
        }
        'paused' {
            if ((Test-InWindow $m $s.FromMin $s.ToMin) -and -not $busy) {
                if (Lz99bb84a952 -RunArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','AddMissing','-Execute') -Label 'Sync new and changed') { $s.State = 'running'; $s.Launches++ }
            } else {
                $nf = Get-NextWindowOpen $s.FromMin
                Lz2a54e04e15 ("Scheduled: waiting for the next window ($($nf.ToString('ddd HH:mm'))) - click to cancel")
            }
        }
    }
}
function Lz753be302fe {
...
$ctrl.BtnSchedule.Add_Click({
    if ($script:Sched) {
        if ((Show-Msg -Text ("A schedule is set:" + [Environment]::NewLine + $script:Sched.Summary + [Environment]::NewLine + [Environment]::NewLine + "Cancel it?") -Caption ('Scheduled run') -Buttons ('YesNo') -Icon ('Question')) -eq 'Yes') { Lz335943c829 -Why 'Schedule cancelled.' }
        return
    }
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')) | Out-Null; return }
    if (Lz81f1acca19) { (Show-Msg -Text ('A run is in progress. Wait for it to finish, then set a schedule.') -Caption ('Cannot schedule now') -Icon ('Warning')) | Out-Null; return }
    if (-not (Save-Mapping)) { return }
    $mappedCount = @($script:Projects | Where-Object { $script:Map.ContainsKey($_.Id) }).Count
    ...
    $script:SchedTimer.Add_Tick({ try { Lz8cb272ad53 } catch {} })
    $script:SchedTimer.Start()
    try { Lz8cb272ad53 } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    if ($script:Sched) { Lz335943c829 -Why 'Schedule cancelled (you stopped the run).' }
    $script:Stopping = $true
    $ctrl.BtnStop.IsEnabled = $false
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz83a1296516 "----- STOP requested by user at $stamp -----"
    Lz522bd82a8b "Stopping $($script:CurLabel)..."
    try { Lze1cb0431f3 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lzcacad11e53 -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lz83a1296516 "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lz5bd552197d {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lz7bd6016d3b -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz83a1296516 "----- PAUSE requested by user at $stamp -----"
    Lz522bd82a8b "Pausing $($script:CurLabel)..."
    try { Lze1cb0431f3 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lzcacad11e53 -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lz83a1296516 "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lz7e10dcbc90 { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lz914127da35 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lzed41c9e9a6 {
...
function Lz8c1949ff1a {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lzb0ceb012ae -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lz25f2968f86 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    ...
    function Lz061e6c1b83 { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz19068816c3 { param($Label,$Control,$Info)
    ...
    function Lz5d16f77b19 { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lz061e6c1b83 'Datto Workplace'
    ...
    $parts = Lz914127da35 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz19068816c3 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lz19068816c3 'Cell (number)' (Lz5d16f77b19 ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lz19068816c3 'Datto Client ID' (Lz5d16f77b19 ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lz19068816c3 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lz061e6c1b83 'Microsoft 365'
    $tbTid = Lz19068816c3 'Tenant ID' (Lz5d16f77b19 ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lz19068816c3 'App (Client) ID' (Lz5d16f77b19 ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lz061e6c1b83 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lz19068816c3 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lz19068816c3 '.pfx file' (Lz5d16f77b19 '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lz19068816c3 '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lz061e6c1b83 'SharePoint and OneDrive'
    ...
    $tbSp  = Lz19068816c3 'SharePoint root URL' (Lz5d16f77b19 ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lz19068816c3 'OneDrive host URL'   (Lz5d16f77b19 ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lz19068816c3 'Team site base URL'  (Lz5d16f77b19 ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lz19068816c3 'Default site URL'    (Lz5d16f77b19 ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lz19068816c3 'Email / UPN domain'  (Lz5d16f77b19 ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lzf5c3b16832 -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lz8c1949ff1a -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz46c4920e94 $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lz7e10dcbc90 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lzf3dcc88e74 }
        ...
        try {
            ...
            else { $tok=Lz8c1949ff1a -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lz7e10dcbc90 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lzf5c3b16832 -Name 'DattoEndpointUrl' -Value $ep
            Lzf5c3b16832 -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lzf5c3b16832 -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lzf5c3b16832 -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lzf5c3b16832 -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lzf5c3b16832 -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lzf5c3b16832 -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lzf5c3b16832 -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lzf5c3b16832 -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lz4fb840b232 -Value $script:AS.pbSec.Password
            }
...
function Lz2087026bee { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lzfc3d66abb4 { param($Cfg,[string]$Path,$Value)
...
function Lz117e575e5e {
    ...
    $curMax=Lz2087026bee -Cfg $cfg -Path 'run.tuning.maxFileSizeMB'; if ($null -eq $curMax) { $curMax=0 }; $sizeBox.Text="$curMax"
    ...
    $curInc=@(Lz2087026bee -Cfg $cfg -Path 'run.tuning.includePatterns' | Where-Object { "$_".Trim() })
    $curExc=@(Lz2087026bee -Cfg $cfg -Path 'run.tuning.excludePatterns' | Where-Object { "$_".Trim() })
    ...
    $afterPick=New-Object System.Windows.Controls.DatePicker; $afterPick.Margin='0,4,0,4'; $afterPick.Width=150; $afterPick.SelectedDate=(& $toDate (Lz2087026bee -Cfg $cfg -Path 'run.tuning.modifiedAfter'))
    ...
    $beforePick=New-Object System.Windows.Controls.DatePicker; $beforePick.Margin='0,4,0,4'; $beforePick.Width=150; $beforePick.SelectedDate=(& $toDate (Lz2087026bee -Cfg $cfg -Path 'run.tuning.modifiedBefore'))
    ...
    $save.Add_Click({
        try {
            ...
            Lzfc3d66abb4 -Cfg $script:FltCfg -Path 'run.tuning.maxFileSizeMB' -Value $sv
            $pats=@("$($script:FltPat.Text)" -split '[;,\r\n]' | ForEach-Object { $_.Trim() } | Where-Object { $_ })
            if ($script:FltInc.IsChecked) { Lzfc3d66abb4 -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value $pats; Lzfc3d66abb4 -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value @() }
            else                          { Lzfc3d66abb4 -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value $pats; Lzfc3d66abb4 -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value @() }
            ...
            Lzfc3d66abb4 -Cfg $script:FltCfg -Path 'run.tuning.modifiedAfter'  -Value $ma
            Lzfc3d66abb4 -Cfg $script:FltCfg -Path 'run.tuning.modifiedBefore' -Value $mb
...
function Lz860cdd4051 {
    ...
    function Lzeffb814ff2 { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec,[string]$Type='text')
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lz2087026bee -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lzeffb814ff2 -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec -Type $s.Type
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lzfc3d66abb4 -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lzfc3d66abb4 -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lzfc3d66abb4 -Cfg $script:SetCfg -Path $s.Path -Value ([bool]$script:SetBoxes[$s.Path].IsChecked) }
                elseif ($s.Type -eq 'date') {
                    ...
                    Lzfc3d66abb4 -Cfg $script:SetCfg -Path $s.Path -Value $raw
                }
                else { Lzfc3d66abb4 -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lz213c7818cb {
    ...
    function Lzcac5ed5816 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lz914127da35 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lzcac5ed5816 -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lzcac5ed5816 -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lzcac5ed5816 -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lzcac5ed5816 -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lz7e10dcbc90 $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lzf3dcc88e74 }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lzf5c3b16832 -Name 'DattoEndpointUrl' -Value $ep
            Lzf5c3b16832 -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lz4fb840b232 -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lza91ccc4b83 {
    ...
    $pre = Lz3dcbab3633 -Title 'Prerequisites'
    ...
    $dat = Lz3dcbab3633 -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lz69290a0bf9 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lz3dcbab3633 -Title 'Microsoft 365' -Buttons @($bMs, (Lz69290a0bf9 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lz3dcbab3633 -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lz213c7818cb } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzaa5a7c762c; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lz29f33bfa10 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzaa5a7c762c; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lzf81fd497b8 } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzaa5a7c762c; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lzaa5a7c762c
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lz5094a0c29f {
...
function Lz5dd6fbec0c {
    ...
    $bOpen.Add_Click({ try { Lz1ead339968 -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lz8797a92f38 {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lzc28ea9f898 {
    ...
    Lz5dd6fbec0c -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lz1ead339968 -Url $pageUrl
    ...
    } finally { Lz8797a92f38 }
}
function Lz70b8937f79 {
    try {
...
function Lzb1eb69f5a8 {
    if (Lz9613505ab3) { return $false }
...
function Lz930bc0f7d6 {
    ...
    if (-not (Get-Module Microsoft.Graph.Authentication -ListAvailable)) {
        Lz5094a0c29f 'The Microsoft Graph module is not installed. Re-run the installer (or run: Install-Module Microsoft.Graph.Authentication -Scope CurrentUser) and try again.' '#B42318' -Strong
        return
    }
    Lz5094a0c29f 'Preparing the Microsoft 365 sign-in. The first time on a new install can take up to a minute while Windows loads the components - please wait, the app has not frozen.' '#475467' -Strong
    try { Import-Module Microsoft.Graph.Authentication -ErrorAction Stop } catch {
        Lz5094a0c29f "Could not load the Microsoft Graph module: $($_.Exception.Message)" '#B42318' -Strong
        return
    }
    Lz5094a0c29f "Sign in with an account that can manage Microsoft 365 (a Global Administrator is simplest). The first time, Microsoft asks you to accept the permissions this setup itself needs; that is expected.$(if ($ForEmail) { ' Email alerts are included, so expect TWO sign-ins: this one, then one for Exchange Online (a separate service with its own sign-in).' })"
    ...
    if (Lzb1eb69f5a8) {
        Lz5094a0c29f 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B5460F' -Strong
        try {
            Lzde685b3f75 -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lz5094a0c29f "The browser sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B5460F' -Strong
        }
    } else {
        Lz5094a0c29f 'Using the code sign-in (the reliable path): a small window will show your code to type at microsoft.com/devicelogin, or on your phone. It stays on top and closes itself when you are done.' '#B5460F' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lzc28ea9f898 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lz5094a0c29f $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lz40a101e441 -Token $at -Claim 'tid'
            $wizTokAcct = Lz40a101e441 -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lz40a101e441 -Token $at -Claim 'preferred_username' }
            ...
            Lz5094a0c29f "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lz5094a0c29f 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lz5094a0c29f "Signed in to tenant $tid as $wizAcct." '#0E9F6E'
    $newThumb = $null
    try {
        Lz5094a0c29f 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lz5094a0c29f "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B5460F' }
            Lz5094a0c29f "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#0E9F6E'
        } else {
            Lz5094a0c29f "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lz5094a0c29f 'App registration created.' '#0E9F6E'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lz5094a0c29f 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lz5094a0c29f "Certificate created in this user's store. Thumbprint $newThumb." '#0E9F6E'
        Lz5094a0c29f 'Uploading the certificate''s public key to the app registration...'
        ...
        Lz5094a0c29f "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#0E9F6E'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lzf94ad4d690 -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lz5094a0c29f 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lzf94ad4d690 -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lz5094a0c29f $consentMsg '#0E9F6E'
        Lzf5c3b16832 -Name 'TenantId'       -Value $tid
        Lzf5c3b16832 -Name 'GraphClientId'  -Value $appId
        Lzf5c3b16832 -Name 'CertThumbprint' -Value $newThumb
        Lz5094a0c29f 'Connection details saved on this computer.' '#0E9F6E'
    } catch {
        ...
        if ($friendly) { Lz5094a0c29f $friendly '#B42318' -Strong; Lz5094a0c29f "Technical detail: $m" }
        else { Lz5094a0c29f "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lz5094a0c29f "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lz5094a0c29f 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lz8c1949ff1a -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lzf5c3b16832 -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lzed41c9e9a6 $spUrl
            if ($der) {
                Lzf5c3b16832 -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lzf5c3b16832 -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lzf5c3b16832 -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lz5094a0c29f "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#0E9F6E'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lzf94ad4d690 -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lz5094a0c29f "The new sign-in has not become active yet: $($_.Exception.Message)" '#B5460F' -Strong
                   Lz5094a0c29f 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz46c4920e94 $org
            if ($u) { Lzf5c3b16832 -Name 'UpnDomain' -Value $u; Lz5094a0c29f "Email domain detected: $u" '#0E9F6E' }
        } catch { Lz5094a0c29f 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lz5094a0c29f 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#0E9F6E' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lz29f33bfa10 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lz930379700e $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lz930bc0f7d6 -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lz46fa939c8e) {
                    Lz5094a0c29f 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lz5094a0c29f $t $c } else { Lz5094a0c29f $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lz5094a0c29f 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B5460F'
                }
            }
            ...
            Lz5094a0c29f "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lzc28ea9f898 -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lzde685b3f75 -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lz1ead339968 -Url 'https://login.microsoft.com/device'
                Lza4c083826f
                try { Lzde685b3f75 -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lz43ce0317a7 }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (Lzb1eb69f5a8) {
            ...
            try {
                Lzde685b3f75 -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lzc28ea9f898 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lz40a101e441 -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lz40a101e441 -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lzf94ad4d690 -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lz569e17aa39 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lzf94ad4d690 -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lzf94ad4d690 -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lzf5c3b16832 -Name 'EmailSender' -Value $senderAddr
        Lzf5c3b16832 -Name 'EmailEnabled' -Value '1'
...
function Lzf81fd497b8 {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lz30c96c135f { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz05df5a4a5f { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lz05df5a4a5f -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lz05df5a4a5f -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lz05df5a4a5f -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lz30c96c135f 'Subject and attachments'
    ...
    [void](Lz05df5a4a5f -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz92be3d8801 -Template $script:ES.tbSubj.Text -Vars (Lz30dfe99415)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz92be3d8801 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz30dfe99415)
            $tok = Lz8c1949ff1a -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lzf5c3b16832 -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lzf5c3b16832 -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lzf5c3b16832 -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lzf5c3b16832 -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lzf5c3b16832 -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lzf5c3b16832 -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lzf5c3b16832 -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lzf5c3b16832 -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lzf5c3b16832 -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lzf5c3b16832 -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lzf5c3b16832 -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lzf5c3b16832 -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lz348ffe3192 {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lzc28ea9f898 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lz9f4e6793b3 {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lz348ffe3192 -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lzaa5a7c762c } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lz35ab417bc7 {
    return ((Lzf1a45e0c85) -and (Lz46fa939c8e))
}
Lzfd65761830
Lz43ce0317a7
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lz83a1296516 "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lz35ab417bc7)) {
        try { Lza91ccc4b83 } catch {}
        if (Lz35ab417bc7) { Lz522bd82a8b 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lz522bd82a8b 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lz522bd82a8b 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ($ctrl.GettingStarted -and $ctrl.GettingStarted.Visibility -eq 'Visible') {
        try { Lz5d22deca64 ([bool](Lz35ab417bc7)) } catch {}
    }
    if ((Lz35ab417bc7) -and -not (Test-Path (Lz4f9f4f2597))) {
        try { Lz28cb5b8cfc } catch {}
    }
