...
function Lz1dda172ed2 { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lz6cfd48781a {
...
function Lzf628aaeb80 {
    ...
    if (-not $s) { $s = Lzb9d71c90f7 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lz7512ccea76 {
    param([string]$Value)
    if (Lz6cfd48781a -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lz3a0c93e49a {
    ...
    return (Lz4482b4d1df -TargetSubFolder $box -SourceSubPath $src)
}
function Lze95e95f344 {
    ...
    try {
        ...
        $norm = Lz4482b4d1df -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lzdf83df0b7e "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lzabc34fcfca
    } catch { }
}
function Lz4482b4d1df {
...
function Lz20e43e16e5 {
    ...
    return @{ SubfolderCount = @(Lzdbb362a442 -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lzbb19a14684 {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lzac9fc6d515 -Path "sites/$siteId"
    } catch { return $null }
}
function Lz91609f2d9f {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lzac9fc6d515 -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lz5a167d7e72 {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lzac9fc6d515 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz41e375c2ec {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lz373a1de23c -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lz91609f2d9f -Search $s.Txt.Text)
...
"@
    ...
    Lz41e375c2ec
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lz41e375c2ec } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lz41e375c2ec } })
    $new.Add_Click({
        $n = Lzcedd1b12aa -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lz88b0b7ad5d -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lz41e375c2ec; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lz42ad99061c -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lzf3a4d12391 -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lz42ad99061c -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lzffa82ab35e {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lzdbb362a442 -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lzffa82ab35e
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lzffa82ab35e } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lzffa82ab35e } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lz42ad99061c -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lzf3a4d12391 -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lz42ad99061c -State $script:SP
    $r = $w.ShowDialog()
...
function Lzdf83df0b7e { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz15a4911335 { param([TimeSpan]$ts)
...
function Lzbbe81009c5 { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lz20bb0442fb {
    ...
    if ($hasRegime) {
        $etaA = Lz7d7d4ceab4 -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz7d7d4ceab4 -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lz7d7d4ceab4 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz7d7d4ceab4 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lz8907f928d1 {
    ...
    try {
        ...
        $etaText = Lz20bb0442fb -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lz62204fe114 {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lzcd2a82a1f9 -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lz37a612d7e2 -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lz8907f928d1 -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lz32efd862b3 $Line) + "`r`n")
...
function Lz17dbd8fb12 { param([string]$Text) Lz62204fe114 $Text }
function Lz0ac5925e23 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lz17568ccc09 $true 'Connecting to Microsoft 365...'
        Lzec9787bca8
    } catch {
        ...
        Lz17568ccc09 $false; return
    } finally { Lz17568ccc09 $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lze341dd7b81 -StartSearch $seed
    if ($picked) {
        ...
        Lzdf83df0b7e "Site set to $picked"
        Lzabc34fcfca
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lz0ac5925e23 }
    Lzabc34fcfca
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lzeda0d2edc2 }
    Lzabc34fcfca
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lzabc34fcfca })
$ctrl.TxtFolder.Add_TextChanged({ Lzabc34fcfca })
$ctrl.TxtSourceSub.Add_TextChanged({ Lzabc34fcfca })
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lzabc34fcfca }); $ctrl.ChkSrcContents.Add_Unchecked({ Lzabc34fcfca }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lz8b173811f4 -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lze95e95f344 })
$ctrl.TxtSourceSub.Add_LostFocus({ Lze95e95f344 })
$ctrl.TxtLoc.Add_TextChanged({ Lzabc34fcfca })
$ctrl.TxtLib.Add_TextChanged({ Lzabc34fcfca })
$script:NestSuspend = $false
function Lz675b2bf600 {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lzef74ac93d1 -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lzdf83df0b7e $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lz675b2bf600 })
$ctrl.ChkNest.Add_Unchecked({ Lz675b2bf600 })
$script:ProjectRows = @()
$script:RunStatus = @{}
function Lz10cc06bc3d {
...
function Lz43655da543 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lz20e43e16e5 -ProjectId $p.Id -SubPath $sub) }
...
function Lz74b2b61d37 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lzac9fc6d515 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz8a218977ce {
    ...
    try { Lz17568ccc09 $true 'Checking source folders...'; $srcOk = Lz43655da543 } finally { Lz17568ccc09 $false }
    if (-not $srcOk) { return }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lz17568ccc09 $true 'Checking destinations...'; $dstOk = Lz74b2b61d37 } finally { Lz17568ccc09 $false }
        if (-not $dstOk) { return }
    }
        ...
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lz62204fe114 $lines[$k] } catch {} }
                }
            }
        } catch {}
        $script:SpinIdx = ($script:SpinIdx + 1) % 4
        $spin = @('|','/','-','\')[$script:SpinIdx]
        $ctrl.LblElapsed.Text = "$spin  Elapsed " + (Lz15a4911335 ((Get-Date) - $script:RunStart))
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lz62204fe114 $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lz62204fe114 $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Lz15a4911335 ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lzca66011aa4) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lz4086adeff8 -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lz62204fe114 "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lz62204fe114 "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lzdf83df0b7e ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
...
function Lz44f1455366 { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lz137b40f04a { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lz4c8a8ed4b9 {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lz97bb9d5ded -ConfigFile $s.Tag; Lzd793d86485 } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lzc2bc9e7132 { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lz1dda172ed2 -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lz4c8a8ed4b9
}
function Lzad0bd6fc1a {
...
function Lz97bb9d5ded {
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lz137b40f04a $true
    Lz44f1455366 $true
    Lzc2bc9e7132 $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lzad0bd6fc1a -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lzdf83df0b7e 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lzcedd1b12aa -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Test-Path $jobDir) { (Show-Msg -Text ("A job folder '$slug' already exists. Open it instead, or choose another name.")); return }
        ...
        Lzef74ac93d1 -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lz97bb9d5ded -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz04f5d2a870 })
$ctrl.MnuSettingsApi.Add_Click({ Lz03d8632438 })
$ctrl.MnuSettingsWizard.Add_Click({ Lzf661f2bdc3 })
$ctrl.MnuSettingsEmail.Add_Click({ Lzd0c30509fe })
$ctrl.MnuSettingsTuning.Add_Click({ Lz8ce25ce34e })
$ctrl.MnuSettingsDecommission.Add_Click({ Lzb7bcf7bbb7 })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lzaeb08d2926 -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lz25fb3cc80b {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lzb9d71c90f7 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lz25fb3cc80b "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lz25fb3cc80b (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lz17568ccc09 $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lz17568ccc09 $false }
    if (-not $b) { return }
...
function Lz286ff46c35 { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lzcdba73967d {
    $path = Lz286ff46c35
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Datto Workplace to SharePoint Migrator" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lzcdba73967d)) -Caption ('Licence')) | Out-Null })
function Lz1cd0ddf716 {
...
function Lzb9123e9f70 {
    param([string]$For = 'copy')
    if (Test-Path (Lz286ff46c35)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        (Show-Msg -Text ("Licence installed.`n`n" + (Lzcdba73967d)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-UpdateCheck {
    try {
        ...
        Lz17568ccc09 $true 'Checking for updates...'
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lz17568ccc09 $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lzaeb08d2926 -Title 'About' -Heading 'Datto Workplace to SharePoint Migrator' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lzf628aaeb80 } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lzac145ce9a3 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lzcedd1b12aa -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lzedb1a08610 {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lz137b40f04a $false
    Lz44f1455366 $false
}
$ctrl.MnuJobClose.Add_Click({ Lzedb1a08610; Lzdf83df0b7e 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lzedb1a08610
    try { if ($dir -and (Test-Path $dir)) { Remove-Item $dir -Recurse -Force -ErrorAction Stop } ; Lz4c8a8ed4b9; Lzdf83df0b7e "Deleted job '$nm'" }
    catch { (Show-Msg -Text ("Could not fully delete: $($_.Exception.Message)")) }
})
function Lze910418bd4 {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lz17568ccc09 $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lz17dbd8fb12 'Signing in to Datto and listing projects...'
        $script:Projects = Lzb2bdd6330d
        Lz17dbd8fb12 "Found $($script:Projects.Count) project(s)."
        Lz71ac28363e
        try { Lzec9787bca8; Lz17dbd8fb12 'Connected to Microsoft 365.' } catch { Lz17dbd8fb12 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lz17dbd8fb12 "Technical detail: $($_.Exception.Message)" }
        Update-ProjectList
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lzdf83df0b7e 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lz17dbd8fb12 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lz17dbd8fb12 "Technical detail: $($_.Exception.Message)"
            Lzdf83df0b7e "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lzdf83df0b7e 'Connect failed'
        }
    }
    finally { Lz17568ccc09 $false }
}
$ctrl.BtnConnect.Add_Click({ Lze910418bd4 })
function Lzd793d86485 {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lzf628aaeb80 } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lzdf83df0b7e "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lze910418bd4 -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lze0f072e829
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lz48fc359767).Count } catch {}
...
function Lzcf9de470a2 {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lz3a0c93e49a) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lz3a0c93e49a) }
}
$ctrl.BtnApply.Add_Click({
    ...
    Lzdf83df0b7e "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lz7b9d0455b4 })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lzc6d4b7f1f3 -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Update-ProjectList; [void](Write-MappingQuiet); Lzdf83df0b7e "Set $($sel.Count) project(s) to skip"; return }
    $d = Lzcf9de470a2
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lz0ac5925e23); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Update-ProjectList
    [void](Write-MappingQuiet)
    Lzdf83df0b7e "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lzc6d4b7f1f3 -Count (@($script:Projects).Count))) { return }
    $d = Lzcf9de470a2
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Update-ProjectList; [void](Write-MappingQuiet); Lzdf83df0b7e 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Update-ProjectList
    [void](Write-MappingQuiet)
    Lzdf83df0b7e "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lz17568ccc09 $true 'Looking up libraries...'
        $libs = Lz19cce9e01c -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lz17568ccc09 $false; return }
    finally { Lz17568ccc09 $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz272e1edfb3 -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lzdf83df0b7e "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lzdf83df0b7e "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lz17568ccc09 $true 'Opening destination...'
        Lzec9787bca8
        $res = Lz5a167d7e72
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lz17568ccc09 $false; return }
    finally { Lz17568ccc09 $false }
    $picked = Lz17e309391f -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lzdf83df0b7e ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnBrowseSource) { $ctrl.BtnBrowseSource.Add_Click({
    ...
    try { $picked = Lz798b08a902 -ProjectId $p.Id -ProjectName $p.Name }
    catch { (Show-Msg -Text ("Could not open the project's folders: $($_.Exception.Message)") -Caption ('Browse source')); return }
    if ($null -ne $picked) { $ctrl.TxtSourceSub.Text = $picked; Lzdf83df0b7e ("Source set to " + $(if ($picked) { "/$picked" } else { 'the whole project' })) }
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    try {
        Lz17568ccc09 $true 'Checking the source folder in Datto...'
        $res = Lz20e43e16e5 -ProjectId $p.Id -SubPath $sub
    } catch { $ctrl.LblSourceCheck.Text = "Source NOT found. $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lz17568ccc09 $false; return }
    finally { Lz17568ccc09 $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lz17568ccc09 $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lzac9fc6d515 -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lz19cce9e01c -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lzabc34fcfca {
    ...
    $srcLabel = Lz7976e82cfa -Project $p
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lz3a0c93e49a) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lz48fc359767).Count } catch { }
...
function Lza2dd77870d {
    ...
    $script:Map[$p.Id] = $d
    Update-ProjectList
    Lzdf83df0b7e "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lz17dbd8fb12 "Saved $n mapping(s)."
    return $true
}
function Lz71ac28363e {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lz17dbd8fb12 "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lz17dbd8fb12 "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lz8a218977ce -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lz8a218977ce -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lz35162dea1f {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lz35162dea1f) }
    ...
    if (-not (Lzaeb08d2926 -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lzb9123e9f70)) { return }
    Lz8a218977ce -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lz32557e64bf {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lz35162dea1f) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lzb9123e9f70)) { return }
    Lz8a218977ce -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lzaeb08d2926 -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lzb9123e9f70 -For 'verify')) { return }
    Lz8a218977ce -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lz8a218977ce -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lzca66011aa4 {
...
function Lzbe6e7cc46c {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lzaa225d77af {
    param([string]$Name)
...
function Lze0d03601b8 {
    ...
    foreach ($f in $files) { $l = Lzaa225d77af $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lze0d03601b8 -Folder (Lzca66011aa4) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnOpenAudit.Add_Click({ Lze0d03601b8 -Folder (Lzbe6e7cc46c) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lzc38b0dfbe3 {
    ...
    Lzdf83df0b7e "Speed limit set: up $uTxt, down $dTxt"
    Lz62204fe114 "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lzc38b0dfbe3 })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lzc38b0dfbe3 } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lzc38b0dfbe3 } })
function Lz2057847184 {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lz2057847184 -ParentId ([int]$_.ProcessId) } } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    Lz62204fe114 "----- STOP requested by user at $stamp -----"
    Lzdf83df0b7e "Stopping $($script:CurLabel)..."
    try { Lz2057847184 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lzad0bd6fc1a -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lz62204fe114 "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lzf90611f365 {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lz8a218977ce -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz62204fe114 "----- PAUSE requested by user at $stamp -----"
    Lzdf83df0b7e "Pausing $($script:CurLabel)..."
    try { Lz2057847184 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lzad0bd6fc1a -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lz62204fe114 "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lz2e265de0ec { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lz968c403f26 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lz9892362411 {
...
function Lzac145ce9a3 {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lzf57a260088 -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lz03d8632438 {
    ...
    function Lz6150a5a1b4 { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz013a032e17 { param($Label,$Control,$Info)
    ...
    function Lz7b78e79149 { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lz6150a5a1b4 'Datto Workplace'
    ...
    $parts = Lz968c403f26 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz013a032e17 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lz013a032e17 'Cell (number)' (Lz7b78e79149 ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lz013a032e17 'Datto Client ID' (Lz7b78e79149 ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lz013a032e17 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lz6150a5a1b4 'Microsoft 365'
    $tbTid = Lz013a032e17 'Tenant ID' (Lz7b78e79149 ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lz013a032e17 'App (Client) ID' (Lz7b78e79149 ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lz6150a5a1b4 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lz013a032e17 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lz013a032e17 '.pfx file' (Lz7b78e79149 '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lz013a032e17 '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lz6150a5a1b4 'SharePoint and OneDrive'
    ...
    $tbSp  = Lz013a032e17 'SharePoint root URL' (Lz7b78e79149 ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lz013a032e17 'OneDrive host URL'   (Lz7b78e79149 ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lz013a032e17 'Team site base URL'  (Lz7b78e79149 ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lz013a032e17 'Default site URL'    (Lz7b78e79149 ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lz013a032e17 'Email / UPN domain'  (Lz7b78e79149 ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lz1dda172ed2 -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lzac145ce9a3 -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz2712b70409 $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lz2e265de0ec $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lzf628aaeb80 }
        ...
        try {
            ...
            else { $tok=Lzac145ce9a3 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lz2e265de0ec $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lz1dda172ed2 -Name 'DattoEndpointUrl' -Value $ep
            Lz1dda172ed2 -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lz1dda172ed2 -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lz1dda172ed2 -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lz1dda172ed2 -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lz1dda172ed2 -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lz1dda172ed2 -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lz1dda172ed2 -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lz1dda172ed2 -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lz7512ccea76 -Value $script:AS.pbSec.Password
            }
...
function Lz8ae734be6f { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lzef74ac93d1 { param($Cfg,[string]$Path,$Value)
...
function Lz8ce25ce34e {
    ...
    function Lz2dcf2a8104 { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec)
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lz8ae734be6f -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lz2dcf2a8104 -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lzef74ac93d1 -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lzef74ac93d1 -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lzef74ac93d1 -Cfg $script:SetCfg -Path $s.Path -Value ([bool]($raw -match '^(1|true|yes|on)$')) }
                else { Lzef74ac93d1 -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lz94dfdd3564 {
    ...
    function Lzb810501e4a { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lz968c403f26 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lzb810501e4a -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lzb810501e4a -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lzb810501e4a -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lzb810501e4a -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lz2e265de0ec $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lzf628aaeb80 }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz1dda172ed2 -Name 'DattoEndpointUrl' -Value $ep
            Lz1dda172ed2 -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lz7512ccea76 -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz04f5d2a870 {
    ...
    $pre = Lzf8e2d13b1a -Title 'Prerequisites'
    ...
    $dat = Lzf8e2d13b1a -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lzb21f907a0b 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lzf8e2d13b1a -Title 'Microsoft 365' -Buttons @($bMs, (Lzb21f907a0b 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lzf8e2d13b1a -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lz94dfdd3564 } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzfcde96e32d; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lzf661f2bdc3 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzfcde96e32d; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lzd0c30509fe } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzfcde96e32d; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lzfcde96e32d
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lzdf0fc57a07 {
...
function Lzadfb7a1ab5 {
    ...
    $bOpen.Add_Click({ try { Lz6235c5a582 -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lz6f9ec25a2d {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lz4d1c4d5af9 {
    ...
    Lzadfb7a1ab5 -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lz6235c5a582 -Url $pageUrl
    ...
    } finally { Lz6f9ec25a2d }
}
function Lzc6e8297c32 {
    try {
...
function Lzc0a6290821 {
    ...
    if (-not (Lzfe71e0a457)) {
        Lzdf0fc57a07 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B54708' -Strong
        try {
            Lz167759b903 -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lzdf0fc57a07 "The standard sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B54708' -Strong
        }
    } else {
        Lzdf0fc57a07 'Using the CODE sign-in (the reliable path on this machine): a small window will show your code, always on top.' '#B54708' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lz4d1c4d5af9 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lzdf0fc57a07 $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lz622a0dffd0 -Token $at -Claim 'tid'
            $wizTokAcct = Lz622a0dffd0 -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lz622a0dffd0 -Token $at -Claim 'preferred_username' }
            ...
            Lzdf0fc57a07 "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lzdf0fc57a07 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lzdf0fc57a07 "Signed in to tenant $tid as $wizAcct." '#12B76A'
    $newThumb = $null
    try {
        Lzdf0fc57a07 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lzdf0fc57a07 "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B54708' }
            Lzdf0fc57a07 "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#12B76A'
        } else {
            Lzdf0fc57a07 "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lzdf0fc57a07 'App registration created.' '#12B76A'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lzdf0fc57a07 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lzdf0fc57a07 "Certificate created in this user's store. Thumbprint $newThumb." '#12B76A'
        Lzdf0fc57a07 'Uploading the certificate''s public key to the app registration...'
        ...
        Lzdf0fc57a07 "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#12B76A'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lz7c8dcee813 -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lzdf0fc57a07 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lz7c8dcee813 -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lzdf0fc57a07 $consentMsg '#12B76A'
        Lz1dda172ed2 -Name 'TenantId'       -Value $tid
        Lz1dda172ed2 -Name 'GraphClientId'  -Value $appId
        Lz1dda172ed2 -Name 'CertThumbprint' -Value $newThumb
        Lzdf0fc57a07 'Connection details saved on this computer.' '#12B76A'
    } catch {
        ...
        if ($friendly) { Lzdf0fc57a07 $friendly '#B42318' -Strong; Lzdf0fc57a07 "Technical detail: $m" }
        else { Lzdf0fc57a07 "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lzdf0fc57a07 "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lzdf0fc57a07 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lzac145ce9a3 -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lz1dda172ed2 -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lz9892362411 $spUrl
            if ($der) {
                Lz1dda172ed2 -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lz1dda172ed2 -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lz1dda172ed2 -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lzdf0fc57a07 "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#12B76A'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lz7c8dcee813 -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lzdf0fc57a07 "The new sign-in has not become active yet: $($_.Exception.Message)" '#B54708' -Strong
                   Lzdf0fc57a07 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz2712b70409 $org
            if ($u) { Lz1dda172ed2 -Name 'UpnDomain' -Value $u; Lzdf0fc57a07 "Email domain detected: $u" '#12B76A' }
        } catch { Lzdf0fc57a07 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lzdf0fc57a07 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#12B76A' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lzf661f2bdc3 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lza78ba01bf9 $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lzc0a6290821 -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lzfe974fe28f) {
                    Lzdf0fc57a07 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lzdf0fc57a07 $t $c } else { Lzdf0fc57a07 $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lzdf0fc57a07 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B54708'
                }
            }
            ...
            Lzdf0fc57a07 "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lz4d1c4d5af9 -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lz167759b903 -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lz6235c5a582 -Url 'https://login.microsoft.com/device'
                Lzdf71d71936
                try { Lz167759b903 -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lz96ab3b24a0 }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (-not (Lzfe71e0a457)) {
            ...
            try {
                Lz167759b903 -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lz4d1c4d5af9 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lz622a0dffd0 -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lz622a0dffd0 -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lz7c8dcee813 -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lz7cb2e3e128 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lz7c8dcee813 -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lz7c8dcee813 -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lz1dda172ed2 -Name 'EmailSender' -Value $senderAddr
        Lz1dda172ed2 -Name 'EmailEnabled' -Value '1'
...
function Lzd0c30509fe {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lzb53000318f { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz83dc90dcca { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lz83dc90dcca -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lz83dc90dcca -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lz83dc90dcca -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lzb53000318f 'Subject and attachments'
    ...
    [void](Lz83dc90dcca -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz077bae2bbd -Template $script:ES.tbSubj.Text -Vars (Lz0d54d9c479)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz077bae2bbd -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz0d54d9c479)
            $tok = Lzac145ce9a3 -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz1dda172ed2 -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lz1dda172ed2 -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lz1dda172ed2 -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lz1dda172ed2 -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lz1dda172ed2 -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lz1dda172ed2 -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lz1dda172ed2 -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lz1dda172ed2 -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lz1dda172ed2 -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lz1dda172ed2 -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lz1dda172ed2 -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lz1dda172ed2 -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lzbdd445f219 {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lz4d1c4d5af9 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lzb7bcf7bbb7 {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lzbdd445f219 -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lzfcde96e32d } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lz8af4d1f33c {
    return ((Lzeca61b9ba2) -and (Lzfe974fe28f))
}
Lz35a4b35656
Lz96ab3b24a0
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lz62204fe114 "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lz8af4d1f33c)) {
        try { Lz04f5d2a870 } catch {}
        if (Lz8af4d1f33c) { Lzdf83df0b7e 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lzdf83df0b7e 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lzdf83df0b7e 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ((Lz8af4d1f33c) -and -not (Test-Path (Lz286ff46c35))) {
        try { Lz504e042f21 } catch {}
    }
})
try {
    Add-Type -MemberDefinition '
