...
function Lzc00bc2376d {
    param([string]$Severity)
    Lza6aecde9a3 -Severity $Severity
    try {
...
function Lzc0847f895b { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lza24773e249 {
...
function Lz1bb750dd0f {
    ...
    if (-not $s) { $s = Lz33392f9830 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lzf15946b3a0 {
    param([string]$Value)
    if (Lza24773e249 -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lzfd6ed7769d {
    $f = @(Lz14c549b426)
    if ($f.Count -eq 1) { return $f[0] }
    return ''
}
function Lza8db185b06 {
...
function Lz26d8cba982 {
    param([string[]]$Picked)
    $result = New-Object System.Collections.Generic.List[string]
    foreach ($e in @(Lz14c549b426)) { [void]$result.Add($e) }
    $dropped = New-Object System.Collections.Generic.List[string]
    foreach ($p in @($Picked | ForEach-Object { "$_".Trim().Trim('/').Trim('\') } | Where-Object { $_ })) {
        if ($result -ccontains $p) { continue }
        $covered = $false
        foreach ($k in @($result)) { if (Lza8db185b06 -Child $p -Parent $k) { [void]$dropped.Add($p); $covered = $true; break } }
        if ($covered) { continue }
        foreach ($narrow in @($result | Where-Object { Lza8db185b06 -Child $_ -Parent $p -Strict })) { [void]$result.Remove($narrow); [void]$dropped.Add($narrow) }
        [void]$result.Add($p)
    }
    Lzbbc26048b2 -Folders @($result)
    if ($dropped.Count) { Lz37efaaf6b8 ("Overlapping folder(s) merged, kept the broader one: " + ((@($dropped) | Select-Object -Unique) -join ', ')) }
}
function Lzb570ab0a10 {
    $one = $true; try { $one = (@(Lz3a05a268f1).Count -le 1) } catch {}
    $n = @(Lz14c549b426).Count
...
function Lzee4ac45bf1 {
    ...
    try {
        ...
        $norm = Lz514350c4f5 -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lz37efaaf6b8 "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz84140713f0
    } catch { }
}
function Lz514350c4f5 {
...
function Lzea4e925d9e {
    ...
    return @{ SubfolderCount = @(Lza88a707fae -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lz3733d90659 {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lz9e17184f8b -Path "sites/$siteId"
    } catch { return $null }
}
function Lz71462f886b {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lz9e17184f8b -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lz7746f0cfc1 {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lz9e17184f8b -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lzdbae37d4da {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lz135408a127 -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
function Lz8e828fa043 {
    param($State, [string]$Current)
    $out = New-Object System.Collections.Generic.List[string]
    foreach ($s in @($State.Lst.SelectedItems)) {
        $name = "$s"
        if (-not (Lz1d30ca4dab -Item $name)) { continue }
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lz71462f886b -Search $s.Txt.Text)
...
"@
    ...
    Lzdbae37d4da
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lzdbae37d4da } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lzdbae37d4da } })
    $new.Add_Click({
        $n = Lz29993345df -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lz8e83f45a10 -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lzdbae37d4da; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lzd5f4b59d2b -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lz362b73d0f6 -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lzd5f4b59d2b -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lz5574a363a2 {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lza88a707fae -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lz5574a363a2
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lz5574a363a2 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lz5574a363a2 } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lzd5f4b59d2b -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lz8e828fa043 -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lzd5f4b59d2b -State $script:SP
    $r = $w.ShowDialog()
...
function Lz37efaaf6b8 { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lzeb2d1d17ef { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lz2f0c0e3627 {
    ...
    if ($hasRegime) {
        $etaA = Lz510bc8808d -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz510bc8808d -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lz510bc8808d -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz510bc8808d -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lz4a02378fe9 {
    ...
    try {
        ...
        $etaText = Lz2f0c0e3627 -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lz50b728fca2 {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lze12b27db41 -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lz065d488374 -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lz4a02378fe9 -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lz069b3f6c9b $Line) + "`r`n")
...
function Lz00f5d6a5d7 { param([string]$Text) Lz50b728fca2 $Text }
function Lz3b9f570e81 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lzca04605905 $true 'Connecting to Microsoft 365...'
        Lzcafe3ded1b
    } catch {
        ...
        Lzca04605905 $false; return
    } finally { Lzca04605905 $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lz9f44025a50 -StartSearch $seed
    if ($picked) {
        ...
        Lz37efaaf6b8 "Site set to $picked"
        Lz84140713f0
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lz3b9f570e81 }
    Lz84140713f0
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lzda8da074e3 }
    Lz84140713f0
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz84140713f0 })
$ctrl.TxtFolder.Add_TextChanged({ Lz84140713f0 })
if ($ctrl.LstSource) { $ctrl.LstSource.Add_SelectionChanged({ Lzb570ab0a10 }) }
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz84140713f0 }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz84140713f0 }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lzcb4557dcd1 -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lzee4ac45bf1 })
$ctrl.TxtLoc.Add_TextChanged({ Lz84140713f0 })
$ctrl.TxtLib.Add_TextChanged({ Lz84140713f0 })
$script:NestSuspend = $false
function Lz1e48af78d7 {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lz9a5390a11b -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lz37efaaf6b8 $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lz1e48af78d7 })
$ctrl.ChkNest.Add_Unchecked({ Lz1e48af78d7 })
$script:ProjectRows = @()
$script:RunStatus = @{}
...
function Lzd80d5b6d77 {
    ...
    if (-not $hasProjects) {
        $sd = $false
        if (Get-Command Test-ConnectionConfigured -ErrorAction SilentlyContinue) { try { $sd = [bool](Lzbb8ea1758f) } catch {} }
        Lz6c46f5a05f $sd
    }
...
function Lzed22d20175 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lzea4e925d9e -ProjectId $p.Id -SubPath $sub) }
...
function Lz887cde9b4f {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lz9e17184f8b -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lza93e590aac {
    ...
    if (($EngineArgs -contains 'Transfer') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck') -or ($EngineArgs -contains 'PreFlight')) {
        try { Lzca04605905 $true 'Checking source folders...'; $srcOk = Lzed22d20175 } finally { Lzca04605905 $false }
        if (-not $srcOk) { return }
    }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lzca04605905 $true 'Checking destinations...'; $dstOk = Lz887cde9b4f } finally { Lzca04605905 $false }
        if (-not $dstOk) { return }
    }
    ...
    $script:Timer.Add_Tick({
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lz50b728fca2 $lines[$k] } catch {} }
                }
            }
        } catch {}
        ...
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lz50b728fca2 $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lz50b728fca2 $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Format-Span ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lz72b8d15888) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lz7291201f39 -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lz50b728fca2 "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lz50b728fca2 "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lz37efaaf6b8 ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
            ...
            Lz09aa354f62
            if (-not $script:Paused) { Lzc00bc2376d -Severity $sev }
            if ($script:RunStatus.Count) { try { Lzd80d5b6d77 } catch {} }
            try {
            ...
            try {
                if ($script:CurLabel -eq 'Check readiness' -or $script:CurLabel -eq 'Preview (no upload)') {
                    $newest = Get-ChildItem (Join-Path (Lz72b8d15888) 'report-*.html') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
function Lz5cc13c0dc1 { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lz04575ca151 { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lzacba09f406 {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lze72a69be5e -ConfigFile $s.Tag; Lz52bbcacf1f } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lzc46c9f97cf { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lzc0847f895b -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lzacba09f406
}
function Lzb0b2b6c909 {
...
function Lze72a69be5e {
    param([string]$ConfigFile)
    if ($script:Sched) { Lzffc472d373 -Why 'Schedule cancelled (job changed).' }
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lz04575ca151 $true
    Lz5cc13c0dc1 $true
    Lzc46c9f97cf $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lzb0b2b6c909 -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lz37efaaf6b8 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
}
function Lz6db51fddf8 {
    param([string]$Dir)
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lz29993345df -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Lz052a275c67 -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Open it instead, or choose another name.")); return }
    if ((Test-Path $jobDir) -and -not (Lz6db51fddf8 -Dir $jobDir)) {
        ...
        Lz9a5390a11b -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lze72a69be5e -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuJobOpen.Add_Click({
    ...
    [void]$w.ShowDialog()
    if ($script:OpenPick) { Lze72a69be5e -ConfigFile $script:OpenPick; Lz52bbcacf1f }
})
$ctrl.MnuJobSave.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')); return }
    if (Save-Mapping) { Lz37efaaf6b8 'Migration job saved.' }
})
$ctrl.MnuJobSaveAs.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open a migration job first, then Save As to copy it.')); return }
    try {
        $name = Lz29993345df -Prompt "New name for the copy:" -Title 'Save job as'
        if (-not $name -or -not $name.Trim()) { return }
        $name = $name.Trim()
        $slug = ($name -replace '[^A-Za-z0-9._-]','_').Trim('_'); if (-not $slug) { $slug = 'job' }
        $jobDir = Join-Path $script:JobsRoot $slug
        if (Lz052a275c67 -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Choose another name.")); return }
        if ((Test-Path $jobDir) -and -not (Lz6db51fddf8 -Dir $jobDir)) {
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz9f194f65ed })
$ctrl.MnuSettingsApi.Add_Click({ Lz0951768c83 })
$ctrl.MnuSettingsWizard.Add_Click({ Lz0f17d36d58 })
$ctrl.MnuSettingsEmail.Add_Click({ Lz8451111fc1 })
$ctrl.MnuSettingsTuning.Add_Click({ Lz91ebecc69b })
$ctrl.BtnFilters.Add_Click({ Lzc0bbbfa8cf })
$ctrl.MnuSettingsDecommission.Add_Click({ Lz9b7bafb533 })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lz5c5e90fe2b -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lz217eb3481f {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lz33392f9830 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lz217eb3481f "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lz217eb3481f (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lzca04605905 $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lzca04605905 $false }
    if (-not $b) { return }
...
function Lzabda0f0ac1 { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lz2a99213409 {
    $path = Lzabda0f0ac1
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Liscaragh Migrate" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lz2a99213409)) -Caption ('Licence')) | Out-Null })
function Lzd3e1a196e6 {
...
function Lz3d8c05657e {
    param([string]$For = 'copy')
    if (Test-Path (Lzabda0f0ac1)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        try {
            ...
            Lzc0847f895b -Name 'LicenceAppliedId'  -Value "$($lpl.LicenceId)"
            Lzc0847f895b -Name 'LicenceAppliedUtc' -Value ((Get-Date).ToUniversalTime().ToString('o'))
        } catch {}
        (Show-Msg -Text ("Licence installed.`n`n" + (Lz2a99213409)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-FullReinstall {
    ...
    Lzca04605905 $true "Downloading the installer for version $($Payload.version)..."
    ...
    catch { Lzca04605905 $false; (Show-Msg -Text ("The installer could not be downloaded, so nothing has been changed.`n`n$($_.Exception.Message)") -Caption ('Reinstall') -Icon ('Warning')) | Out-Null; return }
...
function Invoke-UpdateCheck {
    ...
    try {
        ...
        if (-not $Silent) { Lzca04605905 $true 'Checking for updates...' }
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lzca04605905 $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lz5c5e90fe2b -Title 'About' -Heading 'Liscaragh Migrate' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lz1bb750dd0f } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lz114ac265e8 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lz29993345df -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lz0a244992f6 {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lz04575ca151 $false
    Lz5cc13c0dc1 $false
}
$ctrl.MnuJobClose.Add_Click({ Lz0a244992f6; Lz37efaaf6b8 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lz0a244992f6
    if (Lz6db51fddf8 -Dir $dir) {
        Lzacba09f406; Lz37efaaf6b8 "Deleted job '$nm'"
    } else {
        Lzacba09f406
        ...
        Lz37efaaf6b8 "Could not fully delete job '$nm' (a file is in use)"
    }
})
function Lz74db51a226 {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lzca04605905 $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lz00f5d6a5d7 'Signing in to Datto and listing projects...'
        $script:Projects = Lz4b36c06e62
        Lz00f5d6a5d7 "Found $($script:Projects.Count) project(s)."
        Lz2c1a544ddc
        try { Lzcafe3ded1b; Lz00f5d6a5d7 'Connected to Microsoft 365.' } catch { Lz00f5d6a5d7 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lz00f5d6a5d7 "Technical detail: $($_.Exception.Message)" }
        Lzd80d5b6d77
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lz37efaaf6b8 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lz00f5d6a5d7 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lz00f5d6a5d7 "Technical detail: $($_.Exception.Message)"
            Lz37efaaf6b8 "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lz37efaaf6b8 'Connect failed'
        }
    }
    finally { Lzca04605905 $false }
}
$ctrl.BtnConnect.Add_Click({ Lz74db51a226 })
function Lz52bbcacf1f {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lz1bb750dd0f } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lz37efaaf6b8 "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lz74db51a226 -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lz0129407b32
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lz3a05a268f1).Count } catch {}
    ...
    if ($script:Map.ContainsKey($p.Id)) {
        ...
        $ctrl.TxtLoc.Text = Lzda8da074e3
        $ctrl.TxtLib.Text = ''; $ctrl.TxtFolder.Text = ''; Lzbbc26048b2 -Folders @()
...
function Lzd432e617c0 {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lz89fcd0eaf9) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lz89fcd0eaf9) }
}
$ctrl.BtnApply.Add_Click({
    $p = Lz0129407b32
    if (-not $p) { (Show-Msg -Text ('Select a project first.')); return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Remove($p.Id) | Out-Null; Lzd80d5b6d77; [void](Write-MappingQuiet); return }
    $d = Lzd432e617c0
    if ($d -eq 'ERR') { return }
    ...
    Lz37efaaf6b8 "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lze2c7c028d1 })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lz339a717fe7 -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Lzd80d5b6d77; [void](Write-MappingQuiet); Lz37efaaf6b8 "Set $($sel.Count) project(s) to skip"; return }
    $d = Lzd432e617c0
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lz3b9f570e81); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPaths=$keepSrcList; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Lzd80d5b6d77
    [void](Write-MappingQuiet)
    Lz37efaaf6b8 "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lz339a717fe7 -Count (@($script:Projects).Count))) { return }
    $d = Lzd432e617c0
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Lzd80d5b6d77; [void](Write-MappingQuiet); Lz37efaaf6b8 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Lzd80d5b6d77
    [void](Write-MappingQuiet)
    Lz37efaaf6b8 "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lzca04605905 $true 'Looking up libraries...'
        $libs = Lz97c59dd1e9 -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lzca04605905 $false; return }
    finally { Lzca04605905 $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz2bde77112f -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lz37efaaf6b8 "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lz37efaaf6b8 "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lzca04605905 $true 'Opening destination...'
        Lzcafe3ded1b
        $res = Lz7746f0cfc1
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lzca04605905 $false; return }
    finally { Lzca04605905 $false }
    $picked = Lz92280248ca -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lz37efaaf6b8 ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnAddSource) { $ctrl.BtnAddSource.Add_Click({
    ...
    try { $picked = Lz7266114b7c -ProjectId $p.Id -ProjectName $p.Name }
    ...
    Lz26d8cba982 -Picked $arr
    Lz37efaaf6b8 ("Source folders: " + (@(Lz14c549b426) -join ', '))
}) }
if ($ctrl.BtnRemoveSource) { $ctrl.BtnRemoveSource.Add_Click({
    ...
    Lzbbc26048b2 -Folders @(Lz14c549b426 | Where-Object { $sel -notcontains $_ })
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    $bad = New-Object System.Collections.Generic.List[string]
    try {
        Lzca04605905 $true 'Checking the source folder(s) in Datto...'
        foreach ($s in $subs) { try { $null = Lzea4e925d9e -ProjectId $p.Id -SubPath $s } catch { [void]$bad.Add($s) } }
    } catch { $ctrl.LblSourceCheck.Text = "Could not check the source: $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lzca04605905 $false; return }
    finally { Lzca04605905 $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lzca04605905 $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lz9e17184f8b -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lz97c59dd1e9 -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lz84140713f0 {
    ...
    $srcLabel = Lz0674bce8c4 -Project $p
    $folders  = @(Lz14c549b426)
    $srcSub   = Lzfd6ed7769d
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lz89fcd0eaf9) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lz3a05a268f1).Count } catch { }
...
function Lz699cb8c8de {
    ...
    $script:Map[$p.Id] = $d
    Lzd80d5b6d77
    Lz37efaaf6b8 "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lz00f5d6a5d7 "Saved $n mapping(s)."
    return $true
}
function Lz2c1a544ddc {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lz00f5d6a5d7 "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lz00f5d6a5d7 "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lza93e590aac -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lza93e590aac -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lz47daf85591 {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lz47daf85591) }
    ...
    if (-not (Lz5c5e90fe2b -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lz10a0507939)) { return }
    if (-not (Lz3d8c05657e)) { return }
    Lza93e590aac -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lz1db2f3f000 {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lz47daf85591) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lz10a0507939)) { return }
    if (-not (Lz3d8c05657e)) { return }
    Lza93e590aac -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnRerunFailed.Add_Click({
    ...
    if (-not (Lz5c5e90fe2b -Title 'Rerun failed files only' -Heading 'Rerun failed files only' -Sections $sections -Yes 'Rerun the failed files' -No 'Cancel')) { return }
    if (-not (Lz10a0507939)) { return }
    if (-not (Lz3d8c05657e)) { return }
    Lza93e590aac -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','NewerWins','-Execute','-FailedOnly') -Label 'Rerun failed files only'
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lz5c5e90fe2b -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lz3d8c05657e -For 'verify')) { return }
    Lza93e590aac -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lza93e590aac -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lz72b8d15888 {
...
function Lz09aa354f62 {
    try { if ($ctrl.BtnRerunFailed) { $ctrl.BtnRerunFailed.IsEnabled = (Lz1d653e2953) } } catch {}
}
function Lzf1b995fd50 {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lz656b3a50b5 {
    param([string]$Name)
...
function Lz9ce95eca77 {
    ...
    foreach ($f in $files) { $l = Lz656b3a50b5 $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lz9ce95eca77 -Folder (Lz72b8d15888) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnCertificate.Add_Click({
    if (Save-Mapping) {
        Lza93e590aac -EngineArgs @('-Action','Certificate') -Label 'Completion certificate' -OnComplete {
            $rr = Lz72b8d15888
            $cert = $null
...
$ctrl.BtnOpenAudit.Add_Click({ Lz9ce95eca77 -Folder (Lzf1b995fd50) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lz1bc7701ff1 {
    ...
    Lz37efaaf6b8 "Speed limit set: up $uTxt, down $dTxt"
    Lz50b728fca2 "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lz1bc7701ff1 })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz1bc7701ff1 } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz1bc7701ff1 } })
function Lz12ea3a4b33 {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lz12ea3a4b33 -ParentId ([int]$_.ProcessId) } } catch {}
...
function Lz52fdb83d6c {
    $r = ($script:Proc -and -not $script:Proc.HasExited)
    $f = ($script:FinProc -and -not $script:FinProc.HasExited)
    return ($r -or $f)
}
function Lz80e6a1806d { param([string]$T) try { $ctrl.BtnSchedule.Content.Text = $T } catch {} }
function Lzffc472d373 {
    param([string]$Why = '')
    if ($script:SchedTimer) { try { $script:SchedTimer.Stop() } catch {}; $script:SchedTimer = $null }
    $script:Sched = $null
    Lz80e6a1806d 'Schedule for later / overnight...'
    if ($Why) { Lz37efaaf6b8 $Why }
}
function Lz4cb55a8504 {
    try { $p = Join-Path (Lz72b8d15888) 'lastrun-outcome.json'; if (Test-Path $p) { return (Get-Content $p -Raw | ConvertFrom-Json) } } catch {}
    return $null
}
function Lzeccf12290d {
    if (-not $script:Proc -or $script:Proc.HasExited) { return }
    $script:QuietPausing = $true
    $ctrl.BtnStop.IsEnabled = $false
    Lz50b728fca2 "----- Quiet hours reached at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')): pausing the copy. Files already uploaded are kept. -----"
    Lz37efaaf6b8 'Quiet hours: pausing until the next window...'
    try { Lz12ea3a4b33 -ParentId ([int]$script:Proc.Id) } catch {}
    Lzb0b2b6c909 -Status Cancelled -Note 'Finalising for quiet hours...' -OnDone {
        try { Lz50b728fca2 '----- Paused for quiet hours. It will resume as a Sync at the next window while this app stays open. -----' } catch {}
    }
}
function Lz1a20e8ae3d {
    param([string[]]$RunArgs,[string]$Label)
    Lz50b728fca2 "----- Scheduled: starting '$Label' at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')) -----"
    Lza93e590aac -EngineArgs $RunArgs -Label $Label
    return ($script:Proc -and -not $script:Proc.HasExited)
}
function Lz230ae032ce { param([string]$Status)
    ...
    switch ("$($s.State)") {
        'waiting' {
            if ($now -ge $s.StartAt -and -not $busy) {
                ...
                elseif (Lz1a20e8ae3d -RunArgs $s.Args -Label $s.Label) { $s.State = 'running'; $s.Launches++ }
                else { Lzffc472d373 -Why 'The scheduled run could not start (see the message). Schedule cancelled.' }
            } else {
                $left = $s.StartAt - $now; if ($left.Ticks -lt 0) { $left = [TimeSpan]::Zero }
                Lz80e6a1806d ("Scheduled: starts $($s.StartAt.ToString('ddd HH:mm')) ($(Format-Span $left) to go) - click to cancel")
            }
        }
        'running' {
            if (-not $busy) {
                $oc = Lz4cb55a8504; $status = if ($oc) { "$($oc.Status)" } else { '' }
                if ($s.Mode -eq 'once') { Lzffc472d373 -Why 'Scheduled run finished.'; Lz230ae032ce $status }
                elseif ($status -eq 'COMPLETED') { Lzffc472d373 -Why 'Scheduled migration completed.'; Lz230ae032ce $status }
                else { $s.State = 'paused' }
            } elseif ($s.Mode -eq 'window' -and -not (Test-InWindow $m $s.FromMin $s.ToMin)) {
                Lzeccf12290d; $s.State = 'paused'
            } else {
                $until = if ($s.Mode -eq 'window') { " (until $(Format-MinOfDay $s.ToMin))" } else { '' }
                Lz80e6a1806d ("Scheduled: running$until - click to cancel")
            }
        }
        'paused' {
            if ((Test-InWindow $m $s.FromMin $s.ToMin) -and -not $busy) {
                if (Lz1a20e8ae3d -RunArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','AddMissing','-Execute') -Label 'Sync new and changed') { $s.State = 'running'; $s.Launches++ }
            } else {
                $nf = Get-NextWindowOpen $s.FromMin
                Lz80e6a1806d ("Scheduled: waiting for the next window ($($nf.ToString('ddd HH:mm'))) - click to cancel")
            }
        }
    }
}
function Lzed21dcfc78 {
...
$ctrl.BtnSchedule.Add_Click({
    if ($script:Sched) {
        if ((Show-Msg -Text ("A schedule is set:" + [Environment]::NewLine + $script:Sched.Summary + [Environment]::NewLine + [Environment]::NewLine + "Cancel it?") -Caption ('Scheduled run') -Buttons ('YesNo') -Icon ('Question')) -eq 'Yes') { Lzffc472d373 -Why 'Schedule cancelled.' }
        return
    }
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')) | Out-Null; return }
    if (Lz52fdb83d6c) { (Show-Msg -Text ('A run is in progress. Wait for it to finish, then set a schedule.') -Caption ('Cannot schedule now') -Icon ('Warning')) | Out-Null; return }
    if (-not (Save-Mapping)) { return }
    $mappedCount = @($script:Projects | Where-Object { $script:Map.ContainsKey($_.Id) }).Count
    ...
    $script:SchedTimer.Add_Tick({ try { Lz9ae5510afb } catch {} })
    $script:SchedTimer.Start()
    try { Lz9ae5510afb } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    if ($script:Sched) { Lzffc472d373 -Why 'Schedule cancelled (you stopped the run).' }
    $script:Stopping = $true
    $ctrl.BtnStop.IsEnabled = $false
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz50b728fca2 "----- STOP requested by user at $stamp -----"
    Lz37efaaf6b8 "Stopping $($script:CurLabel)..."
    try { Lz12ea3a4b33 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lzb0b2b6c909 -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lz50b728fca2 "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lzb8ba3a72b6 {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lza93e590aac -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz50b728fca2 "----- PAUSE requested by user at $stamp -----"
    Lz37efaaf6b8 "Pausing $($script:CurLabel)..."
    try { Lz12ea3a4b33 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lzb0b2b6c909 -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lz50b728fca2 "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lzfd8bd1a681 { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lz6e2440765e {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lz0f0454ae2c {
...
function Lz114ac265e8 {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lz7c41860d29 -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lz0951768c83 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    ...
    function Lzf71a27ffba { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lze22eda8541 { param($Label,$Control,$Info)
    ...
    function Lzb25967c5ca { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lzf71a27ffba 'Datto Workplace'
    ...
    $parts = Lz6e2440765e ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lze22eda8541 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lze22eda8541 'Cell (number)' (Lzb25967c5ca ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lze22eda8541 'Datto Client ID' (Lzb25967c5ca ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lze22eda8541 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lzf71a27ffba 'Microsoft 365'
    $tbTid = Lze22eda8541 'Tenant ID' (Lzb25967c5ca ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lze22eda8541 'App (Client) ID' (Lzb25967c5ca ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lzf71a27ffba 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lze22eda8541 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lze22eda8541 '.pfx file' (Lzb25967c5ca '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lze22eda8541 '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lzf71a27ffba 'SharePoint and OneDrive'
    ...
    $tbSp  = Lze22eda8541 'SharePoint root URL' (Lzb25967c5ca ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lze22eda8541 'OneDrive host URL'   (Lzb25967c5ca ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lze22eda8541 'Team site base URL'  (Lzb25967c5ca ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lze22eda8541 'Default site URL'    (Lzb25967c5ca ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lze22eda8541 'Email / UPN domain'  (Lzb25967c5ca ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lzc0847f895b -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lz114ac265e8 -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz16338b0dbb $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lzfd8bd1a681 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lz1bb750dd0f }
        ...
        try {
            ...
            else { $tok=Lz114ac265e8 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lzfd8bd1a681 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lzc0847f895b -Name 'DattoEndpointUrl' -Value $ep
            Lzc0847f895b -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lzc0847f895b -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lzc0847f895b -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lzc0847f895b -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lzc0847f895b -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lzc0847f895b -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lzc0847f895b -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lzc0847f895b -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lzf15946b3a0 -Value $script:AS.pbSec.Password
            }
...
function Lzb49bb3a08d { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lz9a5390a11b { param($Cfg,[string]$Path,$Value)
...
function Lzc0bbbfa8cf {
    ...
    $curMax=Lzb49bb3a08d -Cfg $cfg -Path 'run.tuning.maxFileSizeMB'; if ($null -eq $curMax) { $curMax=0 }; $sizeBox.Text="$curMax"
    ...
    $curInc=@(Lzb49bb3a08d -Cfg $cfg -Path 'run.tuning.includePatterns' | Where-Object { "$_".Trim() })
    $curExc=@(Lzb49bb3a08d -Cfg $cfg -Path 'run.tuning.excludePatterns' | Where-Object { "$_".Trim() })
    ...
    $afterPick=New-Object System.Windows.Controls.DatePicker; $afterPick.Margin='0,4,0,4'; $afterPick.Width=150; $afterPick.SelectedDate=(& $toDate (Lzb49bb3a08d -Cfg $cfg -Path 'run.tuning.modifiedAfter'))
    ...
    $beforePick=New-Object System.Windows.Controls.DatePicker; $beforePick.Margin='0,4,0,4'; $beforePick.Width=150; $beforePick.SelectedDate=(& $toDate (Lzb49bb3a08d -Cfg $cfg -Path 'run.tuning.modifiedBefore'))
    ...
    $save.Add_Click({
        try {
            ...
            Lz9a5390a11b -Cfg $script:FltCfg -Path 'run.tuning.maxFileSizeMB' -Value $sv
            $pats=@("$($script:FltPat.Text)" -split '[;,\r\n]' | ForEach-Object { $_.Trim() } | Where-Object { $_ })
            if ($script:FltInc.IsChecked) { Lz9a5390a11b -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value $pats; Lz9a5390a11b -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value @() }
            else                          { Lz9a5390a11b -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value $pats; Lz9a5390a11b -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value @() }
            ...
            Lz9a5390a11b -Cfg $script:FltCfg -Path 'run.tuning.modifiedAfter'  -Value $ma
            Lz9a5390a11b -Cfg $script:FltCfg -Path 'run.tuning.modifiedBefore' -Value $mb
...
function Lz91ebecc69b {
    ...
    function Lz0ad8c8d668 { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec,[string]$Type='text')
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lzb49bb3a08d -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lz0ad8c8d668 -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec -Type $s.Type
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lz9a5390a11b -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lz9a5390a11b -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lz9a5390a11b -Cfg $script:SetCfg -Path $s.Path -Value ([bool]$script:SetBoxes[$s.Path].IsChecked) }
                elseif ($s.Type -eq 'date') {
                    ...
                    Lz9a5390a11b -Cfg $script:SetCfg -Path $s.Path -Value $raw
                }
                else { Lz9a5390a11b -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lzd542f77652 {
    ...
    function Lz4e35dc128c { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lz6e2440765e ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz4e35dc128c -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lz4e35dc128c -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lz4e35dc128c -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lz4e35dc128c -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lzfd8bd1a681 $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lz1bb750dd0f }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lzc0847f895b -Name 'DattoEndpointUrl' -Value $ep
            Lzc0847f895b -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lzf15946b3a0 -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz9f194f65ed {
    ...
    $pre = Lz891d3783fd -Title 'Prerequisites'
    ...
    $dat = Lz891d3783fd -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lz7c49a962f2 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lz891d3783fd -Title 'Microsoft 365' -Buttons @($bMs, (Lz7c49a962f2 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lz891d3783fd -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lzd542f77652 } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz2319a71513; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lz0f17d36d58 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz2319a71513; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lz8451111fc1 } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz2319a71513; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lz2319a71513
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lzc4bd92b81f {
...
function Lz792a21e201 {
    ...
    $bOpen.Add_Click({ try { Lz756cdff331 -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lzc1e759351c {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lz10096d8940 {
    ...
    Lz792a21e201 -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lz756cdff331 -Url $pageUrl
    ...
    } finally { Lzc1e759351c }
}
function Lzd4e442d38e {
    try {
...
function Lz3de18740c3 {
    if (Lz16e9bc3052) { return $false }
...
function Lz0a88d7805b {
    ...
    if (-not (Get-Module Microsoft.Graph.Authentication -ListAvailable)) {
        Lzc4bd92b81f 'The Microsoft Graph module is not installed. Re-run the installer (or run: Install-Module Microsoft.Graph.Authentication -Scope CurrentUser) and try again.' '#B42318' -Strong
        return
    }
    Lzc4bd92b81f 'Preparing the Microsoft 365 sign-in. The first time on a new install can take up to a minute while Windows loads the components - please wait, the app has not frozen.' '#475467' -Strong
    try { Import-Module Microsoft.Graph.Authentication -ErrorAction Stop } catch {
        Lzc4bd92b81f "Could not load the Microsoft Graph module: $($_.Exception.Message)" '#B42318' -Strong
        return
    }
    Lzc4bd92b81f "Sign in with an account that can manage Microsoft 365 (a Global Administrator is simplest). The first time, Microsoft asks you to accept the permissions this setup itself needs; that is expected.$(if ($ForEmail) { ' Email alerts are included, so expect TWO sign-ins: this one, then one for Exchange Online (a separate service with its own sign-in).' })"
    ...
    if (Lz3de18740c3) {
        Lzc4bd92b81f 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B5460F' -Strong
        try {
            Lz1542f9e267 -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lzc4bd92b81f "The browser sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B5460F' -Strong
        }
    } else {
        Lzc4bd92b81f 'Using the code sign-in (the reliable path): a small window will show your code to type at microsoft.com/devicelogin, or on your phone. It stays on top and closes itself when you are done.' '#B5460F' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lz10096d8940 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lzc4bd92b81f $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lz7349ea8e3f -Token $at -Claim 'tid'
            $wizTokAcct = Lz7349ea8e3f -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lz7349ea8e3f -Token $at -Claim 'preferred_username' }
            ...
            Lzc4bd92b81f "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lzc4bd92b81f 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lzc4bd92b81f "Signed in to tenant $tid as $wizAcct." '#0E9F6E'
    $newThumb = $null
    try {
        Lzc4bd92b81f 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lzc4bd92b81f "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B5460F' }
            Lzc4bd92b81f "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#0E9F6E'
        } else {
            Lzc4bd92b81f "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lzc4bd92b81f 'App registration created.' '#0E9F6E'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lzc4bd92b81f 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lzc4bd92b81f "Certificate created in this user's store. Thumbprint $newThumb." '#0E9F6E'
        Lzc4bd92b81f 'Uploading the certificate''s public key to the app registration...'
        ...
        Lzc4bd92b81f "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#0E9F6E'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lz4e744f9fc3 -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lzc4bd92b81f 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lz4e744f9fc3 -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lzc4bd92b81f $consentMsg '#0E9F6E'
        Lzc0847f895b -Name 'TenantId'       -Value $tid
        Lzc0847f895b -Name 'GraphClientId'  -Value $appId
        Lzc0847f895b -Name 'CertThumbprint' -Value $newThumb
        Lzc4bd92b81f 'Connection details saved on this computer.' '#0E9F6E'
    } catch {
        ...
        if ($friendly) { Lzc4bd92b81f $friendly '#B42318' -Strong; Lzc4bd92b81f "Technical detail: $m" }
        else { Lzc4bd92b81f "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lzc4bd92b81f "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lzc4bd92b81f 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lz114ac265e8 -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lzc0847f895b -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lz0f0454ae2c $spUrl
            if ($der) {
                Lzc0847f895b -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lzc0847f895b -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lzc0847f895b -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lzc4bd92b81f "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#0E9F6E'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lz4e744f9fc3 -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lzc4bd92b81f "The new sign-in has not become active yet: $($_.Exception.Message)" '#B5460F' -Strong
                   Lzc4bd92b81f 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz16338b0dbb $org
            if ($u) { Lzc0847f895b -Name 'UpnDomain' -Value $u; Lzc4bd92b81f "Email domain detected: $u" '#0E9F6E' }
        } catch { Lzc4bd92b81f 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lzc4bd92b81f 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#0E9F6E' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lz0f17d36d58 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lza00d3a40f2 $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lz0a88d7805b -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lz220cd8f47d) {
                    Lzc4bd92b81f 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lzc4bd92b81f $t $c } else { Lzc4bd92b81f $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lzc4bd92b81f 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B5460F'
                }
            }
            ...
            Lzc4bd92b81f "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lz10096d8940 -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lz1542f9e267 -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lz756cdff331 -Url 'https://login.microsoft.com/device'
                Lz361e0df934
                try { Lz1542f9e267 -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lzd1efbc0d4f }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (Lz3de18740c3) {
            ...
            try {
                Lz1542f9e267 -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lz10096d8940 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lz7349ea8e3f -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lz7349ea8e3f -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lz4e744f9fc3 -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lz45b7e4bbee 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lz4e744f9fc3 -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lz4e744f9fc3 -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lzc0847f895b -Name 'EmailSender' -Value $senderAddr
        Lzc0847f895b -Name 'EmailEnabled' -Value '1'
...
function Lz8451111fc1 {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lz4867b99df0 { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz78a3222fb4 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lz78a3222fb4 -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lz78a3222fb4 -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lz78a3222fb4 -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lz4867b99df0 'Subject and attachments'
    ...
    [void](Lz78a3222fb4 -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz711a00fb24 -Template $script:ES.tbSubj.Text -Vars (Lz42cba7a2fb)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz711a00fb24 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz42cba7a2fb)
            $tok = Lz114ac265e8 -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lzc0847f895b -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lzc0847f895b -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lzc0847f895b -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lzc0847f895b -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lzc0847f895b -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lzc0847f895b -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lzc0847f895b -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lzc0847f895b -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lzc0847f895b -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lzc0847f895b -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lzc0847f895b -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lzc0847f895b -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lzeb1daf07d4 {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lz10096d8940 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lz9b7bafb533 {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lzeb1daf07d4 -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lz2319a71513 } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lzbb8ea1758f {
    return ((Lza952c82814) -and (Lz220cd8f47d))
}
Lze1e5ffbf46
Lzd1efbc0d4f
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lz50b728fca2 "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lzbb8ea1758f)) {
        try { Lz9f194f65ed } catch {}
        if (Lzbb8ea1758f) { Lz37efaaf6b8 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lz37efaaf6b8 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lz37efaaf6b8 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ($ctrl.GettingStarted -and $ctrl.GettingStarted.Visibility -eq 'Visible') {
        try { Lz6c46f5a05f ([bool](Lzbb8ea1758f)) } catch {}
    }
    if ((Lzbb8ea1758f) -and -not (Test-Path (Lzabda0f0ac1))) {
        try { Lz536c52ec52 } catch {}
    }
