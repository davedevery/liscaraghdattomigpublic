...
function Lzc71e8ef802 { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lzb47f6217f1 {
...
function Lz44632f89d2 {
    ...
    if (-not $s) { $s = Lzadfc57d92c 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lza162a911ff {
    param([string]$Value)
    if (Lzb47f6217f1 -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lz183a255d4e {
    ...
    return (Lz9772d45404 -TargetSubFolder $box -SourceSubPath $src)
}
function Lz0d459a03bb {
    ...
    try {
        ...
        $norm = Lz9772d45404 -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lz88f11424ec "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lzc3f3830cb5
    } catch { }
}
function Lz9772d45404 {
...
function Lze6978a2715 {
    ...
    return @{ SubfolderCount = @(Lz931886aebf -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lz47a2735702 {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lz20caad18af -Path "sites/$siteId"
    } catch { return $null }
}
function Lzfa23ab9833 {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lz20caad18af -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lz7f31a835c0 {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lz20caad18af -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lzeb17c8ac2f {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lz0ec49a9476 -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lzfa23ab9833 -Search $s.Txt.Text)
...
"@
    ...
    Lzeb17c8ac2f
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lzeb17c8ac2f } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lzeb17c8ac2f } })
    $new.Add_Click({
        $n = Lz4b80fa26e1 -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lz4d1a6a2473 -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lzeb17c8ac2f; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lzfcfd44cc1f -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lz7971fada3b -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lzfcfd44cc1f -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lz63c8add442 {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lz931886aebf -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lz63c8add442
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lz63c8add442 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lz63c8add442 } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lzfcfd44cc1f -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lz7971fada3b -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lzfcfd44cc1f -State $script:SP
    $r = $w.ShowDialog()
...
function Lz88f11424ec { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz2c2c3a8d3e { param([TimeSpan]$ts)
...
function Lz48d8b04973 { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lz51d7b48a66 {
    ...
    if ($hasRegime) {
        $etaA = Lze1b7714f47 -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lze1b7714f47 -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lze1b7714f47 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lze1b7714f47 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lz56ebd87ddd {
    ...
    try {
        ...
        $etaText = Lz51d7b48a66 -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lz4edcf0379f {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lz2cd25b567a -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lz50708b1b07 -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lz56ebd87ddd -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lz07e3c73332 $Line) + "`r`n")
...
function Lz1dd25fc356 { param([string]$Text) Lz4edcf0379f $Text }
function Lzce7bb6a676 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lz797029ac3b $true 'Connecting to Microsoft 365...'
        Lzbe1fd63cec
    } catch {
        ...
        Lz797029ac3b $false; return
    } finally { Lz797029ac3b $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lz48eff8fa39 -StartSearch $seed
    if ($picked) {
        ...
        Lz88f11424ec "Site set to $picked"
        Lzc3f3830cb5
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lzce7bb6a676 }
    Lzc3f3830cb5
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lzdd8f19e000 }
    Lzc3f3830cb5
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lzc3f3830cb5 })
$ctrl.TxtFolder.Add_TextChanged({ Lzc3f3830cb5 })
$ctrl.TxtSourceSub.Add_TextChanged({ Lzc3f3830cb5 })
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lzc3f3830cb5 }); $ctrl.ChkSrcContents.Add_Unchecked({ Lzc3f3830cb5 }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lze55b344eef -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lz0d459a03bb })
$ctrl.TxtSourceSub.Add_LostFocus({ Lz0d459a03bb })
$ctrl.TxtLoc.Add_TextChanged({ Lzc3f3830cb5 })
$ctrl.TxtLib.Add_TextChanged({ Lzc3f3830cb5 })
$script:NestSuspend = $false
function Lzda88b5414a {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lz1b7a0da280 -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lz88f11424ec $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lzda88b5414a })
$ctrl.ChkNest.Add_Unchecked({ Lzda88b5414a })
$script:ProjectRows = @()
$script:RunStatus = @{}
function Lzb9e9b3037b {
...
function Lzfe4e2fd634 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lze6978a2715 -ProjectId $p.Id -SubPath $sub) }
...
function Lzc27c0d85e2 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lz20caad18af -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz40c921cdbf {
    ...
    try { Lz797029ac3b $true 'Checking source folders...'; $srcOk = Lzfe4e2fd634 } finally { Lz797029ac3b $false }
    if (-not $srcOk) { return }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lz797029ac3b $true 'Checking destinations...'; $dstOk = Lzc27c0d85e2 } finally { Lz797029ac3b $false }
        if (-not $dstOk) { return }
    }
        ...
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lz4edcf0379f $lines[$k] } catch {} }
                }
            }
        } catch {}
        $script:SpinIdx = ($script:SpinIdx + 1) % 4
        $spin = @('|','/','-','\')[$script:SpinIdx]
        $ctrl.LblElapsed.Text = "$spin  Elapsed " + (Lz2c2c3a8d3e ((Get-Date) - $script:RunStart))
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lz4edcf0379f $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lz4edcf0379f $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Lz2c2c3a8d3e ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lz36754ef3b4) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lz137ea1f56f -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lz4edcf0379f "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lz4edcf0379f "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lz88f11424ec ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
...
function Lz92e5dedcd9 { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lzd37d13343c { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lz608456e984 {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lz22e1b9430c -ConfigFile $s.Tag; Lz13ee333bff } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lz7843d3cf3a { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lzc71e8ef802 -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lz608456e984
}
function Lz6b09393260 {
...
function Lz22e1b9430c {
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lzd37d13343c $true
    Lz92e5dedcd9 $true
    Lz7843d3cf3a $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lz6b09393260 -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lz88f11424ec 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lz4b80fa26e1 -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Test-Path $jobDir) { (Show-Msg -Text ("A job folder '$slug' already exists. Open it instead, or choose another name.")); return }
        ...
        Lz1b7a0da280 -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lz22e1b9430c -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz1bdf43bea7 })
$ctrl.MnuSettingsApi.Add_Click({ Lz7299986f5c })
$ctrl.MnuSettingsWizard.Add_Click({ Lz2c934a83a9 })
$ctrl.MnuSettingsEmail.Add_Click({ Lz0578fdb785 })
$ctrl.MnuSettingsTuning.Add_Click({ Lz3cf94c491e })
$ctrl.MnuSettingsDecommission.Add_Click({ Lz0f162e58c2 })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lzc76bfa57d0 -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lz4ec9f0c510 {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lzadfc57d92c 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lz4ec9f0c510 "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lz4ec9f0c510 (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lz797029ac3b $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lz797029ac3b $false }
    if (-not $b) { return }
...
function Lz11b30c945e { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lzebcaf83ad0 {
    $path = Lz11b30c945e
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Datto Workplace to SharePoint Migrator" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lzebcaf83ad0)) -Caption ('Licence')) | Out-Null })
function Lz7729eb5fe9 {
...
function Lz0d25e4f9dd {
    param([string]$For = 'copy')
    if (Test-Path (Lz11b30c945e)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        (Show-Msg -Text ("Licence installed.`n`n" + (Lzebcaf83ad0)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-UpdateCheck {
    try {
        ...
        Lz797029ac3b $true 'Checking for updates...'
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lz797029ac3b $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lzc76bfa57d0 -Title 'About' -Heading 'Datto Workplace to SharePoint Migrator' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lz44632f89d2 } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lz48023de656 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lz4b80fa26e1 -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lze58fe778b2 {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lzd37d13343c $false
    Lz92e5dedcd9 $false
}
$ctrl.MnuJobClose.Add_Click({ Lze58fe778b2; Lz88f11424ec 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lze58fe778b2
    try { if ($dir -and (Test-Path $dir)) { Remove-Item $dir -Recurse -Force -ErrorAction Stop } ; Lz608456e984; Lz88f11424ec "Deleted job '$nm'" }
    catch { (Show-Msg -Text ("Could not fully delete: $($_.Exception.Message)")) }
})
function Lz90125674b4 {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lz797029ac3b $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lz1dd25fc356 'Signing in to Datto and listing projects...'
        $script:Projects = Lz045ccd1e3a
        Lz1dd25fc356 "Found $($script:Projects.Count) project(s)."
        Lz68f6d50730
        try { Lzbe1fd63cec; Lz1dd25fc356 'Connected to Microsoft 365.' } catch { Lz1dd25fc356 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lz1dd25fc356 "Technical detail: $($_.Exception.Message)" }
        Update-ProjectList
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lz88f11424ec 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lz1dd25fc356 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lz1dd25fc356 "Technical detail: $($_.Exception.Message)"
            Lz88f11424ec "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lz88f11424ec 'Connect failed'
        }
    }
    finally { Lz797029ac3b $false }
}
$ctrl.BtnConnect.Add_Click({ Lz90125674b4 })
function Lz13ee333bff {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lz44632f89d2 } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lz88f11424ec "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lz90125674b4 -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lz2d2138094c
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lzecdebff618).Count } catch {}
...
function Lzbafa91272a {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lz183a255d4e) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lz183a255d4e) }
}
$ctrl.BtnApply.Add_Click({
    ...
    Lz88f11424ec "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lz2eca390d05 })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lz8738d07224 -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Update-ProjectList; [void](Write-MappingQuiet); Lz88f11424ec "Set $($sel.Count) project(s) to skip"; return }
    $d = Lzbafa91272a
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lzce7bb6a676); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Update-ProjectList
    [void](Write-MappingQuiet)
    Lz88f11424ec "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lz8738d07224 -Count (@($script:Projects).Count))) { return }
    $d = Lzbafa91272a
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Update-ProjectList; [void](Write-MappingQuiet); Lz88f11424ec 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Update-ProjectList
    [void](Write-MappingQuiet)
    Lz88f11424ec "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lz797029ac3b $true 'Looking up libraries...'
        $libs = Lz6c487c4777 -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lz797029ac3b $false; return }
    finally { Lz797029ac3b $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz3cf40057a2 -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lz88f11424ec "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lz88f11424ec "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lz797029ac3b $true 'Opening destination...'
        Lzbe1fd63cec
        $res = Lz7f31a835c0
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lz797029ac3b $false; return }
    finally { Lz797029ac3b $false }
    $picked = Lzfbdc9012f7 -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lz88f11424ec ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnBrowseSource) { $ctrl.BtnBrowseSource.Add_Click({
    ...
    try { $picked = Lz83e6bf30ba -ProjectId $p.Id -ProjectName $p.Name }
    catch { (Show-Msg -Text ("Could not open the project's folders: $($_.Exception.Message)") -Caption ('Browse source')); return }
    if ($null -ne $picked) { $ctrl.TxtSourceSub.Text = $picked; Lz88f11424ec ("Source set to " + $(if ($picked) { "/$picked" } else { 'the whole project' })) }
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    try {
        Lz797029ac3b $true 'Checking the source folder in Datto...'
        $res = Lze6978a2715 -ProjectId $p.Id -SubPath $sub
    } catch { $ctrl.LblSourceCheck.Text = "Source NOT found. $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lz797029ac3b $false; return }
    finally { Lz797029ac3b $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lz797029ac3b $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lz20caad18af -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lz6c487c4777 -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lzc3f3830cb5 {
    ...
    $srcLabel = Lz81e2bb8efa -Project $p
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lz183a255d4e) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lzecdebff618).Count } catch { }
...
function Lz3671264fe8 {
    ...
    $script:Map[$p.Id] = $d
    Update-ProjectList
    Lz88f11424ec "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lz1dd25fc356 "Saved $n mapping(s)."
    return $true
}
function Lz68f6d50730 {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lz1dd25fc356 "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lz1dd25fc356 "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lz40c921cdbf -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lz40c921cdbf -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lzf266d103a5 {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lzf266d103a5) }
    ...
    if (-not (Lzc76bfa57d0 -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lz0d25e4f9dd)) { return }
    Lz40c921cdbf -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lzbafc3aa93a {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lzf266d103a5) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lz0d25e4f9dd)) { return }
    Lz40c921cdbf -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lzc76bfa57d0 -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lz0d25e4f9dd -For 'verify')) { return }
    Lz40c921cdbf -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lz40c921cdbf -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lz36754ef3b4 {
...
function Lz9862de382a {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lz701ae19721 {
    param([string]$Name)
...
function Lzc02f66a8a7 {
    ...
    foreach ($f in $files) { $l = Lz701ae19721 $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lzc02f66a8a7 -Folder (Lz36754ef3b4) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnOpenAudit.Add_Click({ Lzc02f66a8a7 -Folder (Lz9862de382a) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lza21e328670 {
    ...
    Lz88f11424ec "Speed limit set: up $uTxt, down $dTxt"
    Lz4edcf0379f "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lza21e328670 })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lza21e328670 } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lza21e328670 } })
function Lzb9ce67661b {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lzb9ce67661b -ParentId ([int]$_.ProcessId) } } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    Lz4edcf0379f "----- STOP requested by user at $stamp -----"
    Lz88f11424ec "Stopping $($script:CurLabel)..."
    try { Lzb9ce67661b -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lz6b09393260 -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lz4edcf0379f "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lz7ceaeadff5 {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lz40c921cdbf -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz4edcf0379f "----- PAUSE requested by user at $stamp -----"
    Lz88f11424ec "Pausing $($script:CurLabel)..."
    try { Lzb9ce67661b -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lz6b09393260 -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lz4edcf0379f "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lza7e8808c25 { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lza938060406 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lza490b1275f {
...
function Lz48023de656 {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lzb0c1b6e09c -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lz7299986f5c {
    ...
    function Lz9dc5de85b0 { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz57424705e8 { param($Label,$Control,$Info)
    ...
    function Lz670247a96a { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lz9dc5de85b0 'Datto Workplace'
    ...
    $parts = Lza938060406 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz57424705e8 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lz57424705e8 'Cell (number)' (Lz670247a96a ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lz57424705e8 'Datto Client ID' (Lz670247a96a ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lz57424705e8 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lz9dc5de85b0 'Microsoft 365'
    $tbTid = Lz57424705e8 'Tenant ID' (Lz670247a96a ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lz57424705e8 'App (Client) ID' (Lz670247a96a ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lz9dc5de85b0 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lz57424705e8 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lz57424705e8 '.pfx file' (Lz670247a96a '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lz57424705e8 '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lz9dc5de85b0 'SharePoint and OneDrive'
    ...
    $tbSp  = Lz57424705e8 'SharePoint root URL' (Lz670247a96a ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lz57424705e8 'OneDrive host URL'   (Lz670247a96a ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lz57424705e8 'Team site base URL'  (Lz670247a96a ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lz57424705e8 'Default site URL'    (Lz670247a96a ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lz57424705e8 'Email / UPN domain'  (Lz670247a96a ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lzc71e8ef802 -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lz48023de656 -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz20c6b22d86 $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lza7e8808c25 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lz44632f89d2 }
        ...
        try {
            ...
            else { $tok=Lz48023de656 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lza7e8808c25 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lzc71e8ef802 -Name 'DattoEndpointUrl' -Value $ep
            Lzc71e8ef802 -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lzc71e8ef802 -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lzc71e8ef802 -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lzc71e8ef802 -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lzc71e8ef802 -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lzc71e8ef802 -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lzc71e8ef802 -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lzc71e8ef802 -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lza162a911ff -Value $script:AS.pbSec.Password
            }
...
function Lze97c1aad9f { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lz1b7a0da280 { param($Cfg,[string]$Path,$Value)
...
function Lz3cf94c491e {
    ...
    function Lzccbffd035c { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec)
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lze97c1aad9f -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lzccbffd035c -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lz1b7a0da280 -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lz1b7a0da280 -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lz1b7a0da280 -Cfg $script:SetCfg -Path $s.Path -Value ([bool]($raw -match '^(1|true|yes|on)$')) }
                else { Lz1b7a0da280 -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lzea584c5fcb {
    ...
    function Lz8935dcd3e3 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lza938060406 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz8935dcd3e3 -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lz8935dcd3e3 -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lz8935dcd3e3 -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lz8935dcd3e3 -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lza7e8808c25 $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lz44632f89d2 }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lzc71e8ef802 -Name 'DattoEndpointUrl' -Value $ep
            Lzc71e8ef802 -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lza162a911ff -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz1bdf43bea7 {
    ...
    $pre = Lz10178c77f1 -Title 'Prerequisites'
    ...
    $dat = Lz10178c77f1 -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lz94761a9035 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lz10178c77f1 -Title 'Microsoft 365' -Buttons @($bMs, (Lz94761a9035 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lz10178c77f1 -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lzea584c5fcb } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz6716e43697; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lz2c934a83a9 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz6716e43697; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lz0578fdb785 } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz6716e43697; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lz6716e43697
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lz3f0dfd4d21 {
...
function Lz30d301b225 {
    ...
    $bOpen.Add_Click({ try { Lz0c1a62b561 -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lzc500b1cf9e {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lz196996e110 {
    ...
    Lz30d301b225 -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lz0c1a62b561 -Url $pageUrl
    ...
    } finally { Lzc500b1cf9e }
}
function Lzc488b80ab5 {
    try {
...
function Lz7f2afa6ced {
    ...
    if (-not (Lzd9f746c8da)) {
        Lz3f0dfd4d21 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B54708' -Strong
        try {
            Lze71ea67213 -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lz3f0dfd4d21 "The standard sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B54708' -Strong
        }
    } else {
        Lz3f0dfd4d21 'Using the CODE sign-in (the reliable path on this machine): a small window will show your code, always on top.' '#B54708' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lz196996e110 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lz3f0dfd4d21 $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lz2beeea677c -Token $at -Claim 'tid'
            $wizTokAcct = Lz2beeea677c -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lz2beeea677c -Token $at -Claim 'preferred_username' }
            ...
            Lz3f0dfd4d21 "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lz3f0dfd4d21 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lz3f0dfd4d21 "Signed in to tenant $tid as $wizAcct." '#12B76A'
    $newThumb = $null
    try {
        Lz3f0dfd4d21 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lz3f0dfd4d21 "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B54708' }
            Lz3f0dfd4d21 "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#12B76A'
        } else {
            Lz3f0dfd4d21 "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lz3f0dfd4d21 'App registration created.' '#12B76A'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lz3f0dfd4d21 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lz3f0dfd4d21 "Certificate created in this user's store. Thumbprint $newThumb." '#12B76A'
        Lz3f0dfd4d21 'Uploading the certificate''s public key to the app registration...'
        ...
        Lz3f0dfd4d21 "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#12B76A'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lza6b6a5deb1 -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lz3f0dfd4d21 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lza6b6a5deb1 -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lz3f0dfd4d21 $consentMsg '#12B76A'
        Lzc71e8ef802 -Name 'TenantId'       -Value $tid
        Lzc71e8ef802 -Name 'GraphClientId'  -Value $appId
        Lzc71e8ef802 -Name 'CertThumbprint' -Value $newThumb
        Lz3f0dfd4d21 'Connection details saved on this computer.' '#12B76A'
    } catch {
        ...
        if ($friendly) { Lz3f0dfd4d21 $friendly '#B42318' -Strong; Lz3f0dfd4d21 "Technical detail: $m" }
        else { Lz3f0dfd4d21 "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lz3f0dfd4d21 "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lz3f0dfd4d21 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lz48023de656 -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lzc71e8ef802 -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lza490b1275f $spUrl
            if ($der) {
                Lzc71e8ef802 -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lzc71e8ef802 -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lzc71e8ef802 -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lz3f0dfd4d21 "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#12B76A'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lza6b6a5deb1 -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lz3f0dfd4d21 "The new sign-in has not become active yet: $($_.Exception.Message)" '#B54708' -Strong
                   Lz3f0dfd4d21 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz20c6b22d86 $org
            if ($u) { Lzc71e8ef802 -Name 'UpnDomain' -Value $u; Lz3f0dfd4d21 "Email domain detected: $u" '#12B76A' }
        } catch { Lz3f0dfd4d21 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lz3f0dfd4d21 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#12B76A' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lz2c934a83a9 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lzbd07e9e0c4 $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lz7f2afa6ced -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lzffd5f1fb5b) {
                    Lz3f0dfd4d21 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lz3f0dfd4d21 $t $c } else { Lz3f0dfd4d21 $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lz3f0dfd4d21 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B54708'
                }
            }
            ...
            Lz3f0dfd4d21 "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lz196996e110 -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lze71ea67213 -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lz0c1a62b561 -Url 'https://login.microsoft.com/device'
                Lzd3337ac80b
                try { Lze71ea67213 -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lz54c11c4ed9 }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (-not (Lzd9f746c8da)) {
            ...
            try {
                Lze71ea67213 -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lz196996e110 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lz2beeea677c -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lz2beeea677c -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lza6b6a5deb1 -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lzff71328f2f 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lza6b6a5deb1 -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lza6b6a5deb1 -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lzc71e8ef802 -Name 'EmailSender' -Value $senderAddr
        Lzc71e8ef802 -Name 'EmailEnabled' -Value '1'
...
function Lz0578fdb785 {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lz97fe000615 { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz634c44242b { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lz634c44242b -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lz634c44242b -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lz634c44242b -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lz97fe000615 'Subject and attachments'
    ...
    [void](Lz634c44242b -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz6e8054acf5 -Template $script:ES.tbSubj.Text -Vars (Lz1570106122)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz6e8054acf5 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz1570106122)
            $tok = Lz48023de656 -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lzc71e8ef802 -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lzc71e8ef802 -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lzc71e8ef802 -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lzc71e8ef802 -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lzc71e8ef802 -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lzc71e8ef802 -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lzc71e8ef802 -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lzc71e8ef802 -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lzc71e8ef802 -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lzc71e8ef802 -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lzc71e8ef802 -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lzc71e8ef802 -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lz3aca3b00f9 {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lz196996e110 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lz0f162e58c2 {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lz3aca3b00f9 -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lz6716e43697 } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lz8c04da777d {
    return ((Lz987cddaac0) -and (Lzffd5f1fb5b))
}
Lz744221b514
Lz54c11c4ed9
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lz4edcf0379f "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lz8c04da777d)) {
        try { Lz1bdf43bea7 } catch {}
        if (Lz8c04da777d) { Lz88f11424ec 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lz88f11424ec 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lz88f11424ec 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ((Lz8c04da777d) -and -not (Test-Path (Lz11b30c945e))) {
        try { Lz946638a70c } catch {}
    }
})
try {
    Add-Type -MemberDefinition '
