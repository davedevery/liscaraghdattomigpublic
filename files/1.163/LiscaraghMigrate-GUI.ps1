...
function Lze545d2cab1 {
    param([string]$Severity)
    Lz06492d1ec4 -Severity $Severity
    try {
...
function Lzb48aa1f9e5 { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lzc286a1d479 {
...
function Lz50f30adc14 {
    ...
    if (-not $s) { $s = Lz484ca66ac3 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lzcdfd5dc2b3 {
    param([string]$Value)
    if (Lzc286a1d479 -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lz0b8fcc0f04 {
    $f = @(Lzd1834ed54d)
    if ($f.Count -eq 1) { return $f[0] }
    return ''
}
function Lz5fd26fb443 {
...
function Lzb28e59b38d {
    param([string[]]$Picked)
    $result = New-Object System.Collections.Generic.List[string]
    foreach ($e in @(Lzd1834ed54d)) { [void]$result.Add($e) }
    $dropped = New-Object System.Collections.Generic.List[string]
    foreach ($p in @($Picked | ForEach-Object { "$_".Trim().Trim('/').Trim('\') } | Where-Object { $_ })) {
        if ($result -ccontains $p) { continue }
        $covered = $false
        foreach ($k in @($result)) { if (Lz5fd26fb443 -Child $p -Parent $k) { [void]$dropped.Add($p); $covered = $true; break } }
        if ($covered) { continue }
        foreach ($narrow in @($result | Where-Object { Lz5fd26fb443 -Child $_ -Parent $p -Strict })) { [void]$result.Remove($narrow); [void]$dropped.Add($narrow) }
        [void]$result.Add($p)
    }
    Lz631537bef5 -Folders @($result)
    if ($dropped.Count) { Lz51faee01ac ("Overlapping folder(s) merged, kept the broader one: " + ((@($dropped) | Select-Object -Unique) -join ', ')) }
}
function Lzd7a0657a6a {
    $one = $true; try { $one = (@(Lza4f71a160d).Count -le 1) } catch {}
    $n = @(Lzd1834ed54d).Count
...
function Lz92be2335c8 {
    ...
    try {
        ...
        $norm = Lz477bdd204d -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lz51faee01ac "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz8cf88f95b5
    } catch { }
}
function Lz477bdd204d {
...
function Lz37d07a829a {
    ...
    return @{ SubfolderCount = @(Lz55be625135 -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lz1d6885cd7e {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lza94454ff90 -Path "sites/$siteId"
    } catch { return $null }
}
function Lzffff2c76c7 {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lza94454ff90 -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lze4aaa5e13e {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lza94454ff90 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lzcc5c5a117b {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lz21e1035e4a -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
function Lz4519a413ea {
    param($State, [string]$Current)
    $out = New-Object System.Collections.Generic.List[string]
    foreach ($s in @($State.Lst.SelectedItems)) {
        $name = "$s"
        if (-not (Lz059b4d2579 -Item $name)) { continue }
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lzffff2c76c7 -Search $s.Txt.Text)
...
"@
    ...
    Lzcc5c5a117b
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lzcc5c5a117b } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lzcc5c5a117b } })
    $new.Add_Click({
        $n = Lz465d4d54bd -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lz82fd8d2ff2 -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lzcc5c5a117b; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lz2c4ea81bc5 -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lz98d9df5eeb -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lz2c4ea81bc5 -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lze2e245bd1d {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lz55be625135 -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lze2e245bd1d
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lze2e245bd1d } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lze2e245bd1d } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lz2c4ea81bc5 -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lz4519a413ea -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lz2c4ea81bc5 -State $script:SP
    $r = $w.ShowDialog()
...
function Lz51faee01ac { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz7d48afdb50 { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lzacf57348cf {
    ...
    if ($hasRegime) {
        $etaA = Lz277c158f11 -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz277c158f11 -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lz277c158f11 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz277c158f11 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lz1df7d012b5 {
    ...
    try {
        ...
        $etaText = Lzacf57348cf -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lz7e1764855b {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lz8e27ec0884 -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lz2e7ce402e3 -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lz1df7d012b5 -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lzd012240be9 $Line) + "`r`n")
...
function Lze72fe6d41a { param([string]$Text) Lz7e1764855b $Text }
function Lz46eb4c7970 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lzfbb8742cf5 $true 'Connecting to Microsoft 365...'
        Lz4efa6481dc
    } catch {
        ...
        Lzfbb8742cf5 $false; return
    } finally { Lzfbb8742cf5 $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lzbb6457f136 -StartSearch $seed
    if ($picked) {
        ...
        Lz51faee01ac "Site set to $picked"
        Lz8cf88f95b5
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lz46eb4c7970 }
    Lz8cf88f95b5
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lz64cc14487f }
    Lz8cf88f95b5
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz8cf88f95b5 })
$ctrl.TxtFolder.Add_TextChanged({ Lz8cf88f95b5 })
if ($ctrl.LstSource) { $ctrl.LstSource.Add_SelectionChanged({ Lzd7a0657a6a }) }
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz8cf88f95b5 }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz8cf88f95b5 }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lz02c6e5fe6a -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lz92be2335c8 })
$ctrl.TxtLoc.Add_TextChanged({ Lz8cf88f95b5 })
$ctrl.TxtLib.Add_TextChanged({ Lz8cf88f95b5 })
$script:NestSuspend = $false
function Lze206a357fc {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lzfa049b4354 -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lz51faee01ac $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lze206a357fc })
$ctrl.ChkNest.Add_Unchecked({ Lze206a357fc })
$script:ProjectRows = @()
$script:RunStatus = @{}
...
function Lzc1f9016dfd {
    ...
    if (-not $hasProjects) {
        $sd = $false
        if (Get-Command Test-ConnectionConfigured -ErrorAction SilentlyContinue) { try { $sd = [bool](Lzbd2704d91c) } catch {} }
        Lz659c0f8c09 $sd
    }
...
function Lz79f513131e {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lz37d07a829a -ProjectId $p.Id -SubPath $sub) }
...
function Lzfc35edf762 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lza94454ff90 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz2660fec15a {
    ...
    if (($EngineArgs -contains 'Transfer') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck') -or ($EngineArgs -contains 'PreFlight')) {
        try { Lzfbb8742cf5 $true 'Checking source folders...'; $srcOk = Lz79f513131e } finally { Lzfbb8742cf5 $false }
        if (-not $srcOk) { return }
    }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lzfbb8742cf5 $true 'Checking destinations...'; $dstOk = Lzfc35edf762 } finally { Lzfbb8742cf5 $false }
        if (-not $dstOk) { return }
    }
    ...
    $script:Timer.Add_Tick({
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lz7e1764855b $lines[$k] } catch {} }
                }
            }
        } catch {}
        ...
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lz7e1764855b $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lz7e1764855b $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Format-Span ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lz82cff87117) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lz3bd29e7c69 -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lz7e1764855b "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lz7e1764855b "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lz51faee01ac ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
            ...
            Lzcee048f855
            if (-not $script:Paused) { Lze545d2cab1 -Severity $sev }
            if ($script:RunStatus.Count) { try { Lzc1f9016dfd } catch {} }
            try {
            ...
            try {
                if ($script:CurLabel -eq 'Check readiness' -or $script:CurLabel -eq 'Preview (no upload)') {
                    $newest = Get-ChildItem (Join-Path (Lz82cff87117) 'report-*.html') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
function Lzc30e7a1290 { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lz05ca9a0206 { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lz147b530c55 {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lz1eb620f26d -ConfigFile $s.Tag; Lz5196bbacfc } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lz7372c278a5 { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lzb48aa1f9e5 -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lz147b530c55
}
function Lz679ef36d9b {
...
function Lz1eb620f26d {
    param([string]$ConfigFile)
    if ($script:Sched) { Lzeefc9ad81e -Why 'Schedule cancelled (job changed).' }
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lz05ca9a0206 $true
    Lzc30e7a1290 $true
    Lz7372c278a5 $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lz679ef36d9b -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lz51faee01ac 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
}
function Lz190880979e {
    param([string]$Dir)
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lz465d4d54bd -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Lz32c1a27500 -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Open it instead, or choose another name.")); return }
    if ((Test-Path $jobDir) -and -not (Lz190880979e -Dir $jobDir)) {
        ...
        Lzfa049b4354 -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lz1eb620f26d -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuJobOpen.Add_Click({
    ...
    [void]$w.ShowDialog()
    if ($script:OpenPick) { Lz1eb620f26d -ConfigFile $script:OpenPick; Lz5196bbacfc }
})
$ctrl.MnuJobSave.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')); return }
    if (Save-Mapping) { Lz51faee01ac 'Migration job saved.' }
})
$ctrl.MnuJobSaveAs.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open a migration job first, then Save As to copy it.')); return }
    try {
        $name = Lz465d4d54bd -Prompt "New name for the copy:" -Title 'Save job as'
        if (-not $name -or -not $name.Trim()) { return }
        $name = $name.Trim()
        $slug = ($name -replace '[^A-Za-z0-9._-]','_').Trim('_'); if (-not $slug) { $slug = 'job' }
        $jobDir = Join-Path $script:JobsRoot $slug
        if (Lz32c1a27500 -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Choose another name.")); return }
        if ((Test-Path $jobDir) -and -not (Lz190880979e -Dir $jobDir)) {
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz27f5ea078a })
$ctrl.MnuSettingsApi.Add_Click({ Lz82c008d63d })
$ctrl.MnuSettingsWizard.Add_Click({ Lzb6fb01a663 })
$ctrl.MnuSettingsEmail.Add_Click({ Lz6bcd31f673 })
$ctrl.MnuSettingsTuning.Add_Click({ Lz3dbe004a5f })
$ctrl.BtnFilters.Add_Click({ Lz1664302dc0 })
$ctrl.MnuSettingsDecommission.Add_Click({ Lz4fd58e4138 })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lz18939b1e9c -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lz0ce15950bc {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lz484ca66ac3 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lz0ce15950bc "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lz0ce15950bc (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lzfbb8742cf5 $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lzfbb8742cf5 $false }
    if (-not $b) { return }
...
function Lz052d03daa2 { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lzf18d110cad {
    $path = Lz052d03daa2
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Liscaragh Migrate" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lzf18d110cad)) -Caption ('Licence')) | Out-Null })
function Lz1a5862a975 {
...
function Lz096a59d941 {
    param([string]$For = 'copy')
    if (Test-Path (Lz052d03daa2)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        try {
            ...
            Lzb48aa1f9e5 -Name 'LicenceAppliedId'  -Value "$($lpl.LicenceId)"
            Lzb48aa1f9e5 -Name 'LicenceAppliedUtc' -Value ((Get-Date).ToUniversalTime().ToString('o'))
        } catch {}
        (Show-Msg -Text ("Licence installed.`n`n" + (Lzf18d110cad)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-FullReinstall {
    ...
    Lzfbb8742cf5 $true "Downloading the installer for version $($Payload.version)..."
    ...
    catch { Lzfbb8742cf5 $false; (Show-Msg -Text ("The installer could not be downloaded, so nothing has been changed.`n`n$($_.Exception.Message)") -Caption ('Reinstall') -Icon ('Warning')) | Out-Null; return }
...
function Invoke-UpdateCheck {
    ...
    try {
        ...
        if (-not $Silent) { Lzfbb8742cf5 $true 'Checking for updates...' }
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lzfbb8742cf5 $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lz18939b1e9c -Title 'About' -Heading 'Liscaragh Migrate' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lz50f30adc14 } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lz16df13cb0e -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lz465d4d54bd -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lz2cd247531b {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lz05ca9a0206 $false
    Lzc30e7a1290 $false
}
$ctrl.MnuJobClose.Add_Click({ Lz2cd247531b; Lz51faee01ac 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lz2cd247531b
    if (Lz190880979e -Dir $dir) {
        Lz147b530c55; Lz51faee01ac "Deleted job '$nm'"
    } else {
        Lz147b530c55
        ...
        Lz51faee01ac "Could not fully delete job '$nm' (a file is in use)"
    }
})
function Lzee917e05b5 {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lzfbb8742cf5 $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lze72fe6d41a 'Signing in to Datto and listing projects...'
        $script:Projects = Lz4d52917118
        Lze72fe6d41a "Found $($script:Projects.Count) project(s)."
        Lz3d0b776983
        try { Lz4efa6481dc; Lze72fe6d41a 'Connected to Microsoft 365.' } catch { Lze72fe6d41a 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lze72fe6d41a "Technical detail: $($_.Exception.Message)" }
        Lzc1f9016dfd
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lz51faee01ac 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lze72fe6d41a 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lze72fe6d41a "Technical detail: $($_.Exception.Message)"
            Lz51faee01ac "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lz51faee01ac 'Connect failed'
        }
    }
    finally { Lzfbb8742cf5 $false }
}
$ctrl.BtnConnect.Add_Click({ Lzee917e05b5 })
function Lz5196bbacfc {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lz50f30adc14 } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lz51faee01ac "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lzee917e05b5 -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lz80ee42d5c6
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lza4f71a160d).Count } catch {}
    ...
    if ($script:Map.ContainsKey($p.Id)) {
        ...
        $ctrl.TxtLoc.Text = Lz64cc14487f
        $ctrl.TxtLib.Text = ''; $ctrl.TxtFolder.Text = ''; Lz631537bef5 -Folders @()
...
function Lz09fbd01825 {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lzcab04712d5) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lzcab04712d5) }
}
$ctrl.BtnApply.Add_Click({
    $p = Lz80ee42d5c6
    if (-not $p) { (Show-Msg -Text ('Select a project first.')); return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Remove($p.Id) | Out-Null; Lzc1f9016dfd; [void](Write-MappingQuiet); return }
    $d = Lz09fbd01825
    if ($d -eq 'ERR') { return }
    ...
    Lz51faee01ac "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lz41c7bb9743 })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lzbfe50e9da4 -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Lzc1f9016dfd; [void](Write-MappingQuiet); Lz51faee01ac "Set $($sel.Count) project(s) to skip"; return }
    $d = Lz09fbd01825
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lz46eb4c7970); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPaths=$keepSrcList; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Lzc1f9016dfd
    [void](Write-MappingQuiet)
    Lz51faee01ac "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lzbfe50e9da4 -Count (@($script:Projects).Count))) { return }
    $d = Lz09fbd01825
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Lzc1f9016dfd; [void](Write-MappingQuiet); Lz51faee01ac 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Lzc1f9016dfd
    [void](Write-MappingQuiet)
    Lz51faee01ac "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lzfbb8742cf5 $true 'Looking up libraries...'
        $libs = Lzccad69e973 -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lzfbb8742cf5 $false; return }
    finally { Lzfbb8742cf5 $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz39134e9d55 -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lz51faee01ac "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lz51faee01ac "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lzfbb8742cf5 $true 'Opening destination...'
        Lz4efa6481dc
        $res = Lze4aaa5e13e
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lzfbb8742cf5 $false; return }
    finally { Lzfbb8742cf5 $false }
    $picked = Lzf1a70a4dbf -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lz51faee01ac ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnAddSource) { $ctrl.BtnAddSource.Add_Click({
    ...
    try { $picked = Lzb0cc782dfe -ProjectId $p.Id -ProjectName $p.Name }
    ...
    Lzb28e59b38d -Picked $arr
    Lz51faee01ac ("Source folders: " + (@(Lzd1834ed54d) -join ', '))
}) }
if ($ctrl.BtnRemoveSource) { $ctrl.BtnRemoveSource.Add_Click({
    ...
    Lz631537bef5 -Folders @(Lzd1834ed54d | Where-Object { $sel -notcontains $_ })
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    $bad = New-Object System.Collections.Generic.List[string]
    try {
        Lzfbb8742cf5 $true 'Checking the source folder(s) in Datto...'
        foreach ($s in $subs) { try { $null = Lz37d07a829a -ProjectId $p.Id -SubPath $s } catch { [void]$bad.Add($s) } }
    } catch { $ctrl.LblSourceCheck.Text = "Could not check the source: $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lzfbb8742cf5 $false; return }
    finally { Lzfbb8742cf5 $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lzfbb8742cf5 $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lza94454ff90 -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lzccad69e973 -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lz8cf88f95b5 {
    ...
    $srcLabel = Lz76e8aecc21 -Project $p
    $folders  = @(Lzd1834ed54d)
    $srcSub   = Lz0b8fcc0f04
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lzcab04712d5) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lza4f71a160d).Count } catch { }
...
function Lz05e4f947be {
    ...
    $script:Map[$p.Id] = $d
    Lzc1f9016dfd
    Lz51faee01ac "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lze72fe6d41a "Saved $n mapping(s)."
    return $true
}
function Lz3d0b776983 {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lze72fe6d41a "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lze72fe6d41a "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lz2660fec15a -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lz2660fec15a -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lz874f3c1c44 {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lz874f3c1c44) }
    ...
    if (-not (Lz18939b1e9c -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lz3e64c7427c)) { return }
    if (-not (Lz096a59d941)) { return }
    Lz2660fec15a -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lzc5217b450c {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lz874f3c1c44) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lz3e64c7427c)) { return }
    if (-not (Lz096a59d941)) { return }
    Lz2660fec15a -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnRerunFailed.Add_Click({
    ...
    if (-not (Lz18939b1e9c -Title 'Rerun failed files only' -Heading 'Rerun failed files only' -Sections $sections -Yes 'Rerun the failed files' -No 'Cancel')) { return }
    if (-not (Lz3e64c7427c)) { return }
    if (-not (Lz096a59d941)) { return }
    Lz2660fec15a -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','NewerWins','-Execute','-FailedOnly') -Label 'Rerun failed files only'
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lz18939b1e9c -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lz096a59d941 -For 'verify')) { return }
    Lz2660fec15a -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lz2660fec15a -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lz82cff87117 {
...
function Lzcee048f855 {
    try { if ($ctrl.BtnRerunFailed) { $ctrl.BtnRerunFailed.IsEnabled = (Lz14e4593c07) } } catch {}
}
function Lz640735c5a1 {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lzc9ce315b1a {
    param([string]$Name)
...
function Lzc4d2e3f99c {
    ...
    foreach ($f in $files) { $l = Lzc9ce315b1a $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lzc4d2e3f99c -Folder (Lz82cff87117) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnCertificate.Add_Click({
    if (Save-Mapping) {
        Lz2660fec15a -EngineArgs @('-Action','Certificate') -Label 'Completion certificate' -OnComplete {
            $rr = Lz82cff87117
            $cert = $null
...
$ctrl.BtnOpenAudit.Add_Click({ Lzc4d2e3f99c -Folder (Lz640735c5a1) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lz2d16e9a138 {
    ...
    Lz51faee01ac "Speed limit set: up $uTxt, down $dTxt"
    Lz7e1764855b "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lz2d16e9a138 })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz2d16e9a138 } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz2d16e9a138 } })
function Lzabf84a1a1d {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lzabf84a1a1d -ParentId ([int]$_.ProcessId) } } catch {}
...
function Lzde5ec0cb36 {
    $r = ($script:Proc -and -not $script:Proc.HasExited)
    $f = ($script:FinProc -and -not $script:FinProc.HasExited)
    return ($r -or $f)
}
function Lz7f0abaa274 { param([string]$T) try { $ctrl.BtnSchedule.Content.Text = $T } catch {} }
function Lzeefc9ad81e {
    param([string]$Why = '')
    if ($script:SchedTimer) { try { $script:SchedTimer.Stop() } catch {}; $script:SchedTimer = $null }
    $script:Sched = $null
    Lz7f0abaa274 'Schedule for later / overnight...'
    if ($Why) { Lz51faee01ac $Why }
}
function Lz821eb9ca71 {
    try { $p = Join-Path (Lz82cff87117) 'lastrun-outcome.json'; if (Test-Path $p) { return (Get-Content $p -Raw | ConvertFrom-Json) } } catch {}
    return $null
}
function Lzc3619019f6 {
    if (-not $script:Proc -or $script:Proc.HasExited) { return }
    $script:QuietPausing = $true
    $ctrl.BtnStop.IsEnabled = $false
    Lz7e1764855b "----- Quiet hours reached at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')): pausing the copy. Files already uploaded are kept. -----"
    Lz51faee01ac 'Quiet hours: pausing until the next window...'
    try { Lzabf84a1a1d -ParentId ([int]$script:Proc.Id) } catch {}
    Lz679ef36d9b -Status Cancelled -Note 'Finalising for quiet hours...' -OnDone {
        try { Lz7e1764855b '----- Paused for quiet hours. It will resume as a Sync at the next window while this app stays open. -----' } catch {}
    }
}
function Lz442bebe9dd {
    param([string[]]$RunArgs,[string]$Label)
    Lz7e1764855b "----- Scheduled: starting '$Label' at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')) -----"
    Lz2660fec15a -EngineArgs $RunArgs -Label $Label
    return ($script:Proc -and -not $script:Proc.HasExited)
}
function Lz785249e73d { param([string]$Status)
    ...
    switch ("$($s.State)") {
        'waiting' {
            if ($now -ge $s.StartAt -and -not $busy) {
                ...
                elseif (Lz442bebe9dd -RunArgs $s.Args -Label $s.Label) { $s.State = 'running'; $s.Launches++ }
                else { Lzeefc9ad81e -Why 'The scheduled run could not start (see the message). Schedule cancelled.' }
            } else {
                $left = $s.StartAt - $now; if ($left.Ticks -lt 0) { $left = [TimeSpan]::Zero }
                Lz7f0abaa274 ("Scheduled: starts $($s.StartAt.ToString('ddd HH:mm')) ($(Format-Span $left) to go) - click to cancel")
            }
        }
        'running' {
            if (-not $busy) {
                $oc = Lz821eb9ca71; $status = if ($oc) { "$($oc.Status)" } else { '' }
                if ($s.Mode -eq 'once') { Lzeefc9ad81e -Why 'Scheduled run finished.'; Lz785249e73d $status }
                elseif ($status -eq 'COMPLETED') { Lzeefc9ad81e -Why 'Scheduled migration completed.'; Lz785249e73d $status }
                else { $s.State = 'paused' }
            } elseif ($s.Mode -eq 'window' -and -not (Test-InWindow $m $s.FromMin $s.ToMin)) {
                Lzc3619019f6; $s.State = 'paused'
            } else {
                $until = if ($s.Mode -eq 'window') { " (until $(Format-MinOfDay $s.ToMin))" } else { '' }
                Lz7f0abaa274 ("Scheduled: running$until - click to cancel")
            }
        }
        'paused' {
            if ((Test-InWindow $m $s.FromMin $s.ToMin) -and -not $busy) {
                if (Lz442bebe9dd -RunArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','AddMissing','-Execute') -Label 'Sync new and changed') { $s.State = 'running'; $s.Launches++ }
            } else {
                $nf = Get-NextWindowOpen $s.FromMin
                Lz7f0abaa274 ("Scheduled: waiting for the next window ($($nf.ToString('ddd HH:mm'))) - click to cancel")
            }
        }
    }
}
function Lz562406b30c {
...
$ctrl.BtnSchedule.Add_Click({
    if ($script:Sched) {
        if ((Show-Msg -Text ("A schedule is set:" + [Environment]::NewLine + $script:Sched.Summary + [Environment]::NewLine + [Environment]::NewLine + "Cancel it?") -Caption ('Scheduled run') -Buttons ('YesNo') -Icon ('Question')) -eq 'Yes') { Lzeefc9ad81e -Why 'Schedule cancelled.' }
        return
    }
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')) | Out-Null; return }
    if (Lzde5ec0cb36) { (Show-Msg -Text ('A run is in progress. Wait for it to finish, then set a schedule.') -Caption ('Cannot schedule now') -Icon ('Warning')) | Out-Null; return }
    if (-not (Save-Mapping)) { return }
    $mappedCount = @($script:Projects | Where-Object { $script:Map.ContainsKey($_.Id) }).Count
    ...
    $script:SchedTimer.Add_Tick({ try { Lz2d34ce710e } catch {} })
    $script:SchedTimer.Start()
    try { Lz2d34ce710e } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    if ($script:Sched) { Lzeefc9ad81e -Why 'Schedule cancelled (you stopped the run).' }
    $script:Stopping = $true
    $ctrl.BtnStop.IsEnabled = $false
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz7e1764855b "----- STOP requested by user at $stamp -----"
    Lz51faee01ac "Stopping $($script:CurLabel)..."
    try { Lzabf84a1a1d -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lz679ef36d9b -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lz7e1764855b "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lz451abd5c05 {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lz2660fec15a -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz7e1764855b "----- PAUSE requested by user at $stamp -----"
    Lz51faee01ac "Pausing $($script:CurLabel)..."
    try { Lzabf84a1a1d -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lz679ef36d9b -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lz7e1764855b "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lz5d8c765fd0 { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lzab3b54a0d0 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lz7a6354e1f4 {
...
function Lz16df13cb0e {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lz0dc50ee37b -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lz82c008d63d {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    ...
    function Lz943808ad55 { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lzd3db4fa5b0 { param($Label,$Control,$Info)
    ...
    function Lz71f88fc927 { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lz943808ad55 'Datto Workplace'
    ...
    $parts = Lzab3b54a0d0 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lzd3db4fa5b0 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lzd3db4fa5b0 'Cell (number)' (Lz71f88fc927 ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lzd3db4fa5b0 'Datto Client ID' (Lz71f88fc927 ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lzd3db4fa5b0 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lz943808ad55 'Microsoft 365'
    $tbTid = Lzd3db4fa5b0 'Tenant ID' (Lz71f88fc927 ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lzd3db4fa5b0 'App (Client) ID' (Lz71f88fc927 ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lz943808ad55 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lzd3db4fa5b0 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lzd3db4fa5b0 '.pfx file' (Lz71f88fc927 '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lzd3db4fa5b0 '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lz943808ad55 'SharePoint and OneDrive'
    ...
    $tbSp  = Lzd3db4fa5b0 'SharePoint root URL' (Lz71f88fc927 ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lzd3db4fa5b0 'OneDrive host URL'   (Lz71f88fc927 ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lzd3db4fa5b0 'Team site base URL'  (Lz71f88fc927 ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lzd3db4fa5b0 'Default site URL'    (Lz71f88fc927 ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lzd3db4fa5b0 'Email / UPN domain'  (Lz71f88fc927 ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lzb48aa1f9e5 -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lz16df13cb0e -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lzd8997fb902 $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lz5d8c765fd0 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lz50f30adc14 }
        ...
        try {
            ...
            else { $tok=Lz16df13cb0e -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lz5d8c765fd0 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lzb48aa1f9e5 -Name 'DattoEndpointUrl' -Value $ep
            Lzb48aa1f9e5 -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lzb48aa1f9e5 -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lzb48aa1f9e5 -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lzb48aa1f9e5 -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lzb48aa1f9e5 -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lzb48aa1f9e5 -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lzb48aa1f9e5 -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lzb48aa1f9e5 -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lzcdfd5dc2b3 -Value $script:AS.pbSec.Password
            }
...
function Lz26b4dee1e9 { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lzfa049b4354 { param($Cfg,[string]$Path,$Value)
...
function Lz1664302dc0 {
    ...
    $curMax=Lz26b4dee1e9 -Cfg $cfg -Path 'run.tuning.maxFileSizeMB'; if ($null -eq $curMax) { $curMax=0 }; $sizeBox.Text="$curMax"
    ...
    $curInc=@(Lz26b4dee1e9 -Cfg $cfg -Path 'run.tuning.includePatterns' | Where-Object { "$_".Trim() })
    $curExc=@(Lz26b4dee1e9 -Cfg $cfg -Path 'run.tuning.excludePatterns' | Where-Object { "$_".Trim() })
    ...
    $afterPick=New-Object System.Windows.Controls.DatePicker; $afterPick.Margin='0,4,0,4'; $afterPick.Width=150; $afterPick.SelectedDate=(& $toDate (Lz26b4dee1e9 -Cfg $cfg -Path 'run.tuning.modifiedAfter'))
    ...
    $beforePick=New-Object System.Windows.Controls.DatePicker; $beforePick.Margin='0,4,0,4'; $beforePick.Width=150; $beforePick.SelectedDate=(& $toDate (Lz26b4dee1e9 -Cfg $cfg -Path 'run.tuning.modifiedBefore'))
    ...
    $save.Add_Click({
        try {
            ...
            Lzfa049b4354 -Cfg $script:FltCfg -Path 'run.tuning.maxFileSizeMB' -Value $sv
            $pats=@("$($script:FltPat.Text)" -split '[;,\r\n]' | ForEach-Object { $_.Trim() } | Where-Object { $_ })
            if ($script:FltInc.IsChecked) { Lzfa049b4354 -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value $pats; Lzfa049b4354 -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value @() }
            else                          { Lzfa049b4354 -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value $pats; Lzfa049b4354 -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value @() }
            ...
            Lzfa049b4354 -Cfg $script:FltCfg -Path 'run.tuning.modifiedAfter'  -Value $ma
            Lzfa049b4354 -Cfg $script:FltCfg -Path 'run.tuning.modifiedBefore' -Value $mb
...
function Lz3dbe004a5f {
    ...
    function Lza7f445b7b2 { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec,[string]$Type='text')
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lz26b4dee1e9 -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lza7f445b7b2 -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec -Type $s.Type
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lzfa049b4354 -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lzfa049b4354 -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lzfa049b4354 -Cfg $script:SetCfg -Path $s.Path -Value ([bool]$script:SetBoxes[$s.Path].IsChecked) }
                elseif ($s.Type -eq 'date') {
                    ...
                    Lzfa049b4354 -Cfg $script:SetCfg -Path $s.Path -Value $raw
                }
                else { Lzfa049b4354 -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lz01680f7c48 {
    ...
    function Lz83d407fb8e { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lzab3b54a0d0 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz83d407fb8e -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lz83d407fb8e -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lz83d407fb8e -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lz83d407fb8e -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lz5d8c765fd0 $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lz50f30adc14 }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lzb48aa1f9e5 -Name 'DattoEndpointUrl' -Value $ep
            Lzb48aa1f9e5 -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lzcdfd5dc2b3 -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz27f5ea078a {
    ...
    $pre = Lz7028dee64c -Title 'Prerequisites'
    ...
    $dat = Lz7028dee64c -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lz20f815fb11 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lz7028dee64c -Title 'Microsoft 365' -Buttons @($bMs, (Lz20f815fb11 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lz7028dee64c -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lz01680f7c48 } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzc72a1f4813; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lzb6fb01a663 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzc72a1f4813; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lz6bcd31f673 } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lzc72a1f4813; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lzc72a1f4813
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lz018f2caa5d {
...
function Lz125927d23b {
    ...
    $bOpen.Add_Click({ try { Lzb5bea767e8 -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lz9511cb6aff {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lzdd2722952d {
    ...
    Lz125927d23b -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lzb5bea767e8 -Url $pageUrl
    ...
    } finally { Lz9511cb6aff }
}
function Lz27bdd3c217 {
    try {
...
function Lz51c4c58a2c {
    if (Lz55cd6bd9de) { return $false }
...
function Lz8c7aabbb4a {
    ...
    if (-not (Get-Module Microsoft.Graph.Authentication -ListAvailable)) {
        Lz018f2caa5d 'The Microsoft Graph module is not installed. Re-run the installer (or run: Install-Module Microsoft.Graph.Authentication -Scope CurrentUser) and try again.' '#B42318' -Strong
        return
    }
    Lz018f2caa5d 'Preparing the Microsoft 365 sign-in. The first time on a new install can take up to a minute while Windows loads the components - please wait, the app has not frozen.' '#475467' -Strong
    try { Import-Module Microsoft.Graph.Authentication -ErrorAction Stop } catch {
        Lz018f2caa5d "Could not load the Microsoft Graph module: $($_.Exception.Message)" '#B42318' -Strong
        return
    }
    ...
    if (Lz51c4c58a2c) {
        Lz018f2caa5d 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B5460F' -Strong
        try {
            Lz11cd128990 -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lz018f2caa5d "The browser sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B5460F' -Strong
        }
    } else {
        Lz018f2caa5d 'Using the code sign-in (the reliable path): a small window will show your code to type at microsoft.com/devicelogin, or on your phone. It stays on top and closes itself when you are done.' '#B5460F' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lzdd2722952d -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lz018f2caa5d $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lzb9ea108f63 -Token $at -Claim 'tid'
            $wizTokAcct = Lzb9ea108f63 -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lzb9ea108f63 -Token $at -Claim 'preferred_username' }
            ...
            Lz018f2caa5d "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lz018f2caa5d 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lz018f2caa5d "Signed in to tenant $tid as $wizAcct." '#0E9F6E'
    $newThumb = $null
    try {
        Lz018f2caa5d 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lz018f2caa5d "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B5460F' }
            Lz018f2caa5d "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#0E9F6E'
        } else {
            Lz018f2caa5d "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lz018f2caa5d 'App registration created.' '#0E9F6E'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lz018f2caa5d 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lz018f2caa5d "Certificate created in this user's store. Thumbprint $newThumb." '#0E9F6E'
        Lz018f2caa5d 'Uploading the certificate''s public key to the app registration...'
        ...
        Lz018f2caa5d "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#0E9F6E'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lz2b1c445e92 -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lz018f2caa5d 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lz2b1c445e92 -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lz018f2caa5d $consentMsg '#0E9F6E'
        Lzb48aa1f9e5 -Name 'TenantId'       -Value $tid
        Lzb48aa1f9e5 -Name 'GraphClientId'  -Value $appId
        Lzb48aa1f9e5 -Name 'CertThumbprint' -Value $newThumb
        Lz018f2caa5d 'Connection details saved on this computer.' '#0E9F6E'
    } catch {
        ...
        if ($friendly) { Lz018f2caa5d $friendly '#B42318' -Strong; Lz018f2caa5d "Technical detail: $m" }
        else { Lz018f2caa5d "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lz018f2caa5d "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lz018f2caa5d 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lz16df13cb0e -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lzb48aa1f9e5 -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lz7a6354e1f4 $spUrl
            if ($der) {
                Lzb48aa1f9e5 -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lzb48aa1f9e5 -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lzb48aa1f9e5 -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lz018f2caa5d "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#0E9F6E'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lz2b1c445e92 -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lz018f2caa5d "The new sign-in has not become active yet: $($_.Exception.Message)" '#B5460F' -Strong
                   Lz018f2caa5d 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lzd8997fb902 $org
            if ($u) { Lzb48aa1f9e5 -Name 'UpnDomain' -Value $u; Lz018f2caa5d "Email domain detected: $u" '#0E9F6E' }
        } catch { Lz018f2caa5d 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lz018f2caa5d 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#0E9F6E' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lzb6fb01a663 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lze324a357ab $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lz8c7aabbb4a -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lzdff0569aa2) {
                    Lz018f2caa5d 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lz018f2caa5d $t $c } else { Lz018f2caa5d $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lz018f2caa5d 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B5460F'
                }
            }
            ...
            Lz018f2caa5d "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lzdd2722952d -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lz11cd128990 -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lzb5bea767e8 -Url 'https://login.microsoft.com/device'
                Lz6fe6dbc71c
                try { Lz11cd128990 -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lz89545ff310 }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (Lz51c4c58a2c) {
            ...
            try {
                Lz11cd128990 -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lzdd2722952d -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lzb9ea108f63 -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lzb9ea108f63 -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lz2b1c445e92 -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lz445258abe7 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lz2b1c445e92 -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lz2b1c445e92 -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lzb48aa1f9e5 -Name 'EmailSender' -Value $senderAddr
        Lzb48aa1f9e5 -Name 'EmailEnabled' -Value '1'
...
function Lz6bcd31f673 {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lzc7e8b17e50 { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz12f47d7cef { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lz12f47d7cef -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lz12f47d7cef -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lz12f47d7cef -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lzc7e8b17e50 'Subject and attachments'
    ...
    [void](Lz12f47d7cef -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz0c6d9ad241 -Template $script:ES.tbSubj.Text -Vars (Lzc699a1b61d)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz0c6d9ad241 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lzc699a1b61d)
            $tok = Lz16df13cb0e -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lzb48aa1f9e5 -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lzb48aa1f9e5 -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lzb48aa1f9e5 -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lzb48aa1f9e5 -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lzb48aa1f9e5 -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lzb48aa1f9e5 -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lzb48aa1f9e5 -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lzb48aa1f9e5 -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lzb48aa1f9e5 -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lzb48aa1f9e5 -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lzb48aa1f9e5 -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lzb48aa1f9e5 -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lz94ba779b11 {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lzdd2722952d -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lz4fd58e4138 {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lz94ba779b11 -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lzc72a1f4813 } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lzbd2704d91c {
    return ((Lz9e2bde1ad0) -and (Lzdff0569aa2))
}
Lz7a4f49eb07
Lz89545ff310
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lz7e1764855b "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lzbd2704d91c)) {
        try { Lz27f5ea078a } catch {}
        if (Lzbd2704d91c) { Lz51faee01ac 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lz51faee01ac 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lz51faee01ac 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ($ctrl.GettingStarted -and $ctrl.GettingStarted.Visibility -eq 'Visible') {
        try { Lz659c0f8c09 ([bool](Lzbd2704d91c)) } catch {}
    }
    if ((Lzbd2704d91c) -and -not (Test-Path (Lz052d03daa2))) {
        try { Lze8e1901a2f } catch {}
    }
