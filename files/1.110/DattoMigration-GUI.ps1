...
[System.Runtime.InteropServices.DllImport("user32.dll", CharSet=System.Runtime.InteropServices.CharSet.Unicode)] public static extern int GetClassName(System.IntPtr hWnd, System.Text.StringBuilder lpClassName, int nMaxCount);' -Name ConsoleBoot -Namespace Liscaragh -ErrorAction Stop
        ...
        $bootPf7 = Join-Path $env:ProgramFiles 'PowerShell\7\pwsh.exe'
        if (Test-Path $bootPf7) { $bootUnpacked = $bootPf7 }
        if (-not $bootUnpacked) {
...
function Lzc2d4d8c93d { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lz959ab0d29c {
...
function Lz54d26bdb78 {
    ...
    if (-not $s) { $s = Lz0403dc684e 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lza6d4c00b5d {
    param([string]$Value)
    if (Lz959ab0d29c -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lze6b49ba7bb {
    ...
    return (Lz9bc0b7d8c8 -TargetSubFolder $box -SourceSubPath $src)
}
function Lz2f8c7a60ea {
    ...
    try {
        ...
        $norm = Lz9bc0b7d8c8 -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lz7ad492c7de "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz780e776a9b
    } catch { }
}
function Lz9bc0b7d8c8 {
...
function Lz4c32531aed {
    ...
    return @{ SubfolderCount = @(Lz80d5efa507 -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lz2a77ca34cf {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lz2721ac1295 -Path "sites/$siteId"
    } catch { return $null }
}
function Lz8569616a3a {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lz2721ac1295 -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lz361ee91c93 {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lz2721ac1295 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lzf5de368331 {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lz6df122c60d -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lz8569616a3a -Search $s.Txt.Text)
...
"@
    ...
    Lzf5de368331
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lzf5de368331 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lzf5de368331 } })
    $new.Add_Click({
        $n = Lzf0779004ed -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lzc6af10f20a -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lzf5de368331; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lz2aebc9d474 -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lz9c28f47737 -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lz2aebc9d474 -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lz1994cc4679 {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lz80d5efa507 -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lz1994cc4679
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lz1994cc4679 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lz1994cc4679 } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lz2aebc9d474 -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lz9c28f47737 -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lz2aebc9d474 -State $script:SP
    $r = $w.ShowDialog()
...
function Lz7ad492c7de { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz5347f953eb { param([TimeSpan]$ts)
...
function Lz1a40fc2b82 { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lzb9877ae268 {
    ...
    if ($hasRegime) {
        $etaA = Lz1768263feb -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz1768263feb -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lz1768263feb -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz1768263feb -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lza658970705 {
    ...
    try {
        ...
        $etaText = Lzb9877ae268 -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lz5d12701e82 {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lz7b9497c263 -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lzd3979ec5f6 -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lza658970705 -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lza203ed0450 $Line) + "`r`n")
...
function Lze4f5f8c41f { param([string]$Text) Lz5d12701e82 $Text }
function Lz041d74a995 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lz159d723349 $true 'Connecting to Microsoft 365...'
        Lzf21b2c3158
    } catch {
        ...
        Lz159d723349 $false; return
    } finally { Lz159d723349 $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lz688fd6c2b9 -StartSearch $seed
    if ($picked) {
        ...
        Lz7ad492c7de "Site set to $picked"
        Lz780e776a9b
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lz041d74a995 }
    Lz780e776a9b
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lz7c56e022cb }
    Lz780e776a9b
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz780e776a9b })
$ctrl.TxtFolder.Add_TextChanged({ Lz780e776a9b })
$ctrl.TxtSourceSub.Add_TextChanged({ Lz780e776a9b })
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz780e776a9b }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz780e776a9b }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lzcfff1e72ad -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lz2f8c7a60ea })
$ctrl.TxtSourceSub.Add_LostFocus({ Lz2f8c7a60ea })
$ctrl.TxtLoc.Add_TextChanged({ Lz780e776a9b })
$ctrl.TxtLib.Add_TextChanged({ Lz780e776a9b })
$script:NestSuspend = $false
function Lz510f06369b {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lz2ca2d65834 -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lz7ad492c7de $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lz510f06369b })
$ctrl.ChkNest.Add_Unchecked({ Lz510f06369b })
$script:ProjectRows = @()
$script:RunStatus = @{}
function Lzf1003644e8 {
...
function Lz7b3ebc6e49 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lz4c32531aed -ProjectId $p.Id -SubPath $sub) }
...
function Lz88a4d47877 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lz2721ac1295 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz49661531ef {
    ...
    try { Lz159d723349 $true 'Checking source folders...'; $srcOk = Lz7b3ebc6e49 } finally { Lz159d723349 $false }
    if (-not $srcOk) { return }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lz159d723349 $true 'Checking destinations...'; $dstOk = Lz88a4d47877 } finally { Lz159d723349 $false }
        if (-not $dstOk) { return }
    }
        ...
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lz5d12701e82 $lines[$k] } catch {} }
                }
            }
        } catch {}
        $script:SpinIdx = ($script:SpinIdx + 1) % 4
        $spin = @('|','/','-','\')[$script:SpinIdx]
        $ctrl.LblElapsed.Text = "$spin  Elapsed " + (Lz5347f953eb ((Get-Date) - $script:RunStart))
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lz5d12701e82 $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lz5d12701e82 $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Lz5347f953eb ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lz566d7b16da) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lz9518310adc -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lz5d12701e82 "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lz5d12701e82 "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lz7ad492c7de ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
...
function Lz7147c73077 { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lzc7e726ebc4 { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lz37622db7cb {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lzcc9eb7a240 -ConfigFile $s.Tag; Lz76bf4d4694 } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lze3eff3f14d { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lzc2d4d8c93d -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lz37622db7cb
}
function Lzf5c8b2541e {
...
function Lzcc9eb7a240 {
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lzc7e726ebc4 $true
    Lz7147c73077 $true
    Lze3eff3f14d $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lzf5c8b2541e -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lz7ad492c7de 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lzf0779004ed -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Test-Path $jobDir) { (Show-Msg -Text ("A job folder '$slug' already exists. Open it instead, or choose another name.")); return }
        ...
        Lz2ca2d65834 -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lzcc9eb7a240 -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz979e6173b8 })
$ctrl.MnuSettingsApi.Add_Click({ Lz5a93cc726d })
$ctrl.MnuSettingsWizard.Add_Click({ Lzf7040c5b35 })
$ctrl.MnuSettingsEmail.Add_Click({ Lzebdf7fd8ff })
$ctrl.MnuSettingsTuning.Add_Click({ Lz6a86ab2340 })
$ctrl.MnuSettingsDecommission.Add_Click({ Lz47f97f2214 })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lzeb29376774 -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lz704a71fa3d {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lz0403dc684e 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lz704a71fa3d "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lz704a71fa3d (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lz159d723349 $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lz159d723349 $false }
    if (-not $b) { return }
...
function Lzbc231ad03e { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lz0f3ced0b73 {
    $path = Lzbc231ad03e
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Datto Workplace to SharePoint Migrator" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lz0f3ced0b73)) -Caption ('Licence')) | Out-Null })
function Lz3e52c9b3d3 {
...
function Lzd57fad026e {
    param([string]$For = 'copy')
    if (Test-Path (Lzbc231ad03e)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        (Show-Msg -Text ("Licence installed.`n`n" + (Lz0f3ced0b73)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-UpdateCheck {
    try {
        ...
        Lz159d723349 $true 'Checking for updates...'
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lz159d723349 $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lzeb29376774 -Title 'About' -Heading 'Datto Workplace to SharePoint Migrator' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lz54d26bdb78 } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lz9770481f80 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lzf0779004ed -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lze309eb3ddb {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lzc7e726ebc4 $false
    Lz7147c73077 $false
}
$ctrl.MnuJobClose.Add_Click({ Lze309eb3ddb; Lz7ad492c7de 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lze309eb3ddb
    try { if ($dir -and (Test-Path $dir)) { Remove-Item $dir -Recurse -Force -ErrorAction Stop } ; Lz37622db7cb; Lz7ad492c7de "Deleted job '$nm'" }
    catch { (Show-Msg -Text ("Could not fully delete: $($_.Exception.Message)")) }
})
function Lzcf0c95bb63 {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lz159d723349 $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lze4f5f8c41f 'Signing in to Datto and listing projects...'
        $script:Projects = Lz924ade8444
        Lze4f5f8c41f "Found $($script:Projects.Count) project(s)."
        Lz035ffbe64c
        try { Lzf21b2c3158; Lze4f5f8c41f 'Connected to Microsoft 365.' } catch { Lze4f5f8c41f 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lze4f5f8c41f "Technical detail: $($_.Exception.Message)" }
        Update-ProjectList
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lz7ad492c7de 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lze4f5f8c41f 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lze4f5f8c41f "Technical detail: $($_.Exception.Message)"
            Lz7ad492c7de "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lz7ad492c7de 'Connect failed'
        }
    }
    finally { Lz159d723349 $false }
}
$ctrl.BtnConnect.Add_Click({ Lzcf0c95bb63 })
function Lz76bf4d4694 {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lz54d26bdb78 } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lz7ad492c7de "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lzcf0c95bb63 -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lzed261cb5c9
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lze9303648d3).Count } catch {}
...
function Lz2018db3a34 {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lze6b49ba7bb) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lze6b49ba7bb) }
}
$ctrl.BtnApply.Add_Click({
    ...
    Lz7ad492c7de "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lzd6d1ec0c3e })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lz657cb11b02 -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Update-ProjectList; [void](Write-MappingQuiet); Lz7ad492c7de "Set $($sel.Count) project(s) to skip"; return }
    $d = Lz2018db3a34
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lz041d74a995); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Update-ProjectList
    [void](Write-MappingQuiet)
    Lz7ad492c7de "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lz657cb11b02 -Count (@($script:Projects).Count))) { return }
    $d = Lz2018db3a34
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Update-ProjectList; [void](Write-MappingQuiet); Lz7ad492c7de 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Update-ProjectList
    [void](Write-MappingQuiet)
    Lz7ad492c7de "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lz159d723349 $true 'Looking up libraries...'
        $libs = Lza4ea0c897f -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lz159d723349 $false; return }
    finally { Lz159d723349 $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz160ad22b3d -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lz7ad492c7de "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lz7ad492c7de "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lz159d723349 $true 'Opening destination...'
        Lzf21b2c3158
        $res = Lz361ee91c93
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lz159d723349 $false; return }
    finally { Lz159d723349 $false }
    $picked = Lzdc5d41effc -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lz7ad492c7de ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnBrowseSource) { $ctrl.BtnBrowseSource.Add_Click({
    ...
    try { $picked = Lz572b0a19f2 -ProjectId $p.Id -ProjectName $p.Name }
    catch { (Show-Msg -Text ("Could not open the project's folders: $($_.Exception.Message)") -Caption ('Browse source')); return }
    if ($null -ne $picked) { $ctrl.TxtSourceSub.Text = $picked; Lz7ad492c7de ("Source set to " + $(if ($picked) { "/$picked" } else { 'the whole project' })) }
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    try {
        Lz159d723349 $true 'Checking the source folder in Datto...'
        $res = Lz4c32531aed -ProjectId $p.Id -SubPath $sub
    } catch { $ctrl.LblSourceCheck.Text = "Source NOT found. $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lz159d723349 $false; return }
    finally { Lz159d723349 $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lz159d723349 $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lz2721ac1295 -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lza4ea0c897f -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lz780e776a9b {
    ...
    $srcLabel = Lz8798ac06b9 -Project $p
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lze6b49ba7bb) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lze9303648d3).Count } catch { }
...
function Lz449d2d1773 {
    ...
    $script:Map[$p.Id] = $d
    Update-ProjectList
    Lz7ad492c7de "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lze4f5f8c41f "Saved $n mapping(s)."
    return $true
}
function Lz035ffbe64c {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lze4f5f8c41f "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lze4f5f8c41f "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lz49661531ef -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lz49661531ef -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lz050afd6c21 {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lz050afd6c21) }
    ...
    if (-not (Lzeb29376774 -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lzd57fad026e)) { return }
    Lz49661531ef -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lz048458fa98 {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lz050afd6c21) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lzd57fad026e)) { return }
    Lz49661531ef -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lzeb29376774 -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lzd57fad026e -For 'verify')) { return }
    Lz49661531ef -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lz49661531ef -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lz566d7b16da {
...
function Lzb5e2a38539 {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lz733707b889 {
    param([string]$Name)
...
function Lz0dd727433b {
    ...
    foreach ($f in $files) { $l = Lz733707b889 $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lz0dd727433b -Folder (Lz566d7b16da) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnOpenAudit.Add_Click({ Lz0dd727433b -Folder (Lzb5e2a38539) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lz983a8631c3 {
    ...
    Lz7ad492c7de "Speed limit set: up $uTxt, down $dTxt"
    Lz5d12701e82 "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lz983a8631c3 })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz983a8631c3 } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz983a8631c3 } })
function Lz3ff2d53fb1 {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lz3ff2d53fb1 -ParentId ([int]$_.ProcessId) } } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    Lz5d12701e82 "----- STOP requested by user at $stamp -----"
    Lz7ad492c7de "Stopping $($script:CurLabel)..."
    try { Lz3ff2d53fb1 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lzf5c8b2541e -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lz5d12701e82 "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lz342cc20f37 {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lz49661531ef -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz5d12701e82 "----- PAUSE requested by user at $stamp -----"
    Lz7ad492c7de "Pausing $($script:CurLabel)..."
    try { Lz3ff2d53fb1 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lzf5c8b2541e -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lz5d12701e82 "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lz3b914bc64b { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lz1231f7ec88 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lzc8f0be54d6 {
...
function Lz9770481f80 {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lz42ec17158a -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lz5a93cc726d {
    ...
    function Lzdf17ccb4ad { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz0c31fbe775 { param($Label,$Control,$Info)
    ...
    function Lze916b424c0 { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lzdf17ccb4ad 'Datto Workplace'
    ...
    $parts = Lz1231f7ec88 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz0c31fbe775 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lz0c31fbe775 'Cell (number)' (Lze916b424c0 ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lz0c31fbe775 'Datto Client ID' (Lze916b424c0 ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lz0c31fbe775 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lzdf17ccb4ad 'Microsoft 365'
    $tbTid = Lz0c31fbe775 'Tenant ID' (Lze916b424c0 ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lz0c31fbe775 'App (Client) ID' (Lze916b424c0 ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lzdf17ccb4ad 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lz0c31fbe775 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lz0c31fbe775 '.pfx file' (Lze916b424c0 '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lz0c31fbe775 '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lzdf17ccb4ad 'SharePoint and OneDrive'
    ...
    $tbSp  = Lz0c31fbe775 'SharePoint root URL' (Lze916b424c0 ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lz0c31fbe775 'OneDrive host URL'   (Lze916b424c0 ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lz0c31fbe775 'Team site base URL'  (Lze916b424c0 ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lz0c31fbe775 'Default site URL'    (Lze916b424c0 ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lz0c31fbe775 'Email / UPN domain'  (Lze916b424c0 ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lzc2d4d8c93d -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lz9770481f80 -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz60e5bf21b2 $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lz3b914bc64b $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lz54d26bdb78 }
        ...
        try {
            ...
            else { $tok=Lz9770481f80 -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lz3b914bc64b $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lzc2d4d8c93d -Name 'DattoEndpointUrl' -Value $ep
            Lzc2d4d8c93d -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lzc2d4d8c93d -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lzc2d4d8c93d -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lzc2d4d8c93d -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lzc2d4d8c93d -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lzc2d4d8c93d -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lzc2d4d8c93d -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lzc2d4d8c93d -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lza6d4c00b5d -Value $script:AS.pbSec.Password
            }
...
function Lz492d9c94ee { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lz2ca2d65834 { param($Cfg,[string]$Path,$Value)
...
function Lz6a86ab2340 {
    ...
    function Lzfebcdbb55f { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec)
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lz492d9c94ee -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lzfebcdbb55f -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lz2ca2d65834 -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lz2ca2d65834 -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lz2ca2d65834 -Cfg $script:SetCfg -Path $s.Path -Value ([bool]($raw -match '^(1|true|yes|on)$')) }
                else { Lz2ca2d65834 -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lz55eaf94a55 {
    ...
    function Lzb8768aaa60 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lz1231f7ec88 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lzb8768aaa60 -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lzb8768aaa60 -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lzb8768aaa60 -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lzb8768aaa60 -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lz3b914bc64b $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lz54d26bdb78 }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lzc2d4d8c93d -Name 'DattoEndpointUrl' -Value $ep
            Lzc2d4d8c93d -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lza6d4c00b5d -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz979e6173b8 {
    ...
    $pre = Lz05977691b0 -Title 'Prerequisites'
    ...
    $dat = Lz05977691b0 -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lz92473c85e3 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lz05977691b0 -Title 'Microsoft 365' -Buttons @($bMs, (Lz92473c85e3 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lz05977691b0 -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lz55eaf94a55 } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz1a628c90d5; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lzf7040c5b35 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz1a628c90d5; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lzebdf7fd8ff } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz1a628c90d5; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lz1a628c90d5
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lza85977306d {
...
function Lzd0a7adeaf8 {
    ...
    $bOpen.Add_Click({ try { Lzfec552282e -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lz5d6e51ac68 {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lzc19bfae9ab {
    ...
    Lzd0a7adeaf8 -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lzfec552282e -Url $pageUrl
    ...
    } finally { Lz5d6e51ac68 }
}
function Lz2c347b6bf3 {
    try {
...
function Lz7e87fabfce {
    ...
    if (-not (Lz33bdc4d99e)) {
        Lza85977306d 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B54708' -Strong
        try {
            Lzed32ed51bc -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lza85977306d "The standard sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B54708' -Strong
        }
    } else {
        Lza85977306d 'Using the CODE sign-in (the reliable path on this machine): a small window will show your code, always on top.' '#B54708' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lzc19bfae9ab -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lza85977306d $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lzc6fb536c28 -Token $at -Claim 'tid'
            $wizTokAcct = Lzc6fb536c28 -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lzc6fb536c28 -Token $at -Claim 'preferred_username' }
            ...
            Lza85977306d "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lza85977306d 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lza85977306d "Signed in to tenant $tid as $wizAcct." '#12B76A'
    $newThumb = $null
    try {
        Lza85977306d 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lza85977306d "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B54708' }
            Lza85977306d "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#12B76A'
        } else {
            Lza85977306d "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lza85977306d 'App registration created.' '#12B76A'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lza85977306d 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lza85977306d "Certificate created in this user's store. Thumbprint $newThumb." '#12B76A'
        Lza85977306d 'Uploading the certificate''s public key to the app registration...'
        ...
        Lza85977306d "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#12B76A'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lzaec6af3a9a -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lza85977306d 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lzaec6af3a9a -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lza85977306d $consentMsg '#12B76A'
        Lzc2d4d8c93d -Name 'TenantId'       -Value $tid
        Lzc2d4d8c93d -Name 'GraphClientId'  -Value $appId
        Lzc2d4d8c93d -Name 'CertThumbprint' -Value $newThumb
        Lza85977306d 'Connection details saved on this computer.' '#12B76A'
    } catch {
        ...
        if ($friendly) { Lza85977306d $friendly '#B42318' -Strong; Lza85977306d "Technical detail: $m" }
        else { Lza85977306d "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lza85977306d "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lza85977306d 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lz9770481f80 -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lzc2d4d8c93d -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lzc8f0be54d6 $spUrl
            if ($der) {
                Lzc2d4d8c93d -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lzc2d4d8c93d -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lzc2d4d8c93d -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lza85977306d "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#12B76A'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lzaec6af3a9a -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lza85977306d "The new sign-in has not become active yet: $($_.Exception.Message)" '#B54708' -Strong
                   Lza85977306d 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz60e5bf21b2 $org
            if ($u) { Lzc2d4d8c93d -Name 'UpnDomain' -Value $u; Lza85977306d "Email domain detected: $u" '#12B76A' }
        } catch { Lza85977306d 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lza85977306d 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#12B76A' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lzf7040c5b35 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lzbb778b69b0 $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lz7e87fabfce -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lz6782039c66) {
                    Lza85977306d 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lza85977306d $t $c } else { Lza85977306d $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lza85977306d 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B54708'
                }
            }
            ...
            Lza85977306d "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lzc19bfae9ab -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lzed32ed51bc -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lzfec552282e -Url 'https://login.microsoft.com/device'
                Lz3c5e591a79
                try { Lzed32ed51bc -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lz61f7ae700b }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (-not (Lz33bdc4d99e)) {
            ...
            try {
                Lzed32ed51bc -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lzc19bfae9ab -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lzc6fb536c28 -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lzc6fb536c28 -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lzaec6af3a9a -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lza9693bce3f 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lzaec6af3a9a -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lzaec6af3a9a -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lzc2d4d8c93d -Name 'EmailSender' -Value $senderAddr
        Lzc2d4d8c93d -Name 'EmailEnabled' -Value '1'
...
function Lzebdf7fd8ff {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lz509b4176f9 { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz5f568a5e53 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    Lz509b4176f9 'Sender and recipients'
    ...
    [void](Lz5f568a5e53 -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lz5f568a5e53 -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lz5f568a5e53 -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lz509b4176f9 'Subject and attachments'
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz6718682041 -Template $script:ES.tbSubj.Text -Vars (Lz0426f7d0cf)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz6718682041 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz0426f7d0cf)
            $tok = Lz9770481f80 -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lzc2d4d8c93d -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lzc2d4d8c93d -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lzc2d4d8c93d -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lzc2d4d8c93d -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lzc2d4d8c93d -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lzc2d4d8c93d -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lzc2d4d8c93d -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lzc2d4d8c93d -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lzc2d4d8c93d -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lzc2d4d8c93d -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lzc2d4d8c93d -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lzc2d4d8c93d -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lz0b5578465a {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lzc19bfae9ab -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lz47f97f2214 {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lz0b5578465a -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lz1a628c90d5 } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lzae58536ea7 {
    return ((Lz49d74a6ff4) -and (Lz6782039c66))
}
Lz980364a018
Lz61f7ae700b
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lz5d12701e82 "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lzae58536ea7)) {
        try { Lz979e6173b8 } catch {}
        if (Lzae58536ea7) { Lz7ad492c7de 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lz7ad492c7de 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lz7ad492c7de 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ((Lzae58536ea7) -and -not (Test-Path (Lzbc231ad03e))) {
        try { Lzf7b3e0f7b9 } catch {}
    }
})
try {
    Add-Type -MemberDefinition '
