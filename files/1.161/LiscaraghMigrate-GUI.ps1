...
function Lz0003ec5770 {
    param([string]$Severity)
    Lzb34a373a83 -Severity $Severity
    try {
...
function Lz531285efed { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lz93d306bf31 {
...
function Lz788adda938 {
    ...
    if (-not $s) { $s = Lz05bd7caa22 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lz25ddca8bcf {
    param([string]$Value)
    if (Lz93d306bf31 -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lz258e05032f {
    $f = @(Lzffe5096b65)
    if ($f.Count -eq 1) { return $f[0] }
    return ''
}
function Lza4f6504870 {
...
function Lzaeb6d66768 {
    param([string[]]$Picked)
    $result = New-Object System.Collections.Generic.List[string]
    foreach ($e in @(Lzffe5096b65)) { [void]$result.Add($e) }
    $dropped = New-Object System.Collections.Generic.List[string]
    foreach ($p in @($Picked | ForEach-Object { "$_".Trim().Trim('/').Trim('\') } | Where-Object { $_ })) {
        if ($result -ccontains $p) { continue }
        $covered = $false
        foreach ($k in @($result)) { if (Lza4f6504870 -Child $p -Parent $k) { [void]$dropped.Add($p); $covered = $true; break } }
        if ($covered) { continue }
        foreach ($narrow in @($result | Where-Object { Lza4f6504870 -Child $_ -Parent $p -Strict })) { [void]$result.Remove($narrow); [void]$dropped.Add($narrow) }
        [void]$result.Add($p)
    }
    Lze8fa042159 -Folders @($result)
    if ($dropped.Count) { Lzfc19f0d166 ("Overlapping folder(s) merged, kept the broader one: " + ((@($dropped) | Select-Object -Unique) -join ', ')) }
}
function Lz005b2f44c4 {
    $one = $true; try { $one = (@(Lz6dd307d87c).Count -le 1) } catch {}
    $n = @(Lzffe5096b65).Count
...
function Lz54cdb5f9d0 {
    ...
    try {
        ...
        $norm = Lz2349abc2d6 -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lzfc19f0d166 "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz8688f18038
    } catch { }
}
function Lz2349abc2d6 {
...
function Lz9980b80454 {
    ...
    return @{ SubfolderCount = @(Lz47c29a356d -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lz4832923d5b {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lz77722c5682 -Path "sites/$siteId"
    } catch { return $null }
}
function Lza58cf15bfa {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lz77722c5682 -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lzee1ccca89c {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lz77722c5682 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz04fbaa3fd6 {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lzb5b5b5ff13 -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
function Lze3e2af23f2 {
    param($State, [string]$Current)
    $out = New-Object System.Collections.Generic.List[string]
    foreach ($s in @($State.Lst.SelectedItems)) {
        $name = "$s"
        if (-not (Lz3652131e64 -Item $name)) { continue }
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lza58cf15bfa -Search $s.Txt.Text)
...
"@
    ...
    Lz04fbaa3fd6
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lz04fbaa3fd6 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lz04fbaa3fd6 } })
    $new.Add_Click({
        $n = Lza392982cbe -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lz81789f7a37 -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lz04fbaa3fd6; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lzcbfc0c3a0b -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lzc30cb05aeb -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lzcbfc0c3a0b -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lzf5d263f7ec {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lz47c29a356d -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lzf5d263f7ec
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lzf5d263f7ec } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lzf5d263f7ec } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lzcbfc0c3a0b -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lze3e2af23f2 -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lzcbfc0c3a0b -State $script:SP
    $r = $w.ShowDialog()
...
function Lzfc19f0d166 { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lz27323607f7 { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lz4291b91c84 {
    ...
    if ($hasRegime) {
        $etaA = Lz77dc183a9c -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz77dc183a9c -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lz77dc183a9c -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz77dc183a9c -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lz95e027122c {
    ...
    try {
        ...
        $etaText = Lz4291b91c84 -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lz34c537cdcd {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lz4c20f2b4be -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lzc9e3f4e2bd -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lz95e027122c -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lz51a3a7af81 $Line) + "`r`n")
...
function Lz94c3652d09 { param([string]$Text) Lz34c537cdcd $Text }
function Lzf06c5d1806 {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lz334c053912 $true 'Connecting to Microsoft 365...'
        Lzf13f6e6784
    } catch {
        ...
        Lz334c053912 $false; return
    } finally { Lz334c053912 $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lzb18215b94f -StartSearch $seed
    if ($picked) {
        ...
        Lzfc19f0d166 "Site set to $picked"
        Lz8688f18038
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lzf06c5d1806 }
    Lz8688f18038
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lz5a263099d0 }
    Lz8688f18038
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz8688f18038 })
$ctrl.TxtFolder.Add_TextChanged({ Lz8688f18038 })
if ($ctrl.LstSource) { $ctrl.LstSource.Add_SelectionChanged({ Lz005b2f44c4 }) }
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz8688f18038 }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz8688f18038 }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lz0486ce4743 -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lz54cdb5f9d0 })
$ctrl.TxtLoc.Add_TextChanged({ Lz8688f18038 })
$ctrl.TxtLib.Add_TextChanged({ Lz8688f18038 })
$script:NestSuspend = $false
function Lzdedc2a8534 {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lzcd15956d86 -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lzfc19f0d166 $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lzdedc2a8534 })
$ctrl.ChkNest.Add_Unchecked({ Lzdedc2a8534 })
$script:ProjectRows = @()
$script:RunStatus = @{}
...
function Lz7fa280771d {
    ...
    if (-not $hasProjects) {
        $sd = $false
        if (Get-Command Test-ConnectionConfigured -ErrorAction SilentlyContinue) { try { $sd = [bool](Lzbe1515e933) } catch {} }
        Lzba4e28ef83 $sd
    }
...
function Lz8a792840ef {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lz9980b80454 -ProjectId $p.Id -SubPath $sub) }
...
function Lzbc9d17b164 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lz77722c5682 -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz7bf0234f09 {
    ...
    if (($EngineArgs -contains 'Transfer') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck') -or ($EngineArgs -contains 'PreFlight')) {
        try { Lz334c053912 $true 'Checking source folders...'; $srcOk = Lz8a792840ef } finally { Lz334c053912 $false }
        if (-not $srcOk) { return }
    }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lz334c053912 $true 'Checking destinations...'; $dstOk = Lzbc9d17b164 } finally { Lz334c053912 $false }
        if (-not $dstOk) { return }
    }
    ...
    $script:Timer.Add_Tick({
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lz34c537cdcd $lines[$k] } catch {} }
                }
            }
        } catch {}
        ...
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lz34c537cdcd $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lz34c537cdcd $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Format-Span ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lz83a6c248da) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lzcbdf2adf0c -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lz34c537cdcd "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lz34c537cdcd "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lzfc19f0d166 ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
            ...
            Lz2c850b05dc
            if (-not $script:Paused) { Lz0003ec5770 -Severity $sev }
            if ($script:RunStatus.Count) { try { Lz7fa280771d } catch {} }
            try {
            ...
            try {
                if ($script:CurLabel -eq 'Check readiness' -or $script:CurLabel -eq 'Preview (no upload)') {
                    $newest = Get-ChildItem (Join-Path (Lz83a6c248da) 'report-*.html') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
function Lz7967f6e8ee { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lzb30efc7436 { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lz3c33ca69e8 {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lz20de35dfef -ConfigFile $s.Tag; Lz040a816e37 } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lz72f42e262e { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lz531285efed -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lz3c33ca69e8
}
function Lz3c290b47af {
...
function Lz20de35dfef {
    param([string]$ConfigFile)
    if ($script:Sched) { Lz1463b82527 -Why 'Schedule cancelled (job changed).' }
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lzb30efc7436 $true
    Lz7967f6e8ee $true
    Lz72f42e262e $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lz3c290b47af -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lzfc19f0d166 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
}
function Lz06e08d464c {
    param([string]$Dir)
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lza392982cbe -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Lz487c27d1ad -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Open it instead, or choose another name.")); return }
    if ((Test-Path $jobDir) -and -not (Lz06e08d464c -Dir $jobDir)) {
        ...
        Lzcd15956d86 -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lz20de35dfef -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuJobOpen.Add_Click({
    ...
    [void]$w.ShowDialog()
    if ($script:OpenPick) { Lz20de35dfef -ConfigFile $script:OpenPick; Lz040a816e37 }
})
$ctrl.MnuJobSave.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')); return }
    if (Save-Mapping) { Lzfc19f0d166 'Migration job saved.' }
})
$ctrl.MnuJobSaveAs.Add_Click({
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open a migration job first, then Save As to copy it.')); return }
    try {
        $name = Lza392982cbe -Prompt "New name for the copy:" -Title 'Save job as'
        if (-not $name -or -not $name.Trim()) { return }
        $name = $name.Trim()
        $slug = ($name -replace '[^A-Za-z0-9._-]','_').Trim('_'); if (-not $slug) { $slug = 'job' }
        $jobDir = Join-Path $script:JobsRoot $slug
        if (Lz487c27d1ad -Dir $jobDir) { (Show-Msg -Text ("A job named '$slug' already exists. Choose another name.")); return }
        if ((Test-Path $jobDir) -and -not (Lz06e08d464c -Dir $jobDir)) {
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz3053f1ac63 })
$ctrl.MnuSettingsApi.Add_Click({ Lz1880548691 })
$ctrl.MnuSettingsWizard.Add_Click({ Lzb0e54f3fa5 })
$ctrl.MnuSettingsEmail.Add_Click({ Lz957e3d1853 })
$ctrl.MnuSettingsTuning.Add_Click({ Lzfeb2132bba })
$ctrl.BtnFilters.Add_Click({ Lz3b3792aec8 })
$ctrl.MnuSettingsDecommission.Add_Click({ Lzc771795d7b })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lz08bda47256 -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lzb57aa36e25 {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lz05bd7caa22 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lzb57aa36e25 "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lzb57aa36e25 (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lz334c053912 $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lz334c053912 $false }
    if (-not $b) { return }
...
function Lz8eb57c9fff { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lzbbafeadbfb {
    $path = Lz8eb57c9fff
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Liscaragh Migrate" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lzbbafeadbfb)) -Caption ('Licence')) | Out-Null })
function Lz544da050b4 {
...
function Lzca9ced7d88 {
    param([string]$For = 'copy')
    if (Test-Path (Lz8eb57c9fff)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        try {
            ...
            Lz531285efed -Name 'LicenceAppliedId'  -Value "$($lpl.LicenceId)"
            Lz531285efed -Name 'LicenceAppliedUtc' -Value ((Get-Date).ToUniversalTime().ToString('o'))
        } catch {}
        (Show-Msg -Text ("Licence installed.`n`n" + (Lzbbafeadbfb)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-FullReinstall {
    ...
    Lz334c053912 $true "Downloading the installer for version $($Payload.version)..."
    ...
    catch { Lz334c053912 $false; (Show-Msg -Text ("The installer could not be downloaded, so nothing has been changed.`n`n$($_.Exception.Message)") -Caption ('Reinstall') -Icon ('Warning')) | Out-Null; return }
...
function Invoke-UpdateCheck {
    ...
    try {
        ...
        if (-not $Silent) { Lz334c053912 $true 'Checking for updates...' }
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lz334c053912 $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lz08bda47256 -Title 'About' -Heading 'Liscaragh Migrate' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lz788adda938 } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lzc66f41b4bb -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lza392982cbe -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lz346c29e327 {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lzb30efc7436 $false
    Lz7967f6e8ee $false
}
$ctrl.MnuJobClose.Add_Click({ Lz346c29e327; Lzfc19f0d166 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lz346c29e327
    if (Lz06e08d464c -Dir $dir) {
        Lz3c33ca69e8; Lzfc19f0d166 "Deleted job '$nm'"
    } else {
        Lz3c33ca69e8
        ...
        Lzfc19f0d166 "Could not fully delete job '$nm' (a file is in use)"
    }
})
function Lz1072c7583b {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lz334c053912 $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lz94c3652d09 'Signing in to Datto and listing projects...'
        $script:Projects = Lz1a907f6a3c
        Lz94c3652d09 "Found $($script:Projects.Count) project(s)."
        Lzd80c04f4ad
        try { Lzf13f6e6784; Lz94c3652d09 'Connected to Microsoft 365.' } catch { Lz94c3652d09 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lz94c3652d09 "Technical detail: $($_.Exception.Message)" }
        Lz7fa280771d
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lzfc19f0d166 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lz94c3652d09 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lz94c3652d09 "Technical detail: $($_.Exception.Message)"
            Lzfc19f0d166 "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lzfc19f0d166 'Connect failed'
        }
    }
    finally { Lz334c053912 $false }
}
$ctrl.BtnConnect.Add_Click({ Lz1072c7583b })
function Lz040a816e37 {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lz788adda938 } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lzfc19f0d166 "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lz1072c7583b -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lzfc20830d2a
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lz6dd307d87c).Count } catch {}
    ...
    if ($script:Map.ContainsKey($p.Id)) {
        ...
        $ctrl.TxtLoc.Text = Lz5a263099d0
        $ctrl.TxtLib.Text = ''; $ctrl.TxtFolder.Text = ''; Lze8fa042159 -Folders @()
...
function Lz834bd6a825 {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lzc6e13b319b) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lzc6e13b319b) }
}
$ctrl.BtnApply.Add_Click({
    $p = Lzfc20830d2a
    if (-not $p) { (Show-Msg -Text ('Select a project first.')); return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Remove($p.Id) | Out-Null; Lz7fa280771d; [void](Write-MappingQuiet); return }
    $d = Lz834bd6a825
    if ($d -eq 'ERR') { return }
    ...
    Lzfc19f0d166 "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lz3eb21caf82 })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lz61e3d48cd6 -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Lz7fa280771d; [void](Write-MappingQuiet); Lzfc19f0d166 "Set $($sel.Count) project(s) to skip"; return }
    $d = Lz834bd6a825
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lzf06c5d1806); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPaths=$keepSrcList; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Lz7fa280771d
    [void](Write-MappingQuiet)
    Lzfc19f0d166 "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lz61e3d48cd6 -Count (@($script:Projects).Count))) { return }
    $d = Lz834bd6a825
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Lz7fa280771d; [void](Write-MappingQuiet); Lzfc19f0d166 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Lz7fa280771d
    [void](Write-MappingQuiet)
    Lzfc19f0d166 "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lz334c053912 $true 'Looking up libraries...'
        $libs = Lzfd794a8e40 -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lz334c053912 $false; return }
    finally { Lz334c053912 $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz5cbeb05c5b -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lzfc19f0d166 "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lzfc19f0d166 "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lz334c053912 $true 'Opening destination...'
        Lzf13f6e6784
        $res = Lzee1ccca89c
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lz334c053912 $false; return }
    finally { Lz334c053912 $false }
    $picked = Lz97ca06f496 -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lzfc19f0d166 ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnAddSource) { $ctrl.BtnAddSource.Add_Click({
    ...
    try { $picked = Lz81a59f47a4 -ProjectId $p.Id -ProjectName $p.Name }
    ...
    Lzaeb6d66768 -Picked $arr
    Lzfc19f0d166 ("Source folders: " + (@(Lzffe5096b65) -join ', '))
}) }
if ($ctrl.BtnRemoveSource) { $ctrl.BtnRemoveSource.Add_Click({
    ...
    Lze8fa042159 -Folders @(Lzffe5096b65 | Where-Object { $sel -notcontains $_ })
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    $bad = New-Object System.Collections.Generic.List[string]
    try {
        Lz334c053912 $true 'Checking the source folder(s) in Datto...'
        foreach ($s in $subs) { try { $null = Lz9980b80454 -ProjectId $p.Id -SubPath $s } catch { [void]$bad.Add($s) } }
    } catch { $ctrl.LblSourceCheck.Text = "Could not check the source: $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lz334c053912 $false; return }
    finally { Lz334c053912 $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lz334c053912 $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lz77722c5682 -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lzfd794a8e40 -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lz8688f18038 {
    ...
    $srcLabel = Lz71b4afec0b -Project $p
    $folders  = @(Lzffe5096b65)
    $srcSub   = Lz258e05032f
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lzc6e13b319b) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lz6dd307d87c).Count } catch { }
...
function Lz4c74580efb {
    ...
    $script:Map[$p.Id] = $d
    Lz7fa280771d
    Lzfc19f0d166 "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lz94c3652d09 "Saved $n mapping(s)."
    return $true
}
function Lzd80c04f4ad {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lz94c3652d09 "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lz94c3652d09 "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lz7bf0234f09 -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lz7bf0234f09 -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lz5701557f23 {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lz5701557f23) }
    ...
    if (-not (Lz08bda47256 -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lze09a9ee237)) { return }
    if (-not (Lzca9ced7d88)) { return }
    Lz7bf0234f09 -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lzac5cdb5dd7 {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lz5701557f23) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lze09a9ee237)) { return }
    if (-not (Lzca9ced7d88)) { return }
    Lz7bf0234f09 -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnRerunFailed.Add_Click({
    ...
    if (-not (Lz08bda47256 -Title 'Rerun failed files only' -Heading 'Rerun failed files only' -Sections $sections -Yes 'Rerun the failed files' -No 'Cancel')) { return }
    if (-not (Lze09a9ee237)) { return }
    if (-not (Lzca9ced7d88)) { return }
    Lz7bf0234f09 -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','NewerWins','-Execute','-FailedOnly') -Label 'Rerun failed files only'
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lz08bda47256 -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lzca9ced7d88 -For 'verify')) { return }
    Lz7bf0234f09 -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lz7bf0234f09 -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lz83a6c248da {
...
function Lz2c850b05dc {
    try { if ($ctrl.BtnRerunFailed) { $ctrl.BtnRerunFailed.IsEnabled = (Lze579d77180) } } catch {}
}
function Lz7481dfcabc {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lz7c3f19d3a4 {
    param([string]$Name)
...
function Lzd42528b662 {
    ...
    foreach ($f in $files) { $l = Lz7c3f19d3a4 $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lzd42528b662 -Folder (Lz83a6c248da) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnCertificate.Add_Click({
    if (Save-Mapping) {
        Lz7bf0234f09 -EngineArgs @('-Action','Certificate') -Label 'Completion certificate' -OnComplete {
            $rr = Lz83a6c248da
            $cert = $null
...
$ctrl.BtnOpenAudit.Add_Click({ Lzd42528b662 -Folder (Lz7481dfcabc) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lz8070afb5ed {
    ...
    Lzfc19f0d166 "Speed limit set: up $uTxt, down $dTxt"
    Lz34c537cdcd "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lz8070afb5ed })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz8070afb5ed } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz8070afb5ed } })
function Lzf13658e75b {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lzf13658e75b -ParentId ([int]$_.ProcessId) } } catch {}
...
function Lz30f70a2c09 {
    $r = ($script:Proc -and -not $script:Proc.HasExited)
    $f = ($script:FinProc -and -not $script:FinProc.HasExited)
    return ($r -or $f)
}
function Lz02d12a4805 { param([string]$T) try { $ctrl.BtnSchedule.Content.Text = $T } catch {} }
function Lz1463b82527 {
    param([string]$Why = '')
    if ($script:SchedTimer) { try { $script:SchedTimer.Stop() } catch {}; $script:SchedTimer = $null }
    $script:Sched = $null
    Lz02d12a4805 'Schedule for later / overnight...'
    if ($Why) { Lzfc19f0d166 $Why }
}
function Lzfb02ecacf4 {
    try { $p = Join-Path (Lz83a6c248da) 'lastrun-outcome.json'; if (Test-Path $p) { return (Get-Content $p -Raw | ConvertFrom-Json) } } catch {}
    return $null
}
function Lz6e2863df31 {
    if (-not $script:Proc -or $script:Proc.HasExited) { return }
    $script:QuietPausing = $true
    $ctrl.BtnStop.IsEnabled = $false
    Lz34c537cdcd "----- Quiet hours reached at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')): pausing the copy. Files already uploaded are kept. -----"
    Lzfc19f0d166 'Quiet hours: pausing until the next window...'
    try { Lzf13658e75b -ParentId ([int]$script:Proc.Id) } catch {}
    Lz3c290b47af -Status Cancelled -Note 'Finalising for quiet hours...' -OnDone {
        try { Lz34c537cdcd '----- Paused for quiet hours. It will resume as a Sync at the next window while this app stays open. -----' } catch {}
    }
}
function Lz3b20e1114d {
    param([string[]]$RunArgs,[string]$Label)
    Lz34c537cdcd "----- Scheduled: starting '$Label' at $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss')) -----"
    Lz7bf0234f09 -EngineArgs $RunArgs -Label $Label
    return ($script:Proc -and -not $script:Proc.HasExited)
}
function Lzb1e36b6368 { param([string]$Status)
    ...
    switch ("$($s.State)") {
        'waiting' {
            if ($now -ge $s.StartAt -and -not $busy) {
                ...
                elseif (Lz3b20e1114d -RunArgs $s.Args -Label $s.Label) { $s.State = 'running'; $s.Launches++ }
                else { Lz1463b82527 -Why 'The scheduled run could not start (see the message). Schedule cancelled.' }
            } else {
                $left = $s.StartAt - $now; if ($left.Ticks -lt 0) { $left = [TimeSpan]::Zero }
                Lz02d12a4805 ("Scheduled: starts $($s.StartAt.ToString('ddd HH:mm')) ($(Format-Span $left) to go) - click to cancel")
            }
        }
        'running' {
            if (-not $busy) {
                $oc = Lzfb02ecacf4; $status = if ($oc) { "$($oc.Status)" } else { '' }
                if ($s.Mode -eq 'once') { Lz1463b82527 -Why 'Scheduled run finished.'; Lzb1e36b6368 $status }
                elseif ($status -eq 'COMPLETED') { Lz1463b82527 -Why 'Scheduled migration completed.'; Lzb1e36b6368 $status }
                else { $s.State = 'paused' }
            } elseif ($s.Mode -eq 'window' -and -not (Test-InWindow $m $s.FromMin $s.ToMin)) {
                Lz6e2863df31; $s.State = 'paused'
            } else {
                $until = if ($s.Mode -eq 'window') { " (until $(Format-MinOfDay $s.ToMin))" } else { '' }
                Lz02d12a4805 ("Scheduled: running$until - click to cancel")
            }
        }
        'paused' {
            if ((Test-InWindow $m $s.FromMin $s.ToMin) -and -not $busy) {
                if (Lz3b20e1114d -RunArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode','AddMissing','-Execute') -Label 'Sync new and changed') { $s.State = 'running'; $s.Launches++ }
            } else {
                $nf = Get-NextWindowOpen $s.FromMin
                Lz02d12a4805 ("Scheduled: waiting for the next window ($($nf.ToString('ddd HH:mm'))) - click to cancel")
            }
        }
    }
}
function Lz5bc75b2776 {
...
$ctrl.BtnSchedule.Add_Click({
    if ($script:Sched) {
        if ((Show-Msg -Text ("A schedule is set:" + [Environment]::NewLine + $script:Sched.Summary + [Environment]::NewLine + [Environment]::NewLine + "Cancel it?") -Caption ('Scheduled run') -Buttons ('YesNo') -Icon ('Question')) -eq 'Yes') { Lz1463b82527 -Why 'Schedule cancelled.' }
        return
    }
    if (-not $script:JobOpen) { (Show-Msg -Text ('Open or create a migration job first.')) | Out-Null; return }
    if (Lz30f70a2c09) { (Show-Msg -Text ('A run is in progress. Wait for it to finish, then set a schedule.') -Caption ('Cannot schedule now') -Icon ('Warning')) | Out-Null; return }
    if (-not (Save-Mapping)) { return }
    $mappedCount = @($script:Projects | Where-Object { $script:Map.ContainsKey($_.Id) }).Count
    ...
    $script:SchedTimer.Add_Tick({ try { Lz22a4fa9957 } catch {} })
    $script:SchedTimer.Start()
    try { Lz22a4fa9957 } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    if ($script:Sched) { Lz1463b82527 -Why 'Schedule cancelled (you stopped the run).' }
    $script:Stopping = $true
    $ctrl.BtnStop.IsEnabled = $false
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz34c537cdcd "----- STOP requested by user at $stamp -----"
    Lzfc19f0d166 "Stopping $($script:CurLabel)..."
    try { Lzf13658e75b -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lz3c290b47af -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lz34c537cdcd "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lz56482aadfc {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lz7bf0234f09 -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz34c537cdcd "----- PAUSE requested by user at $stamp -----"
    Lzfc19f0d166 "Pausing $($script:CurLabel)..."
    try { Lzf13658e75b -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lz3c290b47af -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lz34c537cdcd "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lz0f800d6d9b { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lz0f8bed5d92 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lz1fc8418a9e {
...
function Lzc66f41b4bb {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lz1b06de4294 -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lz1880548691 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    ...
    function Lz66c9b6e5e7 { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz132b091862 { param($Label,$Control,$Info)
    ...
    function Lz0790014334 { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lz66c9b6e5e7 'Datto Workplace'
    ...
    $parts = Lz0f8bed5d92 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz132b091862 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lz132b091862 'Cell (number)' (Lz0790014334 ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lz132b091862 'Datto Client ID' (Lz0790014334 ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lz132b091862 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lz66c9b6e5e7 'Microsoft 365'
    $tbTid = Lz132b091862 'Tenant ID' (Lz0790014334 ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lz132b091862 'App (Client) ID' (Lz0790014334 ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lz66c9b6e5e7 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lz132b091862 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lz132b091862 '.pfx file' (Lz0790014334 '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lz132b091862 '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lz66c9b6e5e7 'SharePoint and OneDrive'
    ...
    $tbSp  = Lz132b091862 'SharePoint root URL' (Lz0790014334 ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lz132b091862 'OneDrive host URL'   (Lz0790014334 ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lz132b091862 'Team site base URL'  (Lz0790014334 ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lz132b091862 'Default site URL'    (Lz0790014334 ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lz132b091862 'Email / UPN domain'  (Lz0790014334 ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lz531285efed -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lzc66f41b4bb -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz2cfb127b23 $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lz0f800d6d9b $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lz788adda938 }
        ...
        try {
            ...
            else { $tok=Lzc66f41b4bb -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lz0f800d6d9b $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lz531285efed -Name 'DattoEndpointUrl' -Value $ep
            Lz531285efed -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lz531285efed -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lz531285efed -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lz531285efed -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lz531285efed -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lz531285efed -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lz531285efed -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lz531285efed -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lz25ddca8bcf -Value $script:AS.pbSec.Password
            }
...
function Lzedef4476a0 { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lzcd15956d86 { param($Cfg,[string]$Path,$Value)
...
function Lz3b3792aec8 {
    ...
    $curMax=Lzedef4476a0 -Cfg $cfg -Path 'run.tuning.maxFileSizeMB'; if ($null -eq $curMax) { $curMax=0 }; $sizeBox.Text="$curMax"
    ...
    $curInc=@(Lzedef4476a0 -Cfg $cfg -Path 'run.tuning.includePatterns' | Where-Object { "$_".Trim() })
    $curExc=@(Lzedef4476a0 -Cfg $cfg -Path 'run.tuning.excludePatterns' | Where-Object { "$_".Trim() })
    ...
    $afterPick=New-Object System.Windows.Controls.DatePicker; $afterPick.Margin='0,4,0,4'; $afterPick.Width=150; $afterPick.SelectedDate=(& $toDate (Lzedef4476a0 -Cfg $cfg -Path 'run.tuning.modifiedAfter'))
    ...
    $beforePick=New-Object System.Windows.Controls.DatePicker; $beforePick.Margin='0,4,0,4'; $beforePick.Width=150; $beforePick.SelectedDate=(& $toDate (Lzedef4476a0 -Cfg $cfg -Path 'run.tuning.modifiedBefore'))
    ...
    $save.Add_Click({
        try {
            ...
            Lzcd15956d86 -Cfg $script:FltCfg -Path 'run.tuning.maxFileSizeMB' -Value $sv
            $pats=@("$($script:FltPat.Text)" -split '[;,\r\n]' | ForEach-Object { $_.Trim() } | Where-Object { $_ })
            if ($script:FltInc.IsChecked) { Lzcd15956d86 -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value $pats; Lzcd15956d86 -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value @() }
            else                          { Lzcd15956d86 -Cfg $script:FltCfg -Path 'run.tuning.excludePatterns' -Value $pats; Lzcd15956d86 -Cfg $script:FltCfg -Path 'run.tuning.includePatterns' -Value @() }
            ...
            Lzcd15956d86 -Cfg $script:FltCfg -Path 'run.tuning.modifiedAfter'  -Value $ma
            Lzcd15956d86 -Cfg $script:FltCfg -Path 'run.tuning.modifiedBefore' -Value $mb
...
function Lzfeb2132bba {
    ...
    function Lz1d7251c01e { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec,[string]$Type='text')
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lzedef4476a0 -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lz1d7251c01e -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec -Type $s.Type
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lzcd15956d86 -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lzcd15956d86 -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lzcd15956d86 -Cfg $script:SetCfg -Path $s.Path -Value ([bool]$script:SetBoxes[$s.Path].IsChecked) }
                elseif ($s.Type -eq 'date') {
                    ...
                    Lzcd15956d86 -Cfg $script:SetCfg -Path $s.Path -Value $raw
                }
                else { Lzcd15956d86 -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lz0b7df7c6a6 {
    ...
    function Lzfe37d0845c { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lz0f8bed5d92 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lzfe37d0845c -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lzfe37d0845c -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lzfe37d0845c -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lzfe37d0845c -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lz0f800d6d9b $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lz788adda938 }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz531285efed -Name 'DattoEndpointUrl' -Value $ep
            Lz531285efed -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lz25ddca8bcf -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz3053f1ac63 {
    ...
    $pre = Lzc3e49a2583 -Title 'Prerequisites'
    ...
    $dat = Lzc3e49a2583 -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lzb788654aaa 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lzc3e49a2583 -Title 'Microsoft 365' -Buttons @($bMs, (Lzb788654aaa 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lzc3e49a2583 -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lz0b7df7c6a6 } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz2f8a5fe735; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lzb0e54f3fa5 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz2f8a5fe735; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lz957e3d1853 } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz2f8a5fe735; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lz2f8a5fe735
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lzbe4ef6d8d0 {
...
function Lz62288e6be2 {
    ...
    $bOpen.Add_Click({ try { Lzd2f09eb753 -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lz63dbc6d3c6 {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lzcbeacd9af2 {
    ...
    Lz62288e6be2 -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lzd2f09eb753 -Url $pageUrl
    ...
    } finally { Lz63dbc6d3c6 }
}
function Lzcd93b1d3b1 {
    try {
...
function Lz4ce6446cd4 {
    if (Lzffb5b7facd) { return $false }
...
function Lz7b59ed8bed {
    ...
    if (-not (Get-Module Microsoft.Graph.Authentication -ListAvailable)) {
        Lzbe4ef6d8d0 'The Microsoft Graph module is not installed. Re-run the installer (or run: Install-Module Microsoft.Graph.Authentication -Scope CurrentUser) and try again.' '#B42318' -Strong
        return
    }
    Lzbe4ef6d8d0 'Preparing the Microsoft 365 sign-in. The first time on a new install can take up to a minute while Windows loads the components - please wait, the app has not frozen.' '#475467' -Strong
    try { Import-Module Microsoft.Graph.Authentication -ErrorAction Stop } catch {
        Lzbe4ef6d8d0 "Could not load the Microsoft Graph module: $($_.Exception.Message)" '#B42318' -Strong
        return
    }
    Lzbe4ef6d8d0 "Sign in with an account that can manage Microsoft 365 (a Global Administrator is simplest). The first time, Microsoft asks you to accept the permissions this setup itself needs; that is expected.$(if ($ForEmail) { ' Email alerts are included, so expect TWO sign-ins: this one, then one for Exchange Online (a separate service with its own sign-in).' })"
    ...
    if (Lz4ce6446cd4) {
        Lzbe4ef6d8d0 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B5460F' -Strong
        try {
            Lz881d975f4c -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lzbe4ef6d8d0 "The browser sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B5460F' -Strong
        }
    } else {
        Lzbe4ef6d8d0 'Using the code sign-in (the reliable path): a small window will show your code to type at microsoft.com/devicelogin, or on your phone. It stays on top and closes itself when you are done.' '#B5460F' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lzcbeacd9af2 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lzbe4ef6d8d0 $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lzcd649e0168 -Token $at -Claim 'tid'
            $wizTokAcct = Lzcd649e0168 -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lzcd649e0168 -Token $at -Claim 'preferred_username' }
            ...
            Lzbe4ef6d8d0 "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lzbe4ef6d8d0 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lzbe4ef6d8d0 "Signed in to tenant $tid as $wizAcct." '#0E9F6E'
    $newThumb = $null
    try {
        Lzbe4ef6d8d0 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lzbe4ef6d8d0 "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B5460F' }
            Lzbe4ef6d8d0 "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#0E9F6E'
        } else {
            Lzbe4ef6d8d0 "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lzbe4ef6d8d0 'App registration created.' '#0E9F6E'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lzbe4ef6d8d0 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lzbe4ef6d8d0 "Certificate created in this user's store. Thumbprint $newThumb." '#0E9F6E'
        Lzbe4ef6d8d0 'Uploading the certificate''s public key to the app registration...'
        ...
        Lzbe4ef6d8d0 "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#0E9F6E'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lz727bab0cdb -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lzbe4ef6d8d0 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lz727bab0cdb -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lzbe4ef6d8d0 $consentMsg '#0E9F6E'
        Lz531285efed -Name 'TenantId'       -Value $tid
        Lz531285efed -Name 'GraphClientId'  -Value $appId
        Lz531285efed -Name 'CertThumbprint' -Value $newThumb
        Lzbe4ef6d8d0 'Connection details saved on this computer.' '#0E9F6E'
    } catch {
        ...
        if ($friendly) { Lzbe4ef6d8d0 $friendly '#B42318' -Strong; Lzbe4ef6d8d0 "Technical detail: $m" }
        else { Lzbe4ef6d8d0 "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lzbe4ef6d8d0 "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lzbe4ef6d8d0 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lzc66f41b4bb -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lz531285efed -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lz1fc8418a9e $spUrl
            if ($der) {
                Lz531285efed -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lz531285efed -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lz531285efed -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lzbe4ef6d8d0 "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#0E9F6E'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lz727bab0cdb -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lzbe4ef6d8d0 "The new sign-in has not become active yet: $($_.Exception.Message)" '#B5460F' -Strong
                   Lzbe4ef6d8d0 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz2cfb127b23 $org
            if ($u) { Lz531285efed -Name 'UpnDomain' -Value $u; Lzbe4ef6d8d0 "Email domain detected: $u" '#0E9F6E' }
        } catch { Lzbe4ef6d8d0 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lzbe4ef6d8d0 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#0E9F6E' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lzb0e54f3fa5 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lz7d34a057df $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lz7b59ed8bed -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lzd633b0a277) {
                    Lzbe4ef6d8d0 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lzbe4ef6d8d0 $t $c } else { Lzbe4ef6d8d0 $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lzbe4ef6d8d0 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B5460F'
                }
            }
            ...
            Lzbe4ef6d8d0 "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lzcbeacd9af2 -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lz881d975f4c -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lzd2f09eb753 -Url 'https://login.microsoft.com/device'
                Lz862ced22c5
                try { Lz881d975f4c -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lzedfa6cc1dc }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (Lz4ce6446cd4) {
            ...
            try {
                Lz881d975f4c -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lzcbeacd9af2 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lzcd649e0168 -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lzcd649e0168 -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lz727bab0cdb -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lza2138e4d48 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lz727bab0cdb -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lz727bab0cdb -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lz531285efed -Name 'EmailSender' -Value $senderAddr
        Lz531285efed -Name 'EmailEnabled' -Value '1'
...
function Lz957e3d1853 {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lzd30212e8ff { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz37d8d68aa8 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    Lzd30212e8ff 'Sender and recipients'
    ...
    [void](Lz37d8d68aa8 -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lz37d8d68aa8 -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lz37d8d68aa8 -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lzd30212e8ff 'Subject and attachments'
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz8e50cdeab7 -Template $script:ES.tbSubj.Text -Vars (Lz2015e19e6f)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz8e50cdeab7 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz2015e19e6f)
            $tok = Lzc66f41b4bb -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz531285efed -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lz531285efed -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lz531285efed -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lz531285efed -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lz531285efed -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lz531285efed -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lz531285efed -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lz531285efed -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lz531285efed -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lz531285efed -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lz531285efed -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lz531285efed -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lzfd4813f89e {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lzcbeacd9af2 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lzc771795d7b {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lzfd4813f89e -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lz2f8a5fe735 } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lzbe1515e933 {
    return ((Lz66eb5897c3) -and (Lzd633b0a277))
}
Lzbb91842f43
Lzedfa6cc1dc
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lz34c537cdcd "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lzbe1515e933)) {
        try { Lz3053f1ac63 } catch {}
        if (Lzbe1515e933) { Lzfc19f0d166 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lzfc19f0d166 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lzfc19f0d166 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ($ctrl.GettingStarted -and $ctrl.GettingStarted.Visibility -eq 'Visible') {
        try { Lzba4e28ef83 ([bool](Lzbe1515e933)) } catch {}
    }
    if ((Lzbe1515e933) -and -not (Test-Path (Lz8eb57c9fff))) {
        try { Lza52cb78c3f } catch {}
    }
