...
function Lzbf80252112 { param([int]$Done,[int]$Total,[int64]$BytesDone,[int64]$BytesTotal,[string]$Space,
...
function Lzd83bfe3b8c { param([int]$Max,[int]$HardMax,[int]$PausedSeconds,[int]$Events,[int]$Code=0)
...
function Lz320223c1e0 {
    param($Headers)
    $p = $script:DattoPace
    $rl = Lz855251d550 $Headers
    if (-not $rl.HasBudget) {
        [System.Threading.Monitor]::Enter($p.SyncRoot)
        try {
    ...
    if (-not $p.HeadersSeen) { $p.HeadersSeen = $true; Lzcfa2a806c1 INFO "  Datto rate budget detected ($([string]::Join(', ', $rl.Raw))). Flying close: pacing to stay just under it and avoid forced pauses." }
    ...
    if ($note) {
        $now = [DateTime]::UtcNow
        if ((-not $script:DattoBudgetLogAt) -or (($now - $script:DattoBudgetLogAt).TotalSeconds -ge 10)) { $script:DattoBudgetLogAt = $now; Lzcfa2a806c1 INFO "  $note" }
    }
}
function Lz4bef8d86cf {
    $p = $script:DattoPace
    [System.Threading.Monitor]::Enter($p.SyncRoot)
    try {
        $cur = [int]$p.GapMs
...
function Lzf5bfcd7225 {
    ...
    $tv = Lz1fdfbd87f7
    Lzcfa2a806c1 INFO ("=== Stage '$Stage' started (pid $PID" + $(if ($tv) { ", v$tv" } else { '' }) + ") ===")
}
...
function Resolve-Secret {
    ...
    if ("$Value" -match '^env:(.+)$') {
        ...
        if (-not $v) { $v = Lz4599e96f40 -Name $n }
...
function Lz4e715694ae {
    ...
    if ($script:RenameSeenDirs -is [hashtable] -and -not $script:RenameSeenDirs.ContainsKey($key)) {
        ...
        Lzcfa2a806c1 INFO "  tidied name(s) to suit SharePoint in: $dir  (every change is listed in the report and the renames CSV)"
    }
}
function Lz369be8a1f8 {
    ...
    Lzcfa2a806c1 WARN "  NAME COLLISION in [$Space]: '$SecondSource' and '$FirstSource' both become '$Final' after tidying. In plain terms: two Datto items would land on the same SharePoint path, so one could overwrite the other. Both are in the collisions CSV and the report."
}
function Lz1d5f6c951a {
    param([string]$Space, [string]$Rp, $Item)
    $safe = ConvertTo-SafeRelPath $Rp
    if ($safe -ne $Rp) {
        Lz4e715694ae -Space $Space -Original $Rp -Final $safe
    ...
    if ($script:FinalSeen -is [hashtable]) {
        ...
        if ($script:FinalSeen.ContainsKey($k)) { $prev = $script:FinalSeen[$k]; if ($prev -ne $Rp) { Lz369be8a1f8 -Space $Space -Final $safe -FirstSource $prev -SecondSource $Rp } }
        else { $script:FinalSeen[$k] = $Rp }
    }
    return $safe
}
function Lz0ecb0be693 {
...
function Invoke-WithRetry {
    ...
    for ($a = 1; $a -le $throttleMax; $a++) {
        try { return & $Action }
            ...
            $isTransient = Lz63a42f68be $_
            $cap = if ($isThrottle) { $throttleMax } else { [int]$t.maxRetries }
            if ((($retryCodes -notcontains $status) -and (-not $isTransient)) -or $a -ge $cap) {
                Lzcfa2a806c1 ERROR "$What failed (attempt $a): $($_.Exception.Message)"; throw
            }
            $delay = Lz400a52c942 -Attempt $a -Throttle $t -RetryAfter $ra
            if ($isTransient) {
                Lzcfa2a806c1 WARN "$What hit a network blip and is retrying in $delay s (attempt $a/$cap): $($_.Exception.Message). In plain terms: a brief connection glitch, not a real failure. It retries here so nothing has to wait for a later Sync."
            } elseif ($What -like 'Datto*' -and @(429,503) -contains $status) {
                $learnedTxt = Lz4bef8d86cf
                Lzcfa2a806c1 WARN "  Datto is limiting how fast we can read (HTTP $status). Pausing $delay s as it asked, then settling to the fastest rate your account allows.$learnedTxt This is normal on large projects, not an error, and it gets smarter after the first one or two."
                ...
                Lzcfa2a806c1 WARN "$What retrying (HTTP $status): $why, waiting $delay s (attempt $a/$cap)."
            }
            $script:RetryEvents++
...
function Test-DestructiveGate { param([switch]$Execute) if (-not $Execute){ Lzcfa2a806c1 WARN "Preview only: nothing will be uploaded. In plain terms: this is a safe rehearsal. Use 'Upload all files' when you are ready to do it for real." } return [bool]$Execute }
...
function Lzb8666bd351 {
    param($Config)
    if ($Config.datto.provider -eq 'LocalSim') { Lzcfa2a806c1 OK "Datto provider: LocalSim ($($Config.datto.sim.rootPath))"; return }
    ...
    Lzcfa2a806c1 OK "Datto API authenticated (Basic auth)."
}
function Invoke-DattoApi {
    ...
    Lz320223c1e0 -Headers $script:DattoLastHeaders
    return $r
}
...
function Lzfd947d544f {
    ...
    Lzcfa2a806c1 WARN "RERUN FAILED ONLY: re-copying just the $($failed.Count) file(s) that failed in $([System.IO.Path]::GetFileName($AuditPath)). Everything else is left exactly as it is."
    return $set
}
...
function Get-DattoItems {
    ...
    if ($subPath) {
        Lzcfa2a806c1 WARN "  SOURCE IS SCOPED to [$subPath] inside [$($Space.Name)]. Only that folder is being listed. This is a partial view of the project: do not read the result as a complete migration of it."
        if ($contentsOnly) { Lzcfa2a806c1 WARN "  CONTENTS ONLY is on for this project: the [$subPath] folder itself is NOT created at the destination; its contents land directly in the target folder. Only runs of this mapping with the setting on understand that layout: a run WITHOUT it would not find these files and would copy them again to [$subPath/...]." }
    }
    ...
    $emitEnum = {
        ...
        Lzcfa2a806c1 INFO "  SOURCE [$scopeLabel]: $('{0:N0}' -f $prog.Folders) folder(s) read, $('{0:N0}' -f $prog.Files) file(s) found so far.$w"
    ...
    function Lz186826c9d6 { param($ParentId, [string]$Prefix)
        ...
        foreach ($it in @($items)) {
            ...
            $isFolder = $false; try { $isFolder = [bool]$it.($f.itemFolder) } catch {}
            if ($isFolder) {
                Lz186826c9d6 -ParentId $it.($f.itemId) -Prefix $rel
            } else {
    ...
    Lz186826c9d6 -ParentId $startId -Prefix $(if ($contentsOnly) { '' } else { $subPath })
    & $emitEnum -Force
    $script:SrcTotal = $prog.Files
    Lzcfa2a806c1 OK "  SOURCE [$scopeLabel] FINISHED: $('{0:N0}' -f $prog.Files) file(s) across $('{0:N0}' -f $prog.Folders) folder(s)."
    Lzb72059be4d -Config $Config -Space $Space -Suspects $suspects
    return $out
}
$script:EnumCacheMaxAgeSec = 86400
function Lz20ad2ea95e {
    param($Config, $Space)
    if (-not $Config.run.stateRoot) { return $null }
    return (Join-Path $Config.run.stateRoot ("enum-" + (ConvertTo-Slug "$($Space.Name)") + ".json"))
}
function Lza88499e23c {
    param($Config, $Space, $Items)
    $p = Lz20ad2ea95e -Config $Config -Space $Space
    if (-not $p) { return }
    try {
        ...
        Lzcfa2a806c1 INFO "  (could not save the file list for a later Resume: $($_.Exception.Message). Harmless: a Resume would read Datto again instead.)"
    }
}
...
function Lz647a8ab0aa {
    ...
    } catch { Lzcfa2a806c1 WARN "  permissions unavailable for $($Space.Name)"; return @() }
}
function Lzaf46c10a15 {
    ...
    if ($code -eq 5) {
        ...
        if ("$reason".Trim()) { Lzcfa2a806c1 WARN "Integrity check detail: $reason" }
...
function Connect-Destination {
    param($Config)
    if ($Config.destination.provider -eq 'LocalSim') { Lzcfa2a806c1 OK "Destination provider: LocalSim ($($Config.destination.sim.rootPath))"; return }
    ...
    $auth = Lzaf46c10a15 -Broker $broker -Config $Config
    Connect-MgGraph -AccessToken (ConvertTo-SecureString "$($auth.Token)" -AsPlainText -Force) -NoWelcome
    Lzcfa2a806c1 OK "Microsoft 365 sign-in via the licence broker (LiscaraAuth): the token is bound to a verified licence."
    $liveTenant = ''
    ...
    if ($script:ScopeRefId -and $liveTenant -and ($script:ScopeRefId -ne $liveTenant) -and ($null -eq $script:TrialCap)) {
        ...
        Lzcfa2a806c1 WARN "Evaluation limits apply to this run."
    }
    Lzcfa2a806c1 OK "Connected to Graph (tenant $liveTenant)."
}
...
function Send-FileToDestination {
    ...
    if (($Config.run.PSObject.Properties.Name -contains 'sanitiseNames') -and $Config.run.sanitiseNames) {
        $relPath = Lz1d5f6c951a -Space $Space -Rp $relPath -Item $Item
    }
    if ($Config.destination.provider -eq 'LocalSim') {
        ...
        return @{ Size = (Get-Item $dest).Length; Hash = (Lz2f1981f60e $dest); Path = $relPath; Method = 'LocalSim'; Retries = 0 }
    }
    if ($relPath -match '/') { Lzc5c26dcd1f -Config $Config -DriveId $DriveId -FolderPath ($relPath -replace '/[^/]*$','') }
    ...
    if ($size -eq 0) {
    ...
    } elseif ($size -le $script:SmallFilePutThreshold -and (Lz245406824d) -and -not $script:SpoolKey) {
        ...
        try {
            ...
            Lzcfa2a806c1 WARN "  direct PUT failed for ${relPath}; falling back to upload session: $($_.Exception.Message). In plain terms: the quick upload did not work for this file, so the tool is switching to the slower resumable method. Usually harmless, the file should still upload."
        }
    }
            ...
            try {
                ...
                if ($size -le $chunk) {
                    if ($script:SpoolKey) {
                        ...
                        $finalResp = Lza41e734ee3 -Config $Config -UploadUrl $uploadUrl -Bytes $plain -From 0 -To ($size - 1) -Total $size
                    } else {
                        $finalResp = Lza41e734ee3 -Config $Config -UploadUrl $uploadUrl -FilePath $LocalFile -From 0 -To ($size - 1) -Total $size
                    }
                    ...
                    try {
                        ...
                        while ($true) {
                            ...
                            $finalResp = Lza41e734ee3 -Config $Config -UploadUrl $uploadUrl -Bytes $plain -From $pos -To ($pos + $plain.Length - 1) -Total $size
                            $pos += $plain.Length
                    ...
                    try {
                        ...
                        while (($read = $fs.Read($buffer, 0, $buffer.Length)) -gt 0) {
                            ...
                            $finalResp = Lza41e734ee3 -Config $Config -UploadUrl $uploadUrl -FilePath $chunkTmp -From $pos -To ($pos + $read - 1) -Total $size
                            $pos += $read
                        }
                ...
                if ($st -eq 400 -and $hasFsi -and -not $strippedFsi) {
                    ...
                    if (-not $script:OmitFsi) {
                        $script:OmitFsi = $true
                        Lzcfa2a806c1 WARN "  destination rejects the modified-date field; omitting it for the rest of this run. In plain terms: SharePoint would not accept the file's last-modified date, so the tool will stop sending it. Files still upload fine."
                    }
                    $sessBody = @{ item = @{ '@microsoft.graph.conflictBehavior' = 'replace' } } | ConvertTo-Json -Depth 5
                    Lzcfa2a806c1 WARN "  createUploadSession 400 for ${relPath}: retrying without the modified-date field. In plain terms: the first upload attempt was refused, so the tool is retrying without the date. Usually harmless."
                    $script:RetryEvents++; $localRetries++
                    continue
                }
                if ((@(401, 404, 408, 429, 500, 502, 503, 504) -contains $st) -and $attempt -lt $t.maxRetries) {
                    $ra = Lz7db5703273 $_
                    $delay = Lz400a52c942 -Attempt $attempt -Throttle $t -RetryAfter $ra
                    ...
                    Lzcfa2a806c1 WARN "  upload retry for $relPath (HTTP $st): new session, waiting $delay s (attempt $attempt/$($t.maxRetries)). In plain terms: $why, so it will wait $delay seconds and try again. Normal on busy runs."
                    $script:RetryEvents++; $localRetries++
                ...
                if ($gBody) { Lzcfa2a806c1 ERROR "  Graph reason for $relPath (HTTP $st): $gBody" }
                throw
            }
        }
    }
    ...
    switch ($choice.Trim()) {
        ...
        default {
            ...
            while ($true) {
                ...
                if ($validate) {
                    $siteObj = Lz2b2313b619 -Url $site.Trim()
...
function Lzcd047c7b4a {
    param($Config, [switch]$NonInteractive)
    Lzf5bfcd7225 -Config $Config -Stage 'api-discovery'
    Lzb8666bd351 -Config $Config
    ...
    if ($NonInteractive) {
        foreach ($s in $projects) {
            $rows.Add((Lz5e0e6ea3c2 -Project $s -Dest @{ Action='MIGRATE'; DestinationType='SharePoint'; DestinationUrl="$($Config.destination.teamSiteBaseUrl)/$((ConvertTo-Slug $s.Name))"; TargetPrincipal=''; TargetLibrary=''; TargetSubFolder=''; Notes='auto (non-interactive)' }))
        }
    } else {
        ...
        while ($remaining.Count -gt 0) {
            ...
            $proj = $remaining[$n - 1]
            $dest = Lz6ffe2c5a88 -Config $Config -Project $proj
            if ($dest.Action -eq 'MIGRATE') {
                $newRow = Lz5e0e6ea3c2 -Project $proj -Dest $dest
                $rows.Add($newRow)
    ...
    if ($rows.Count -eq 0) { Lzcfa2a806c1 WARN "No sources mapped, nothing to migrate. mapping.csv not written."; return }
    ...
    Lzcfa2a806c1 OK "Discovery complete. $($rows.Count) project(s) mapped. Review: $out"
}
function Lza601daf2d8 {
    param($Config)
    Lzf5bfcd7225 -Config $Config -Stage 'api-preflight'
    Lzb8666bd351 -Config $Config
    ...
    $t = $map.Count; $i = 0
    foreach ($row in $map) {
        $i++; Lz0ecb0be693 -Activity 'Pre-flight' -Status $row.Space -Current $i -Total $t | Out-Null
        ...
        if ($verdict -eq 'BLOCKED') { Lzcfa2a806c1 ERROR "  BLOCKED: $($row.Space) needs $([math]::Round($bytes/1GB,2))GB, free $freeTxt. In plain terms: the destination does not have enough free space for this project. Free up space or increase the storage quota, then run this again." }
    }
    ...
    Lzcfa2a806c1 OK "Readiness check complete. $blocked destination(s) blocked for lack of storage - 'Upload all files' will skip those."
    if ($renameCount -gt 0) { Lzcfa2a806c1 INFO "$renameCount file(s) have illegal characters and will be AUTO-RENAMED on upload (they migrate, nothing skipped)." }
    if ($failCount -gt 0)   { Lzcfa2a806c1 WARN "$failCount file(s) WILL FAIL (path too long, or illegal char with sanitiseNames off). See api-preflight-issues.csv. In plain terms: some files have names or folder paths SharePoint cannot accept. They will be skipped unless shortened or renamed first." }
    if ($issues.Count -eq 0) { Lzcfa2a806c1 OK "No file name/path issues." }
    ...
    if ($largestRows.Count) { Lzcfa2a806c1 INFO "Largest file: $(Format-Bytes ([int64]$largestRows[0].SizeBytes)) - $($largestRows[0].Space)/$($largestRows[0].RelativePath). The biggest $($largestRows.Count) are in api-preflight-largest-files.csv." }
    ...
    if ($gid -gt 0) {
        ...
        Lzcfa2a806c1 INFO "$gid set(s) of duplicate files (identical content), about $(Format-Bytes $wasted) in redundant copies. See api-preflight-duplicates.csv. Nothing is de-duplicated automatically; this is for review before migrating."
    } else {
        Lzcfa2a806c1 INFO "No duplicate files detected by content hash."
    }
    $rate = $null; $rateFrom = ''
    ...
    if ($rate -and $rate -gt 0 -and $allBytes -gt 0) {
        ...
        Lzcfa2a806c1 INFO "Estimated transfer time for $(Format-Bytes $allBytes): about $(Format-Duration ([TimeSpan]::FromSeconds($secs))), at this job's last measured throughput of $([math]::Round($rate,1)) Mb/s (from $rateFrom). Actual time depends on your line speed and Microsoft 365 throttling."
    } else {
        Lzcfa2a806c1 INFO "No duration estimate yet: this job has no completed upload to measure real throughput from. One will appear here after the first upload finishes."
    }
}
function Lzdbc1f6a323 {
...
function Lz748ca30e31 { param($Config) return (Join-Path $Config.run.reportRoot 'run-active.json') }
function Lz034a533d97 {
    ...
    try { [System.IO.File]::WriteAllText((Lz748ca30e31 $Config), ($o | ConvertTo-Json -Depth 4), [System.Text.UTF8Encoding]::new($false)) } catch {}
}
function Lzf50658685f { param($Config) try { Remove-Item (Lz748ca30e31 $Config) -Force -ErrorAction SilentlyContinue } catch {} }
...
function Get-EmailCommonVars {
    ...
    return @{ JobName = $job; Tenant = $tenant; Version = (Lz1fdfbd87f7) }
}
...
function Send-RunEmail {
    ...
    try {
        ...
        if (-not "$($s.Sender)".Trim() -or -not "$($s.Recipients)".Trim()) {
            Lzcfa2a806c1 WARN 'Email alerts are switched on but the sender or recipients are missing (Settings > Email alerts). No email was sent; the run itself is unaffected.'
            return
        }
        ...
        foreach ($r in ("$($s.Recipients)" -split '[;,\r\n]+')) {
            ...
            elseif ($a) { Lzcfa2a806c1 WARN "Email alerts: '$a' does not look like an email address and was skipped." }
        }
        if (-not $to.Count) {
            Lzcfa2a806c1 WARN 'Email alerts: no valid recipient address is configured. No email was sent; the run itself is unaffected.'
            return
        }
        $atts = @(); $attachNote = ''
        if ("$($s.Attach)" -ne '0') {
            ...
            foreach ($f in @($set.Files)) {
                ...
                } catch { Lzcfa2a806c1 WARN "Email alerts: could not read attachment $f ($($_.Exception.Message)); sending without it." }
            }
        }
        ...
        Lzcfa2a806c1 OK "Completion email sent from $emSender to $(@($to).Count) recipient(s)$(if ($atts.Count) { " with $($atts.Count) attachment(s)" })."
    } catch {
        Lzcfa2a806c1 WARN "The completion email could not be sent: $($_.Exception.Message). The run itself is unaffected; the outcome above stands. Check Settings > Email alerts (sender, recipients, Mail.Send permission)."
    }
}
function Invoke-RunFinalize {
    ...
    try {
        Lzdbc1f6a323 -AuditFile $AuditFile -Status $status -Reason $reason -Processed $processed -Expected $ExpectedFiles `
    ...
    try {
        switch ($status) {
            'COMPLETED'             { Lzcfa2a806c1 OK    "=== RUN COMPLETE: all $SpacesPlanned project(s) finished. Uploaded $copied file(s) ($szTxt), skipped $skipped unchanged, 0 failures, in $el. ===" }
            'COMPLETED_WITH_ERRORS' { Lzcfa2a806c1 ERROR "=== RUN COMPLETE WITH ERRORS: reached the end of all $SpacesPlanned project(s). Uploaded $copied ($szTxt), but $errCount file(s) failed or could not be verified and are NOT marked done. Run 'Sync new and changed' to retry just those. ===" }
            'CANCELLED'             { Lzcfa2a806c1 WARN  "=== RUN CANCELLED by user. Uploaded $copied file(s) ($szTxt) before stopping; those are saved. The rest were NOT uploaded. Click 'Sync new and changed' to carry on from where it stopped: it copies only what is not already there. ===" }
            'INCOMPLETE'            { Lzcfa2a806c1 ERROR "=== RUN INCOMPLETE: the previous run stopped without finalising (a crash, a forced close, or power loss). $copied file(s) ($szTxt) were recorded as uploaded. Run 'Sync new and changed' to continue and confirm. ===" }
            default                 { Lzcfa2a806c1 ERROR "=== RUN ENDED EARLY - DID NOT FINISH. Processed $processed file(s) (of ~$ExpectedFiles listed) across $SpacesCompleted of $SpacesPlanned project(s) before stopping. Reason: $reason. The remaining files were NOT uploaded. Fix the cause, then click 'Sync new and changed' to carry on from where it stopped. ===" }
        }
    } catch {}
    ...
    Lzf50658685f -Config $Config
    return $outcome
}
function Lze11eeb339e {
...
function Lz7897f308f4 {
    param($Config,[string]$Status='Incomplete',[string]$AuditPath='')
    Lzf5bfcd7225 -Config $Config -Stage 'api-finalize'
    $act = $null
    $ap = Lz748ca30e31 $Config
    ...
    if (-not $AuditPath -or -not (Test-Path $AuditPath)) {
        Lzcfa2a806c1 WARN "Record outcome: no audit CSV found, so there is nothing to record."; Lzf50658685f -Config $Config; return
    }
    $sidecar = ($AuditPath -replace '\.csv$','') + '.outcome.json'
    if (Test-Path $sidecar) { Lzcfa2a806c1 INFO "Record outcome: the run already has an outcome; leaving it as it is."; Lzf50658685f -Config $Config; return }
...
function Lz69fa995513 {
    ...
    try {
        ...
        if ($days -lt 0) {
            ...
            Lzcfa2a806c1 WARN "  the sign-in certificate expires in $([int]$days) day(s) (on $($cert.NotAfter.ToString('yyyy-MM-dd'))). Renew it soon to avoid a mid-run authentication failure."
        }
    } catch { throw }
}
function Lz6bb2cd29e9 {
    param($Config)
    if ($Config.datto.provider -eq 'LocalSim') { return }
...
function Lzc9336ca389 {
    ...
    try {
        ...
        Lzcfa2a806c1 INFO "  found $($old.Count) resume record(s) from an older version in [$root]. They are no longer used: the destination is now the source of truth for what has already been copied. They are harmless, and you can delete them whenever you like. Nothing here will delete them for you."
    } catch {}
}
function Lzf8e51d3176 {
    param($Config)
    ...
    try {
        ...
        if ($tmpRoot -and (Test-Path $tmpRoot)) {
            ...
            if ($orphans.Count) {
                ...
                Lzcfa2a806c1 INFO "  cleared $($orphans.Count) leftover temp file(s) from a previous run."
            }
            $freeB = Lz60be02d2da -Path $tmpRoot
            if ($freeB -ge 0) {
                Lzcfa2a806c1 INFO "  temp drive free space: $(Format-Bytes $freeB) (files are staged here then deleted immediately after upload, so only a handful sit on disk at once)."
                if ($freeB -lt 2GB) { Lzcfa2a806c1 WARN "  the temp drive has under 2 GB free. Everyday files are fine (only a handful stage at a time), but any single file larger than the free space is skipped and reported. Free space, or point run.tempWorkingFolder at a bigger drive." }
            }
        }
    } catch {}
    Lz69fa995513 -Config $Config
}
function Lz36d24a0b0b {
    param($Config, $Rows)
    if ($Config.destination.provider -eq 'LocalSim') { return }
    $seen = @{}
    foreach ($row in $Rows) {
        ...
        try {
            ...
            if ($null -ne $free -and $free -gt 0) {
                ...
                if ($free -lt 2GB) { Lzcfa2a806c1 WARN "  destination check: $label is reachable but has only $ftxt free. A large upload may fail part-way; free up space or pick another destination." }
                else { Lzcfa2a806c1 OK "  destination check: $label reachable, $ftxt free." }
            } else {
                Lzcfa2a806c1 OK "  destination check: $label reachable."
            }
        } catch {
...
function Invoke-Transfer {
    ...
    if ($Mode -eq 'Resume') {
        $Mode = 'Delta'
        Lzcfa2a806c1 INFO "Resume now runs as a Sync: it compares Datto against the destination itself, file by file, instead of trusting a local record of the last run. Same intent, and it cannot be fooled by a stale or damaged record."
    }
    Lzf5bfcd7225 -Config $Config -Stage "api-transfer-$Mode" -Friendly $(
    ...
    if ($FailedOnly) {
        ...
        $script:FailedOnlySet = Lzfd947d544f -Config $Config -AuditPath $FailedFromAudit
    }
    Lzc9336ca389 -Config $Config
    Lz6bb2cd29e9 -Config $Config
    if ($Execute -and $Config.destination.provider -ne 'LocalSim' -and $null -eq $script:TrialCap) {
        ...
        if ($lzOk) { $script:ScopeRefId = $lzTen }
            ...
            Lzcfa2a806c1 WARN "Evaluation limits apply to this run: up to $lzRem file(s) will be copied. Licence this Microsoft tenant at https://www.liscaragh.com to remove the limit."
        }
    }
    $runStart = Get-Date
    ...
    else { Lzcfa2a806c1 INFO "No readiness check was run for this job (it is optional). The upload checks the destination is reachable before it starts, tidies names as it goes, and flags any problems in the report." }
    $runnable = @($map | Where-Object { $blockedSpaces -notcontains $_.Space })
    if ($MaxParallelSpaces -gt 1 -and $runnable.Count -gt 1) {
        Lz3597c95c7c -Config $Config -Rows $runnable -Mode $Mode -DeltaMode $DeltaMode -SpoolAhead $SpoolAhead -MaxParallelSpaces $MaxParallelSpaces -Execute:$Execute
        return
    }
    Lzcfa2a806c1 INFO "This run: $($map.Count) project(s). (Read-ahead: $SpoolAhead file(s).)"
    if ($script:SpoolEncrypt) {
        $script:SpoolKey = New-SpoolKey
        Lzcfa2a806c1 OK "Files staged on this computer are ENCRYPTED (AES-256) while they wait to upload. The key exists only in this run's memory and dies with it, staged names are random identifiers, and the destination receives the original files unchanged."
    } else {
        $script:SpoolKey = $null
        Lzcfa2a806c1 INFO "Spool encryption is OFF (run.encryptSpool = false): files staged on this computer are readable while a run is in flight."
    }
    Lzb8666bd351 -Config $Config
    Connect-Destination -Config $Config
    $expectedTotal   = 0
    ...
    $spacesCompleted = 0
    $transferError   = $null
    if ($willWrite -and -not $ResultDir) { Lz034a533d97 -Config $Config -Mode $Mode -StartUtc $runStart -AuditFile $script:AuditFile -ExpectedFiles 0 -SpacesPlanned $spacesPlanned -SpacesCompleted 0 }
    $summary = New-Object System.Collections.Generic.List[object]
    $sTotal = $map.Count; $sIdx = 0
    try {
    if ($willWrite) { Lz36d24a0b0b -Config $Config -Rows $map; Lzf8e51d3176 -Config $Config }
    foreach ($row in $map) {
        $sIdx++
        Lz0ecb0be693 -Activity "Transfer ($Mode)" -Status $row.Space -Current $sIdx -Total $sTotal -Id 1 | Out-Null
        if ($blockedSpaces -contains $row.Space) { Lzcfa2a806c1 ERROR "SKIPPED '$($row.Space)': the readiness check found too little storage at this destination. Increase the quota or free up space, then run again."; continue }
        Lzcfa2a806c1 INFO "[$(if (-not $willWrite) { 'Preview' } elseif ($Mode -eq 'Delta') { 'Sync' } else { 'Upload' })] source [$($row.Space)] -> destination $(Format-Destination -Row $row)"
        $space = New-SpaceRef -Row $row
        ...
        if ($streamFP) {
            Lzcfa2a806c1 INFO "  Upload all: listing and uploading run together (files start moving while the folder list is still being read)."
            $items = @()
        } else {
            $rowScope = "$($row.Space)"
            ...
            Lzcfa2a806c1 INFO "  listing files in [$rowScope] (large projects can take a few minutes, nothing uploads yet)..."
            $items = @(Get-SpaceItemsCached -Config $Config -Space $space)
            Lzcfa2a806c1 INFO "  listing complete: $($items.Count) file(s)."
        }
        ...
        if ($effSub) { Lzcfa2a806c1 INFO "  files land under: [$effSub]$nestNote." }
        elseif ($nestNote) { Lzcfa2a806c1 INFO "  files land under the project's own folder at the destination root." }
        ...
        if ($restoreSet.Count) {
            ...
            Lzcfa2a806c1 INFO "  restore: $($toDo.Count) selected file(s) will be overwritten at the destination with the Datto version."
        }
        ...
        elseif ($Mode -eq 'Delta') {
            ...
            $toDo = @($items | Where-Object {
                ...
                if ($null -ne $ds -and -not (Lza75b0392fc -RelPath $rp -SourceSize ([int64]$_.Size) -DestSize $ds)) {
            ...
            Lzcfa2a806c1 INFO "  sync ($modeTxt): $($toDo.Count) of $($items.Count) file(s) to copy; the rest are already present and are left as they are."
            if ($repair.Count) {
                Lzcfa2a806c1 WARN "  $($repair.Count) of those file(s) are ALREADY at the destination but are the wrong size, and will be re-copied to repair them. These are uploads that did not complete correctly on an earlier run, not new or changed files. They are recorded as 'repaired' in the audit CSV, not as ordinary copies."
            }
        }
        else {
            $toDo = @($items)
        ...
        if ($willWrite) { Lzbf80252112 -Done 0 -Total $fTotal -BytesDone 0 -BytesTotal $spaceBytesTotal -Space $row.Space }
        ...
        if ($willWrite -and $Mode -ne 'FirstPass') {
            ...
            foreach ($sk in @($items | Where-Object { $toDoIds -notcontains $_.Id })) {
                Lza321649d43 -Space $row.Space -Item $sk -Status 'Skipped' -Reason 'already at the destination, intact and not older than the source'
            }
        }
        if (-not $willWrite) {
            $copied = $fTotal; $bytes = $spaceBytesTotal
            foreach ($it in $toDo) { Lza321649d43 -Space $row.Space -Item $it -Status 'WouldCopy' -Reason 'dry-run' }
        }
        elseif ($UploadWorkers -gt 1) {
            ...
            foreach ($it in $toDo) {
                ...
                $rp = $rp -replace '\\','/'
                if ($sanitise) { $rp = Lz1d5f6c951a -Space $row.Space -Rp $rp -Item $it }
            ...
            if ($Config.destination.provider -ne 'LocalSim') { $tk = Lz5ac175df7a -Config $Config; $tokenHolder.Token = $tk.Token; $tokenHolder.Exp = $tk.Exp }
            $ensured = [hashtable]::Synchronized(@{})
            Lzcfa2a806c1 INFO "  copying up to $downloadThreads file(s) at a time from Datto, and up to $UploadWorkers at a time into Microsoft 365."
            ...
            if ($maxUpMbps -gt 0 -or $maxDownMbps -gt 0) {
                ...
                Lzcfa2a806c1 INFO "  bandwidth: $upTxt, $dnTxt (so the client's line is not saturated)."
            }
            ...
            $process = {
                ...
                switch ($res.Status) {
                    'Copied'   { $script:copiedRef.Value++; $script:bytesRef.Value+=$it.Size; if($script:VerboseFiles){Lzcfa2a806c1 INFO "  + [$($it.RelativePath)] $($res.Reason)"} }
                    'ZeroByte' { $script:copiedRef.Value++; $script:zeroRef.Value++; $script:bytesRef.Value+=$it.Size; Lzcfa2a806c1 WARN "  empty file (0 bytes), uploaded as-is: $($it.RelativePath)" }
                    'VerifyFail'{ $script:verifyFailRef.Value++; Lzcfa2a806c1 ERROR "  VERIFY FAILED $($it.RelativePath): $($res.Reason). In plain terms: the uploaded copy did not match the original, so it was not marked done. It will be retried on the next Sync." }
                    'DownloadError'{ $script:failedRef.Value++; Lzcfa2a806c1 ERROR "  DOWNLOAD FAILED $($it.RelativePath): $($res.Error). In plain terms: the file could not be downloaded from Datto. It will be retried on the next Sync." }
                    'SkippedTooLarge'{ $script:failedRef.Value++; Lzcfa2a806c1 ERROR "  SKIPPED (too large for temp drive) $($it.RelativePath): $($res.Error). In plain terms: this one file will not fit in the temp drive's free space, so it was left for a later run. Free up space (or point the temp folder at a bigger drive) and Sync to pick it up." }
                    default    { $script:failedRef.Value++; Lzcfa2a806c1 ERROR "  UPLOAD FAILED $($it.RelativePath): $($res.Error). In plain terms: the file could not be uploaded to Microsoft 365. It will be retried on the next Sync." }
                }
                $rDl = 0.0; try { $rDl = [double]$res.DlMs } catch {}
                $rsn = Lz2e711c99af -Repair $repair -Item $it -Status $res.Status -Reason $res.Reason
                Lza321649d43 -Space $row.Space -Item $it -Status $res.Status -Reason $rsn -DestSize $res.DestSize -DestPath $res.DestPath -DestHash $res.DestHash -Method $res.Method -Retries $res.Retries -DurationMs $res.Ms -DownloadMs $rDl -ErrorMsg $res.Error
                Lzbf80252112 -Done $script:fIdxRef.Value -Total $fTotal -BytesDone $script:bytesRef.Value -BytesTotal $spaceBytesTotal -Space $row.Space `
            ...
            if ($streamFP) {
                ...
                Get-DattoItems -Config $Config -Space $space -OnItem {
                    ...
                    if ($Config.destination.provider -ne 'LocalSim' -and [DateTimeOffset]::UtcNow -gt $tokenHolder.Exp) {
                        try { $tk = Lz5ac175df7a -Config $Config; $tokenHolder.Token = $tk.Token; $tokenHolder.Exp = $tk.Exp } catch {}
                    }
                    $rp = if ($effSub) { ($effSub.Trim('/') + '/' + $it.RelativePath) } else { $it.RelativePath }
                    $rp = $rp -replace '\\','/'
                    if ($sanitise) { $rp = Lz1d5f6c951a -Space $row.Space -Rp $rp -Item $it }
                ...
                $expectedTotal += $fTotal
                Lzcfa2a806c1 INFO "  listing finished: $fTotal file(s), $([math]::Round($spaceBytesTotal/1MB,1)) MB to move."
            ...
            while (@($workers | Where-Object { -not $_.Handle.IsCompleted }).Count -gt 0 -or $resQ.Count -gt 0) {
                $res=$null
                while ($resQ.TryDequeue([ref]$res)) { & $process $res; Lz0ecb0be693 -Activity "Files: $($row.Space)" -Status $res.DestPath -Current $script:fIdxRef.Value -Total $fTotal -Id 2 | Out-Null }
                ...
                if ($Config.destination.provider -ne 'LocalSim' -and [DateTimeOffset]::UtcNow -gt $tokenHolder.Exp) { try { $tk=Lz5ac175df7a -Config $Config; $tokenHolder.Token=$tk.Token; $tokenHolder.Exp=$tk.Exp } catch {} }
                $nowT=[DateTime]::UtcNow.Ticks
                [System.Threading.Monitor]::Enter($thr.SyncRoot)
                try {
                ...
                if ($curMax -ne $script:LastThrMax -or $isPaused -ne $script:LastThrPaused) {
                    Lzd83bfe3b8c -Max $curMax -HardMax $thr.HardMax -PausedSeconds $pausedSecs -Events $curEvents -Code $curCode
                    ...
                    if ($curMax -lt $thr.HardMax -or $isPaused) {
                        ...
                        Lzcfa2a806c1 WARN "  $msg In plain terms: this is normal protection, the job keeps running and speeds back up on its own."
                    } elseif ($script:LastThrMax -ne -1) {
                        Lzcfa2a806c1 OK "  Throttling cleared: back to full speed ($curMax of $($thr.HardMax) uploaders)."
                    }
                    ...
                    if (Test-Path $bwControlPath) {
                        ...
                        if ($st -ne $bwCtrlStamp) {
                            ...
                            if ($nu -ne $bwLastUp -or $nd -ne $bwLastDown) {
                                ...
                                Lzcfa2a806c1 INFO "  bandwidth changed on the fly: upload $ut, download $dt. Applied immediately."
                            }
                        }
                    }
                } catch {}
            ...
            if ($script:LastThrMax -ne -1 -and $script:LastThrMax -lt $thr.HardMax) { Lzd83bfe3b8c -Max $thr.HardMax -HardMax $thr.HardMax -PausedSeconds 0 -Events $thr.Events }
        ...
        elseif ($SpoolAhead -le 1) {
            ...
            foreach ($it in $toDo) {
                ...
                Lz0ecb0be693 -Activity "Files: $($row.Space)" -Status $it.RelativePath -Current $fIdx -Total $fTotal -Id 2 | Out-Null
                ...
                try {
                    Lzcbed0ba163 -Config $Config -Item $it -OutFile $tempFile | Out-Null
                    ...
                    $v = Lz732f8bacdc -Item $it -LocalFile $tempFile -DestInfo $destInfo
                    $upSw.Stop()
                    if ($v.Ok) {
                        if ($v.Zero) { Lzcfa2a806c1 WARN "  empty file (0 bytes), uploaded as-is: $($it.RelativePath)"; $zero++ }
                        $copied++; $bytes += $it.Size
                        if ($script:VerboseFiles) { Lzcfa2a806c1 INFO "  + [$($it.RelativePath)] $($v.Reason)" }
                        $stat = if($v.Zero){'ZeroByte'}else{'Copied'}
                        Lza321649d43 -Space $row.Space -Item $it -Status $stat -Reason (Lz2e711c99af -Repair $repair -Item $it -Status $stat -Reason $v.Reason) -DestSize $destInfo.Size -DestPath $destInfo.Path -DestHash $destInfo.Hash -Method $destInfo.Method -Retries $destInfo.Retries -DurationMs $upSw.Elapsed.TotalMilliseconds -DownloadMs $dlSw.Elapsed.TotalMilliseconds
                    } else {
                        $verifyFail++; Lzcfa2a806c1 ERROR "  VERIFY FAILED $($it.RelativePath): $($v.Reason). In plain terms: the uploaded copy did not match the original, so it was not marked done. It will be retried on the next Sync."
                        Lza321649d43 -Space $row.Space -Item $it -Status 'VerifyFail' -Reason $v.Reason -DestSize $destInfo.Size -DestPath $destInfo.Path -DestHash $destInfo.Hash -Method $destInfo.Method -Retries $destInfo.Retries -DurationMs $upSw.Elapsed.TotalMilliseconds -DownloadMs $dlSw.Elapsed.TotalMilliseconds
                    }
                } catch { if ($dlSw.IsRunning){$dlSw.Stop()}; if($upSw.IsRunning){$upSw.Stop()}; $failed++; Lzcfa2a806c1 ERROR "  FAILED $($it.RelativePath): $($_.Exception.Message). In plain terms: this file could not be copied. It will be retried on the next Sync."; Lza321649d43 -Space $row.Space -Item $it -Status 'Error' -DurationMs $upSw.Elapsed.TotalMilliseconds -DownloadMs $dlSw.Elapsed.TotalMilliseconds -ErrorMsg $_.Exception.Message }
                finally { if (Test-Path $tempFile) { Remove-Item $tempFile -Force -ErrorAction SilentlyContinue } }
                Lzbf80252112 -Done $fIdx -Total $fTotal -BytesDone $bytes -BytesTotal $spaceBytesTotal -Space $row.Space -SmallDone $etaSmallDone -SmallTotal $etaSmallTotal -LargeBytesDone $etaLargeDone -LargeBytesTotal $etaLargeTotal -Final 1
            }
        }
        else {
            ...
            foreach ($ready in $spool.GetConsumingEnumerable()) {
                ...
                if (-not $ready.Ok) { $failed++; Lzcfa2a806c1 ERROR "  DOWNLOAD FAILED $($it.RelativePath): $($ready.Error). In plain terms: the file could not be downloaded from Datto. It will be retried on the next Sync."; Lza321649d43 -Space $row.Space -Item $it -Status 'DownloadError' -DownloadMs $dlMs -ErrorMsg $ready.Error; if ($ready.TempFile) { $delQ.Enqueue($ready.TempFile) }; continue }
                ...
                try {
                    ...
                    $v = Lz732f8bacdc -Item $it -LocalFile $ready.TempFile -DestInfo $destInfo
                    $sw.Stop()
                    if ($v.Ok) {
                        if ($v.Zero) { Lzcfa2a806c1 WARN "  empty file (0 bytes), uploaded as-is: $($it.RelativePath)"; $zero++ }
                        $copied++; $bytes += $it.Size
                        if ($script:VerboseFiles) { Lzcfa2a806c1 INFO "  + [$($it.RelativePath)] $($v.Reason)" }
                        $stat = if($v.Zero){'ZeroByte'}else{'Copied'}
                        Lza321649d43 -Space $row.Space -Item $it -Status $stat -Reason (Lz2e711c99af -Repair $repair -Item $it -Status $stat -Reason $v.Reason) -DestSize $destInfo.Size -DestPath $destInfo.Path -DestHash $destInfo.Hash -Method $destInfo.Method -Retries $destInfo.Retries -DurationMs $sw.Elapsed.TotalMilliseconds -DownloadMs $dlMs
                    } else {
                        $verifyFail++; Lzcfa2a806c1 ERROR "  VERIFY FAILED $($it.RelativePath): $($v.Reason). In plain terms: the uploaded copy did not match the original, so it was not marked done. It will be retried on the next Sync."
                        Lza321649d43 -Space $row.Space -Item $it -Status 'VerifyFail' -Reason $v.Reason -DestSize $destInfo.Size -DestPath $destInfo.Path -DestHash $destInfo.Hash -Method $destInfo.Method -Retries $destInfo.Retries -DurationMs $sw.Elapsed.TotalMilliseconds -DownloadMs $dlMs
                    }
                } catch { $sw.Stop(); $failed++; Lzcfa2a806c1 ERROR "  UPLOAD FAILED $($it.RelativePath): $($_.Exception.Message). In plain terms: the file could not be uploaded to Microsoft 365. It will be retried on the next Sync."; Lza321649d43 -Space $row.Space -Item $it -Status 'Error' -DurationMs $sw.Elapsed.TotalMilliseconds -DownloadMs $dlMs -ErrorMsg $_.Exception.Message }
        ...
        if ($willWrite) {
            Lzcfa2a806c1 $lvl "  $($row.Space): copied $copied, skipped $skipped, failed $failed, failed verification $verifyFail, empty files $zero ($szTxt)"
        } else {
            Lzcfa2a806c1 INFO "  $($row.Space): PREVIEW - would copy $copied file(s) ($szTxt) and skip $skipped unchanged. Nothing was uploaded."
        }
        ...
        $spacesCompleted++
        if ($willWrite -and -not $ResultDir) { Lz034a533d97 -Config $Config -Mode $Mode -StartUtc $runStart -AuditFile $script:AuditFile -ExpectedFiles $expectedTotal -SpacesPlanned $spacesPlanned -SpacesCompleted $spacesCompleted }
    }
    ...
    if ($willWrite) {
        ...
        Lzcfa2a806c1 $lvl "$(if ($Mode -eq 'Delta') { 'Sync' } else { 'Upload' }) finished in $(Format-Duration $elapsed) of copying. Totals: copied $tc, skipped $ts, failed $tf, failed verification $tv, $szTxt.$rate  Rate-limit pauses and retries: $script:RetryEvents."
        if ($script:RenameCount -gt 0) { Lzcfa2a806c1 INFO "Tidied $script:RenameCount name(s) to suit SharePoint (trailing spaces, illegal characters). Every change is listed in the report and in: $script:RenameFile" }
        if ($script:CollisionCount -gt 0) { Lzcfa2a806c1 WARN "$script:CollisionCount name collision(s): two Datto items would land on the same SharePoint path. Review before relying on this migration. Details in the report and in: $script:CollisionFile" }
        if (($tf + $tv) -gt 0) {
            Lzcfa2a806c1 WARN "In plain terms: uploaded $tc file(s) ($szTxt), but $($tf + $tv) did not make it ($tf failed, $tv could not be verified). Next step: click 'Sync new and changed' to retry the ones that did not upload, then 'Verify files arrived' to confirm everything is there."
            Lzcfa2a806c1 WARN "Review this log for [ERROR] lines (files that failed or failed verification). Failed files are not recorded as done; 'Sync new and changed' will retry them."
        } else {
            Lzcfa2a806c1 OK "In plain terms: all done. Uploaded $tc file(s) ($szTxt) with no failures. Next step (optional): click 'Verify files arrived' to double-check, or 'Compare sizes' for a quick overview."
        }
        if (-not $ResultDir -and $script:AuditFile -and (Test-Path $script:AuditFile)) {
            Lzcfa2a806c1 OK "Audit trail written: $script:AuditFile"
            try {
                $diag = Lz99f612e36b -Rows @(Import-Csv $script:AuditFile) -Elapsed $elapsed -ConfiguredWorkers $UploadWorkers -ThrottleEvents $script:RetryEvents
                if ($diag) {
                    Lzcfa2a806c1 INFO "What limited this run: $($diag.Verdict). $($diag.Why)"
                    Lzcfa2a806c1 INFO "  $($diag.Action)"
                    Lzcfa2a806c1 INFO "  Numbers: moved $(Format-Bytes $diag.Bytes) at about $($diag.AggMbps) Mb/s overall; single file about $($diag.PerStreamMbps) Mb/s; $($diag.UpSec)s uploading vs $($diag.DlSec)s downloading."
                    if ($diag.Concurrency) { Lzcfa2a806c1 INFO "  Concurrency: $($diag.Concurrency)" }
                    if ($diag.RetryDetail) { Lzcfa2a806c1 INFO "  Retries: $($diag.RetryDetail)" }
                    if ($diag.Projection)  { Lzcfa2a806c1 INFO "  Planning: $($diag.Projection)" }
                    if ($diag.Headroom)    { Lzcfa2a806c1 INFO "  Headroom: $($diag.Headroom)" }
                }
            } catch {}
        }
    }
    else { Lzcfa2a806c1 OK "Preview finished in $el. This was a rehearsal, nothing was uploaded. It would copy $tc file(s), $szTxt. In plain terms: if this looks right, click 'Upload all files' to do it for real." }
    }
    catch {
        $transferError = $_
        Lzcfa2a806c1 ERROR "RUN ABORTED before all files were processed: $($_.Exception.Message)"
    }
    finally {
        Lze11eeb339e -Config $Config -Mode $Mode -RunStart $runStart -ExpectedFiles $expectedTotal `
...
function Lz3597c95c7c {
    ...
    Lzcfa2a806c1 INFO "Parallel projects: $($Rows.Count) project(s), $MaxParallelSpaces at a time."
    $parallelStart = Get-Date
    $childExitCodes = New-Object System.Collections.Generic.List[int]
    if ($willWrite) { Lz034a533d97 -Config $Config -Mode $Mode -StartUtc $parallelStart -AuditFile '' -ExpectedFiles 0 -SpacesPlanned $Rows.Count -SpacesCompleted 0 }
    ...
    if ($auditRows.Count) {
        ...
        Lzcfa2a806c1 OK "Audit trail written: $auditOut ($($auditRows.Count) file actions)"
        if ($willWrite) {
    ...
    Lzcfa2a806c1 OK "Parallel projects finished. Copied $tc, failed $tf, failed verification $tv. Record: $combined"
}
...
function Get-DestinationInventory {
    ...
    $emitDest = {
        ...
        Lzcfa2a806c1 INFO "  DESTINATION: $('{0:N0}' -f $folders) folder(s) read, $('{0:N0}' -f $count) file(s) found so far.$w"
    ...
    while ($stack.Count -gt 0) {
        ...
        try {
            ...
            if ($cur.Root -and $st -eq 404) {
                Lzcfa2a806c1 INFO "  destination inventory: the folder does not exist at the destination yet, so there is nothing to compare against and every file will be copied. This is normal the first time you copy into a new folder."
    ...
    $extra = $pages - $folders
    Lzcfa2a806c1 INFO "  DESTINATION FINISHED: $('{0:N0}' -f $count) file(s) across $('{0:N0}' -f $folders) folder(s), $pages page(s) followed."
    if ($extra -gt 0) {
        Lzcfa2a806c1 WARN "  $extra destination folder(s) held more than one page of children. Before the paging fix everything past the first page was invisible here, so a Sync would have judged those files missing and re-uploaded over the top of them. They are now enumerated in full."
    }
...
function Lzb0c7e0fb81 {
    ...
    if (-not $agg.Count) { Lzcfa2a806c1 OK "  [$($Row.Space)] every file matches by path, both ways. Nothing extra, nothing missing."; return ([pscustomobject]@{ Missing = 0; Extra = 0 }) }
    $rows = New-Object System.Collections.Generic.List[object]
    ...
    try {
        ...
        Lzcfa2a806c1 OK "  [$($Row.Space)] difference written (one row per file): $out"
        if ($script:GuiMode) { Write-Host "##CHECKFILES##|$nMissing|$nExtra|0|$out" }
    } catch {
        Lzcfa2a806c1 WARN "  [$($Row.Space)] could not write the difference CSV: $($_.Exception.Message)"
    }
    if ($nMissing) {
        Lzcfa2a806c1 WARN "  [$($Row.Space)] $('{0:N0}' -f $nMissing) file(s) in Datto that are NOT at the destination:"
        foreach ($p in @($missingPaths | Select-Object -First 15)) { Lzcfa2a806c1 WARN "      missing: /$p" }
        if ($nMissing -gt 15) { Lzcfa2a806c1 WARN "      ... and $('{0:N0}' -f ($nMissing - 15)) more. Every one is named in the CSV above." }
    }
    if ($nExtra)   { Lzcfa2a806c1 INFO "  [$($Row.Space)] $('{0:N0}' -f $nExtra) file(s) at the destination that are NOT in Datto (usually pre-existing content). Biggest folders:" }
    foreach ($r in @($sortedAgg | Where-Object { $_.Side -eq 'AtDestinationOnly' } | Select-Object -First 8)) {
        Lzcfa2a806c1 INFO ("      {0,8:N0}  /{1}" -f $r.Files, $r.Folder)
    }
    return ([pscustomobject]@{ Missing = $nMissing; Extra = $nExtra })
}
function Lz3e14144d85 {
    param($Config, [string]$SpaceName, [string]$ResultDir)
    Lzf5bfcd7225 -Config $Config -Stage 'api-destinv'
    ...
    Lzcfa2a806c1 OK "  DESTINATION inventory written: $('{0:N0}' -f [int]$inv.Count) file(s). $out"
}
function Lz631e304c99 {
    param($Config, $Row)
    try {
        $pwshExe = [System.Diagnostics.Process]::GetCurrentProcess().MainModule.FileName
        $self = $PSCommandPath
        ...
        Lzcfa2a806c1 INFO "  Reading the DESTINATION in the background (helper process $($proc.Id)) while the Datto list is read."
        return @{ Process = $proc; File = (Join-Path $dir "destinv-$slug.json"); OutLog = $out; ErrLog = $err; Pos = [long]0; Partial = '' }
    } catch {
        Lzcfa2a806c1 WARN "  could not start the background destination read ($($_.Exception.Message)). The destination will be read after the source instead - slower, same answer."
        return $null
    }
}
function Lz88e80db9a2 {
    param($Job)
...
function Receive-DestInventoryJob {
    ...
    try {
        ...
        Lzcfa2a806c1 OK "  DESTINATION list received from the background helper: $('{0:N0}' -f [int]$doc.Count) file(s)."
        return @{ Count = [int]$doc.Count; Bytes = [int64]$doc.Bytes; Paths = $paths; Times = $times; Sizes = $sizes }
    } catch {
        Lzcfa2a806c1 WARN "  the background destination list could not be used ($($_.Exception.Message)). Reading the destination directly instead - slower, same answer."
...
function Invoke-SizeCheck {
    ...
    Lzf5bfcd7225 -Config $Config -Stage 'api-sizecheck'
    $ckStart = Get-Date
    Lzb8666bd351 -Config $Config
    Connect-Destination -Config $Config
    $mapPath = Join-Path $Config.run.reportRoot 'mapping.csv'
    if (-not (Test-Path $mapPath)) { Lzcfa2a806c1 ERROR "No mapping.csv found. Set a destination and save the mapping first."; return }
    $map = @(Import-Csv $mapPath | Where-Object Action -eq 'MIGRATE')
    if ($OnlySpace) { $map = @($map | Where-Object { $_.Space -eq $OnlySpace }) }
    if (-not $map.Count) { Lzcfa2a806c1 WARN "No mapped projects to check."; return }
    $tSrcC = 0; $tSrcB = [int64]0; $tDstC = 0; $tDstB = [int64]0; $tMissing = 0; $tExtra = 0; $i = 0; $t = $map.Count
    foreach ($row in $map) {
        $i++; Lzbf80252112 -Done $i -Total $t -BytesDone 0 -BytesTotal 0 -Space $row.Space
        try {
            $null = Resolve-DestinationDriveId -Config $Config -Row $row
            Lzcfa2a806c1 OK "  destination reachable: $(Get-RowDestLabel $row). Reading the source now."
        } catch {
            Lzcfa2a806c1 ERROR "  destination NOT reachable for [$($row.Space)], so the source was not read: $($_.Exception.Message)"
            continue
        }
        $space = New-SpaceRef -Row $row
        $dj = Lz631e304c99 -Config $Config -Row $row
        $items = @(Get-DattoItems -Config $Config -Space $space)
        $sc = $items.Count
        ...
        $d = Lzb0c7e0fb81 -Config $Config -Row $row -Items $items -Inv $inv
        ...
        Lzcfa2a806c1 $lvl ("  [$($row.Space)]  source $sc files / $(Format-Bytes $sb)   ->   destination $($inv.Count) files / $(Format-Bytes $inv.Bytes)   [$tag]")
    }
    Lzcfa2a806c1 OK ("TOTAL  source $tSrcC files / $(Format-Bytes $tSrcB)    destination $tDstC files / $(Format-Bytes $tDstB)")
    if ($tMissing -gt 0) {
        Lzcfa2a806c1 WARN "$('{0:N0}' -f $tMissing) file(s) in Datto are not at the destination (named above and in the difference CSV), so some files may not have migrated. Click 'Sync new and changed' to copy them, then compare again.$(if ($tExtra) { " The destination also holds $('{0:N0}' -f $tExtra) extra file(s), which is usually pre-existing content." })"
    } elseif ($tExtra -gt 0) {
        Lzcfa2a806c1 INFO "The destination has more files than the source. That is normal if the destination already held content before the migration. To confirm every source file actually arrived, click 'Verify files arrived': it checks file by file and ignores pre-existing extras."
    }
    Lzcfa2a806c1 INFO "Bytes differ from the source because Microsoft 365 rewrites Office files on upload, so treat totals as indicative. 'Verify files arrived' is the definitive per-file check."
    if ($script:GuiMode) {
...
function Invoke-HtmlReport {
    ...
    if (-not $NoStageLog) { Lzf5bfcd7225 -Config $Config -Stage 'api-report' }
    if (-not $AuditPath) {
        $latest = Get-ChildItem (Join-Path $Config.run.reportRoot 'audit-*.csv') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if ($latest) { $AuditPath = $latest.FullName }
    }
    if (-not $AuditPath -or -not (Test-Path $AuditPath)) { Lzcfa2a806c1 ERROR "No audit CSV found in $($Config.run.reportRoot). In plain terms: there is no record to build a report from yet. Run an upload first, then build the report."; return }
    ...
    if (-not $rows.Count) { Lzcfa2a806c1 WARN "Audit CSV is empty: $AuditPath"; return }
    function _enc { param($s) [System.Net.WebUtility]::HtmlEncode("$s") }
    function Lzd89f2d3f8a { param($st) @($rows | Where-Object { $_.Status -eq $st }).Count }
    ...
    $total = $rows.Count
    $copied = (Lzd89f2d3f8a 'Copied') + (Lzd89f2d3f8a 'ZeroByte')
    $skipped = Lzd89f2d3f8a 'Skipped'
    $errors = (Lzd89f2d3f8a 'Error') + (Lzd89f2d3f8a 'DownloadError') + (Lzd89f2d3f8a 'SkippedTooLarge')
    $verifyFail = Lzd89f2d3f8a 'VerifyFail'
    $wouldCopy = Lzd89f2d3f8a 'WouldCopy'
    ...
    $diagSection = ''
    if (-not $dryRun) {
        $diag = $null; try { $diag = Lz99f612e36b -Rows $rows -Elapsed $elapsed } catch {}
        if ($diag) {
            $diagSection = @"
...
"@
    ...
    Lzcfa2a806c1 OK "HTML report written: $outPath"
    return $outPath
}
function Invoke-CompletionCertificate {
    param($Config, [string]$AuditPath, [switch]$NoStageLog)
    if (-not $NoStageLog) { Lzf5bfcd7225 -Config $Config -Stage 'api-certificate' }
    if (-not $AuditPath) {
        $latest = Get-ChildItem (Join-Path $Config.run.reportRoot 'audit-*.csv') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if ($latest) { $AuditPath = $latest.FullName }
    }
    if (-not $AuditPath -or -not (Test-Path $AuditPath)) { Lzcfa2a806c1 ERROR "No audit CSV found in $($Config.run.reportRoot). A certificate is built from a completed run's audit trail; run a migration to completion first."; return }
    $rows = @(Import-Csv $AuditPath)
    $endRow = @($rows | Where-Object { "$($_.Status)" -like 'RUN_END:*' }) | Select-Object -Last 1
    $rows   = @($rows | Where-Object { "$($_.Status)" -notlike 'RUN_END:*' })
    if (-not $rows.Count) { Lzcfa2a806c1 WARN "Audit CSV is empty: $AuditPath"; return }
    ...
    if ($dryRun -or @('PREVIEW','INCOMPLETE','ENDED_EARLY','CANCELLED') -contains $ocStatus) {
        Lzcfa2a806c1 ERROR ("A completion certificate is only issued for a completed migration. This run is '" + $ocStatus + "'. Run a full sync through to completion (then a verify), and issue the certificate from that run.")
        return
    }
    ...
    $tv = Lz1fdfbd87f7; $verSub = if ($tv) { " &middot; v$tv" } else { '' }
    $now = Get-Date
...
"@
    ...
    Set-Content -Path $outPath -Value $html -Encoding UTF8
    Lzcfa2a806c1 OK "Completion certificate written: $outPath"
    return $outPath
}
function Invoke-Validate {
    param($Config)
    Lzf5bfcd7225 -Config $Config -Stage 'api-validation'
    $ckStart = Get-Date
    Lzb8666bd351 -Config $Config
    Connect-Destination -Config $Config
    $map = @(Import-Csv (Join-Path $Config.run.reportRoot 'mapping.csv') | Where-Object Action -eq 'MIGRATE')
    ...
    $t = $map.Count; $i = 0
    foreach ($row in $map) {
        $i++; Lz0ecb0be693 -Activity 'Verify' -Status $row.Space -Current $i -Total $t | Out-Null
        ...
        if ($verdict -eq 'PASS') {
            Lzcfa2a806c1 OK "  $($row.Space): PASS - all $srcCount file(s) are present and up to date at the destination.$extraNote"
        } elseif ($verdict -eq 'OUT OF DATE') {
            Lzcfa2a806c1 WARN "  $($row.Space): OUT OF DATE - all $srcCount file(s) are present, but $($stale.Count) have a newer version in Datto than at the destination (see api-validation-stale.csv). In plain terms: those files were changed in Datto since they were copied. Run 'Sync new and changed' with 'Update where Datto is newer' to bring them up to date."
        } else {
            Lzcfa2a806c1 ERROR "  $($row.Space): FAIL - $($missing.Count) of $srcCount file(s) were NOT found at the destination (see api-validation-missing.csv).$(if($stale.Count){" $($stale.Count) present file(s) are also out of date."}) In plain terms: some files did not make it across. Run 'Sync new and changed' to copy them, then verify again."
            foreach ($m in @($missing | Select-Object -First 15)) { Lzcfa2a806c1 ERROR "      missing: /$m" }
            if ($missing.Count -gt 15) { Lzcfa2a806c1 ERROR "      ... and $('{0:N0}' -f ($missing.Count - 15)) more, all named in api-validation-missing.csv." }
        }
    }
    ...
    if ($fail -gt 0) { Lzcfa2a806c1 ERROR "Verify complete. $fail project(s) have missing files (see api-validation-missing.csv). Run 'Sync new and changed' to re-copy them, then verify again." }
    elseif ($stalep -gt 0) { Lzcfa2a806c1 WARN "Verify complete. Everything is present, but $stalep project(s) have file(s) that are newer in Datto than at the destination (see api-validation-stale.csv). Run 'Sync new and changed' with 'Update where Datto is newer' to refresh them." }
    else { Lzcfa2a806c1 OK "Verify complete. Every file is present at the destination and up to date (no file is newer in Datto). Extra files already in the destination are expected and were ignored." }
    if ($script:GuiMode) {
...
function Lzd6e0f3b57f {
    param($Config)
    Lzf5bfcd7225 -Config $Config -Stage 'api-permissions'
    Lzb8666bd351 -Config $Config
    $map = @(Import-Csv (Join-Path $Config.run.reportRoot 'mapping.csv') | Where-Object Action -eq 'MIGRATE')
    function Lz3735704304 { param([string]$r) switch -Regex ($r.ToLower()) { 'owner|manage|admin'{'Site Owner (Full Control)';break} 'edit|write|contrib'{'Member (Edit)';break} 'view|read'{'Visitor (Read)';break} default{'REVIEW'} } }
    $plan = New-Object System.Collections.Generic.List[object]
    $t = $map.Count; $i = 0
    foreach ($row in $map) {
        $i++; Lz0ecb0be693 -Activity 'Permissions' -Status $row.Space -Current $i -Total $t | Out-Null
        $space = New-SpaceRef -Row $row
        foreach ($p in @(Lz647a8ab0aa -Config $Config -Space $space)) {
            $plan.Add([pscustomobject]@{ Space=$row.Space; Principal=$p.Principal; DattoRole=$p.Role; DestinationUrl=$row.DestinationUrl; ProposedM365Role=(Lz3735704304 $p.Role) })
        }
    }
    ...
    if ($plan.Count -eq 0) { Lzcfa2a806c1 WARN "No permissions returned by the API (endpoint may not be configured). $out is empty." }
    else { Lzcfa2a806c1 OK "Permissions plan written (NOT applied): $($plan.Count) entries. $out" }
}
function Lze6a056e345 {
    param($Config)
    Lzf5bfcd7225 -Config $Config -Stage 'api-test'
    Lzb8666bd351 -Config $Config
    Connect-Destination -Config $Config
    $spaces = @(Get-DattoSpaces -Config $Config)
    Lzcfa2a806c1 OK "Datto returned $($spaces.Count) project(s)."
    if ($spaces.Count) {
        $s = $spaces[0]
        Lzcfa2a806c1 INFO "First project: $($s.Name) ($($s.Type))"
        $items = @(Get-DattoItems -Config $Config -Space $s | Select-Object -First 5)
        foreach ($it in $items) { Lzcfa2a806c1 INFO "  file: $($it.RelativePath) ($([math]::Round($it.Size/1KB,1)) KB, mod $($it.ModifiedUtc))" }
        Lzcfa2a806c1 OK "If the project and files above look right, the Datto connection is reading real data correctly."
    }
}
function Lz1386377977 {
    param($Config)
    while ($true) {
        ...
        switch ((Read-Host 'Select').ToUpper()) {
            '1' { Lze6a056e345 -Config $Config }
            '2' { Lzcd047c7b4a -Config $Config }
            '3' { Lza601daf2d8 -Config $Config }
            ...
            '8' {
                Lzcd047c7b4a -Config $Config
                $mapPath = Join-Path $Config.run.reportRoot 'mapping.csv'
                if (Test-Path $mapPath) {
                    Lza601daf2d8 -Config $Config
...
if (-not $parallelEnabled -and $MaxParallelSpaces -gt 1) {
    Lzcfa2a806c1 INFO "Parallel projects are off (run.parallel.enabled is not set); running one project at a time."
    $MaxParallelSpaces = 1
}
...
try {
    ...
    if ($needsLicence -and ($cfg.destination.provider -ne 'LocalSim')) {
        ...
        if ($lic.Licensed) {
            ...
            Lzcfa2a806c1 OK "Licence accepted: $($lic.Customer), Microsoft tenant $($lic.TenantId) (licence $($lic.LicenceId))."
        }
        elseif ($lic.Reason -match '^no licence file') {
            if ($Action -eq 'Validate') {
                if ($script:GuiMode) { Write-Host "##TRIAL##|START|0|Validate" }
                Lzcfa2a806c1 WARN "No licence: EVALUATION MODE. Verify will still check whatever is at the destination, so you can confirm the trial copy landed correctly. Licence this Microsoft tenant at https://www.liscaragh.com to remove the limit."
            }
            else {
                ...
                Lzcfa2a806c1 WARN "No licence: EVALUATION MODE. Up to $remaining $what file(s) will be copied for real so you can prove the migration works end to end, then it stops. This is a sample, not a full migration: this tenant's free $what allowance is $($script:TrialFileLimit) file(s) in total, and $spent have been used. Licence this Microsoft tenant at https://www.liscaragh.com, then Help > Install licence file, to migrate everything."
            }
        }
        else {
            if ($script:GuiMode) { Write-Host "##LICENCE##|$($lic.Reason)" }
            Lzcfa2a806c1 ERROR "This action needs a valid licence: $($lic.Reason). Without a valid licence you can still test the connection, list projects, Preview (see what would copy, without copying), and run 'Compare sizes'. Obtain a licence at https://www.liscaragh.com, then install it via Help > Install licence file."
            exit 1
        }
    }
    switch ($Action) {
        'TestApi'     { Lze6a056e345        -Config $cfg }
        'Discover'    { Lzcd047c7b4a -Config $cfg -NonInteractive:$NonInteractive }
        'PreFlight'   { Lza601daf2d8 -Config $cfg }
        ...
        'DestInventory' { Lz3e14144d85 -Config $cfg -SpaceName $Spaces -ResultDir $ResultDir }
        ...
        'Finalize'    { Lz7897f308f4 -Config $cfg -Status $FinalizeStatus -AuditPath $AuditPath; exit $script:ExitCode }
        default       { Lz1386377977       -Config $cfg }
    }
}
catch {
    $e = $_
    $emit = {
        ...
        try { Lzcfa2a806c1 $Level $Text } catch { Write-Host "[$Level] $Text" }
    }
