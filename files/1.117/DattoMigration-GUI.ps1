...
function Lz03190c8928 { param([string]$Name,[string]$Value) if (-not (Test-Path $script:RegPath)) { New-Item -Path $script:RegPath -Force | Out-Null }; Set-ItemProperty -Path $script:RegPath -Name $Name -Value ([string]$Value) }
$script:SecretEntropy = [Text.Encoding]::UTF8.GetBytes('Liscaragh.DattoMigration.v1')
function Lzb774bfe06d {
...
function Lza14ddadf3a {
    ...
    if (-not $s) { $s = Lzd4a57de353 'DATTO_CLIENT_SECRET' }
    return $s
}
function Lzc55bd37b3e {
    param([string]$Value)
    if (Lzb774bfe06d -Name 'DATTO_CLIENT_SECRET' -Value $Value) {
...
function Lze79eaff979 {
    ...
    return (Lze075c51213 -TargetSubFolder $box -SourceSubPath $src)
}
function Lz746f404d5e {
    ...
    try {
        ...
        $norm = Lze075c51213 -TargetSubFolder $box -SourceSubPath $src
        if ($norm -eq $box) { return }
        $ctrl.TxtFolder.Text = $norm
        $dropped = $box.Substring($norm.Length).Trim('/')
        Lz4bc61a2b16 "Removed '$dropped' from the folder box: the source folder you picked is already part of the path, so it would have landed twice."
        Lz6609498780
    } catch { }
}
function Lze075c51213 {
...
function Lzd5e51df0bc {
    ...
    return @{ SubfolderCount = @(Lzeab08ab46f -ParentId $curId).Count }
}
function ConvertTo-Slug { param([string]$n) return ($n -replace '[^\w\-]','-') -replace '-+','-' }
function Lz47d94c0b24 {
    param([string]$Url)
    try {
        $u = [Uri]$Url
        ...
        return Lz07ef1ec22b -Path "sites/$siteId"
    } catch { return $null }
}
function Lz47511e6986 {
    param([string]$Search)
    $q = "$Search".Trim()
    if (-not $q) { $q = '*' }
    $resp = Lz07ef1ec22b -Path ("sites?search=" + [uri]::EscapeDataString($q))
    if (-not $resp) { return @() }
...
function Lzcd335707cb {
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        $d = Lz07ef1ec22b -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lz35a18986ea {
    $fp = $script:FP
    $fp.Lst.Items.Clear()
    $fp.Lbl.Text = if ($fp.Path) { "Current folder:  /$($fp.Path)" } else { "Current folder:  / (top level)" }
    try {
        $folders = @(Lze922f0eccd -DriveId $fp.Drive -RelPath $fp.Path | Sort-Object)
...
"@
    ...
    $search = {
        ...
        try {
            $hits = @(Lz47511e6986 -Search $s.Txt.Text)
...
"@
    ...
    Lz35a18986ea
    $up.Add_Click({ if ($script:FP.Path) { $script:FP.Path = ($script:FP.Path -replace '/?[^/]*$',''); Lz35a18986ea } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:FP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(')) { $script:FP.Path = if ($script:FP.Path) { "$($script:FP.Path)/$it" } else { $it }; Lz35a18986ea } })
    $new.Add_Click({
        $n = Lz4096143d9e -Prompt 'Name for the new folder:' -Title 'New folder'
        if ($n) { $n = $n.Trim(); if ($n) { try { Lz76ee834813 -DriveId $script:FP.Drive -RelPath $script:FP.Path -Name $n; Lz35a18986ea; $script:FP.Lst.SelectedItem = $n } catch { (Show-Msg -Text ("Could not create the folder: $($_.Exception.Message)") -Caption ('New folder')) } } }
    })
    $script:FP.Use = $use
    $lst.Add_SelectionChanged({ Lz92ce167537 -State $script:FP })
    $use.Add_Click({
        $script:FP.Result = Lz3fd8eb2cb7 -State $script:FP -Current $script:FP.Path
        $script:FP.Win.DialogResult = $true
    })
    Lz92ce167537 -State $script:FP
    $r = $w.ShowDialog()
    if ($r -eq $true) { return $script:FP.Result } else { return $null }
}
function Lzb65c414692 {
    $sp = $script:SP
    $sp.Lst.Items.Clear()
    ...
    try {
        $folders = @(Lzeab08ab46f -ParentId $node.Id | Sort-Object Name)
        $sp.Current = @{}
...
"@
    ...
    Lzb65c414692
    $up.Add_Click({ if ($script:SP.Stack.Count -gt 1) { $script:SP.Stack.RemoveAt($script:SP.Stack.Count - 1); Lzb65c414692 } })
    $lst.Add_MouseDoubleClick({ $it = "$($script:SP.Lst.SelectedItem)"; if ($it -and ($it -notmatch '^\(') -and $script:SP.Current.ContainsKey($it)) { [void]$script:SP.Stack.Add(@{ Id = $script:SP.Current[$it]; Name = $it }); Lzb65c414692 } })
    $script:SP.Use = $use
    $lst.Add_SelectionChanged({ Lz92ce167537 -State $script:SP })
    $use.Add_Click({
        $cur = (@($script:SP.Stack | Select-Object -Skip 1 | ForEach-Object { $_.Name }) -join '/')
        $script:SP.Result = Lz3fd8eb2cb7 -State $script:SP -Current $cur
        $script:SP.Win.DialogResult = $true
    })
    Lz92ce167537 -State $script:SP
    $r = $w.ShowDialog()
...
function Lz4bc61a2b16 { param([string]$Text) $ctrl.LblStatus.Text = $Text }
...
function Lzc8e2c8a681 { param([TimeSpan]$ts)
...
function Lz11e7dc4375 { param([double]$sec)
    if ($sec -lt 0) { $sec = 0 }
...
function Lz6efd72f355 {
    ...
    if ($hasRegime) {
        $etaA = Lz374fdc4a94 -SmallRate $smallRateFull   -SmallRemaining $smallRem -LargeRate $largeRateFull   -LargeBytesRemaining $largeRem -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz374fdc4a94 -SmallRate $smallRateRecent -SmallRemaining $smallRem -LargeRate $largeRateRecent -LargeBytesRemaining $largeRem -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    } else {
        $etaA = Lz374fdc4a94 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateFull   -BytesRemaining $bytesRem
        $etaB = Lz374fdc4a94 -SmallRate 0 -SmallRemaining 0 -LargeRate 0 -LargeBytesRemaining 0 -BytesRate $bytesRateRecent -BytesRemaining $bytesRem
    }
...
function Lz9ed385b957 {
    ...
    try {
        ...
        $etaText = Lz6efd72f355 -Samples $script:EtaSamples -SmallDone $SmallDone -SmallTotal $SmallTotal -LargeDone $LargeDone -LargeTotal $LargeTotal -BytesDone $BytesDone -BytesTotal $BytesTotal -Final $Final
...
function Lz13c7159e0b {
    ...
    if ($Line -match '^##DATTOPACE##\|(-?\d+)\|(\d+)\|(\d+)$') {
        Lzef53dee65e -GapMs ([int]$Matches[2]) -Code ([int]$Matches[3])
        return
    }
    if ($Line -match '^##THROTTLE##\|(-?\d+)\|(\d+)\|(-?\d+)\|(\d+)(?:\|(\d+))?$') {
        $code = if ($Matches[5]) { [int]$Matches[5] } else { 0 }
        Lzfeebc6357c -Max ([int]$Matches[1]) -HardMax ([int]$Matches[2]) -PausedSeconds ([int]$Matches[3]) -Code $code
        return
    }
    if ($Line -match '^##PROGRESS##\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|(\d+)\|([01])\|(.*)$') {
        Lz9ed385b957 -Done ([int]$Matches[1]) -Total ([int]$Matches[2]) -BytesDone ([int64]$Matches[3]) -BytesTotal ([int64]$Matches[4]) `
    ...
    $ctrl.TxtLog.AppendText((Lza1b6cf691c $Line) + "`r`n")
...
function Lzfe163ad786 { param([string]$Text) Lz13c7159e0b $Text }
function Lz3e590589ed {
...
if ($ctrl.BtnFindSite) { $ctrl.BtnFindSite.Add_Click({
    ...
    try {
        Lz793a4a830a $true 'Connecting to Microsoft 365...'
        Lz954459ca6b
    } catch {
        ...
        Lz793a4a830a $false; return
    } finally { Lz793a4a830a $false }
    $seed = "$($ctrl.TxtLoc.Text)".Trim()
    if ($seed -match '://') { $seed = '' }
    $picked = Lz836e4e0491 -StartSearch $seed
    if ($picked) {
        ...
        Lz4bc61a2b16 "Site set to $picked"
        Lz6609498780
    }
}) }
$ctrl.RbSite.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and ($ctrl.TxtLoc.Text -notmatch '://')) { $ctrl.TxtLoc.Text = Lz3e590589ed }
    Lz6609498780
})
$ctrl.RbOneDrive.Add_Checked({
    Set-DestModeUI
    if ($script:Cfg -and (($ctrl.TxtLoc.Text -match '://') -or (-not $ctrl.TxtLoc.Text.Trim()))) { $ctrl.TxtLoc.Text = Lz99f50b6e2b }
    Lz6609498780
})
$ctrl.RbSkip.Add_Checked({ Set-DestModeUI; Lz6609498780 })
$ctrl.TxtFolder.Add_TextChanged({ Lz6609498780 })
$ctrl.TxtSourceSub.Add_TextChanged({ Lz6609498780 })
if ($ctrl.ChkSrcContents) { $ctrl.ChkSrcContents.Add_Checked({ Lz6609498780 }); $ctrl.ChkSrcContents.Add_Unchecked({ Lz6609498780 }) }
if ($ctrl.BtnShowFiles) { $ctrl.BtnShowFiles.Add_Click({ Lz4324778797 -Paths @(@($script:CheckFiles) | ForEach-Object { "$($_.Path)" }) }) }
$ctrl.TxtFolder.Add_LostFocus({ Lz746f404d5e })
$ctrl.TxtSourceSub.Add_LostFocus({ Lz746f404d5e })
$ctrl.TxtLoc.Add_TextChanged({ Lz6609498780 })
$ctrl.TxtLib.Add_TextChanged({ Lz6609498780 })
$script:NestSuspend = $false
function Lzd7a1bb7642 {
    if ($script:NestSuspend) { return }
    if (-not $script:JobOpen -or -not $script:ConfigPath) { return }
    try {
        $cfg = Read-ConfigJson $script:ConfigPath
        Lz42b949c667 -Cfg $cfg -Path 'destination.nestUnderProjectFolder' -Value ([bool]$ctrl.ChkNest.IsChecked)
        Write-ConfigJson -Cfg $cfg -Path $script:ConfigPath
        try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { }
        Lz4bc61a2b16 $(if ($ctrl.ChkNest.IsChecked) {
...
$ctrl.ChkNest.Add_Checked({ Lzd7a1bb7642 })
$ctrl.ChkNest.Add_Unchecked({ Lzd7a1bb7642 })
$script:ProjectRows = @()
$script:RunStatus = @{}
function Lz0e09053092 {
...
function Lz219aa951a1 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if (-not $sub) { continue }
        try { [void](Lzd5e51df0bc -ProjectId $p.Id -SubPath $sub) }
...
function Lz3178aeef88 {
    ...
    foreach ($p in $script:Projects) {
        ...
        if ($d.DestinationType -eq 'OneDrive') {
            ...
            $seen[$key] = ''
            try {
                $drv = Lz07ef1ec22b -Path "users/$([uri]::EscapeDataString($upn))/drive"
...
function Lzcea1dd7ea1 {
    ...
    try { Lz793a4a830a $true 'Checking source folders...'; $srcOk = Lz219aa951a1 } finally { Lz793a4a830a $false }
    if (-not $srcOk) { return }
    if (($EngineArgs -contains '-Execute') -or ($EngineArgs -contains 'Validate') -or ($EngineArgs -contains 'SizeCheck')) {
        try { Lz793a4a830a $true 'Checking destinations...'; $dstOk = Lz3178aeef88 } finally { Lz793a4a830a $false }
        if (-not $dstOk) { return }
    }
        ...
        try {
            if (Test-Path $script:OutFile) {
                ...
                if ($chunk) {
                    ...
                    for ($k = 0; $k -lt $lines.Count - 1; $k++) { try { Lz13c7159e0b $lines[$k] } catch {} }
                }
            }
        } catch {}
        $script:SpinIdx = ($script:SpinIdx + 1) % 4
        $spin = @('|','/','-','\')[$script:SpinIdx]
        $ctrl.LblElapsed.Text = "$spin  Elapsed " + (Lzc8e2c8a681 ((Get-Date) - $script:RunStart))
        if ($script:Proc.HasExited) {
            $script:Timer.Stop()
            if ($script:Partial) { Lz13c7159e0b $script:Partial; $script:Partial = '' }
            $errFile = "$($script:OutFile).err"
            if (Test-Path $errFile) { $e = Get-Content $errFile -Raw; if ($e) { Lz13c7159e0b $e } }
            $ctrl.Prog.IsIndeterminate = $false; $ctrl.Prog.Value = 100
            $ctrl.LblElapsed.Text = 'Elapsed ' + (Lzc8e2c8a681 ((Get-Date) - $script:RunStart))
            $ctrl.LblEta.Text = ''
            $oc = $null
            try {
                $ocPath = Join-Path (Lz9b2a5857c0) 'lastrun-outcome.json'
            ...
            if ($script:TrialCapped) {
                ...
                $sev = 'warn'
                Lz256e0ff5b4 -Copied $script:TrialCopied
            }
            if ($script:Paused) { Lz13c7159e0b "----- Paused. What had already uploaded and verified is saved. Click Resume to carry on. -----" }
            elseif ($script:Stopping) { Lz13c7159e0b "----- Stopped. What had already uploaded and verified is saved. Click 'Sync new and changed' to carry on. -----" }
            $stripWord = switch ($sev) { 'ok' { 'finished' } 'warn' { 'finished, with notes' } default { 'finished, with problems' } }
            Lz4bc61a2b16 ("$($script:CurLabel) $stripWord - the verdict is in the summary below.")
            $ctrl.ThrottleBanner.Visibility = 'Collapsed'
...
function Lzcce963fa05 { param([bool]$On) foreach ($n in $script:JobButtons) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
$script:JobMenuItems = @('MnuJobSave','MnuJobSaveAs','MnuJobRename','MnuJobOpenFolder','MnuJobClose','MnuJobDelete')
function Lz2c3b9b93b8 { param([bool]$On) foreach ($n in $script:JobMenuItems) { if ($ctrl[$n]) { $ctrl[$n].IsEnabled = $On } } }
function Lz9ff71e9144 {
    try {
        ...
        foreach ($p in $list) {
            ...
            $mi.Add_Click({ param($s,$e) if (Test-Path $s.Tag) { Lz5e384ca3d8 -ConfigFile $s.Tag; Lzc7a2ac1352 } else { (Show-Msg -Text ('That job no longer exists.')) } })
            [void]$ctrl.MnuJobRecent.Items.Add($mi)
        }
    } catch {}
}
function Lz4b175e469e { param([string]$ConfigFile)
    try {
        $cur = Get-RegSetting 'RecentJobs'
        $list = if ($cur) { @($cur -split "`n" | Where-Object { $_ }) } else { @() }
        $list = @(@($ConfigFile) + @($list | Where-Object { $_ -ne $ConfigFile })) | Select-Object -First 8
        Lz03190c8928 -Name 'RecentJobs' -Value ($list -join "`n")
    } catch {}
    Lz9ff71e9144
}
function Lzc4705decb6 {
...
function Lz5e384ca3d8 {
    ...
    $script:JobOpen = $true
    $ctrl.BtnConnect.IsEnabled = $true; Lz2c3b9b93b8 $true
    Lzcce963fa05 $true
    Lz4b175e469e $ConfigFile
    try {
        if ($script:Cfg -and $script:Cfg.run.reportRoot) {
            $ra = Join-Path $script:Cfg.run.reportRoot 'run-active.json'
            if (Test-Path $ra) {
                Lzc4705decb6 -Status Incomplete -Note 'Recovering a previous run that did not finish...' -OnDone {
                    Lz4bc61a2b16 'A previous run had not finished; it has been recorded as incomplete. Open the report to see what completed, then ''Sync new and changed'' to continue.'
                }
            }
        }
    } catch {}
...
$ctrl.MnuJobNew.Add_Click({
  try {
    $name = Lz4096143d9e -Prompt "Name for this migration job (e.g. the company or project name):" -Title 'New migration job'
    ...
    if (Test-Path $jobDir) { (Show-Msg -Text ("A job folder '$slug' already exists. Open it instead, or choose another name.")); return }
        ...
        Lz42b949c667 -Cfg $tmpl -Path 'destination.nestUnderProjectFolder' -Value $false
        ...
        Lz5e384ca3d8 -ConfigFile (Join-Path $jobDir 'config.json')
...
$ctrl.MnuSettingsChecklist.Add_Click({ Lz7efc902c43 })
$ctrl.MnuSettingsApi.Add_Click({ Lz14e2a00aaf })
$ctrl.MnuSettingsWizard.Add_Click({ Lzce21c87c16 })
$ctrl.MnuSettingsEmail.Add_Click({ Lz7d9c6f59ab })
$ctrl.MnuSettingsTuning.Add_Click({ Lz3ed075ad16 })
$ctrl.MnuSettingsDecommission.Add_Click({ Lz8bdaf3302d })
$ctrl.MnuHelpHowto.Add_Click({
    [void](Lz131537a88a -Title 'How to use' -Heading 'How to use this tool' -Sections $script:QuickStartSections -No 'Close')
})
function Lz88103fed00 {
    param([string]$Text)
    $t = "$Text"
    ...
    $s2 = Lzd4a57de353 'DATTO_CLIENT_SECRET'
...
function Build-SupportBundle {
    ...
    try {
        ...
        try { Set-Content -Path (Join-Path $stage 'gui-window-log.txt') -Value (Lz88103fed00 "$($ctrl.TxtLog.Text)") -Encoding UTF8 } catch {}
        $copied = 0
        if ($script:Cfg) {
            $sets = @(
        ...
        foreach ($f in @(Get-ChildItem -Path $stage -Recurse -File -Include '*.log','*.txt','*.json','*.csv','*.html' -ErrorAction SilentlyContinue)) {
            try { Set-Content -Path $f.FullName -Value (Lz88103fed00 (Get-Content $f.FullName -Raw -ErrorAction Stop)) -Encoding UTF8 } catch {}
        }
...
$ctrl.MnuHelpSupport.Add_Click({
    ...
    Lz793a4a830a $true 'Collecting logs and reports for support...'
    try { $b = Build-SupportBundle } finally { Lz793a4a830a $false }
    if (-not $b) { return }
...
function Lz191551ebc5 { return (Join-Path (Split-Path $PSScriptRoot -Parent) 'licence.json') }
function Lzcfdcc817ce {
    $path = Lz191551ebc5
...
$ctrl.MnuHelpLicence.Add_Click({ (Show-Msg -Text ("Datto Workplace to SharePoint Migrator" + "`n" + "(c) Liscaragh Software. All rights reserved." + "`n`n" + (Lzcfdcc817ce)) -Caption ('Licence')) | Out-Null })
function Lzefd2f3760e {
...
function Lz6e23f85b62 {
    param([string]$For = 'copy')
    if (Test-Path (Lz191551ebc5)) { return $true }
...
$ctrl.MnuHelpLicInstall.Add_Click({
    ...
    try {
        ...
        (Show-Msg -Text ("Licence installed.`n`n" + (Lzcfdcc817ce)) -Caption 'Licence installed') | Out-Null
    } catch {
...
function Invoke-UpdateCheck {
    try {
        ...
        Lz793a4a830a $true 'Checking for updates...'
        $descJson = $null
        ...
        New-Item -ItemType Directory -Path $stage -Force | Out-Null
        try {
            Lz793a4a830a $true "Downloading version $($payload.version)..."
            foreach ($f in $payload.files) {
...
$ctrl.MnuHelpAbout.Add_Click({
    ...
    [void](Lz131537a88a -Title 'About' -Heading 'Datto Workplace to SharePoint Migrator' -Sections $sections -No 'Close' -Brand)
})
$ctrl.MnuHelpCheck.Add_Click({
    ...
    $sec = $null; try { $sec = Lza14ddadf3a } catch {}
    ...
    try {
        ...
        if ($tid -and $app -and $certOk) { $tok=Lzf911a10d4c -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{Authorization="Bearer $tok"} -ErrorAction Stop; & $mark $true 'Microsoft 365 reachable' "$($s.webUrl)" }
...
$ctrl.MnuJobRename.Add_Click({
    if (-not $script:JobOpen) { return }
    $new = Lz4096143d9e -Prompt 'New display name for this job:' -Title 'Rename job' -Default "$($ctrl.LblJob.Text)"
...
function Lz69b5af03dd {
    ...
    $ctrl.BtnConnect.IsEnabled = $false; Lz2c3b9b93b8 $false
    Lzcce963fa05 $false
}
$ctrl.MnuJobClose.Add_Click({ Lz69b5af03dd; Lz4bc61a2b16 'Job closed' })
$ctrl.MnuJobDelete.Add_Click({
    ...
    Lz69b5af03dd
    try { if ($dir -and (Test-Path $dir)) { Remove-Item $dir -Recurse -Force -ErrorAction Stop } ; Lz9ff71e9144; Lz4bc61a2b16 "Deleted job '$nm'" }
    catch { (Show-Msg -Text ("Could not fully delete: $($_.Exception.Message)")) }
})
function Lzeec9f9138f {
    param([switch]$Quiet)
    if (-not $script:JobOpen) { if (-not $Quiet) { (Show-Msg -Text ('Create or open a named migration job before connecting.')) }; return }
    try {
        Lz793a4a830a $true 'Connecting...'
        $script:Cfg = Import-ResolvedConfig $script:ConfigPath
        Lzfe163ad786 'Signing in to Datto and listing projects...'
        $script:Projects = Lz097e51fdd4
        Lzfe163ad786 "Found $($script:Projects.Count) project(s)."
        Lz8e1b42f7cd
        try { Lz954459ca6b; Lzfe163ad786 'Connected to Microsoft 365.' } catch { Lzfe163ad786 'Could not sign in to Microsoft 365 yet. You can still set up mappings, but you will need this connected before uploading.'; Lzfe163ad786 "Technical detail: $($_.Exception.Message)" }
        Update-ProjectList
        $ctrl.LblConn.Text = "Connected - $($script:Projects.Count) projects"
        $ctrl.LblConn.Foreground = 'Green'
        Lz4bc61a2b16 'Connected'
    } catch {
        if ($Quiet) {
            $ctrl.LblConn.Text = 'Not connected - automatic connect failed'; $ctrl.LblConn.Foreground = 'Red'
            Lzfe163ad786 'Could not connect automatically. Click ''Connect and list projects'' to try again.'
            Lzfe163ad786 "Technical detail: $($_.Exception.Message)"
            Lz4bc61a2b16 "Could not connect automatically ($($_.Exception.Message)). Click 'Connect and list projects' to try again."
        } else {
            ...
            Lz4bc61a2b16 'Connect failed'
        }
    }
    finally { Lz793a4a830a $false }
}
$ctrl.BtnConnect.Add_Click({ Lzeec9f9138f })
function Lzc7a2ac1352 {
    $ep = [string](Get-RegSetting 'DattoEndpointUrl'); $id = [string](Get-RegSetting 'DattoClientId')
    $sec = $null; try { $sec = Lza14ddadf3a } catch {}
    if (-not ($ep -and $id -and $sec)) {
        Lz4bc61a2b16 "Job ready. Set up your API details first (Settings > API settings), then click 'Connect and list projects'."
        return
    }
    Lzeec9f9138f -Quiet
}
$ctrl.LstProjects.Add_SelectionChanged({
    $p = Lz2a5d6ac5e4
    if (-not $p) { return }
    $selCount = 0; try { $selCount = @(Lz6f3d252518).Count } catch {}
...
function Lzfa0a293ecb {
    ...
    if ($ctrl.RbOneDrive.IsChecked) {
        ...
        return @{ DestinationType='OneDrive'; DestinationUrl="$($script:Cfg.destination.oneDriveHostUrl)/personal/$((ConvertTo-Slug $upn))"; TargetPrincipal=$upn; TargetLibrary=''; TargetSubFolder=(Lze79eaff979) }
    }
    $site = $ctrl.TxtLoc.Text.Trim().TrimEnd('/')
    if (-not $site) { if (-not $Quiet) { (Show-Msg -Text ('Enter a SharePoint site URL.')) }; return 'ERR' }
    return @{ DestinationType='SharePoint'; DestinationUrl=$site; TargetPrincipal=''; TargetLibrary="$($ctrl.TxtLib.Text)".Trim(); TargetSubFolder=(Lze79eaff979) }
}
$ctrl.BtnApply.Add_Click({
    ...
    Lz4bc61a2b16 "Mapped and saved: $($p.Name)"
})
$ctrl.TxtFilter.Add_TextChanged({ Lz2ecfd30270 })
$ctrl.BtnApplySel.Add_Click({
    ...
    if (-not (Lz6c5303b601 -Count $sel.Count)) { return }
    if ($ctrl.RbSkip.IsChecked) { foreach ($p in $sel) { $script:Map.Remove($p.Id) | Out-Null }; Update-ProjectList; [void](Write-MappingQuiet); Lz4bc61a2b16 "Set $($sel.Count) project(s) to skip"; return }
    $d = Lzfa0a293ecb
    if ($d -eq 'ERR') { return }
    ...
    foreach ($proj in $sel) {
        ...
        if ($ctrl.RbOneDrive.IsChecked) {
            ...
            $script:Map[$proj.Id] = @{ DestinationType='SharePoint'; DestinationUrl=(Lz3e590589ed); TargetPrincipal=''; TargetLibrary=$lib; TargetSubFolder=$sub; SourceSubPath=$keepSrc; SourceContentsOnly=$keepCo }
        }
    }
    Update-ProjectList
    [void](Write-MappingQuiet)
    Lz4bc61a2b16 "Applied and saved for $($sel.Count) selected project(s)"
})
$ctrl.BtnApplyAll.Add_Click({
    if (-not (Lz6c5303b601 -Count (@($script:Projects).Count))) { return }
    $d = Lzfa0a293ecb
    if ($d -eq 'ERR') { return }
    if ($ctrl.RbSkip.IsChecked) { $script:Map.Clear(); Update-ProjectList; [void](Write-MappingQuiet); Lz4bc61a2b16 'All projects set to skip'; return }
    $lib = $d.TargetLibrary; $folder = $d.TargetSubFolder
    ...
    Update-ProjectList
    [void](Write-MappingQuiet)
    Lz4bc61a2b16 "Applied and saved for all $($script:Projects.Count) projects"
})
if ($ctrl.BtnPickLib) { $ctrl.BtnPickLib.Add_Click({
    ...
    $libs = $null
    try {
        Lz793a4a830a $true 'Looking up libraries...'
        $libs = Lz20813fa942 -Url $site -Refresh
    } catch { $ctrl.LblCheck.Text = "Error: $($_.Exception.Message)"; $ctrl.LblCheck.Foreground = 'Red'; Lz793a4a830a $false; return }
    finally { Lz793a4a830a $false }
    if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
    $picked = Lz2e9fe69661 -SiteUrl $site -Libraries $libs
    if ($picked) {
        $changed = ("$($ctrl.TxtLib.Text)".Trim() -ne "$picked")
        $ctrl.TxtLib.Text = $picked
        if ($changed -and "$($ctrl.TxtFolder.Text)".Trim()) {
            $ctrl.TxtFolder.Text = ''
            Lz4bc61a2b16 "Library set to $picked. The Folder box was cleared: its path belonged to the previous library."
        } else {
            Lz4bc61a2b16 "Library set to $picked"
        }
...
if ($ctrl.BtnBrowseFolder) { $ctrl.BtnBrowseFolder.Add_Click({
    ...
    try {
        Lz793a4a830a $true 'Opening destination...'
        Lz954459ca6b
        $res = Lzcd335707cb
    } catch { (Show-Msg -Text ($_.Exception.Message) -Caption ('Browse folders')); Lz793a4a830a $false; return }
    finally { Lz793a4a830a $false }
    $picked = Lz9794810807 -DriveId $res.DriveId -Label $res.Label -StartPath ("$($ctrl.TxtFolder.Text)".Trim())
    if ($null -ne $picked) { $ctrl.TxtFolder.Text = $picked; Lz4bc61a2b16 ("Folder set to " + $(if ($picked) { "/$picked" } else { 'top level' })) }
}) }
if ($ctrl.BtnBrowseSource) { $ctrl.BtnBrowseSource.Add_Click({
    ...
    try { $picked = Lzf676a681f7 -ProjectId $p.Id -ProjectName $p.Name }
    catch { (Show-Msg -Text ("Could not open the project's folders: $($_.Exception.Message)") -Caption ('Browse source')); return }
    if ($null -ne $picked) { $ctrl.TxtSourceSub.Text = $picked; Lz4bc61a2b16 ("Source set to " + $(if ($picked) { "/$picked" } else { 'the whole project' })) }
}) }
if ($ctrl.BtnTestSource) { $ctrl.BtnTestSource.Add_Click({
    ...
    try {
        Lz793a4a830a $true 'Checking the source folder in Datto...'
        $res = Lzd5e51df0bc -ProjectId $p.Id -SubPath $sub
    } catch { $ctrl.LblSourceCheck.Text = "Source NOT found. $($_.Exception.Message)"; $ctrl.LblSourceCheck.Foreground = 'Red'; Lz793a4a830a $false; return }
    finally { Lz793a4a830a $false }
...
$ctrl.BtnCheck.Add_Click({
    try {
        Lz793a4a830a $true 'Checking destination...'
        if ($ctrl.RbOneDrive.IsChecked) {
            $upn = $ctrl.TxtLoc.Text.Trim()
            $d = Lz07ef1ec22b -Path "users/$([uri]::EscapeDataString($upn))/drive"
            $ctrl.LblCheck.Text = if ($d) { 'OneDrive found for this user.' } else { 'Not found.' }
        } else {
            $libs = Lz20813fa942 -Url $ctrl.TxtLoc.Text.Trim()
            if ($null -eq $libs) { $ctrl.LblCheck.Text = 'Site not found - check the URL.'; $ctrl.LblCheck.Foreground = 'Red'; return }
...
function Lz6609498780 {
    ...
    $srcLabel = Lze3ceb9b45a -Project $p
    ...
    $land = Get-GuiLandingPath -DestType $type -SiteUrl $loc -TargetSubFolder (Lze79eaff979) `
...
function Update-ApplyButtonState {
    ...
    $sel = 0; try { $sel = @(Lz6f3d252518).Count } catch { }
...
function Lz2977689fc6 {
    ...
    $script:Map[$p.Id] = $d
    Update-ProjectList
    Lz4bc61a2b16 "Captured your on-screen changes for '$($p.Name)' before running."
}
function Save-Mapping {
    ...
    Lzfe163ad786 "Saved $n mapping(s)."
    return $true
}
function Lz8e1b42f7cd {
    $script:Map = @{}
    if (-not $script:Cfg) { return }
    $out = Join-Path $script:Cfg.run.reportRoot 'mapping.csv'
    ...
    try {
        ...
        if ($script:Map.Count) { Lzfe163ad786 "Loaded $($script:Map.Count) saved mapping(s) for this job." }
    } catch { Lzfe163ad786 "Could not read saved mappings: $($_.Exception.Message)" }
}
$ctrl.BtnPreflight.Add_Click({ if (Save-Mapping) { Lzcea1dd7ea1 -EngineArgs @('-Action','PreFlight') -Label 'Check readiness' } })
$ctrl.BtnDryRun.Add_Click({ if (Save-Mapping) { Lzcea1dd7ea1 -EngineArgs @('-Action','Transfer','-Mode','FirstPass') -Label 'Preview (no upload)' } })
function Lze32d7c38ef {
...
$ctrl.BtnTransfer.Add_Click({
    ...
    $sections = @(
        @{ H = 'What this does'; B = "Copies every file from " + (Lze32d7c38ef) }
    ...
    if (-not (Lz131537a88a -Title 'Upload all files' -Heading 'Upload all files  -  Datto is the source of truth' -Sections $sections -Yes 'Upload all files' -No 'Cancel')) { return }
    if (-not (Lz6e23f85b62)) { return }
    Lzcea1dd7ea1 -EngineArgs @('-Action','Transfer','-Mode','FirstPass','-Execute') -Label 'Upload all files'
})
function Lz167988e995 {
    [xml]$sx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
"@
    ...
    $intro.Text = "How should Sync decide what to copy for " + (Lze32d7c38ef) + "`n`nIt compares Datto to the Microsoft 365 copy by modified date, file by file. Pick one:"
...
$ctrl.BtnDelta.Add_Click({
    ...
    if (-not (Lz6e23f85b62)) { return }
    Lzcea1dd7ea1 -EngineArgs @('-Action','Transfer','-Mode','Delta','-DeltaMode',$mode,'-Execute') -Label $label
})
$ctrl.BtnValidate.Add_Click({
    ...
    if (-not (Lz131537a88a -Title 'Verify files arrived' -Heading 'Verify files arrived' -Sections $sections -Yes 'Run the check' -No 'Cancel')) { return }
    if (-not (Lz6e23f85b62 -For 'verify')) { return }
    Lzcea1dd7ea1 -EngineArgs @('-Action','Validate') -Label 'Verify files arrived'
})
$ctrl.BtnSizeCheck.Add_Click({ if (Save-Mapping) { Lzcea1dd7ea1 -EngineArgs @('-Action','SizeCheck') -Label 'Compare sizes' } })
function Lz9b2a5857c0 {
...
function Lz88918d719d {
    if (-not $script:Cfg) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch { return $null } }
    return $script:Cfg.run.logRoot
}
function Lz3061b1a1ab {
    param([string]$Name)
...
function Lzf03af5c5be {
    ...
    foreach ($f in $files) { $l = Lz3061b1a1ab $f.Name; $rows.Add([pscustomobject]@{ What = $l.What; When = $l.When; File = $f.Name; Path = $f.FullName }) }
    [xml]$px = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
$ctrl.BtnOpenReport.Add_Click({ Lzf03af5c5be -Folder (Lz9b2a5857c0) -Patterns @('report-*.html') -Title 'Open a report' -EmptyMsg 'No reports yet. A report is created automatically after each upload or sync.' })
$ctrl.BtnOpenAudit.Add_Click({ Lzf03af5c5be -Folder (Lz88918d719d) -Patterns @('*.log') -Title 'Open a log' -EmptyMsg 'No logs yet. Run an action first.' -Exclude @('*api-report-*.log','*api-finalize-*.log') })
function Lz568e399337 {
    ...
    Lz4bc61a2b16 "Speed limit set: up $uTxt, down $dTxt"
    Lz13c7159e0b "----- Speed limit set: up $uTxt, down $dTxt (applies immediately to a running upload) -----"
}
$ctrl.BtnApplyCap.Add_Click({ Lz568e399337 })
$ctrl.TxtCapUp.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz568e399337 } })
$ctrl.TxtCapDown.Add_KeyDown({ param($s,$e) if ($e.Key -eq 'Return') { Lz568e399337 } })
function Lz0fc33c1323 {
    param([int]$ParentId)
    try { Get-CimInstance Win32_Process -Filter "ParentProcessId=$ParentId" -ErrorAction SilentlyContinue | ForEach-Object { Lz0fc33c1323 -ParentId ([int]$_.ProcessId) } } catch {}
...
$ctrl.BtnStop.Add_Click({
    ...
    Lz13c7159e0b "----- STOP requested by user at $stamp -----"
    Lz4bc61a2b16 "Stopping $($script:CurLabel)..."
    try { Lz0fc33c1323 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
    ...
    Lzc4705decb6 -Status Cancelled -Note 'Finalising the cancelled run...' -OnDone {
        try { Lz13c7159e0b "----- Cancellation recorded. A report has been saved; click 'Open report' to view it. -----" } catch {}
    }
})
function Lz9fed4eb053 {
    [xml]$rx = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
...
if ($ctrl.BtnPause) { $ctrl.BtnPause.Add_Click({
    if ($script:Paused) {
        ...
        Lzcea1dd7ea1 -EngineArgs $rargs -Label $rlabel
        return
    }
    ...
    $stamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
    Lz13c7159e0b "----- PAUSE requested by user at $stamp -----"
    Lz4bc61a2b16 "Pausing $($script:CurLabel)..."
    try { Lz0fc33c1323 -ParentId ([int]$script:Proc.Id) } catch {}
    try {
        if ($script:Cfg -and $script:Cfg.run.logRoot -and (Test-Path $script:Cfg.run.logRoot)) {
            $lf = Get-ChildItem (Join-Path $script:Cfg.run.logRoot '*.log') -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    Lzc4705decb6 -Status Cancelled -Note 'Finalising the paused run...' -OnDone {
        try { Lz13c7159e0b "----- Paused. What had already uploaded is saved. Click Resume to carry on. -----" } catch {}
    }
}) }
$script:DattoDomains = @(
...
function Lz94adc4dbb7 { param([string]$Domain,[string]$Cell) if (-not $Domain -or -not "$Cell".Trim()) { return '' }; return "https://$Domain/$("$Cell".Trim())/api/v1" }
function Lz080a2c51c1 {
    param([string]$Url)
    if ($Url -match '^https?://([^/]+)/([^/]+)/api/v1/?$') { return @{ Domain=$Matches[1]; Cell=$Matches[2] } }
    return @{ Domain=''; Cell='' }
}
function Lzf07855cf99 {
...
function Lzf911a10d4c {
    param([string]$TenantId,[string]$ClientId,[string]$Thumbprint,[string]$CertStore)
    return (Lz22cf414ae5 -TenantId $TenantId -ClientId $ClientId -Thumbprint $Thumbprint -CertStore $CertStore).Token
}
function Lz14e2a00aaf {
    ...
    function Lz8928d13a0f { param($Text) $h=New-Object System.Windows.Controls.TextBlock; $h.Text=$Text; $h.FontWeight='Bold'; $h.Margin='0,12,0,4'; [void]$root.Children.Add($h) }
    function Lz3bac484a85 { param($Label,$Control,$Info)
    ...
    function Lze75ebf8ee8 { param($Text) $t=New-Object System.Windows.Controls.TextBox; $t.Margin='0,3,0,3'; $t.Text="$Text"; return $t }
    Lz8928d13a0f 'Datto Workplace'
    ...
    $parts = Lz080a2c51c1 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz3bac484a85 'Region (domain)' $cmbDom 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on. If you are not sure, ask whoever set up your Datto account.')
    $tbCell = Lz3bac484a85 'Cell (number)' (Lze75ebf8ee8 ($parts.Cell)) 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). You can see it on the Datto API page, or ask your Datto administrator.'
    $tbDId  = Lz3bac484a85 'Datto Client ID' (Lze75ebf8ee8 ([string](Get-RegSetting 'DattoClientId'))) 'The username for the Datto connection. In the Datto Workplace admin portal you create an "API integration", which gives you a Client ID and a Secret. This is the Client ID.'
    $pbSec  = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin='0,3,0,3'
    [void](Lz3bac484a85 'Datto Secret' $pbSec 'The password that goes with the Datto Client ID, from the same Datto API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    Lz8928d13a0f 'Microsoft 365'
    $tbTid = Lz3bac484a85 'Tenant ID' (Lze75ebf8ee8 ([string](Get-RegSetting 'TenantId'))) 'Your Microsoft 365 organisation''s unique ID (a long code with dashes). Find it at entra.microsoft.com (Microsoft Entra admin center) on the Overview page, shown as "Tenant ID". Or ask whoever manages your Microsoft 365.'
    $tbApp = Lz3bac484a85 'App (Client) ID' (Lze75ebf8ee8 ([string](Get-RegSetting 'GraphClientId'))) 'The ID of the app registration this tool signs in as. In entra.microsoft.com go to App registrations, open the app created for this migration, and copy "Application (client) ID".'
    Lz8928d13a0f 'Certificate (sign-in to Microsoft 365)'
    ...
    [void](Lz3bac484a85 'Thumbprint' $lblThumb 'A fingerprint of the sign-in certificate. You do not type this: it fills in automatically when you install the .pfx file below.')
    $tbPfx = Lz3bac484a85 '.pfx file' (Lze75ebf8ee8 '') 'The certificate file (its name ends in .pfx) that lets the tool sign in to Microsoft 365. Your IT contact provides it. Click Browse to pick it.'
    ...
    [void](Lz3bac484a85 '.pfx password' $pbPfx 'The password for the .pfx file, supplied together with the certificate by your IT contact.')
    ...
    Lz8928d13a0f 'SharePoint and OneDrive'
    ...
    $tbSp  = Lz3bac484a85 'SharePoint root URL' (Lze75ebf8ee8 ([string](Get-RegSetting 'SharePointRootUrl'))) 'Your SharePoint web address, for example https://yourcompany.sharepoint.com. It is the address you see when you open SharePoint in a browser. Auto-detect can fill this for you.'
    $tbOd  = Lz3bac484a85 'OneDrive host URL'   (Lze75ebf8ee8 ([string](Get-RegSetting 'OneDriveHostUrl'))) 'Your OneDrive web address. It is usually your SharePoint address with "-my" added, for example https://yourcompany-my.sharepoint.com. Auto-detect fills it.'
    $tbTs  = Lz3bac484a85 'Team site base URL'  (Lze75ebf8ee8 ([string](Get-RegSetting 'TeamSiteBaseUrl'))) 'Where your SharePoint team sites live, usually your SharePoint address followed by /sites. Auto-detect fills it.'
    $tbDef = Lz3bac484a85 'Default site URL'    (Lze75ebf8ee8 ([string](Get-RegSetting 'DefaultSiteUrl'))) 'The SharePoint site suggested by default when you set up a new project. Auto-detect fills it, and you can change it for each project.'
    $tbUpn = Lz3bac484a85 'Email / UPN domain'  (Lze75ebf8ee8 ([string](Get-RegSetting 'UpnDomain'))) 'Your organisation''s email domain, for example @yourcompany.com. It is used to find each person''s OneDrive. Auto-detect can fill it if the permission is granted; otherwise type it.'
    ...
    $btnInstall.Add_Click({
        try {
            ...
            if ($have -and $have.HasPrivateKey) {
                Lz03190c8928 -Name 'CertThumbprint' -Value $th
    ...
    $btnAuto.Add_Click({
        ...
        try {
            ...
            $tok = Lzf911a10d4c -TenantId $tid -ClientId $app -Thumbprint $th
            ...
            $upnMsg=''
            try { $org=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/organization' -Headers @{ Authorization="Bearer $tok" }; $u=Lz177fd36c37 $org; if ($u) { $script:AS.tbUpn.Text=$u } else { $upnMsg=' (email domain not found)' } }
    ...
    $btnTest.Add_Click({
        ...
        $msgs=@()
        try {
            $ep = Lz94adc4dbb7 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            $secPlain = if ($script:AS.pbSec.Password) { $script:AS.pbSec.Password } else { Lza14ddadf3a }
        ...
        try {
            ...
            else { $tok=Lzf911a10d4c -TenantId $tid -ClientId $app -Thumbprint $th; $s=Invoke-RestMethod -Method GET -Uri 'https://graph.microsoft.com/v1.0/sites/root' -Headers @{ Authorization="Bearer $tok" }; $msgs += "Microsoft 365: OK ($($s.webUrl))." }
    ...
    $btnSave.Add_Click({
        try {
            $ep = Lz94adc4dbb7 $script:AS.cmbDom.SelectedItem $script:AS.tbCell.Text
            Lz03190c8928 -Name 'DattoEndpointUrl' -Value $ep
            Lz03190c8928 -Name 'DattoClientId'    -Value ("$($script:AS.tbDId.Text)".Trim())
            Lz03190c8928 -Name 'TenantId'         -Value ("$($script:AS.tbTid.Text)".Trim())
            Lz03190c8928 -Name 'GraphClientId'    -Value ("$($script:AS.tbApp.Text)".Trim())
            Lz03190c8928 -Name 'SharePointRootUrl'-Value ("$($script:AS.tbSp.Text)".Trim())
            Lz03190c8928 -Name 'OneDriveHostUrl'  -Value ("$($script:AS.tbOd.Text)".Trim())
            Lz03190c8928 -Name 'TeamSiteBaseUrl'  -Value ("$($script:AS.tbTs.Text)".Trim())
            Lz03190c8928 -Name 'DefaultSiteUrl'   -Value ("$($script:AS.tbDef.Text)".Trim())
            Lz03190c8928 -Name 'UpnDomain'        -Value ("$($script:AS.tbUpn.Text)".Trim())
            if ($script:AS.pbSec.Password) {
                Lzc55bd37b3e -Value $script:AS.pbSec.Password
            }
...
function Lza27d6f19aa { param($Cfg,[string]$Path)
    $node=$Cfg
...
function Lz42b949c667 { param($Cfg,[string]$Path,$Value)
...
function Lz3ed075ad16 {
    ...
    function Lz18c4ebb7eb { param($Grid,[int]$Row,[string]$Label,[string]$Value,[string]$Info,[string]$Rec)
    ...
    foreach ($s in $script:SettingSpecs) {
        $val = Lza27d6f19aa -Cfg $cfg -Path $s.Path
        $disp = if ($s.Type -eq 'list') { (@($val) -join ';') } else { "$val" }
        $script:SetBoxes[$s.Path] = Lz18c4ebb7eb -Grid $grid -Row $r -Label $s.Label -Value $disp -Info $s.Info -Rec $s.Rec
        $r++
    }
    ...
    $save.Add_Click({
        try {
            foreach ($s in $script:SettingSpecs) {
                ...
                if ($s.Type -eq 'int') { $v=0; [void][int]::TryParse($raw,[ref]$v); Lz42b949c667 -Cfg $script:SetCfg -Path $s.Path -Value $v }
                elseif ($s.Type -eq 'list') { $arr=@($raw -split ';' | ForEach-Object { $_.Trim() } | Where-Object { $_ }); Lz42b949c667 -Cfg $script:SetCfg -Path $s.Path -Value $arr }
                elseif ($s.Type -eq 'bool') { Lz42b949c667 -Cfg $script:SetCfg -Path $s.Path -Value ([bool]($raw -match '^(1|true|yes|on)$')) }
                else { Lz42b949c667 -Cfg $script:SetCfg -Path $s.Path -Value $raw }
            }
...
function Lz13407d2998 {
    ...
    function Lz1a1c773c01 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    $parts = Lz080a2c51c1 ([string](Get-RegSetting 'DattoEndpointUrl'))
    if ($parts.Domain) { $cmbDom.SelectedItem = $parts.Domain } else { $cmbDom.SelectedIndex = 0 }
    [void](Lz1a1c773c01 -Label 'Region (domain)' -Control $cmbDom -Info 'Your Datto Workplace region. If the web address you use for Datto starts with "eu", pick the eu one; "us", pick us, and so on.')
    $tbCell = New-Object System.Windows.Controls.TextBox; $tbCell.Margin = '0,3,0,3'; $tbCell.Text = "$($parts.Cell)"
    [void](Lz1a1c773c01 -Label 'Cell (number)' -Control $tbCell -Info 'A small number that is part of your Datto web address (for example the 2 in .../2/api/v1). It is shown on the Datto API integration page.')
    $tbDId = New-Object System.Windows.Controls.TextBox; $tbDId.Margin = '0,3,0,3'; $tbDId.Text = [string](Get-RegSetting 'DattoClientId')
    [void](Lz1a1c773c01 -Label 'Client ID' -Control $tbDId -Info 'The username for the Datto connection, from the API integration you created in the Datto Workplace admin portal.')
    $pbSec = New-Object System.Windows.Controls.PasswordBox; $pbSec.Margin = '0,3,0,3'
    [void](Lz1a1c773c01 -Label 'Secret' -Control $pbSec -Info 'The password that goes with the Client ID, from the same API integration. Treat it like a password. Leave blank to keep the one already saved.')
    ...
    $btnTest.Add_Click({
        ...
        try {
            $ep = Lz94adc4dbb7 $script:DS.cmbDom.SelectedItem $script:DS.tbCell.Text
            $secPlain = if ($script:DS.pbSec.Password) { $script:DS.pbSec.Password } else { Lza14ddadf3a }
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz03190c8928 -Name 'DattoEndpointUrl' -Value $ep
            Lz03190c8928 -Name 'DattoClientId'    -Value ("$($script:DS.tbDId.Text)".Trim())
            if ($script:DS.pbSec.Password) { Lzc55bd37b3e -Value $script:DS.pbSec.Password }
            if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
...
function Lz7efc902c43 {
    ...
    $pre = Lz3f4f84301b -Title 'Prerequisites'
    ...
    $dat = Lz3f4f84301b -Title 'Datto Workplace credentials' -Buttons @($bDat, (Lza69e1cba86 'Step-by-step Datto setup guide (opens www.liscaragh.com in your browser)'))
    ...
    $ms = Lz3f4f84301b -Title 'Microsoft 365' -Buttons @($bMs, (Lza69e1cba86 'What the wizard does, explained (opens www.liscaragh.com in your browser)'))
    $bEm = New-Object System.Windows.Controls.Button
    $bEm.Content = 'Set up...'; $bEm.Padding = '10,4'; $bEm.VerticalAlignment = 'Top'
    $em = Lz3f4f84301b -Title 'Email alerts (optional)' -Buttons @($bEm)
    ...
    $bDat.Add_Click({ try { Lz13407d2998 } catch { (Show-Msg -Text ("The Datto step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz111c883475; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bMs.Add_Click({ try { Lzce21c87c16 } catch { (Show-Msg -Text ("The Microsoft 365 setup hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz111c883475; $script:SC.Win.Activate() | Out-Null } catch {} })
    $bEm.Add_Click({ try { Lz7d9c6f59ab } catch { (Show-Msg -Text ("The email step hit a problem: $($_.Exception.Message)") -Icon ('Error')) | Out-Null }; try { Lz111c883475; $script:SC.Win.Activate() | Out-Null } catch {} })
    $btnGo.Add_Click({ try { $script:SC.Win.Close() } catch {} })
    Lz111c883475
    [void]$w.ShowDialog()
    $script:SC = $null
}
function Lz9edd810370 {
...
function Lza61621f77a {
    ...
    $bOpen.Add_Click({ try { Lz404e768d9f -Url "$($script:DC.Url)" } catch {} })
    $w.Show()
}
function Lzad2db9f672 {
    if ($script:DC) { try { $script:DC.Win.Close() } catch {}; $script:DC = $null }
}
function Lzcae1862d91 {
    ...
    Lza61621f77a -Code $code -Url $pageUrl -Step $Step -Purpose $Purpose
    Lz404e768d9f -Url $pageUrl
    ...
    } finally { Lzad2db9f672 }
}
function Lz980b4185b0 {
    try {
...
function Lz3e682cdb0e {
    ...
    if (-not (Lz2fcffdf4ad)) {
        Lz9edd810370 'A MICROSOFT SIGN-IN WINDOW IS ABOUT TO OPEN. It can hide behind other windows - if you do not see it, check the taskbar. The app will not respond while it is open; that is normal.' '#B54708' -Strong
        try {
            Lza1d66a8cbc -Action { Connect-MgGraph -Scopes $wizScopes -NoWelcome -ErrorAction Stop }
            $wizConnected = $true
        } catch {
            Lz9edd810370 "The standard sign-in did not complete ($($_.Exception.Message)). Switching to a CODE sign-in: a small window will show your code, always on top." '#B54708' -Strong
        }
    } else {
        Lz9edd810370 'Using the CODE sign-in (the reliable path on this machine): a small window will show your code, always on top.' '#B54708' -Strong
    }
    if (-not $wizConnected) {
        try {
            $at = Lzcae1862d91 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($wizScopes -join ' ') -Say { param($t, $c) Lz9edd810370 $t $(if ($c) { $c } else { '#475467' }) -Strong } `
            ...
            $wizTokTid = Lz01f502400a -Token $at -Claim 'tid'
            $wizTokAcct = Lz01f502400a -Token $at -Claim 'upn'
            if (-not $wizTokAcct) { $wizTokAcct = Lz01f502400a -Token $at -Claim 'preferred_username' }
            ...
            Lz9edd810370 "Sign-in did not complete: $($_.Exception.Message)" '#B42318' -Strong
            Lz9edd810370 'Nothing was changed. You can try again, or use Settings > API settings to enter the details by hand.'
            return
        }
    }
    $mgc = Get-MgContext
    ...
    Lz9edd810370 "Signed in to tenant $tid as $wizAcct." '#12B76A'
    $newThumb = $null
    try {
        Lz9edd810370 'Reading the tenant''s Microsoft Graph service catalogue...'
        ...
        if ($found.Count) {
            $app = $found[0]
            if ($found.Count -gt 1) { Lz9edd810370 "Note: $($found.Count) app registrations share the name '$($script:WizAppName)'. Using the oldest; consider removing the duplicates in Microsoft Entra." '#B54708' }
            Lz9edd810370 "This tenant is already set up: reusing the existing app registration '$($script:WizAppName)' and adding this computer's certificate to it." '#12B76A'
        } else {
            Lz9edd810370 "Creating the app registration '$($script:WizAppName)'..."
            ...
            Lz9edd810370 'App registration created.' '#12B76A'
        }
        $appId = "$($app['appId'])"; $appObj = "$($app['id'])"
        Lz9edd810370 'Generating a sign-in certificate for this computer (valid one year)...'
        ...
        Lz9edd810370 "Certificate created in this user's store. Thumbprint $newThumb." '#12B76A'
        Lz9edd810370 'Uploading the certificate''s public key to the app registration...'
        ...
        Lz9edd810370 "Public key uploaded. The app registration now holds $($after.Count) certificate(s); private keys stay on their own machines." '#12B76A'
        ...
        $spId = "$($sp['id'])"
        if ($spJustCreated) { Lz9279370a3a -Seconds 15 -Message 'Letting the new app registration settle before granting consent' }
        Lz9edd810370 'Granting admin consent for the five permissions...'
        $haveRoleIds = @{}
        try {
            ...
            for ($ca = 1; $ca -le 4 -and -not $done; $ca++) {
                try {
                    ...
                    elseif (($cm -match 'BadRequest|Bad Request|NotFound|Not Found|does not exist|Request_ResourceNotFound') -and $ca -lt 4) {
                        Lz9279370a3a -Seconds 15 -Message "Permission '$rn' not ready yet (attempt $ca of 4), the new app is still propagating"
                    } else { throw }
                }
            }
        }
        ...
        Lz9edd810370 $consentMsg '#12B76A'
        Lz03190c8928 -Name 'TenantId'       -Value $tid
        Lz03190c8928 -Name 'GraphClientId'  -Value $appId
        Lz03190c8928 -Name 'CertThumbprint' -Value $newThumb
        Lz9edd810370 'Connection details saved on this computer.' '#12B76A'
    } catch {
        ...
        if ($friendly) { Lz9edd810370 $friendly '#B42318' -Strong; Lz9edd810370 "Technical detail: $m" }
        else { Lz9edd810370 "Setup stopped: $m" '#B42318' -Strong }
        if ($newThumb) { Lz9edd810370 "The certificate generated for this attempt (thumbprint $newThumb) is still in this user's certificate store; re-running the wizard after fixing the cause will create a fresh one, and unused ones can be deleted from certmgr.msc at any time." }
        if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
        return
    }
    if (-not $ForEmail) { try { Disconnect-MgGraph -ErrorAction SilentlyContinue | Out-Null } catch {} }
    Lz9edd810370 'Checking the new sign-in works (a brand-new setup can take a minute or two to become active)...'
    $tok = $null
    for ($i = 1; $i -le 8; $i++) {
        try {
            $tok = Lzf911a10d4c -TenantId $tid -ClientId $appId -Thumbprint $newThumb
            ...
            Lz03190c8928 -Name 'SharePointRootUrl' -Value $spUrl
            $der = Lzf07855cf99 $spUrl
            if ($der) {
                Lz03190c8928 -Name 'OneDriveHostUrl' -Value $der.OneDriveHostUrl
                Lz03190c8928 -Name 'TeamSiteBaseUrl' -Value $der.TeamSiteBaseUrl
                Lz03190c8928 -Name 'DefaultSiteUrl'  -Value $der.DefaultSiteUrl
            }
            Lz9edd810370 "Sign-in works. SharePoint and OneDrive addresses detected from $spUrl." '#12B76A'
            break
        } catch {
            $tok = $null
            if ($i -lt 8) { Lz9279370a3a -Seconds 15 -Message "Not active yet (attempt $i of 8), this is normal for a brand-new setup" }
            else { Lz9edd810370 "The new sign-in has not become active yet: $($_.Exception.Message)" '#B54708' -Strong
                   Lz9edd810370 'Everything is saved, so this usually just needs a few more minutes. Use Test connection under Settings > API settings to confirm, and Auto-detect URLs there to fill the addresses.' }
        }
    }
    if ($tok) {
        try {
            ...
            $u = Lz177fd36c37 $org
            if ($u) { Lz03190c8928 -Name 'UpnDomain' -Value $u; Lz9edd810370 "Email domain detected: $u" '#12B76A' }
        } catch { Lz9edd810370 'The email (UPN) domain could not be read yet; enter it under Settings > API settings if OneDrive destinations need it.' }
        Lz9edd810370 'Microsoft 365 setup is COMPLETE. Datto Workplace credentials are the only remaining step (if not already done).' '#12B76A' -Strong
        $script:WizSetupOk = $true
    }
    if ($script:JobOpen -and $script:ConfigPath) { try { $script:Cfg = Import-ResolvedConfig $script:ConfigPath } catch {} }
}
function Lzce21c87c16 {
    Add-Type -AssemblyName PresentationFramework | Out-Null
    $w = New-Object System.Windows.Window
    Lzfc38646221 $w
    ...
    $btnRun.Add_Click({
        ...
        try {
            ...
            Lz3e682cdb0e -ForEmail:$wantEmail
            $emailOkRun = $true
            if ($wantEmail) {
                $emailOkRun = $false
                if (Lzec8bb3b922) {
                    Lz9edd810370 'Email alerts: setting up the sender...' -Strong
                    $ret = @(Invoke-EmailSenderSetup -ReuseGraphSession -Log { param($t, $c) if ($c) { Lz9edd810370 $t $c } else { Lz9edd810370 $t } })
                    $emailOkRun = ($ret.Count -gt 0 -and $ret[-1] -eq $true)
                } else {
                    Lz9edd810370 'Email alerts were not set up, because the Microsoft 365 setup did not complete. Fix that first, then use Settings > Email alerts.' '#B54708'
                }
            }
            ...
            Lz9edd810370 "Setup hit an unexpected problem: $($_.Exception.Message). Nothing was left half-applied that a re-run will not sort out; you can also use Settings > API settings by hand." '#B42318' -Strong
        } finally {
...
function Invoke-EmailSenderSetup {
        ...
        if (-not $done) {
            ...
            if ($canTok -and $exoOrg) {
                try {
                    ...
                    $exoTok = Lzcae1862d91 -ClientId 'fb78d390-0c51-40cd-8e17-fdbfab77341b' -Scope 'https://outlook.office365.com/.default' -Say $say `
                    ...
                    Lza1d66a8cbc -Action { Connect-ExchangeOnline -AccessToken $exoTok -Organization $exoOrg -ShowBanner:$false -ErrorAction Stop }
                    $done = $true
            ...
            if (-not $done) {
                ...
                Lz404e768d9f -Url 'https://login.microsoft.com/device'
                Lzc1b487bbfa
                try { Lza1d66a8cbc -Action { Connect-ExchangeOnline -ShowBanner:$false -Device -ErrorAction Stop } }
                finally { Lz71d7e6eb92 }
            }
        }
    }
    ...
    if ($ReuseGraphSession -and $haveCtx) {
        ...
        if (-not (Lz2fcffdf4ad)) {
            ...
            try {
                Lza1d66a8cbc -Action { Connect-MgGraph -Scopes $esScopes -NoWelcome -ErrorAction Stop }
                $esConnected = $true
            } catch {
        ...
        if (-not $esConnected) {
            try {
                $at = Lzcae1862d91 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope ($esScopes -join ' ') -Say $say `
                ...
                $tokUpn = Lz01f502400a -Token $at -Claim 'upn'
                if (-not $tokUpn) { $tokUpn = Lz01f502400a -Token $at -Claim 'preferred_username' }
    ...
    try {
        ...
        if (-not $mailboxExists) {
            ...
            if (-not $existing) {
                ...
                for ($w = 1; $w -le 6; $w++) {
                    ...
                    catch { if ($w -lt 6) { Lz9279370a3a -Seconds 10 -Message "Still provisioning (check $w of 6)" } }
                }
            } else {
            ...
            for ($attempt = 1; $attempt -le 3; $attempt++) {
                try {
                    ...
                    if ($m2 -match 'Enable-OrganizationCustomization' -and $attempt -lt 3) {
                        ...
                        Lz282ef62b8f 'Applying it now. THIS TAKES UP TO TWO MINUTES and the window may look frozen while Microsoft applies it - that is normal, please do not close it.'
                        ...
                        Lz9279370a3a -Seconds 30 -Message 'Applied. Letting it take effect, then retrying the restriction'
                    } elseif ($attempt -lt 3) {
                        Lz9279370a3a -Seconds 15 -Message 'Retrying the restriction'
                    } else { throw }
                }
            }
        } catch {
        ...
        Lz03190c8928 -Name 'EmailSender' -Value $senderAddr
        Lz03190c8928 -Name 'EmailEnabled' -Value '1'
...
function Lz7d9c6f59ab {
    ...
    $intro.TextWrapping = 'Wrap'; $intro.Foreground = 'Gray'; $intro.Margin = '0,0,0,6'; [void]$root.Children.Add($intro)
    function Lz30adf807e0 { param($Text) $h = New-Object System.Windows.Controls.TextBlock; $h.Text = $Text; $h.FontWeight = 'Bold'; $h.Margin = '0,12,0,4'; [void]$root.Children.Add($h) }
    function Lza287072b50 { param($Label, $Control, $Info)
        $g = New-Object System.Windows.Controls.Grid
    ...
    [void](Lza287072b50 -Label 'Send to' -Control $tbTo -Info 'Who receives the alerts. Put ONE address per line (press Enter for the next one). A pasted list separated with ; or , also works.')
    ...
    [void](Lza287072b50 -Label 'Outcomes' -Control $outRow -Info 'Which results trigger an email. For unattended overnight runs, at least "Failure / ended early" is strongly recommended.')
    ...
    [void](Lza287072b50 -Label 'Actions' -Control $actRow -Info 'Which run types trigger an email. Previews never send one (they change nothing).')
    Lz30adf807e0 'Subject and attachments'
    ...
    [void](Lza287072b50 -Label 'Subject' -Control $tbSubj -Info ('The email subject. These placeholders are filled in from the run: {JobName} {Action} {Outcome} {Source} {Destination} {FilesCopied} {FilesFailed} {Errors} {SizeCopied} {Duration} {StartTime} {EndTime} {Tenant} {Version}. Checks (Verify, Compare sizes) fill the ones that apply to them; the rest are left out.'))
    ...
    $updatePrev = {
        try { $script:ES.lblPrev.Text = 'Preview:  ' + (Lz7ee18bd2b1 -Template $script:ES.tbSubj.Text -Vars (Lz85511c4d26)) } catch {}
    }
    ...
    $btnTest.Add_Click({
        ...
        try {
            ...
            $subj = Lz7ee18bd2b1 -Template ("$($script:ES.tbSubj.Text)") -Vars (Lz85511c4d26)
            $tok = Lzf911a10d4c -TenantId $tid -ClientId $app -Thumbprint $th
    ...
    $btnSave.Add_Click({
        try {
            ...
            Lz03190c8928 -Name 'EmailEnabled'    -Value (& $b $script:ES.chkOn)
            Lz03190c8928 -Name 'EmailSender'     -Value ("$($script:ES.tbFrom.Text)".Trim())
            Lz03190c8928 -Name 'EmailRecipients' -Value ((("$($script:ES.tbTo.Text)" -split '[;,\r\n]+' | ForEach-Object { "$_".Trim() } | Where-Object { $_ })) -join '; ')
            Lz03190c8928 -Name 'EmailSubject'    -Value ("$($script:ES.tbSubj.Text)".Trim())
            Lz03190c8928 -Name 'EmailOnSuccess'  -Value (& $b $script:ES.chkOk)
            Lz03190c8928 -Name 'EmailOnWarning'  -Value (& $b $script:ES.chkWarn)
            Lz03190c8928 -Name 'EmailOnFailure'  -Value (& $b $script:ES.chkBad)
            Lz03190c8928 -Name 'EmailOnTransfer' -Value (& $b $script:ES.chkFull)
            Lz03190c8928 -Name 'EmailOnDelta'    -Value (& $b $script:ES.chkSync)
            Lz03190c8928 -Name 'EmailOnValidate' -Value (& $b $script:ES.chkVer)
            Lz03190c8928 -Name 'EmailOnSizeCheck'-Value (& $b $script:ES.chkCmp)
            Lz03190c8928 -Name 'EmailAttach'     -Value (& $b $script:ES.chkAtt)
...
function Lz2cfd3c49c7 {
    ...
    if ($Tenant) {
        try {
            ...
            $at = Lzcae1862d91 -ClientId '14d82eec-204b-4c2f-b7e8-296a70dab67e' -Scope 'Application.ReadWrite.All User.ReadWrite.All Organization.Read.All' -Say $say `
...
function Lz8bdaf3302d {
    ...
    $btnGo.Add_Click({
        try {
            ...
            $okD = Lz2cfd3c49c7 -Tenant:([bool]$script:DX.chkTenant.IsChecked) -Exchange:([bool]$script:DX.chkExo.IsChecked) -Local:([bool]$script:DX.chkLocal.IsChecked) -Log $sayUi
            ...
            try { if ($script:SC -and $script:SC.Win -and $script:SC.Win.IsVisible) { Lz111c883475 } } catch {}
        } catch {
            try {
                $tbE = New-Object System.Windows.Controls.TextBlock
...
function Lza8fed5c9b3 {
    return ((Lz4f6999728c) -and (Lzec8bb3b922))
}
Lz91c8aef8ce
Lz71d7e6eb92
$win.Dispatcher.Add_UnhandledException({
    $ev = $args[1]
    try { Lz13c7159e0b "[app] An unexpected background error was contained (the app carries on): $($ev.Exception.Message)" } catch {}
    $ev.Handled = $true
})
$win.Add_ContentRendered({
    ...
    if (-not (Lza8fed5c9b3)) {
        try { Lz7efc902c43 } catch {}
        if (Lza8fed5c9b3) { Lz4bc61a2b16 'Connection ready. Create a migration with Job > New, or open an existing one.' }
        else { Lz4bc61a2b16 'Finish setup any time: the checklist reappears at startup, and each step is under Settings.' }
    } elseif (-not $script:JobOpen) {
        Lz4bc61a2b16 'Connection ready. Create a migration with Job > New, or open an existing one.'
    }
    if ((Lza8fed5c9b3) -and -not (Test-Path (Lz191551ebc5))) {
        try { Lz66b0b340d1 } catch {}
    }
})
try {
    Add-Type -MemberDefinition '
